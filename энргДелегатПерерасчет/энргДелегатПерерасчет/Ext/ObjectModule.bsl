...
Процедура Инициализировать(МВТ, ПолучаемыеНаборы)  Экспорт 
	...
	Если НЕ ПараметрыЗапроса.Найти("НаборПредоставленныеПоказания") = Неопределено Тогда
		Запрос.УстановитьПараметр("НаборПредоставленныеПоказания",		НаборПредоставленныеПоказания.Выгрузить());
	КонецЕсли;
	Если НЕ ПараметрыЗапроса.Найти("НаборМаксимальныеПоказанияСрез") = Неопределено Тогда
		Запрос.УстановитьПараметр("НаборМаксимальныеПоказанияСрез",		НаборМаксимальныеПоказанияСрез.Выгрузить());
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыВТ(ПараметрыВТ)  
	...
	Если НЕ мПомещения = Неопределено И НЕ мПомещения.Количество() = 0 Тогда
