...
Процедура ПримененитьПриоритетыПоказанийПУНП(Отбор,НачалоПериодаРасчета)
	...
	Текст = 
		...
		|	ЕСТЬNULL(бестПриоритетПередачиПоказанийСрезПоследних.Приоритет, 0) КАК Приоритет
		|ПОМЕСТИТЬ ВТ_ЗаписиПоледовательности
		|ИЗ
		|	РегистрСведений.бестПоказанияПУНППоследовательность КАК бестПоказанияПУНППоследовательность
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.бестПриоритетПередачиПоказаний.СрезПоследних(
		|				&КонецПериода,
		|				Организация = &Организация
		|					И Район = &Район) КАК бестПриоритетПередачиПоказанийСрезПоследних
		|		ПО бестПоказанияПУНППоследовательность.Организация = бестПриоритетПередачиПоказанийСрезПоследних.Организация
		|			И бестПоказанияПУНППоследовательность.Район = бестПриоритетПередачиПоказанийСрезПоследних.Район
		//|			И бестПоказанияПУНППоследовательность.ИсточникПоказаний = бестПриоритетПередачиПоказанийСрезПоследних.ИсточникПоказаний
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			...
			Если ЗначениеЗаполнено(Выборка.ДокРегистратор) Тогда
				
				ДокРегистратор = Выборка.ДокРегистратор.ПолучитьОбъект();
				Если Ложь Тогда
					ДокРегистратор = Документы.бестРегистрацияПоказанийНП.СоздатьДокумент();
				КонецЕсли;
...
Процедура ВосстановитьПоследовательностьПоказанияПУНП(Организация, ПериодНачисления,Район,МКД = Неопределено, Отдельностоящие = Ложь, Договор=Неопределено) Экспорт 
	...
	Если Метаданные.РегистрыСведений.Найти("бестПриоритетПередачиПоказаний") <> Неопределено Тогда
