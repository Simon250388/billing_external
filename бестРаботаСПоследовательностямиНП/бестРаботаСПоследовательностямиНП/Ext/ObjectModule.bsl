...
Процедура ОтложенныеДвиженияДокументовНачисленияНП(Организация,ПериодНачисления, Район,  МКД = Неопределено, ИсключаяОтдельностоящие = Ложь, Договор = Неопределено,СбрасыватьСостояниеПоследовательности=ложь) Экспорт
	...
	МассивОбъектРасчета = Результат[2].Выгрузить().ВыгрузитьКолонку("ОбъектРасчета");
	
	ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивДоговоров);
	ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивОбъектРасчета);
	
	ОчиститьНаборыЗаписей(Организация, ПериодНачисления,Район,МассивДоговоров, МассивОбъектРасчета);
...
Процедура ОчиститьНаборыЗаписей(Организация, ПериодНачисления,Район,МассивДоговоров, МассивОбъектРасчета)
	...
	Для каждого ОбъектРасчета из МассивОбъектРасчета Цикл
		...
		НаборМаксимальныеПоказанияСрез.Отбор.ОбъектРасчета.установить(ОбъектРасчета);
...
Функция РезультатЗапросаСтабильныеПериодыНП(Отбор) 
	...
	Текст ="ВЫБРАТЬ
		   ...
		   Если Отбор.ИсключаяОтдельностоящие тогда
			   Текст 		= СтрЗаменить(Текст,"И (бестОбъектыРасчета.МКД = &МКД)","И (бестОбъектыРасчета.МКД <> &МКД)");
		   Иначе
