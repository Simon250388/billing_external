...
//
Процедура ОтложенныеДвиженияДокументовНачисленияНП(Организация,ПериодНачисления, Район,  МКД = Неопределено, ИсключаяОтдельностоящие = Ложь,СбрасыватьСостояниеПоследовательности=ложь) Экспорт
	...
	Результат 													= РезультатНачальныеОстаткиСтабильногоПериодаНП(ПериодНачисления,Организация,Район,НачалоПериодаРасчета,ИсключаяОтдельностоящие,МКД);
	...
	ОчиститьНаборыЗаписей(Организация, ПериодНачисления,Район,МКД);
		...
		Если ТекМКД <> Выборка.МКД или ТекОбъект <> Выборка.ОбъектРасчета или ТекПрибор <> Выборка.ПриборУчета Или ТекШкала <> Выборка.Шкала или ТекТарифнаяЗона <> Выборка.ТарифнаяЗона тогда 			  			
			Если ТекМКД <> Неопределено тогда
				...
				Если Не Отказ Тогда
					...
					|Организация, ПериодНачисления,район,МКД,ОбъектРасчета,ПриборУчета,Шкала,ТарифнаяЗона,НомерПозиции,ПоказанияС,ПоказанияПО,НачалоПериодаРасчета",
					Организация, ПериодНачисления,район,ТекМКД,ТекОбъект,ТекПрибор,Текшкала,ТекТарифнаяЗона,НомерПозиции,ПоказанияС,ПоказанияПО,НачалоПериодаРасчета);
...
Процедура ОчиститьНаборыЗаписей(Организация, ПериодНачисления,Район,МКД)
		НаборСтабильныеПериоды					= РегистрыСведений.бестСтабильныеПериодыНП.СоздатьНаборЗаписей();
...
Функция ПровестиПоказанияПриборовУчетаНП(ПараметрыПроведений,МассивДокументов,ДанныеСреднегоОбъема,ДанныеМаксимальногоПотребления)
	...
	Попытка
		...
		для Каждого Выборка из МассивДокументов цикл
			...
			ПоказанияТекущегоПериода 					= ?(ЭтоОткрытие, ЛОЖЬ,Выборка.ПериодРегистрации >= НачалоПериодаРасчета);
			...
			//Если  Выборка.СостояниеПоказаний = ПоказанияПриняты или ВСрок тогда
				...
				СтрокаНабор.ВСрок  						= ?(ЭтоОткрытие, ЛОЖЬ, ВСрок);;
...
Функция РезультатНачальныеОстаткиСтабильногоПериодаНП(ПериодНачисления,Организация,Район,ПериодРегистрации,ИсключаяОтдельностоящие = Ложь, МКД)
	...
	|	0 КАК НомерПозиции,	
	|" + энргНачисления.СформироватьСтрокуДляЗапроса("бестСтабильныеПериодыПриборыУзловУчета","СтабильныеПериодыПриборыУзлаУчета","Организация,ПериодНачисления,ДатаРегистратора,ДокРегистратор,НомерПозиции,МКД") + "
	...
	|			И ДокументыНачальныхОстатков.МКД = СтабильныеПериодыПриборыУзлаУчета.МКД
