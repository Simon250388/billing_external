...
Функция ПровестиПоказанияПриборовУчета(ПараметрыПроведений,МассивДокументов,ДанныеСреднегоОбъема,ДанныеМаксимальногоПотребления)
	...
	Попытка
		...
		для Каждого Выборка из МассивДокументов цикл
			...
			Если Не РегистрируетОбъем Тогда
				// Получаем таблицу максимальных показаний по регистратору.
				НаборМаксПоказаний  				= РегистрыСведений.энргМаксимальныеПоказания.СоздатьНаборЗаписей();
				НаборМаксПоказаний.Отбор.регистратор.установить(Выборка.ДокРегистратор);
			...
			Если (ЭтоМаксимальные или ВСрок) И НЕ РегистрируетОбъем тогда 				
...
Процедура СписатьЗадолженность(ПараметрыОперации,ДанныеДокумента,Сумма,Услуга = Неопределено, Поставщик = Неопределено, ПериодНачисления  = Неопределено)
	...
	Если НЕ Услуга = Неопределено ИЛИ НЕ ПериодНачисления = Неопределено ИЛИ НЕ Поставщик = Неопределено Тогда
...
Процедура СписатьЗадолженностьПоУслуге(ПараметрыОперации,ДанныеДокумента,НадоСписать,Услуга,Поставщик,ПериодРасчета)
	...
	Если НЕ яПериодРасчета = Неопределено И НЕ яУслуга = Неопределено И НЕ яПоставщик = Неопределено Тогда
		
		СтруктураПоиска  						= Новый Структура("Услуга,ПериодРасчета,Поставщик");
		СтруктураПоиска.Услуга 					= яУслуга;
		СтруктураПоиска.ПериодРасчета 			= яПериодРасчета;
		СтруктураПоиска.Поставщик 				= яПоставщик;
		
	ИначеЕсли НЕ яПериодРасчета = Неопределено И НЕ яУслуга = Неопределено И яПоставщик = Неопределено Тогда
		
		СтруктураПоиска  						= Новый Структура("ПериодРасчета,Услуга");
		СтруктураПоиска.ПериодРасчета 			= яПериодРасчета;
		СтруктураПоиска.Услуга 					= яУслуга;
		
	ИначеЕсли НЕ яПериодРасчета = Неопределено И яУслуга = Неопределено И НЕ яПоставщик = Неопределено Тогда 
		
		СтруктураПоиска  						= Новый Структура("ПериодРасчета,Поставщик");
		СтруктураПоиска.ПериодРасчета 			= яПериодРасчета;
		СтруктураПоиска.Поставщик 				= яПоставщик;
		
	ИначеЕсли яПериодРасчета = Неопределено И НЕ яУслуга = Неопределено И НЕ яПоставщик = Неопределено Тогда 
		
		СтруктураПоиска  						= Новый Структура("Услуга,Поставщик");
		СтруктураПоиска.Услуга 					= яУслуга;
		СтруктураПоиска.Поставщик 				= яПоставщик;
		
	ИначеЕсли НЕ яПериодРасчета = Неопределено И яУслуга = Неопределено И яПоставщик = Неопределено Тогда
		
		СтруктураПоиска  						= Новый Структура("ПериодРасчета");
		СтруктураПоиска.ПериодРасчета 			= яПериодРасчета;
		
	ИначеЕсли яПериодРасчета = Неопределено И яУслуга = Неопределено И НЕ яПоставщик = Неопределено Тогда
		
		СтруктураПоиска  						= Новый Структура("Поставщик");
		СтруктураПоиска.Поставщик 				= яПоставщик;
		
	ИначеЕсли яПериодРасчета = Неопределено И НЕ яУслуга = Неопределено И яПоставщик = Неопределено Тогда
...
Функция РезультатОплата(МВТ)
	...
	|	ДокументыДляПроведения.Абонент,
	|	ДокументыДляПроведения.ДокРегистратор,
	|	энргРасходныйКассовыйОрдер.ПериодОплатыНачало,
	|	ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка),
	|	ЗНАЧЕНИЕ(Справочник.энргКонтрагенты.ПустаяСсылка),
	|	-энргРасходныйКассовыйОрдер.СуммаОплаты
	|ИЗ
	|	ДокументыДляПроведения КАК ДокументыДляПроведения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.энргРасходныйКассовыйОрдер КАК энргРасходныйКассовыйОрдер
	|		ПО ДокументыДляПроведения.ДокРегистратор = энргРасходныйКассовыйОрдер.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
