...
Процедура ВыполнитьОтложененоеПроведение(Организация, ПериодНачисления,Район,Строение,Абонент)
	...
	|	ИЗ
	|		ДокументыНачисления КАК ДокументыНачисления
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргОбъемВременныхНачислений КАК Документ
	|			ПО ДокументыНачисления.ДокРегистратор = Документ.ДокРегистратор
	|				И (ДокументыНачисления.ДокРегистратор ССЫЛКА Документ.энргВременноеНачисление)
	|				И (Документ.Абонент = &Абонент)
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.энргВременноеНачисление КАК ШапкаДокумента
	|			ПО ДокументыНачисления.ДокРегистратор = ШапкаДокумента.Ссылка
	|				И (ДокументыНачисления.ДокРегистратор ССЫЛКА Документ.энргВременноеНачисление)) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	Организация,
	|	ПериодНачисления,
...
Функция РезультатДанныеНачислений(МВТ)
	...
	|		ДокументыДляПроведения.Абонент КАК Абонент,
	|		ДокументыДляПроведения.ДокРегистратор КАК ДокРегистратор,
	|		ОбъемВременныхНачислений.ПериодНачисления КАК ПериодРасчета,
	|		ОбъемВременныхНачислений.Поставщик КАК Поставщик,
	|		ОбъемВременныхНачислений.Услуга КАК Услуга,
	|		СУММА(ОбъемВременныхНачислений.Сумма) КАК СуммаНачислений
	|	ИЗ
	|		ДокументыДляПроведения КАК ДокументыДляПроведения
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргОбъемВременныхНачислений КАК ОбъемВременныхНачислений
	|			ПО ДокументыДляПроведения.ДокРегистратор = ОбъемВременныхНачислений.ДокРегистратор
	|				И ДокументыДляПроведения.Абонент = ОбъемВременныхНачислений.Абонент
	|				И (ОбъемВременныхНачислений.Сумма <> 0)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ДокументыДляПроведения.ДокРегистратор,
	|		ОбъемВременныхНачислений.ПериодНачисления,
	|		ОбъемВременныхНачислений.Поставщик,
	|		ОбъемВременныхНачислений.Услуга,
	|		ДокументыДляПроведения.ПериодНачисления,
	|		ДокументыДляПроведения.Абонент,
	|		ДокументыДляПроведения.Район,
