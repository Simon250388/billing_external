...
//
Процедура ОтложенныеДвиженияДокументовНачисления(Организация, ПериодНачисления, Район, МКД=Неопределено,СбрасыватьСостояниеПоследовательности=Истина) Экспорт
	...
	Результат 													= РезультатНачальныеОстаткиСтабильногоПериода(Организация, ПериодНачисления,Район,МКД,НачалоПериодаРасчета);
	...
	Пока Выборка.Следующий() цикл		
		Строение 												= ?(НЕ Выборка.ЧастныйСектор,Выборка.Строение,ПустоеСтроение);
		Если ТекСтроение <> Строение тогда  						
			Если ТекСтроение <>Неопределено тогда
				
				Пока ЕстьСледующийПоказания И ?(НЕ ВыборкаПоказания.ЧастныйСектор,ВыборкаПоказания.Строение,ПустоеСтроение) = ТекСтроение  Цикл
					...
					Если ВыборкаПоказания.ЭтоВводНачальныхОстатков Тогда
						...
						НаборМаксимальныеПоказания.Отбор.ЧастныйСектор.установить(НЕ ЗначениеЗаполнено(ТекСтроение));
						Если ЗначениеЗаполнено(ТекСтроение) Тогда 
							НаборМаксимальныеПоказания.Отбор.Строение.установить(ТекСтроение);
						КонецЕсли;
				...
				Пока ?(НЕ ВыборкаШкалы.ЧастныйСектор,ВыборкаШкалы.Строение,ПустоеСтроение) = ТекСтроение и ЕстьСледующийШКалы Цикл
				...
				Попытка 										
					Если СбрасыватьСостояниеПоследовательности тогда
						СброситьСостояниеПоследовательности("энргСтабильныеПериодыПоследовательность",	Организация,ПериодНачисления,Район,ТекСтроение);
						СброситьСостояниеПоследовательности("энргПоказанияПУПоследовательность", 		Организация,ПериодНачисления,Район,ТекСтроение);
						СброситьСостояниеПоследовательности("энргВзаиморасчетыПоследовательность",		Организация,ПериодНачисления,Район,ТекСтроение);
					КонецЕсли;
					...
					НаборОтложенные.Отбор.ЧастныйСектор.установить(НЕ ЗначениеЗаполнено(ТекСтроение));
					НаборОтложенные.отбор.Строение.установить(ТекСтроение);
		...
		ТекСтроение 											= ?(НЕ Выборка.ЧастныйСектор,Выборка.Строение,ПустоеСтроение);
	КонецЦикла;  
	...
	Если ТекСтроение <>Неопределено тогда
		...
		Пока ЕстьСледующийШКалы И ?(НЕ ВыборкаШкалы.ЧастныйСектор,ВыборкаШкалы.Строение,ПустоеСтроение) = ТекСтроение Цикл
	...
	ПересчитатьСреднийОбъемПотребленияАбонентов(Организация, ПериодНачисления,Район,Ложь);	
КонецПроцедуры
...
Процедура ПересчитатьСреднийОбъемПотребленияАбонентов(Организация, ПериодНачисления,Район,Строение=Неопределено,Помещение=Неопределено) Экспорт 
	...
	Если НЕ Строение = Неопределено Тогда
		Набор.Отбор.ЧастныйСектор.Установить(Помещение = Неопределено);
...
Процедура ВосстановитьПоследовательностьСтабильныеПериоды(Организация, ПериодНачисления,Район,Строение=Неопределено,Помещение=Неопределено) Экспорт 
	...
	Пока Выборка.следующий() цикл  		
		...
		Если НЕ ТекПомещение = Выборка.Помещение ИЛИ НЕ ТекСтроение = Выборка.Строение Тогда 						
			Если НЕ ТекПомещение = Неопределено тогда // Это не первая строка.				
				Отказ 							= ПровестиПоСтабильнымПериодам(МассивДокументов,НачалоПериодаРасчета);
				Если не Отказ тогда
		...
		|Строение,Помещение,ПериодРегистрации,Документ,МоментВремени,Дата");
	...
	Если ЕстьОшибки Тогда
		ВызватьИсключение НСтр("ru = 'Операция восстановления последовательности документов завершена с ошибками'");
	КонецЕсли;		
	...
	Пока Выборка.следующий() цикл  		
		Если ТекМКД <> Выборка.МКД тогда 						
			Если ТекМКД <> Неопределено тогда // Это не первая строка.				
				Отказ 							= ПровестиПоСтабильнымПериодамМКД(МассивДокументов,Ошибки,НачалоПериодаРасчета);
				Если не Отказ тогда
					// Если все выполнено тогда изменяем состояние в последовательности.
		...
		Если НЕ ТекЧастныйСектор = Выборка.ЧастныйСектор
			...
			Если ТекПомещение <> Неопределено тогда
				...
				Если Не Отказ Тогда
					...
					|Организация, ПериодНачисления,Район,ЧастныйСектор,Строение,Помещение,КлючПомещения,ПриборУчета,Шкала, ТарифнаяЗона,НомерПозиции,ПоказанияС,ПоказанияПО,НачалоПериодаРасчета",
					Организация, ПериодНачисления,район,ТекЧастныйСектор,ТекСтроение,ТекПомещение,ТекКлючПомещения,ТекПрибор,Текшкала,ТекТарифнаяЗона,НомерПозиции,ПоказанияС,ПоказанияПО,НачалоПериодаРасчета);
...
Процедура ВосстановитьПоследовательностьВзаиморасчеты(Организация, ПериодНачисления,район,МКД=Неопределено,Абонент=Неопределено) Экспорт 
	перем ТекАбонент,ТекМКД,СуммаАванса,МассивДокументов,ДанныеЗадолженности;
	...
	ОписаниеСтроки  									= Новый ФиксированнаяСтруктура("
	|МКД,Абонент,СостояниеПроведения,ДатаРегистратора,ДокРегистратор,Сумма,ЭтоОплата,ПериодОплатыНачало,ПериодОплатыКонец,Услуга,Кассир,ИсточникПоступления");
	...
	Пока ВыборкаДокументы.следующий() цикл 					
		...
		Если ТекАбонент <> ВыборкаДокументы.Абонент тогда	
			...
			Если ТекАбонент <> Неопределено тогда
				...
				СуммаАванса 							= АвансНаГраницу(ТекАбонент,ВыборкаАвансы);				
				...
				ПараметрыПроведения.СуммаАванса			= СуммаАванса;
...
Процедура СброситьСостояниеПоследовательности(ИмяПоследовательности,Организация,ПериодНачисления,Район,Строение=Неопределено,Помещение=Неопределено) Экспорт 
	...
	ИначеЕсли НЕ Строение = Неопределено Тогда
		Набор.Отбор.ЧастныйСектор.установить(НЕ ЗначениеЗаполнено(Строение));
		Набор.Отбор.Строение.установить(Строение); 
		Если НЕ Помещение = Неопределено тогда
			Набор.Отбор.Помещение.установить(Помещение);
		КонецЕсли;
...
Функция ПровестиПоСтабильнымПериодам(Документы,НачалоПериодаРасчета)
	...
	для Каждого ОписаниеДокумента из Документы цикл
		...
		|ссылка,ДатаДок,Организация, ПериодНачисления, РазделенныйЛС,
		...
		Документ,Дата,ОписаниеДокумента.Организация, ОписаниеДокумента.ПериодНачисления, Ложь,
		НаборСтабильныеПериоды,НаборСтабильныеПериодыШкалы,НаборСтабильныеПериодыИзмерителиНаправлений,НаборПоследнихНомеров);
		...
		Если не Параметры.РазрешенПустойНабор и НаборСтабильныеПериоды.Количество() =0 тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось восставновить последовательности документов по помещению " + Строение + "-" + Помещение +": " + Документ);
			Возврат истина;
		КонецЕсли; 		
		...
		//если разделенный ЛС, продублировать движения в Стабильные периоды по помещению
		КлючПомещенияСтруктура 			= Новый Структура("Строение, Помещение",Строение,Помещение);
		ПолучаемыеСвойства  = Новый Структура("ТипПомещения, КлючПомещенияРодителя");
		энргРаботаСЛицевымСчетомСервер.ПараметрыРасчетаПомещения(Организация,Район,КлючПомещенияСтруктура,Дата,ПолучаемыеСвойства);
		...
		Если ПолучаемыеСвойства.ТипПомещения = Перечисления.энргТипыПомещений.Комната И ПолучаемыеСвойства.КлючПомещенияРодителя <> Неопределено Тогда
			...
			ПомещениеРодителя =  ПолучаемыеСвойства.КлючПомещенияРодителя.Помещение;
			...
			ПараметрыЗапроса							= Новый Структура("
			|Организация,Район,ЧастныйСектор,Строение,Помещение,РазделенныйЛС,ПериодРегистрации",
			Организация,Район,ЧастныйСектор,Строение,ПомещениеРодителя,Помещение,Макс(ПериодРегистрации,НачалоПериодаРасчета));
			Параметры 									= Новый Структура("ПараметрыЗапроса,Отказ,МоментВремени,РазрешенПустойНабор,
			|ссылка,ДатаДок,Организация, ПериодНачисления, РазделенныйЛС,
			|НаборСтабильныеПериоды,НаборСтабильныеПериодыШкалы,НаборСтабильныеПериодыИзмерителиНаправлений,НаборПоследнихНомеров",
			ПараметрыЗапроса,Ложь,МоментВремени,Ложь,
			Документ,Дата,ОписаниеДокумента.Организация, ОписаниеДокумента.ПериодНачисления,Истина,
			НаборСтабильныеПериоды,НаборСтабильныеПериодыШкалы,НаборСтабильныеПериодыИзмерителиНаправлений,НаборПоследнихНомеров);
			
			энргРаботаСПериодомДействия.ПровестиПоСтабильнымПериодам(Документ,Параметры);
			
			Если не Параметры.РазрешенПустойНабор и НаборСтабильныеПериоды.Количество() =0 тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось восставновить последовательности документов по помещению " + Строение + "-" + ПомещениеРодителя +": " + Документ);
				Возврат истина;
			КонецЕсли; 		
...
Функция ПровестиПоСтабильнымПериодамМКД(Документы,Ошибки,НачалоПериодаРасчета)
	...
	для Каждого ОписаниеДокумента из Документы цикл
		...
		Если не Параметры.РазрешенПустойНабор и НаборСтабильныеПериоды.Количество() =0 тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось восставновить последовательности документов по мкд " + МКД +": " + Документ);
			Возврат истина;
		КонецЕсли; 		
...
Функция РезультатСреднийОбъемПотребления(МВТ,Отбор)
	...
	ИначеЕсли  НЕ Отбор.Строение  = Неопределено И ЗначениеЗаполнено(Отбор.Строение) Тогда
		...
		Запрос.УстановитьПараметр("Строение", 			Отбор.Строение);
		Если НЕ Отбор.Помещение  = Неопределено Тогда
			  Запрос.УстановитьПараметр("Помешение", 	Отбор.Помешение);
		КонецЕсли;
	КонецЕсли; 
...
Процедура ВыполнитьОтложененоеПроведение(Организация, ПериодНачисления,Район,Строение,Абонент)
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.энргОбъемНачислений КАК Документ
	|			ПО ДокументыНачисления.ДокРегистратор = Документ.Регистратор
	|				И (ДокументыНачисления.ДокРегистратор ССЫЛКА Документ.энргНачислениеПоФормуламРасчета)
	|				И (Документ.Абонент = &Абонент)
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.энргНачислениеПоФормуламРасчета КАК ШапкаДокумента
	|			ПО ДокументыНачисления.ДокРегистратор = ШапкаДокумента.Ссылка
	|				И (ДокументыНачисления.ДокРегистратор ССЫЛКА Документ.энргНачислениеПоФормуламРасчета)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	...
	|		Документ.Абонент,
	|		ДокументыНачисления.ДокРегистратор,
	|		Документ.ДатаДокументаОснования
	|	ИЗ
	|		ДокументыНачисления КАК ДокументыНачисления
	...
	Пока Выборка.Следующий() цикл		
		Попытка
			Если ТекДок <> Выборка.ДокРегистратор тогда 
				Если Абонент = Неопределено тогда
...
Процедура УменьшитьАванс(ПараметрыОперации, ДанныеНачислений)	
	...
	Если ПараметрыОперации.СпособРаспределенияОплаты = Перечисления.энргСпособыРаспределенияОплаты.ПоУслугамСогласноНастройкамРаспределения тогда
		УменьшитьАвансСогласноНастройкиРаспределения(ПараметрыОперации, ДанныеНачислений);
	иначе
		УменьшитьАвансПропорциональноЗадолженности(ПараметрыОперации, ДанныеНачислений);
	КонецЕсли;
	
КонецПроцедуры

Процедура УменьшитьАвансСогласноНастройкиРаспределения(ПараметрыОперации, ДанныеНачислений)
	
	Абонент  													= ПараметрыОперации.Абонент;
	Район  														= ПараметрыОперации.Район;
	МКД  														= ПараметрыОперации.МКД;
	ТаблицаАвансы 												= ПараметрыОперации.СтруктураТаблиц.энргАвансы;
	ТаблицаАвансыИспользованные 								= ПараметрыОперации.СтруктураТаблиц.энргАвансыИспользованные;
		...
		Если НачисленияПоУслуге <> Неопределено тогда 
			...
			для Каждого ПериодНачисления из ПериодыНачислений цикл 								
				...
				Если ПериодНачисления.СуммаНачислений < 0 тогда
					...
					ОтрицательнаяСуммаНачислений(ПараметрыОперации,-ПериодНачисления.СуммаНачислений, ВыборкаПорядок.услуга,ПериодНачисления.Поставщик,ПериодНачисления.ПериодРасчета);
...
Процедура УменьшитьАвансПропорциональноЗадолженности(ПараметрыОперации, ДанныеНачислений)
	...
	Для Сч = 0 По ПериодыЗадолженности.Количество() - 1 Цикл
		...
		Для ИндексСтроки = 0 По СтрокиВПериоде.ВГраница() Цикл			
			...
			Списываем 										= Мин(СписываемПоДоле, СтрокиВПериоде[ИндексСтроки].СуммаНачислений);
			...
			ЗаписьАвансы.МКД	  							= ПараметрыОперации.МКД;
			ЗаписьАвансы.Абонент  							= ПараметрыОперации.Абонент;
			ЗаписьАвансы.Сумма 								= Списываем;
			ЗаписьАвансы.ПериодНачисления 					= ПараметрыОперации.ПериодНачисления;
			ЗаписьАвансы.Организация 						= ПараметрыОперации.Организация;
			...
			ЗаписьОплатаУслуг.МКД	 						= ПараметрыОперации.МКД;
			ЗаписьОплатаУслуг.Абонент						= ПараметрыОперации.Абонент;
...
Процедура УвеличитьАванс(ПараметрыОперации,Сумма)
	...
	Если ПараметрыОперации.свойство("СнятьТекущие") тогда
		...
		ЗаписьАвансыИспользованные.Период  			= ПараметрыОперации.период;
		ЗаписьАвансыИспользованные.Район  			= ПараметрыОперации.Район;
		ЗаписьАвансыИспользованные.МКД	  			= ПараметрыОперации.МКД;
		ЗаписьАвансыИспользованные.Абонент  		= ПараметрыОперации.Абонент;
	...
	Иначе			
		ТаблицаАвансыПоступившие  					= ПараметрыОперации.СтруктураТаблиц.энргАвансыПоступившие;
		ЗаписьАвансыПоступившие						= ТаблицаАвансыПоступившие.добавить();
		ЗаписьАвансыПоступившие.Активность  		= истина;
		ЗаписьАвансыПоступившие.Период  			= ПараметрыОперации.период;
		ЗаписьАвансыПоступившие.Район  				= ПараметрыОперации.Район;
		ЗаписьАвансыПоступившие.МКД	  				= ПараметрыОперации.МКД;
		ЗаписьАвансыПоступившие.Абонент  			= ПараметрыОперации.Абонент;
		...
		ЗаписьАвансыПоступившие.Кассир				= ПараметрыОперации.Кассир;
		ЗаписьАвансыПоступившие.ИсточникПоступления	= ПараметрыОперации.ИсточникПоступления;
	КонецЕсли;  	 
	
КонецПроцедуры

Процедура УвеличитьЗадолженность(ПараметрыОперации, ДанныеНачислений)
	
	Если ПараметрыОперации.СуммаАванса > 0 тогда
		УменьшитьАванс(ПараметрыОперации, ДанныеНачислений);				
	КонецЕсли;
	...
	ТаблицаВзаиморасчетыПоМесяцам 						= ПараметрыОперации.СтруктураТаблиц.энргВзаиморасчетыПоМесяцам;
	...
	для Каждого СтрокаНачисления из РасшифровкаНачислений цикл
		...
		ИначеЕсли  СтрокаНачисления.СуммаНачислений < 0 тогда
			ОтрицательнаяСуммаНачислений(ПараметрыОперации,-СтрокаНачисления.СуммаНачислений,СтрокаНачисления.Услуга,СтрокаНачисления.Поставщик,СтрокаНачисления.ПериодРасчета);
			Продолжить;
		КонецЕсли;
		...
		СтрокаВзаиморасчеты.Период 						= ПараметрыОперации.Период;
		СтрокаВзаиморасчеты.Район 						= ПараметрыОперации.Район;
		СтрокаВзаиморасчеты.МКД 						= ПараметрыОперации.МКД;
		СтрокаВзаиморасчеты.Абонент 					= ПараметрыОперации.Абонент;
...
Процедура СписатьЗадолженность(ПараметрыОперации,Сумма,Услуга = Неопределено, Поставщик = Неопределено, ПериодНачисления  = Неопределено)
	
	Если Услуга <> Неопределено тогда
		СписатьЗадолженностьПоУслуге(ПараметрыОперации,Сумма,Услуга,Поставщик,ПериодНачисления);	
	иначеЕсли ПараметрыОперации.СпособРаспределенияОплаты = Перечисления.энргСпособыРаспределенияОплаты.ПоУслугамСогласноНастройкамРаспределения тогда
		СписатьЗадолженностьСогласноНастройкиРаспределения(ПараметрыОперации,Сумма);
	иначе
		СписатьЗадолженностьПропорциональноЗадолженности(ПараметрыОперации, Сумма);
	КонецЕсли;
	
КонецПроцедуры 

Процедура СписатьЗадолженностьСогласноНастройкиРаспределения(ПараметрыОперации,НадоСписать)
	
	ТаблицаВзаиморасчетыПоМесяцам 						= ПараметрыОперации.СтруктураТаблиц.энргВзаиморасчетыПоМесяцам;
	
	Если ПараметрыОперации.ЭтоОплата тогда
		ТаблицаОплатаУслуг			 					= ПараметрыОперации.СтруктураТаблиц.энргОплатаУслуг;
	иначе
		ТаблицаАвансыИспользованные 					= ПараметрыОперации.СтруктураТаблиц.энргАвансыИспользованные
	КонецЕсли;
	...
	для Каждого СтрокаЗадолженности из Расшифровка цикл
		...
		Если ПараметрыОперации.ЭтоОплата тогда 
			...
			СтрокаОплатаУслуг.Период 					= ПараметрыОперации.Период;
			СтрокаОплатаУслуг.Организация				= ПараметрыОперации.Организация;
			СтрокаОплатаУслуг.Район 					= ПараметрыОперации.Район;
			СтрокаОплатаУслуг.МКД 						= ПараметрыОперации.МКД;
			СтрокаОплатаУслуг.Абонент 					= ПараметрыОперации.Абонент;
			...
			СтрокаОплатаУслуг.ПериодНачисления			= ПараметрыОперации.ПериодРасчета;
			СтрокаОплатаУслуг.ПериодРасчета				= СтрокаЗадолженности.ПериодНачисления;
			СтрокаОплатаУслуг.Кассир					= ПараметрыОперации.Кассир;
			СтрокаОплатаУслуг.ИсточникПоступления		= ПараметрыОперации.ИсточникПоступления;
		иначе
			ТаблицаАвансыИспользованные					= ПараметрыОперации.СтруктураТаблиц.энргАвансыИспользованные;
			ЗаписьАвансыИспользованные					= ТаблицаАвансыИспользованные.добавить();
			ЗаписьАвансыИспользованные.Активность  		= истина;
			...
			ЗаписьАвансыИспользованные.ПериодНачисления	= ПараметрыОперации.ПериодРасчета;
	...
	Если НадоСписать > 0 тогда
		УвеличитьАванс(ПараметрыОперации,НадоСписать);
	КонецЕсли;

КонецПроцедуры

Процедура СписатьЗадолженностьПропорциональноЗадолженности(ПараметрыОперации, НадоСписать)
...
Процедура СписатьЗадолженностьПоУслуге(ПараметрыОперации,НадоСписать,Услуга,Поставщик,ПериодРасчета)
...
Процедура ОтрицательнаяСуммаНачислений(ПараметрыОперации,Сумма,Услуга,Поставщик,ПериодРасчета)
	...
	СписатьЗадолженность(ПараметрыОперации,Сумма,Услуга,Поставщик,ПериодРасчета);
	// Списываем задолженность по услуге за предыдущие периоды.
	СписатьЗадолженность(ПараметрыОперации,Сумма,Услуга,Поставщик,Неопределено);	
	// Списываем пропорционально и записываем в аванс если что то останется.
	СписатьЗадолженность(ПараметрыОперации,Сумма);
	
	Если Сумма > 0 Тогда
		УвеличитьАванс(ПараметрыОперации,Сумма);
	КонецЕсли;
...
Функция ТекстАвансыНаГраницу()
	...
	|		МинимумРегистратор.Район,
	|		МинимумРегистратор.МКД,
	|		МинимумРегистратор.Абонент) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстМВТ_Взаиморасчеты()
	...
	|	МинимумДатаРегистратора.СостояниеПроведения КАК СостояниеПроведения,
	...
	|			И МинимумДатаРегистратора.СостояниеПроведения = энргВзаиморасчетыПоследовательность.СостояниеПроведения
	...
	|	МинимумДатаРегистратора.СостояниеПроведения,
	...
	|	МинимумДопПорядокРегистратора.СостояниеПроведения КАК СостояниеПроведения,
	...
	|			И МинимумДопПорядокРегистратора.СостояниеПроведения = энргВзаиморасчетыПоследовательность.СостояниеПроведения
	...
	|	МинимумДопПорядокРегистратора.СостояниеПроведения,
...
Функция ПровестиВзаиморасчеты(ПараметрыПроведения,МассивДокументовНачисления)
	...
	СуммаАванса								= ПараметрыПроведения.СуммаАванса;
	...
	Исключен  								= Перечисления.энргСостояниеДокументаВПоследовательности.ИсключенИзПоследовательности;
	ПроведенВПоследоватености  				= Перечисления.энргСостояниеДокументаВПоследовательности.ПроведенВПоследовательности;
	ТекСуммаАванса 							= СуммаАванса;
	Начисления								= ПараметрыПроведения.ДанныеНачислений;
	текДокРегистратор 						= Неопределено;
	
	для Каждого Документ из МассивДокументов цикл  		
		ОчищатьСтроки 						= Истина;
		Абонент  							= Документ.Абонент;
		МКД 					 			= Документ.МКД;
		Если текДокРегистратор = Документ.ДокРегистратор Тогда
			ОчищатьСтроки 					= Ложь;
		КонецЕсли;		
		ДокРегистратор  					= Документ.ДокРегистратор;
		СуммаОперации 						= Документ.Сумма; 		 
		Период  							= Документ.ДатаРегистратора;
		Услуга  							= Документ.Услуга;
		Кассир  							= Документ.Кассир;
		ИсточникПоступления  				= Документ.ИсточникПоступления;
		...
		Если (ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргНачисление")
			...
			или ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргНачислениеПоФормуламРасчета")
			...
			СтруктураНаборов  				= МассивДокументовНачисления[ДокРегистратор].СтруктураНаборов;
			СтруктураТаблиц 				= МассивДокументовНачисления[ДокРегистратор].СтруктураТаблиц;
		иначе 			
			...
			для Каждого ИмяРегистра из МассивИмен цикл
				...
				Набор.Очистить();
				СтруктураНаборов.Вставить(ИмяРегистра,Набор);
				СтруктураТаблиц.Вставить(ИмяРегистра,Таблица);
			КонецЦикла;							
		КонецЕсли;
		
		Если ОчищатьСтроки  Тогда
			для Каждого СтрокаТаблиц из СтруктураТаблиц цикл
				Строки 						= СтрокаТаблиц.Значение.НайтиСтроки(Новый Структура("Абонент",Абонент));
		...
		|Организация, ПериодНачисления,Район,МКД,Абонент,
		|Сумма,СуммаАванса,Период,ЭтоОплата,
		|СтруктураТаблиц,ДанныеЗадолженности,
		|СпособРаспределенияОплаты",
		Организация, ПериодНачисления,район,МКД,Абонент,
		СуммаОперации,0,Период,Ложь,
		СтруктураТаблиц,ДанныеЗадолженности,
		СпособРаспределенияОплаты);
		
		Параметры.СуммаАванса 					= ТекСуммаАванса;
		
		Если Не Документ.ЭтоОплата тогда			
			ДанныеНачисления					= Начисления[ДокРегистратор];									
			Если  ДанныеНачисления <> Неопределено тогда				
				УвеличитьЗадолженность(Параметры, ДанныеНачисления);
			КонецЕсли; 		
		Иначе
			Параметры.ЭтоОплата  				= Истина;
			Параметры.Вставить("Кассир",				Кассир);
			Параметры.Вставить("ИсточникПоступления",	ИсточникПоступления);			
			СписатьЗадолженность(Параметры, Документ.Сумма); 			 			
		КонецЕсли;
		...
		ТекСуммаАванса 							= Параметры.СуммаАванса;
		...
		Если ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргНачисление")
			...
			или ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргНачислениеПоФормуламРасчета")
			или ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргПлатежноеПоручениеВходящие")
			или ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргПоступлениеОплатыЧерезКонтроллера") тогда
			
			СтруктураДанных 						= Новый Структура;
			СтруктураДанных.Вставить("СтруктураНаборов",Новый Структура("энргВзаиморасчетыПоМесяцам,энргАвансы,энргАвансыИспользованные",СтруктураНаборов.энргВзаиморасчетыПоМесяцам,СтруктураНаборов.энргАвансы,СтруктураНаборов.энргАвансыИспользованные));
			СтруктураДанных.Вставить("СтруктураТаблиц",	Новый Структура("энргАвансы,энргАвансыИспользованные,энргВзаиморасчетыПоМесяцам",СтруктураТаблиц.энргАвансы,СтруктураТаблиц.энргАвансыИспользованные,СтруктураТаблиц.энргВзаиморасчетыПоМесяцам));
			
			Если ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргПлатежноеПоручениеВходящие")
				или ТипЗнч(ДокРегистратор) = Тип("ДокументСсылка.энргПоступлениеОплатыЧерезКонтроллера") тогда
				//СтруктураНаборов
				СтруктураДанных.СтруктураНаборов.Вставить("энргОплатаУслуг",СтруктураНаборов.энргОплатаУслуг);
				СтруктураДанных.СтруктураНаборов.Вставить("энргАвансыПоступившие",СтруктураНаборов.энргАвансыПоступившие);
				//СтруктураТаблиц
				СтруктураДанных.СтруктураТаблиц.Вставить("энргОплатаУслуг",СтруктураТаблиц.энргОплатаУслуг);
				СтруктураДанных.СтруктураТаблиц.Вставить("энргАвансыПоступившие",СтруктураТаблиц.энргАвансыПоступившие);
			КонецЕсли;  	
			
			Если МассивДокументовНачисления.получить(ДокРегистратор) = Неопределено тогда				
				МассивДокументовНачисления.вставить(ДокРегистратор,СтруктураДанных);
			иначе
				МассивДокументовНачисления[ДокРегистратор] = СтруктураДанных;
			КонецЕсли;
			ПараметрыПроведения.вставить("ЕстьДокументыНачисления",Истина); 
			Продолжить;
		иначе
			для Каждого  СтрокаНабора из СтруктураНаборов цикл
				СтрокаНабора.Значение.загрузить(СтруктураТаблиц[СтрокаНабора.ключ]);
			КонецЦикла;			 			
		КонецЕсли;   	
		
		Попытка
			для Каждого СтрокаНаборов из СтруктураНаборов цикл
...
Функция ЗаписатьНаборыДокументовНачисления(Организация, ПериодНачисления,Район,Строение,ПроведенСНарушением,ПроведенВПоследовательности,МассивДокументовНачисления,Абонент)
	...
	Попытка
		для Каждого стр из МассивДокументовНачисления цикл
			...
			СтруктураНаборов  	= СтруктураДанных.СтруктураНаборов;
			СтруктураТаблиц 	= СтруктураДанных.СтруктураТаблиц;
			для Каждого Набор из СтруктураДанных.СтруктураНаборов цикл				
				Набор.Значение.загрузить(СтруктураТаблиц[Набор.ключ]);
...
Функция СтабильныеПериодыВводНачальныхОстатков(Строение, Помещение)
	...
	|ПОМЕСТИТЬ ПараметрыСтабильногоПериодаВводНачальныхОстатков
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВложенныйЗапрос.Организация КАК Организация,
	|		ВложенныйЗапрос.ПериодНачисления КАК ПериодНачисления,
	...
	|						ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргКлючиПомещений КАК энргКлючиПомещений
	|						ПО ВложенныйЗапрос.КлючПомещения = энргКлючиПомещений.КлючПомещения) КАК ВложенныйЗапрос
	...
	|		СГРУППИРОВАТЬ ПО
	|			энргУстановленныеПараметрыРасчетаНормативаСрезПоследних.Услуга,
	|			энргУстановленныеПараметрыРасчетаНормативаСрезПоследних.ТипНачисления) КАК ВложенныйЗАпрос
	...
	|	ВложенныйЗапрос.КоэффициентРасчета КАК КоэффициентРасчета,
	...
	|	ВложенныйЗапрос.ФиксированныйОбъемПотребления КАК ФиксированныйОбъемПотребления,
	|	ВложенныйЗапрос.ДоляПотребленияЧислитель КАК ДоляПотребленияЧислитель,
	|	ВложенныйЗапрос.ДоляПотребленияЗнаменатель КАК ДоляПотребленияЗнаменатель,
	|	ВложенныйЗапрос.КоэффициентРаспределенияОстатка КАК КоэффициентРаспределенияОстатка,
	|	ВложенныйЗапрос.СоставнаяУслуга КАК СоставнаяУслуга,
	|	ЗНАЧЕНИЕ(Справочник.энргНаправлениеИспользованияТочекУчета.ПустаяСсылка) КАК НаправлениеИспользованияТУ,
	|	ЛОЖЬ КАК ЯвляетсяПомещениемСРазделеннымиЛС
	|ПОМЕСТИТЬ СтабильныеПериодыВводНачальныхОстатков
	|ИЗ
	|	(ВЫБРАТЬ
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Организация КАК Организация,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ПериодНачисления КАК ПериодНачисления,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Район КАК Район,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ЧастныйСектор КАК ЧастныйСектор,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Строение КАК Строение,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Помещение КАК Помещение,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Абонент КАК Абонент,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Услуга КАК Услуга,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ТочкаУчета КАК ТочкаУчета,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.НомерПозиции КАК НомерПозиции,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ПериодРегистрации КАК ПериодРегистрации,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ДатаРегистратора КАК ДатаРегистратора,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ДокРегистратор КАК ДокРегистратор,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ЛСОткрыт КАК ЛСОткрыт,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.УслугаПодключена КАК УслугаПодключена,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ПриборВведен КАК ПриборВведен,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ПриборУчета КАК ПриборУчета,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоличествоПрописанных КАК КоличествоПрописанных,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоличествоСобственников КАК КоличествоСобственников,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоличествоПроживающих КАК КоличествоПроживающих,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоличествоЖильцов КАК КоличествоЖильцов,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоличествоКомнат КАК КоличествоКомнат,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ЗначениеПлощади КАК ЗначениеПлощади,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоэффициентПУ КАК КоэффициентПУ,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.РасчетПоФормуле КАК РасчетПоФормуле,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ВидБлагоустройства КАК ВидБлагоустройства,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ТарифнаяГруппа КАК ТарифнаяГруппа,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ДатаПоверки КАК ДатаПоверки,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ОтключатьВНеполивнойСезон КАК ОтключатьВНеполивнойСезон,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Поставщик КАК Поставщик,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ДатаСостоянияПрибора КАК ДатаСостоянияПрибора,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.НетВозможностьУстановкиПУ КАК НетВозможностьУстановкиПУ,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.IDСтрокиТочкиУчета КАК IDСтрокиТочкиУчета,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ВидСобственности КАК ВидСобственности,
	|		-1 КАК ГодПостройки,
	|		-1 КАК КоличествоЭтажей,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ЗависимаяТочкаУчета КАК ЗависимаяТочкаУчета,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоэффициентРасчета КАК КоэффициентРасчета,
	...
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.Шкала КАК Шкала,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ТипСхемы КАК ТипСхемы,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ВидТепловогоПункта КАК ВидТепловогоПункта,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.СпособРаспределенияОбъемаПомещения КАК СпособРаспределенияОбъемаПомещения,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ФиксированныйОбъемПотребления КАК ФиксированныйОбъемПотребления,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ДоляПотребленияЧислитель КАК ДоляПотребленияЧислитель,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.ДоляПотребленияЗнаменатель КАК ДоляПотребленияЗнаменатель,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.КоэффициентРаспределенияОстатка КАК КоэффициентРаспределенияОстатка,
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков.СоставнаяУслуга КАК СоставнаяУслуга
	|	ИЗ
	|		ПараметрыСтабильногоПериодаВводНачальныхОстатков КАК ПараметрыСтабильногоПериодаВводНачальныхОстатков
	|			ЛЕВОЕ СОЕДИНЕНИЕ РазрезыНормативаУслуг КАК РазрезыНормативаУслуг
	|			ПО ПараметрыСтабильногоПериодаВводНачальныхОстатков.Услуга = РазрезыНормативаУслуг.Услуга) КАК ВложенныйЗапрос
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.энргКлючиНормативов КАК энргКлючиНормативов
	|		ПО (ВЫБОР
	|				КОГДА ВложенныйЗапрос.ЕстьРазрезВидБлагоустройства
	...
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ВложенныйЗапрос.ЕстьРазрезВидСобственности
	...
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ВложенныйЗапрос.ЕстьРазрезГодПостройки
	|					ТОГДА ВложенныйЗапрос.ГодПостройки = энргКлючиНормативов.ГодПостройки
	|				ИНАЧЕ энргКлючиНормативов.ГодПостройки = 0
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ВложенныйЗапрос.ЕстьРазрезКоличествоКомнат
	|					ТОГДА ВЫБОР
	...
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ВложенныйЗапрос.ЕстьРазрезКоличествоПрописанных
	|					ТОГДА ВЫБОР
	...
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ВложенныйЗапрос.ЕстьРазрезКоличествоЭтажей
	|					ТОГДА ВЫБОР
...
Функция СтабильныеПериодыДокументыНачисления(Строение, Помещение)
	...
	|	&ПериодНачисления КАК ПериодНачисления,
	|	0 КАК НомерПозиции,
	| "+ энргНачисления.СформироватьСтрокуДляЗапроса("энргСтабильныеПериоды","СтабильныеПериоды","Организация,ПериодНачисления,НомерПозиции") + "
	...
	Если Строение = Неопределено Тогда
		Текст = СтрЗаменить(Текст,"И энргНомераСтрокСтабильногоПериода.МКД = &МКД","");
		Текст = СтрЗаменить(Текст,"И (СтабильныеПериоды.МКД = &МКД)","");
		Текст = СтрЗаменить(Текст,"И (СтабильныеПериодыШкалы.МКД = &МКД)","");
	КонецЕсли;
	
	Если Помещение = Неопределено Тогда
		Текст = СтрЗаменить(Текст,"И энргНомераСтрокСтабильногоПериода.Абонент = &Абонент","");
		Текст = СтрЗаменить(Текст,"И (СтабильныеПериоды.Абонент = &Абонент)","");
		Текст = СтрЗаменить(Текст,"И (СтабильныеПериодыШкалы.Абонент = &Абонент)","");
	КонецЕсли;
...
Функция РезультатНачальныеОстаткиСтабильногоПериодаМКД(Организация, ПериодНачисления,Район,Строение,ПериодРегистрации)
	...
	|	0 КАК НомерПозиции,	
	|" + энргНачисления.СформироватьСтрокуДляЗапроса("энргСтабильныеПериодыМКДШкалы","ПараметрыШкалСтаблиногоПериода","Организация,ПериодНачисления,ДатаРегистратора,ДокРегистратор,НомерПозиции") + "
	|ИЗ
	|	ДокументыНачальныхОстатков КАК ДокументыНачальныхОстатков
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПараметрыШкалСтаблиногоПериода КАК ПараметрыШкалСтаблиногоПериода
...
Функция СтабильныеПериодыДокументыНачисленияМКД(Строение)
	...
	|	&Организация КАК Организация,
	|	&ПериодНачисления КАК ПериодНачисления,
	|" + энргНачисления.СформироватьСтрокуДляЗапроса("энргСтабильныеПериодыМКДШкалы","СтабильныеПериодыШкалы","Организация,ПериодНачисления,НомерПозиции,ДатаРегистратора,ДокРегистратор") + "
	|		
	|ПОМЕСТИТЬ ПараметрыШкалСтаблиногоПериода
	|ИЗ
	|	НомерСтрок КАК НомерСтрок
...
Функция РезультатаЗапросаСреднегоПотребленияАбонентов(Организация, ПериодНачисления,Район,Строение,Помещение)
	...
	Если НЕ Строение = Неопределено Тогда
		Запрос.УстановитьПараметр("ЧастныйСектор",	Помещение = Неопределено);
		Запрос.УстановитьПараметр("Строение",		Строение);
		Если НЕ Помещение = Неопределено Тогда
			Запрос.УстановитьПараметр("Помещение",	Помещение);
		Иначе	
...
Функция ТекстЗапросаСреднийОбъемАбонентов()
	...
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(энргРасчетСреднегоОбъема.ПериодНачисления, МЕСЯЦ)) >= МАКСИМУМ(СтабильныеПериоды.КоличествоПериодов)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				И (энргРасчетСреднегоОбъема.ПериодНачисления <= ДОБАВИТЬКДАТЕ(ТочкиДляРасчета.КонецПериода, МЕСЯЦ, -6 + 1))
	...
	|				И ТочкиДляРасчета.Шкала = энргРасчетСреднегоОбъема.Шкала
	|				И ТочкиДляРасчета.ТарифнаяЗона = энргРасчетСреднегоОбъема.ТарифнаяЗона
	|				И (энргРасчетСреднегоОбъема.ПериодНачисления <= ДОБАВИТЬКДАТЕ(ТочкиДляРасчета.КонецПериода, МЕСЯЦ, -3 + 1))
	|				И (энргРасчетСреднегоОбъема.ПериодНачисления >= ДОБАВИТЬКДАТЕ(ТочкиДляРасчета.КонецПериода, МЕСЯЦ, -5 + 1))
	|				И (энргРасчетСреднегоОбъема.Организация = &Организация)
	|				И (энргРасчетСреднегоОбъема.Район = &Район)
	|				И (энргРасчетСреднегоОбъема.ЧастныйСектор = &ЧастныйСектор)
	|				И (энргРасчетСреднегоОбъема.Строение = &Строение)
