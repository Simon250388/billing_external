...
Процедура ВосстановитьПоследовательностьВзаиморасчеты(Организация, ПериодНачисления,Район,МКД=Неопределено,Абонент=Неопределено) Экспорт 
	...
	РезультатОплата										= РезультатОплата(МВТ);
	...
	Если НЕ Отбор.Строение = Неопределено тогда
		...
		Если ЗначениеЗаполнено(Отбор.Строение) Тогда
			...
			Если НЕ Отбор.Помещение = Неопределено Тогда  				
				...
				СтруктураКлючПомещения = РегистрыСведений.энргКлючиПомещений.Получить(Новый Структура("Строение, Помещение", Отбор.Строение, Отбор.Помещение));
...
Функция СтабильныеПериодыВводНачальныхОстатков(Строение, Помещение)
	...
	|ПОМЕСТИТЬ ВсеПараметрыРасчетаБезФормул
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВложенныйЗапрос.Организация КАК Организация,
	|		ВложенныйЗапрос.ПериодНачисления КАК ПериодНачисления,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВсеПараметрыРасчетаБезФормул.Организация КАК Организация,
	|	ВсеПараметрыРасчетаБезФормул.ПериодНачисления КАК ПериодНачисления,
	|	ВсеПараметрыРасчетаБезФормул.Район КАК Район,
	|	ВсеПараметрыРасчетаБезФормул.ЧастныйСектор КАК ЧастныйСектор,
	|	ВсеПараметрыРасчетаБезФормул.Строение КАК Строение,
	|	ВсеПараметрыРасчетаБезФормул.Помещение КАК Помещение,
	|	ВсеПараметрыРасчетаБезФормул.Абонент КАК Абонент,
	|	ВсеПараметрыРасчетаБезФормул.Услуга КАК Услуга,
	|	энргВводНачальныхОстатковФормулыРасчета.ТочкаУчета КАК ТочкаУчета,
	|	ВсеПараметрыРасчетаБезФормул.НомерПозиции КАК НомерПозиции,
	|	ВсеПараметрыРасчетаБезФормул.ПериодРегистрации КАК ПериодРегистрации,
	|	ВсеПараметрыРасчетаБезФормул.ДатаРегистратора КАК ДатаРегистратора,
	|	ВсеПараметрыРасчетаБезФормул.ДокРегистратор КАК ДокРегистратор,
	|	ВсеПараметрыРасчетаБезФормул.ЛСОткрыт КАК ЛСОткрыт,
	|	ВсеПараметрыРасчетаБезФормул.УслугаПодключена КАК УслугаПодключена,
	|	ВсеПараметрыРасчетаБезФормул.ПриборВведен КАК ПриборВведен,
	|	ВсеПараметрыРасчетаБезФормул.ПриборУчета КАК ПриборУчета,
	|	ВсеПараметрыРасчетаБезФормул.КоличествоПрописанных КАК КоличествоПрописанных,
	|	ВсеПараметрыРасчетаБезФормул.КоличествоСобственников КАК КоличествоСобственников,
	|	ВсеПараметрыРасчетаБезФормул.КоличествоПроживающих КАК КоличествоПроживающих,
	|	ВсеПараметрыРасчетаБезФормул.КоличествоЖильцов КАК КоличествоЖильцов,
	|	ВсеПараметрыРасчетаБезФормул.КоличествоКомнат КАК КоличествоКомнат,
	|	ВсеПараметрыРасчетаБезФормул.ЗначениеПлощади КАК ЗначениеПлощади,
	|	ВсеПараметрыРасчетаБезФормул.КоэффициентПУ КАК КоэффициентПУ,
	|	ЛОЖЬ КАК РасчетПоФормуле,
	|	ВсеПараметрыРасчетаБезФормул.ВидБлагоустройства КАК ВидБлагоустройства,
	|	ВсеПараметрыРасчетаБезФормул.ТарифнаяГруппа КАК ТарифнаяГруппа,
	|	ВсеПараметрыРасчетаБезФормул.ДатаПоверки КАК ДатаПоверки,
	|	ВсеПараметрыРасчетаБезФормул.ОтключатьВНеполивнойСезон КАК ОтключатьВНеполивнойСезон,
	|	ВсеПараметрыРасчетаБезФормул.Поставщик КАК Поставщик,
	|	ВсеПараметрыРасчетаБезФормул.ДатаСостоянияПрибора КАК ДатаСостоянияПрибора,
	|	ВсеПараметрыРасчетаБезФормул.НетВозможностьУстановкиПУ КАК НетВозможностьУстановкиПУ,
	|	ВсеПараметрыРасчетаБезФормул.IDСтрокиТочкиУчета КАК IDСтрокиТочкиУчета,
	|	ВсеПараметрыРасчетаБезФормул.ВидСобственности КАК ВидСобственности,
	|	ВсеПараметрыРасчетаБезФормул1.ТочкаУчета КАК ЗависимаяТочкаУчета,
	|	ЕСТЬNULL(энргВводНачальныхОстатковФормулыРасчета.Коэффициент, ВсеПараметрыРасчетаБезФормул.КоэффициентФормула) КАК КоэффициентФормула,
	|	ВсеПараметрыРасчетаБезФормул.Шкала КАК Шкала,
	|	ВсеПараметрыРасчетаБезФормул.ТипСхемы КАК ТипСхемы,
	|	ВсеПараметрыРасчетаБезФормул.ВидТепловогоПункта КАК ВидТепловогоПункта,
	|	ВсеПараметрыРасчетаБезФормул.СпособРаспределенияОбъемаПомещения КАК СпособРаспределенияОбъемаПомещения,
	|	ВсеПараметрыРасчетаБезФормул.ФиксированныйОбъемПотребления КАК ФиксированныйОбъемПотребления,
	|	ВсеПараметрыРасчетаБезФормул.ДоляПотребленияЧислитель КАК ДоляПотребленияЧислитель,
	|	ВсеПараметрыРасчетаБезФормул.ДоляПотребленияЗнаменатель КАК ДоляПотребленияЗнаменатель,
	|	ВсеПараметрыРасчетаБезФормул.КоэффициентРаспределенияОстатка КАК КоэффициентРаспределенияОстатка,
	|	ВсеПараметрыРасчетаБезФормул.СоставнаяУслуга КАК СоставнаяУслуга,
	|	ВсеПараметрыРасчетаБезФормул.ГодПостройки КАК ГодПостройки,
	|	ВсеПараметрыРасчетаБезФормул.КоличествоЭтажей КАК КоличествоЭтажей,
	|	ВсеПараметрыРасчетаБезФормул.НаправлениеИспользованияТУ КАК НаправлениеИспользованияТУ,
	|	ВсеПараметрыРасчетаБезФормул.БлокировочныйАкт КАК БлокировочныйАкт,
	|	ВсеПараметрыРасчетаБезФормул.ИспользоватьДиаметрСечения КАК ИспользоватьДиаметрСечения,
	|	ВсеПараметрыРасчетаБезФормул.ДиаметрСечения КАК ДиаметрСечения,
	|	ВсеПараметрыРасчетаБезФормул.Техническая КАК Техническая,
	|	ВсеПараметрыРасчетаБезФормул.ТочкаУчетаРодитель КАК ТочкаУчетаРодитель
	|ПОМЕСТИТЬ ТочкиПоФормулам
	|ИЗ
	|	ВсеПараметрыРасчетаБезФормул КАК ВсеПараметрыРасчетаБезФормул
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.энргВводНачальныхОстатков.ФормулыРасчета КАК энргВводНачальныхОстатковФормулыРасчета
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВсеПараметрыРасчетаБезФормул КАК ВсеПараметрыРасчетаБезФормул1
	|			ПО (ВсеПараметрыРасчетаБезФормул1.IDСтрокиТочкиУчета = энргВводНачальныхОстатковФормулыРасчета.IDСтрокиТочкиУчета)
	|				И (ВсеПараметрыРасчетаБезФормул1.ДокРегистратор = энргВводНачальныхОстатковФормулыРасчета.Ссылка)
	|		ПО ВсеПараметрыРасчетаБезФормул.ТочкаУчета = энргВводНачальныхОстатковФормулыРасчета.ТочкаУчета
	|			И ВсеПараметрыРасчетаБезФормул.ДокРегистратор = энргВводНачальныхОстатковФормулыРасчета.Ссылка
	|			И (ВЫБОР
	|				КОГДА ВсеПараметрыРасчетаБезФормул.СоставнаяУслуга = ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.Пустаяссылка)
	|					ТОГДА ВсеПараметрыРасчетаБезФормул.Услуга = энргВводНачальныхОстатковФормулыРасчета.Услуга
	|				ИНАЧЕ ВсеПараметрыРасчетаБезФормул.СоставнаяУслуга = энргВводНачальныхОстатковФормулыРасчета.Услуга
	|			КОНЕЦ)
	|			И ВсеПараметрыРасчетаБезФормул.КлючПомещения = энргВводНачальныхОстатковФормулыРасчета.КлючПомещения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ВсеПараметрыРасчетаБезФормул.Абонент КАК Абонент,
	|	ВсеПараметрыРасчетаБезФормул.Услуга КАК Услуга,
	|	ВсеПараметрыРасчетаБезФормул.ТочкаУчета КАК ТочкаУчета,
	|	ВсеПараметрыРасчетаБезФормул.НомерПозиции КАК НомерПозиции,
	|	ВсеПараметрыРасчетаБезФормул.ПериодРегистрации КАК ПериодРегистрации,
	|	ВсеПараметрыРасчетаБезФормул.ДатаРегистратора КАК ДатаРегистратора,
	|	ВсеПараметрыРасчетаБезФормул.ДокРегистратор КАК ДокРегистратор,
	...
	|	ВсеПараметрыРасчетаБезФормул.КоэффициентПУ КАК КоэффициентПУ,
	|	ВЫБОР
	|		КОГДА ТочкиПоФормулам.ТочкаУчета ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|	ВсеПараметрыРасчетаБезФормул.IDСтрокиТочкиУчета КАК IDСтрокиТочкиУчета,
	|	ВсеПараметрыРасчетаБезФормул.ВидСобственности КАК ВидСобственности,
	|	ВсеПараметрыРасчетаБезФормул.ЗависимаяТочкаУчета КАК ЗависимаяТочкаУчета,
	|	ВсеПараметрыРасчетаБезФормул.КоэффициентФормула КАК КоэффициентФормула,
	|	ВсеПараметрыРасчетаБезФормул.Шкала КАК Шкала,
	|	ВсеПараметрыРасчетаБезФормул.ТипСхемы КАК ТипСхемы,
	|	ВсеПараметрыРасчетаБезФормул.ВидТепловогоПункта КАК ВидТепловогоПункта,
	|	ВсеПараметрыРасчетаБезФормул.СпособРаспределенияОбъемаПомещения КАК СпособРаспределенияОбъемаПомещения,
	...
	|ИЗ
	|	ВсеПараметрыРасчетаБезФормул КАК ВсеПараметрыРасчетаБезФормул
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТочкиПоФормулам КАК ТочкиПоФормулам
	|		ПО ВсеПараметрыРасчетаБезФормул.ДокРегистратор = ТочкиПоФормулам.ДокРегистратор
	|			И ВсеПараметрыРасчетаБезФормул.ТочкаУчета = ТочкиПоФормулам.ЗависимаяТочкаУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТочкиПоФормулам КАК ТочкиПоФормулам1
	|		ПО ВсеПараметрыРасчетаБезФормул.ТочкаУчета = ТочкиПоФормулам1.ТочкаУчета
	|			И ВсеПараметрыРасчетаБезФормул.Услуга = ТочкиПоФормулам1.Услуга
	|			И ВсеПараметрыРасчетаБезФормул.ДокРегистратор = ТочкиПоФормулам1.ДокРегистратор
	|			И ВсеПараметрыРасчетаБезФормул.ПериодНачисления = ТочкиПоФормулам1.ПериодНачисления
	|			И ВсеПараметрыРасчетаБезФормул.ЧастныйСектор = ТочкиПоФормулам1.ЧастныйСектор
	|			И ВсеПараметрыРасчетаБезФормул.Строение = ТочкиПоФормулам1.Строение
	|			И ВсеПараметрыРасчетаБезФормул.Помещение = ТочкиПоФормулам1.Помещение
	|ГДЕ
	|	ТочкиПоФормулам1.ТочкаУчета ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТочкиПоФормулам.Организация,
	|	ТочкиПоФормулам.ПериодНачисления,
	|	ТочкиПоФормулам.Район,
	|	ТочкиПоФормулам.ЧастныйСектор,
	|	ТочкиПоФормулам.Строение,
	|	ТочкиПоФормулам.Помещение,
	|	ТочкиПоФормулам.Абонент,
	|	ТочкиПоФормулам.Услуга,
	|	ТочкиПоФормулам.ТочкаУчета,
	|	ТочкиПоФормулам.НомерПозиции,
	|	ТочкиПоФормулам.ПериодРегистрации,
	|	ТочкиПоФормулам.ДатаРегистратора,
	|	ТочкиПоФормулам.ДокРегистратор,
	|	ТочкиПоФормулам.ЛСОткрыт,
	|	ТочкиПоФормулам.УслугаПодключена,
	|	ТочкиПоФормулам.ПриборВведен,
	|	ТочкиПоФормулам.ПриборУчета,
	|	ТочкиПоФормулам.КоличествоПрописанных,
	|	ТочкиПоФормулам.КоличествоСобственников,
	|	ТочкиПоФормулам.КоличествоПроживающих,
	|	ТочкиПоФормулам.КоличествоЖильцов,
	|	ТочкиПоФормулам.КоличествоКомнат,
	|	ТочкиПоФормулам.ЗначениеПлощади,
	|	ТочкиПоФормулам.КоэффициентПУ,
	|	ТочкиПоФормулам.РасчетПоФормуле,
	|	ТочкиПоФормулам.ВидБлагоустройства,
	|	ТочкиПоФормулам.ТарифнаяГруппа,
	|	ТочкиПоФормулам.ДатаПоверки,
	|	ТочкиПоФормулам.ОтключатьВНеполивнойСезон,
	|	ТочкиПоФормулам.Поставщик,
	|	ТочкиПоФормулам.ДатаСостоянияПрибора,
	|	ТочкиПоФормулам.НетВозможностьУстановкиПУ,
	|	ТочкиПоФормулам.IDСтрокиТочкиУчета,
	|	ТочкиПоФормулам.ВидСобственности,
	|	ТочкиПоФормулам.ЗависимаяТочкаУчета,
	|	ТочкиПоФормулам.КоэффициентФормула,
	|	ТочкиПоФормулам.Шкала,
	|	ТочкиПоФормулам.ТипСхемы,
	|	ТочкиПоФормулам.ВидТепловогоПункта,
	|	ТочкиПоФормулам.СпособРаспределенияОбъемаПомещения,
	|	ТочкиПоФормулам.ФиксированныйОбъемПотребления,
	|	ТочкиПоФормулам.ДоляПотребленияЧислитель,
	|	ТочкиПоФормулам.ДоляПотребленияЗнаменатель,
	|	ТочкиПоФормулам.КоэффициентРаспределенияОстатка,
	|	ТочкиПоФормулам.СоставнаяУслуга,
	|	ТочкиПоФормулам.ГодПостройки,
	|	ТочкиПоФормулам.КоличествоЭтажей,
	|	ТочкиПоФормулам.НаправлениеИспользованияТУ,
	|	ТочкиПоФормулам.БлокировочныйАкт,
	|	ТочкиПоФормулам.ИспользоватьДиаметрСечения,
	|	ТочкиПоФормулам.ДиаметрСечения,
	|	ТочкиПоФормулам.Техническая,
	|	ТочкиПоФормулам.ТочкаУчетаРодитель
	|ИЗ
	|	ТочкиПоФормулам КАК ТочкиПоФормулам
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаСреднийОбъемАбонентов()
	...
	|		СтабильныеПериоды.Услуга,
	|		энргРасчетСреднегоОбъема.Шкала,
	|		энргРасчетСреднегоОбъема.ТарифнаяЗона) КАК ВложенныйЗапрос
	|ГДЕ
	|	ВЫБОР
	...
	|		ТочкиДляРасчета.ТарифнаяЗона,
	|		ТочкиДляРасчета.КонецПериода,
	|		ТочкиДляРасчета.ЭтоОбъемПотребленный) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Район,
	|	ВложенныйЗапрос.ЧастныйСектор,
...
Функция ТекстЗапросаСреднийОбъемМКД()
	...
	|		ТочкиДляРасчета.Шкала,
	|		ТочкиДляРасчета.ТарифнаяЗона,
	|		МАКСИМУМ(энргРасчетСреднегоОбъемаМКД.ПериодНачисления),
	|		МИНИМУМ(энргРасчетСреднегоОбъемаМКД.ПериодНачисления)
	|	ИЗ
	|		ТочкиДляРасчета КАК ТочкиДляРасчета
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргРасчетСреднегоОбъемаМКД КАК энргРасчетСреднегоОбъемаМКД
