...
Функция РезультатаЗапросаСреднегоПотребленияАбонентов(Организация, ПериодНачисления,Район,Строение,Помещение)
	...
	ПараметрыДлительности.Вставить("Период", Новый Структура("ИмяПараметра","НачалоГода"));
	ПараметрыДлительности.Вставить("ПараметрыВТ", ПараметрыВТ);
	РегистрыСведений.энргДлительностиПериодов.ПериодыСезонности(Схема,ПараметрыДлительности,"ПериодыСезонностиНачалоГода"); 
	Запрос 											= Новый Запрос(Схема.ПолучитьТекстЗапроса());
	Запрос.МенеджерВременныхТаблиц 					= МВТ;
	Запрос.УстановитьПараметр("Организация", 				Организация);
	Запрос.УстановитьПараметр("Район", 						Район);
	Запрос.УстановитьПараметр("НачалоГода", 				НачалоГода(ПериодНачисления));
	Запрос.УстановитьПараметр("ПериодНачисления", 			НачалоГода(ПериодНачисления));
	Запрос.УстановитьПараметр("НачалоПериодаРасчета", 		ГраницыПериода.НачалоОП);
	Запрос.УстановитьПараметр("ЗавершениеПериодаРасчета", 	ГраницыПериода.ЗавершениеОП);
	Запрос.Выполнить();
	...
	ПараметрыВТ  									= Новый Массив;
	ПараметрыВТ.Добавить(Новый Структура("ТипПараметра, СоотвествиеПолей", "ТипПараметраРавно", СоотвествиеПолей));
	ПараметрыДлительности.Вставить("Период", Новый Структура("ИмяПараметра","ПериодНачисления"));
	ПараметрыДлительности.Вставить("ПараметрыВТ", ПараметрыВТ);
	РегистрыСведений.энргДлительностиПериодов.ПериодыСезонности(Схема,ПараметрыДлительности,"ПериодыСезонностиПериодНачисления"); 
	Запрос 											= Новый Запрос(Схема.ПолучитьТекстЗапроса());
	Запрос.МенеджерВременныхТаблиц 					= МВТ;
	Запрос.УстановитьПараметр("Организация", 				Организация);
	Запрос.УстановитьПараметр("Район", 						Район);
	...
	Если НЕ Строение = Неопределено Тогда
		...
		Если ЧастныйСектор Тогда
			...
			Текст 		= СтрЗаменить(Текст,"МКД = &Строение","ЛОЖЬ");
	...
	|					И (ВЫБОР
	|						КОГДА ПериодыРасчетаСреднего.ДатаСостоянияЛицевогоСчета = ДАТАВРЕМЯ(1, 1, 1)
	|							ТОГДА ИСТИНА
	|						ИНАЧЕ НАЧАЛОПЕРИОДА(ПериодыРасчетаСреднего.ДатаСостоянияЛицевогоСчета, МЕСЯЦ) <= энргРасчетСреднегоОбъема.ПериодНачисления
	|					КОНЕЦ)
...
Функция ТекстЗапросаСреднийОбъемАбонентов()
	...
	|	энргНастройкиРасчетаСреднегоПотребленияСрезПоследних.НеПрименятьСезонность КАК НеПрименятьСезонность,
	|	энргНастройкиРасчетаСреднегоПотребленияСрезПоследних.НеПрименятьСезонностьПотребленный КАК НеПрименятьСезонностьПотребленный
	...
	|	ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)) КАК КонецСезонностиНачалоГода,
	|	ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)) КАК НачалоСезонностиТекущийПериод,
	|	ВЫБОР
	|		КОГДА ВложенныйЗапрос.Сезонность = ЗНАЧЕНИЕ(Справочник.энргВидыСезонности.ПустаяСсылка)
	|			ТОГДА 0
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)) > ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1))
	|					ТОГДА РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), МЕСЯЦ)
	|				ИНАЧЕ РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), МЕСЯЦ)
	|			КОНЕЦ - 1
	|	КОНЕЦ КАК КоличествоМесяцевИсключая
	...
	|			РАЗНОСТЬДАТ(МАКСИМУМ(энргРасчетСреднегоОбъема.ПериодНачисления), &ПериодНачисления, МЕСЯЦ) КАК СрокНеПредоставления,
	...
	|				энргСтабильныеПериоды.ДатаСостоянияЛицевогоСчета КАК ДатаСостоянияЛицевогоСчета,
	...
	|						И (энргСтабильныеПериоды.ПриборВведен)) КАК СтабильныеПериоды
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргРасчетСреднегоОбъема КАК энргРасчетСреднегоОбъема
	...
	|			РАЗНОСТЬДАТ(МАКСИМУМ(энргРасчетСреднегоОбъема.ПериодНачисления), &ПериодНачисления, МЕСЯЦ),
	...
	|				И (НЕ ВложенныйЗапрос.ЗаПрошлыйГод)) КАК ВложенныйЗапрос
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПериодыСезонностиНачалоГода КАК ПериодыСезонностиНачалоГода
	|		ПО ВложенныйЗапрос.Район = ПериодыСезонностиНачалоГода.Район
	|			И ВложенныйЗапрос.Сезонность = ПериодыСезонностиНачалоГода.Сезонность
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПериодыСезонностиПериодНачисления КАК ПериодыСезонностиПериодНачисления
	|		ПО ВложенныйЗапрос.Район = ПериодыСезонностиПериодНачисления.Район
	|			И ВложенныйЗапрос.Сезонность = ПериодыСезонностиПериодНачисления.Сезонность
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ВложенныйЗапрос.ЗаПрошлыйГод
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ВложенныйЗапрос.КоличествоПредоставленных >= 1
	|					И ВложенныйЗапрос.СрокНеПредоставления - ВЫБОР
	|						КОГДА ВложенныйЗапрос.Сезонность = ЗНАЧЕНИЕ(Справочник.энргВидыСезонности.ПустаяСсылка)
	|							ТОГДА 0
	|						КОГДА ВложенныйЗапрос.КонецПериода МЕЖДУ ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)) И ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1))
	|								И (&НачалоПериодаРасчета МЕЖДУ ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)) И ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)))
	|							ТОГДА 0
	|						ИНАЧЕ ВЫБОР
	|								КОГДА ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)) > ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1))
	|									ТОГДА РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), МЕСЯЦ)
	|								ИНАЧЕ РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), МЕСЯЦ)
	|							КОНЕЦ - 1
	|					КОНЕЦ <= ВложенныйЗапрос.НачислятьСреднийОбъемНеБолееМесяцев
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	ВложенныйЗапрос.КоличествоПоказаний КАК КоличествоПоказаний,
	|	ВЫБОР
	|		КОГДА РАЗНОСТЬДАТ(ВложенныйЗапрос.НачалоПериода, ВложенныйЗапрос.КонецПериода, МЕСЯЦ) + 1 >= ВложенныйЗапрос.КоличествоПоказаний
	|			ТОГДА РАЗНОСТЬДАТ(ВложенныйЗапрос.НачалоПериода, ВложенныйЗапрос.КонецПериода, МЕСЯЦ) + 1
	|		КОГДА РАЗНОСТЬДАТ(ВложенныйЗапрос.ДатаСостоянияПрибора, ВложенныйЗапрос.КонецПериода, МЕСЯЦ) + 1 >= ВложенныйЗапрос.КоличествоПоказаний
	|			ТОГДА РАЗНОСТЬДАТ(ВложенныйЗапрос.ДатаСостоянияПрибора, ВложенныйЗапрос.КонецПериода, МЕСЯЦ) + 1
	|		ИНАЧЕ -1
	|	КОНЕЦ КАК КоличествоМесяцев
	...
	|			ТочкиДляРасчета.КонецПериода,
	|			ТочкиДляРасчета.ЭтоОбъемПотребленный,
	|			НАЧАЛОПЕРИОДА(ТочкиДляРасчета.ДатаСостоянияПрибора, МЕСЯЦ),
	|			ТочкиДляРасчета.Организация,
	|			ТочкиДляРасчета.НаправлениеИспользованияТУ,
	|			ТочкиДляРасчета.Сезонность,
	|			ТочкиДляРасчета.КоличествоПоказаний
