...
Функция СтабильныеПериодыДокументыНачисления(Строение, Помещение)
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.энргПриборыТочекУчета.СрезПоследних(
	|					&ПериодРегистрации,
	|					(Организация, Район, ТочкаУчета) В
	|						(ВЫБРАТЬ
	|							НомерСтрок.Организация КАК Организация,
	|							НомерСтрок.Район КАК Район,
	|							НомерСтрок.ТочкаУчета КАК ТочкаУчета
	|						ИЗ
	|							НомерСтрок КАК НомерСтрок)) КАК энргПриборыТочекУчетаСрезПоследних
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргСостояниеПриборовУчетаАбонента.СрезПоследних(
	|						&ПериодРегистрации,
	|						(Организация, Район, ТочкаУчета) В
	|							(ВЫБРАТЬ
	|								НомерСтрок.Организация КАК Организация,
	...
	|								НомерСтрок.ТочкаУчета КАК ТочкаУчета
	|							ИЗ
	|								НомерСтрок КАК НомерСтрок)) КАК энргСостояниеПриборовУчетаАбонентаСрезПоследних
	|				ПО энргПриборыТочекУчетаСрезПоследних.Организация = энргСостояниеПриборовУчетаАбонентаСрезПоследних.Организация
	|					И энргПриборыТочекУчетаСрезПоследних.Район = энргСостояниеПриборовУчетаАбонентаСрезПоследних.Район
	|					И энргПриборыТочекУчетаСрезПоследних.КлючПомещения = энргСостояниеПриборовУчетаАбонентаСрезПоследних.КлючПомещения
	|					И энргПриборыТочекУчетаСрезПоследних.ТочкаУчета = энргСостояниеПриборовУчетаАбонентаСрезПоследних.ТочкаУчета
	|					И энргПриборыТочекУчетаСрезПоследних.ПриборУчета = энргСостояниеПриборовУчетаАбонентаСрезПоследних.ПриборУчета
	|					И (энргСостояниеПриборовУчетаАбонентаСрезПоследних.Состояние = ЗНАЧЕНИЕ(Перечисление.энргСостояниеПриборовУчета.Ввод))
	|			ПО СтабильныеПериоды.Организация = энргПриборыТочекУчетаСрезПоследних.Организация
	|				И СтабильныеПериоды.Район = энргПриборыТочекУчетаСрезПоследних.Район
	|				И СтабильныеПериоды.Абонент.КлючПомещения = энргПриборыТочекУчетаСрезПоследних.КлючПомещения
	|				И СтабильныеПериоды.ТочкаУчета = энргПриборыТочекУчетаСрезПоследних.ТочкаУчета
	...
	// Исправление ошибки при получении прибора учета
	Текст = СтрЗаменить(Текст, "СтабильныеПериоды.ПриборУчета КАК ПриборУчета,", "ЕСТЬNULL(энргПриборыТочекУчетаСрезПоследних.ПриборУчета, СтабильныеПериоды.ПриборУчета) КАК ПриборУчета,");
	Возврат Текст;
...
Функция ТекстЗапросаСреднийОбъемАбонентов()
	...
	|	МАКСИМУМ(СтабильныеПериоды.КоличествоПоказанийОбъемПотребленный) КАК КоличествоПоказанийОбъемПотребленный,
	|	МАКСИМУМ(СтабильныеПериоды.КоличествоПериодовОбъемПотребленный) КАК КоличествоПериодовОбъемПотребленный,
	|	МАКСИМУМ(СтабильныеПериоды.ЗаПрошлыйГодОбъемПотребленный) КАК ЗаПрошлыйГодОбъемПотребленный,
	|	МАКСИМУМ(СтабильныеПериоды.ПоВсемНачислениямОбъемПотребленный) КАК ПоВсемНачислениямОбъемПотребленный
	...
	|ИМЕЮЩИЕ
	|	(ВЫБОР
	|			КОГДА МАКСИМУМ(СтабильныеПериоды.ЗаПрошлыйГод)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(энргРасчетСреднегоОбъема.ПериодНачисления, МЕСЯЦ)) >= МАКСИМУМ(СтабильныеПериоды.КоличествоПоказаний)
	|					И РАЗНОСТЬДАТ(МАКСИМУМ(энргРасчетСреднегоОбъема.ПериодНачисления), &ПериодНачисления, МЕСЯЦ) <= МАКСИМУМ(СтабильныеПериоды.НачислятьСреднийОбъемНеБолееМесяцев)
	|		КОНЕЦ
	|		ИЛИ ВЫБОР
	|			КОГДА МАКСИМУМ(СтабильныеПериоды.ЗаПрошлыйГодОбъемПотребленный)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(энргРасчетСреднегоОбъема.ПериодНачисления, МЕСЯЦ)) >= МАКСИМУМ(СтабильныеПериоды.КоличествоПоказанийОбъемПотребленный)
	|					И РАЗНОСТЬДАТ(МАКСИМУМ(энргРасчетСреднегоОбъема.ПериодНачисления), &ПериодНачисления, МЕСЯЦ) <= МАКСИМУМ(СтабильныеПериоды.НачислятьСреднийОбъемНеБолееМесяцевОбъемПотребленный)
	|		КОНЕЦ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	МИНИМУМ(ВложенныйЗапрос.НачалоПериодаОбъемПотребленный) КАК НачалоПериодаОбъемПотребленный
	...
	|		ДАТАВРЕМЯ(3999, 12, 31, 23, 59, 59) КАК НачалоПериодаОбъемПотребленный
	|	ИЗ
	...
	|				И ТочкиДляРасчета.Шкала = энргРасчетСреднегоОбъема.Шкала
	|				И ТочкиДляРасчета.ТарифнаяЗона = энргРасчетСреднегоОбъема.ТарифнаяЗона
	|				И (энргРасчетСреднегоОбъема.ПериодНачисления <= ДОБАВИТЬКДАТЕ(ТочкиДляРасчета.КонецПериода, МЕСЯЦ, -ТочкиДляРасчета.КоличествоПериодовОбъемПотребленный + 1))
	|				И (энргРасчетСреднегоОбъема.Организация = &Организация)
	|				И (энргРасчетСреднегоОбъема.Район = &Район)
	|				И (энргРасчетСреднегоОбъема.ЧастныйСектор = &ЧастныйСектор)
	|				И (энргРасчетСреднегоОбъема.Строение = &Строение)
	|				И (энргРасчетСреднегоОбъема.Помещение = &Помещение)
	|				И (НЕ ТочкиДляРасчета.ЗаПрошлыйГодОбъемПотребленный)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ТочкиДляРасчета.Район,
	|		ТочкиДляРасчета.ЧастныйСектор,
	...
	|				И ТочкиДляРасчета.Шкала = энргРасчетСреднегоОбъема.Шкала
	|				И ТочкиДляРасчета.ТарифнаяЗона = энргРасчетСреднегоОбъема.ТарифнаяЗона
	|				И (энргРасчетСреднегоОбъема.ПериодНачисления <= ДОБАВИТЬКДАТЕ(ТочкиДляРасчета.КонецПериода, МЕСЯЦ, -ТочкиДляРасчета.КоличествоПоказанийОбъемПотребленный + 1))
	|				И (энргРасчетСреднегоОбъема.ПериодНачисления >= ДОБАВИТЬКДАТЕ(ТочкиДляРасчета.КонецПериода, МЕСЯЦ, -ТочкиДляРасчета.КоличествоПериодовОбъемПотребленный + 1 + 1))
	|				И (энргРасчетСреднегоОбъема.Организация = &Организация)
	|				И (энргРасчетСреднегоОбъема.Район = &Район)
	|				И (энргРасчетСреднегоОбъема.ЧастныйСектор = &ЧастныйСектор)
	|				И (энргРасчетСреднегоОбъема.Строение = &Строение)
	...
	|				И (энргРасчетСреднегоОбъема.Строение = &Строение)
	|				И (энргРасчетСреднегоОбъема.Помещение = &Помещение)
	|				И (ТочкиДляРасчета.ЗаПрошлыйГодОбъемПотребленный)
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ТочкиДляРасчета.Район,
	|		ТочкиДляРасчета.ЧастныйСектор,
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ВложенныйЗапрос.КолВоМесяцевОбъемПотребленный < 0
	|				ТОГДА -1
	|			КОГДА ВложенныйЗапрос.КолВоМесяцевОбъемПотребленный = 0
	|				ТОГДА 0
	|			ИНАЧЕ ВложенныйЗапрос.ОбъемУслугиПотребленный / ВложенныйЗапрос.КолВоМесяцевОбъемПотребленный
	|		КОНЕЦ КАК ЧИСЛО(15, 6)) КАК ОбъемПотребленный
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВложенныйЗапрос.Район КАК Район,
	|		ВложенныйЗапрос.ЧастныйСектор КАК ЧастныйСектор,
	...
	|		ВложенныйЗапрос.Шкала КАК Шкала,
	|		ВложенныйЗапрос.ТарифнаяЗона КАК ТарифнаяЗона,
	|		МАКСИМУМ(ВложенныйЗапрос.ОбъемУслуги) КАК ОбъемУслуги,
	|		МАКСИМУМ(ВложенныйЗапрос.ОбъемУслугиПотребленный) КАК ОбъемУслугиПотребленный,
	|		МАКСИМУМ(ВложенныйЗапрос.КолВоМесяцев) КАК КолВоМесяцев,
	|		МАКСИМУМ(ВложенныйЗапрос.КолВоМесяцевОбъемПотребленный) КАК КолВоМесяцевОбъемПотребленный
	|	ИЗ
	|		(ВЫБРАТЬ
	...
	|			ВЫБОР
	|				КОГДА ПериодыРасчетаСреднего.НачалоПериода = ДАТАВРЕМЯ(3999, 12, 31, 23, 59, 59)
	|					ТОГДА -1
	|				ИНАЧЕ РАЗНОСТЬДАТ(ПериодыРасчетаСреднего.НачалоПериода, ПериодыРасчетаСреднего.КонецПериода, МЕСЯЦ) + 1
	|			КОНЕЦ КАК КолВоМесяцев,
	|			-1 КАК КолВоМесяцевОбъемПотребленный
	|		ИЗ
	...
	|			ПериодыРасчетаСреднего.ТарифнаяЗона,
	|			0,
	|			СУММА(энргРасчетСреднегоОбъема.ОбъемУслуги),
	|			-1,
	|			ВЫБОР
	|				КОГДА ПериодыРасчетаСреднего.НачалоПериодаОбъемПотребленный = ДАТАВРЕМЯ(3999, 12, 31, 23, 59, 59)
	|					ТОГДА -1
	|				ИНАЧЕ РАЗНОСТЬДАТ(ПериодыРасчетаСреднего.НачалоПериодаОбъемПотребленный, ПериодыРасчетаСреднего.КонецПериода, МЕСЯЦ) + 1
	|			КОНЕЦ
	|		ИЗ
	|			ПериодыРасчетаСреднего КАК ПериодыРасчетаСреднего
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргРасчетСреднегоОбъема КАК энргРасчетСреднегоОбъема
	...
	|					И ПериодыРасчетаСреднего.Шкала = энргРасчетСреднегоОбъема.Шкала
	|					И ПериодыРасчетаСреднего.ТарифнаяЗона = энргРасчетСреднегоОбъема.ТарифнаяЗона
	|					И (энргРасчетСреднегоОбъема.ПериодНачисления МЕЖДУ ПериодыРасчетаСреднего.НачалоПериодаОбъемПотребленный И ПериодыРасчетаСреднего.КонецПериода)
	|					И (энргРасчетСреднегоОбъема.Организация = &Организация)
	|					И (энргРасчетСреднегоОбъема.Район = &Район)
	|					И (энргРасчетСреднегоОбъема.ЧастныйСектор = &ЧастныйСектор)
	|					И (энргРасчетСреднегоОбъема.Строение = &Строение)
