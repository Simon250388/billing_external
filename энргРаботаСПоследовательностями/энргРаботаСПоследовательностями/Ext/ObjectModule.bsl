...
Функция ТекстЗапросаСреднийОбъемМКД()
	...
	|			НастройкиРасчетаСреднегоПотребления.НачислятьСреднийОбъемНеБолееМесяцев КАК НачислятьСреднийОбъемНеБолееМесяцев,
	|			ВЫБОР
	|				КОГДА СтабильныеПериоды.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|							КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	|								ТОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27)
	|							ИНАЧЕ ДЕНЬ(КОНЕЦПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	|						КОНЕЦ)
	|					ТОГДА НАЧАЛОПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ)
	|				ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), МЕСЯЦ, 1)
	|			КОНЕЦ КАК ДатаСостоянияПрибора,
	|			СтабильныеПериоды.Организация КАК Организация
	|		ИЗ
	...
	|				ЛЕВОЕ СОЕДИНЕНИЕ ДатыОтчетногоПериодыОбщиеНастройки КАК ДатыОтчетногоПериодыОбщиеНастройки
	|				ПО СтабильныеПериоды.Организация = ДатыОтчетногоПериодыОбщиеНастройки.Организация
	|		ГДЕ
	|			РАЗНОСТЬДАТ(ВЫБОР
	|						КОГДА СтабильныеПериоды.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|									КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	...
	|				ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(СтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), МЕСЯЦ, 1)
	|			КОНЕЦ,
	|			СтабильныеПериоды.Организация) КАК СтабильныеПериоды
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргРасчетСреднегоОбъемаМКД КАК РасчетСреднегоОбъема
