...
Функция ПровестиПоказанияПриборовУчета(ПараметрыПроведений,МассивДокументов,ДанныеСреднегоОбъема,ДанныеМаксимальногоПотребления)
	...
	Попытка
		...
		для Каждого Выборка из МассивДокументов цикл
			...
			Если Константы.энргРасчетПриОтклоненииПоказанийПоСреднемуИлиНормативу.Получить() и ВСрок и СостояниеПоказаний = Перечисления.энргСостояниеПоказаний.Отклонены Тогда
				Продолжить;
			КонецЕсли;
