...
Процедура ВосстановитьПоследовательностьВзаиморасчеты(Организация, ПериодНачисления,Район,МКД=Неопределено,Абонент=Неопределено) Экспорт 
	...
	ОписаниеСтроки  									= Новый ФиксированнаяСтруктура("
	|МКД,Абонент,СостояниеПроведения,ДатаРегистратора,ДокРегистратор,ЭтоОплата,Кассир,ИсточникПоступления");
...
Процедура УвеличитьЗадолженность(ПараметрыОперации, ДанныеДокумента, ДанныеНачислений)
	...
	Если ПараметрыОперации.СуммаАванса > 0 тогда
		Если ДанныеДокумента.ПерерасчетКорректировкаОплаты Тогда
	...
	для Каждого СтрокаНачисления из РасшифровкаНачислений цикл
		...
		Если СтрокаНачисления.СуммаНачислений = 0 ИЛИ (ДанныеДокумента.ПерерасчетКорректировкаОплаты И СтрокаНачисления.ПериодРасчета > ПараметрыОперации.ПериодНачисления) тогда
			Продолжить;
...
Функция ПровестиВзаиморасчеты(ПараметрыПроведения,МассивДокументовНачисления)
	...
	для Каждого Документ из МассивДокументов цикл  		
		...
		ДанныеДокумента.Вставить("ПерерасчетКорректировкаОплаты",	Ложь);
				
		Если ТипЗнч(Документ.ДокРегистратор) = Тип("ДокументСсылка.энргПерерасчет") и НЕ Документ.ЭтоОплата Тогда
			Если  Документ.ДокРегистратор.ВидОперации = Перечисления.энргВидыОпераций.энргПерерасчет_КорректировкаОплаты Тогда
				ДанныеДокумента.ПерерасчетКорректировкаОплаты = Истина;
			КонецЕсли;
			...
			КонецЕсли;
			//Если Параметры.Авансы.Количество() > 0 и не ДанныеДокумента.ПерерасчетКорректировкаОплаты тогда
