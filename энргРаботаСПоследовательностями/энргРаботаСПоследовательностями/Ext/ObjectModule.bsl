...
Функция СтабильныеПериодыДокументыНачисления(Строение, Помещение)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПараметрыСтаблиногоПериода.Организация КАК Организация,
	|	ПараметрыСтаблиногоПериода.Район КАК Район,
	|	ПараметрыСтаблиногоПериода.Абонент КАК Абонент,
	|	ПараметрыСтаблиногоПериода.Строение КАК Строение,
	|	ПараметрыСтаблиногоПериода.Помещение КАК Помещение,
	|	ПараметрыСтаблиногоПериода.ТочкаУчета КАК ТочкаУчета,
	|	ПараметрыСтаблиногоПериода.Услуга КАК Услуга,
	|	МАКСИМУМ(ПараметрыСтаблиногоПериода.ДатаСостоянияПрибора) КАК ДатаСостоянияПрибора,
	|	ЕСТЬNULL(энргДиаметрСеченияТочекУчетаСрезПоследних.Использовать, ЛОЖЬ) КАК ИспользоватьДиаметрСечения,
	|	ЕСТЬNULL(энргДиаметрСеченияТочекУчетаСрезПоследних.ДиаметрСечения, 0) КАК ДиаметрСечения
	|ПОМЕСТИТЬ ВТ_ПарамСечения
	|ИЗ
	|	ПараметрыСтаблиногоПериода КАК ПараметрыСтаблиногоПериода
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргДиаметрСеченияТочекУчета.СрезПоследних(
	|				,
	...
	|						ПараметрыСтаблиногоПериода.ТочкаУчета КАК ТочкаУчета
	|					ИЗ
	|						ПараметрыСтаблиногоПериода КАК ПараметрыСтаблиногоПериода)) КАК энргДиаметрСеченияТочекУчетаСрезПоследних
	|		ПО ПараметрыСтаблиногоПериода.Организация = энргДиаметрСеченияТочекУчетаСрезПоследних.Организация
	|			И ПараметрыСтаблиногоПериода.Район = энргДиаметрСеченияТочекУчетаСрезПоследних.Район
	|			И ПараметрыСтаблиногоПериода.ТочкаУчета = энргДиаметрСеченияТочекУчетаСрезПоследних.ТочкаУчета
	|ГДЕ
	|	ПараметрыСтаблиногоПериода.ИспользоватьДиаметрСечения
	|
	|СГРУППИРОВАТЬ ПО
	|	ПараметрыСтаблиногоПериода.Организация,
	|	ПараметрыСтаблиногоПериода.Район,
	|	ПараметрыСтаблиногоПериода.Абонент,
	|	ПараметрыСтаблиногоПериода.Строение,
	|	ПараметрыСтаблиногоПериода.Помещение,
	|	ПараметрыСтаблиногоПериода.ТочкаУчета,
	|	ПараметрыСтаблиногоПериода.Услуга,
	|	ЕСТЬNULL(энргДиаметрСеченияТочекУчетаСрезПоследних.Использовать, ЛОЖЬ),
	|	ЕСТЬNULL(энргДиаметрСеченияТочекУчетаСрезПоследних.ДиаметрСечения, 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|				КОНЕЦ)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ПарамСечения КАК ВТ_ПарамСечения
	|		ПО ПараметрыСтаблиногоПериода.Организация = ВТ_ПарамСечения.Организация
	|			И ПараметрыСтаблиногоПериода.Район = ВТ_ПарамСечения.Район
	|			И ПараметрыСтаблиногоПериода.Абонент = ВТ_ПарамСечения.Абонент
	|			И ПараметрыСтаблиногоПериода.Строение = ВТ_ПарамСечения.Строение
	|			И ПараметрыСтаблиногоПериода.Помещение = ВТ_ПарамСечения.Помещение
	|			И ПараметрыСтаблиногоПериода.ТочкаУчета = ВТ_ПарамСечения.ТочкаУчета
	|			И ПараметрыСтаблиногоПериода.Услуга = ВТ_ПарамСечения.Услуга
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////	
	|ВЫБРАТЬ
	...
	КонецЕсли;
	Текст = СтрЗаменить(Текст,"ПараметрыСтаблиногоПериода.ИспользоватьДиаметрСечения КАК ИспользоватьДиаметрСечения", "ЕСТЬNULL(ВТ_ПарамСечения.ИспользоватьДиаметрСечения, ЛОЖЬ) КАК ИспользоватьДиаметрСечения");
	Текст = СтрЗаменить(Текст,"ПараметрыСтаблиногоПериода.ДиаметрСечения КАК ДиаметрСечения", "ЕСТЬNULL(ВТ_ПарамСечения.ДиаметрСечения, 0) КАК ДиаметрСечения");
	Текст = СтрЗаменить(Текст,"ПараметрыСтаблиногоПериода.ДатаСостоянияПрибора КАК ДатаСостоянияПрибора", "ЕСТЬNULL(ВТ_ПарамСечения.ДатаСостоянияПрибора,ПараметрыСтаблиногоПериода.ДатаСостоянияПрибора) КАК ДатаСостоянияПрибора");
