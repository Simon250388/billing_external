...
Функция ПровестиПоСтабильнымПериодам(Документы,НачалоПериодаРасчета)
	...
	для Каждого ОписаниеДокумента из Документы цикл
		...
		ПомещениеСРазделеннымиЛС                    = Ложь;
				
		Если ЯвляетсяПомещениемСРазделеннымиЛС(Помещение) Тогда
			ПомещениеСРазделеннымиЛС = Истина;
		КонецЕсли;			
		...
		Если не Параметры.РазрешенПустойНабор и НаборСтабильныеПериоды.Количество() =0 тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не удалось восстановить последовательности документов по помещению " + Строение + "-" + Помещение +": " + Документ);
			Возврат истина;
		КонецЕсли; 		
...
Функция ЯвляетсяПомещениемСРазделеннымиЛС(Помещение)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	энргПомещения.Родитель = &Помещение
	...
	Если РезультатЗапроса.Пустой() Тогда Возврат Ложь
