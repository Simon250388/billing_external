...
Функция РезультатЗапросаСтабильныеПериоды(Отбор) 
	...
	|				И НЕ ПомещенияСРазделеннымиЛС.Помещение ЕСТЬ NULL
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК СортировкаПоУслуге,
	|	ВЫБОР
	|		КОГДА СтабильныеПериоды.ДокРегистратор.КлючПомещения.Помещение = СтабильныеПериоды.Помещение
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК СортировкаПоПомещению
	|ИЗ
	|	СтабильныеПериоды КАК СтабильныеПериоды
...
Функция РезультатОплата(МВТ)
	...
	|			И ДокументыДляПроведения.ДатаРегистратора = ОплатаПоУслугам.ДатаПлатежногоДокумента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
