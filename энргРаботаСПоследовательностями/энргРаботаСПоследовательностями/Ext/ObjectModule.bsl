...
Процедура ВосстановитьПоследовательностьВзаиморасчеты(Организация, ПериодНачисления,Район,МКД=Неопределено,Абонент=Неопределено) Экспорт 
	...
	РезультатДанныеНачисленийПени						= РезультатДанныеНачисленийПени(МВТ);
	ВыборкаДанныеНачисленийПени							= РезультатДанныеНачисленийПени.выбрать();
	...
	ПустойРезультатНачисленияПени						= Истина;
	...
	Если ВыборкаДокументы.количество() > 0 тогда
		...
		ПустойРезультатНачисленияПени 					= ВыборкаДанныеНачисленийПени.следующий();
	...
	ПараметрыПроведения.Вставить("ДанныеНачисленийПени"					, Новый Соответствие);
	...
	Пока ВыборкаДокументы.следующий() цикл 					
		...
		Если ТекАбонент <> ВыборкаДокументы.Абонент тогда	
			...
			Если ТекАбонент <> Неопределено тогда
				...
				КоллекцияВыборок 			= Новый Структура(
				"ВыборкаДанныеНачислений,ВыборкаДанныеНачисленийПени,ВыборкаАвансы,ВыборкаЗадолженность,ВыборкаЗадолженностьПени,ВыборкаОплата"
				,ВыборкаДанныеНачислений,ВыборкаДанныеНачисленийПени,ВыборкаАвансы,ВыборкаЗадолженность,ВыборкаЗадолженностьПени,ВыборкаОплата); 				
				КоллекцияПустыхРезультатов 	= Новый Структура("ПустойРезультатОплаты", ПустойРезультатОплаты);
				ДопПараметры 				= Новый Структура("МассивДокументов", МассивДокументов);
				
				ЗаполнитьСвойстваПараметрыПроведенияПоследовательностиВзаиморасчетов(ПараметрыПроведения, ТекАбонент, КоллекцияВыборок, КоллекцияПустыхРезультатов, ДопПараметры);
...
Процедура ЗаполнитьСвойстваПараметрыПроведенияПоследовательностиВзаиморасчетов(ПараметрыПроведения, ТекАбонент, КоллекцияВыборок, КоллекцияПустыхРезультатов, ДопПараметры)
	ДанныеНачислений 							= ДанныеНачислений(ТекАбонент, КоллекцияВыборок.ВыборкаДанныеНачислений);
	ДанныеНачисленийПени						= ДанныеНачисленийПени(ТекАбонент, КоллекцияВыборок.ВыборкаДанныеНачисленийПени);
	ДанныеАвансы 								= АвансНаГраницу(ТекАбонент, КоллекцияВыборок.ВыборкаАвансы);			
	ДанныеЗадолженности  						= ЗадолженностьНаГраницу(ТекАбонент, КоллекцияВыборок.ВыборкаЗадолженность);
	ДанныеЗадолженностиПени 					= ЗадолженностьНаГраницуПени(ТекАбонент, КоллекцияВыборок.ВыборкаЗадолженностьПени);
	
	ПараметрыПроведения.ДанныеОплаты 			= Новый Соответствие;
	Если Не КоллекцияПустыхРезультатов.ПустойРезультатОплаты Тогда
		ПараметрыПроведения.ДанныеОплаты 	= ДанныеОплаты(ТекАбонент, КоллекцияВыборок.ВыборкаОплата); 
	КонецЕсли;
	
	ПараметрыПроведения.МассивДокументов		= ДопПараметры.МассивДокументов;
	...
	ПараметрыПроведения.ДанныеНачисленийПени 	= ДанныеНачисленийПени;
КонецПроцедуры
...
Функция РезультатЗапросаПоказанияПУ(МВТ,Отбор,НачалоПериодаРасчета) 
	...
	|		МАКСИМУМ(энргПоказанияПУПоследовательность.Разделитель) КАК Разделитель
	|	ИЗ
	|		МинимальныйРегистратор КАК МинимальныйРегистратор
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргПоказанияПУПоследовательность КАК энргПоказанияПУПоследовательность
...
Функция ПровестиПоказанияПриборовУчета(ПараметрыПроведений,МассивДокументов,ДанныеСреднегоОбъема,ДанныеМаксимальногоПотребления)
	...
	Попытка
		...
		Если ТипЗнч(РегистраторМаксимальных) = Тип("ДокументСсылка.энргОперацияСУслугами")
			И РегистраторМаксимальных.Видоперации  = Перечисления.энргВидыОпераций.энргОперацияСУслугами_ВозобновлениеРежимаПотребления тогда
		...
		для Каждого Выборка из МассивДокументов цикл
			...
			ЭтоОткрытие 							= Ложь;
			ЭтоВозобновлениеУслуги					= Ложь;
			ПоказанияДоВозобновленияУслуги			= ПредыдущиеМакс;
			...
			ИначеЕсли ТипЗнч(Выборка.ДокРегистратор) = Тип("ДокументСсылка.энргОперацияСУслугами")	тогда
				Если Выборка.ДокРегистратор.Видоперации  = Перечисления.энргВидыОпераций.энргОперацияСУслугами_ПодключениеУслуги тогда
					...
					ЭтоМаксимальные 				= Истина;
					ЭтоОткрытие 					= Истина;
				ИначеЕсли Выборка.ДокРегистратор.Видоперации  = Перечисления.энргВидыОпераций.энргОперацияСУслугами_ВозобновлениеРежимаПотребления тогда
					ЭтоВозобновлениеУслуги			= Истина;	
				КонецЕсли;
			...
			Если  СостояниеПоказаний = ПоказанияПриняты ИЛИ ВСрок Тогда
				...
				Если ЭтоВозобновлениеУслуги И СостояниеПоказаний = ПоказанияПриняты Тогда
					...
					Иначе
						СтрокаНабор.Показания  				= ?(ПоказанияДоВозобновленияУслуги < 0, 0, ПоказанияДоВозобновленияУслуги);
					КонецЕсли;
					СтрокаНабор.ВСрок  						= ВСрок;
					СтрокаНабор.ПоказанияТекущегоПериода	= Ложь;
				...
				КонецЕсли;
				Если ЭтоВозобновлениеУслуги И СостояниеПоказаний = ПоказанияПриняты Тогда
					СтрокаНабор.Разделитель		  		= Выборка.Разделитель + 1;
				иначе
					СтрокаНабор.Разделитель		  		= Выборка.Разделитель;
				КонецЕсли; 				
				Если ЭтоВозобновлениеУслуги И НЕ СостояниеПоказаний = ПоказанияПриняты Тогда
					СтрокаНабор.ПоказанияТекущегоПериода	= Ложь;
				иначе
...
Функция ПровестиВзаиморасчеты(ПараметрыПроведения,МассивДокументовНачисления)
	...
	НачисленияПени								= ПараметрыПроведения.ДанныеНачисленийПени;
	Оплата										= ПараметрыПроведения.ДанныеОплаты[Перечисления.энргВидыЗадолженности.ОсновнойДолг];
	ОплатаЗадолженностиПени						= ПараметрыПроведения.ДанныеОплаты[Перечисления.энргВидыЗадолженности.Пеня];// новое распределение оплаты
	...
	для Каждого Документ из МассивДокументов цикл 
		...
		ИначеЕсли ТипЗнчРегистратора = Тип("ДокументСсылка.энргПерерасчет") и Документ.ЭтоОплата Тогда
			Если ДанныеДокумента.ЭтоКорректировкаОплаты Тогда
				...
				СторнироватьАвансыИОплату(Параметры, ДанныеДокумента,ДанныеНачисления);
			...
			Если НЕ ОплатаЗадолженностиПени = Неопределено Тогда 
				ДанныеОплатыПени					= ОплатаЗадолженностиПени[текДокРегистратор];// новое распределение оплаты
				РаспределитьОплатуПени(Параметры, ДанныеОплатыПени, ДанныеДокумента);
			КонецЕсли;
			
			ДанныеОплаты						= Оплата[текДокРегистратор];// новое распределение оплаты
			РаспределитьОплату(Параметры, ДанныеОплаты, ДанныеДокумента);
		...
		ИначеЕсли Не Документ.ЭтоОплата тогда			
			...
			ДанныеНачисленияПени				= НачисленияПени[текДокРегистратор];									
			Если ДанныеНачисленияПени <> Неопределено тогда				
				УвеличитьЗадолженностьПени(Параметры, ДанныеДокумента, ДанныеНачисленияПени);
			КонецЕсли;
		Иначе
...
#Область УвеличениеЗадолженностиПени

Процедура УвеличитьЗадолженностьПени(ПараметрыОперации, ДанныеДокумента, ДанныеНачисленийПени)
	...
	Абонент  											= ДанныеДокумента.Абонент;	
	МКД  												= ДанныеДокумента.МКД;
	ДанныеЗадолженности 								= ПараметрыОперации.ДанныеЗадолженностиПени;
	ТаблицаВзаиморасчетыПоМесяцам 						= ДанныеДокумента.НаборТаблиц.энргВзаиморасчетыПоПени;
		
	ТаблицаЗадолженности  								= ДанныеЗадолженности.Расшифровка;
	РасшифровкаНачислений 								= ДанныеНачисленийПени.Расшифровка;
	
	РасшифровкаНачислений.Сортировать("СуммаНачислений, ПериодРасчета, ПериодНачисления");
	//// сначала отрицательные строки записываем в аванс
	//для Каждого СтрокаНачисления из РасшифровкаНачислений цикл
	//	
	//	Если СтрокаНачисления.СуммаНачислений = 0 ИЛИ (ДанныеДокумента.ЭтоКорректировкаОплаты И СтрокаНачисления.ПериодРасчета > ПараметрыОперации.ПериодНачисления) тогда
	...
	Если БылиИзменения Тогда
		ТаблицаЗадолженности.Свернуть("ПериодРасчета,ПериодНачисления,ПорядокПериодРасчета,ПорядокПогашенияУслуги,Поставщик,Услуга","СуммаЗадолженности");
		ТаблицаЗадолженности.Сортировать("ПорядокПериодРасчета,ПериодРасчета,ПериодНачисления,ПорядокПогашенияУслуги,Услуга");
		ДанныеЗадолженности.СуммаЗадолженности 			= ТаблицаЗадолженности.Итог("СуммаЗадолженности");
	КонецЕсли;
...
Процедура СписатьЗадолженностьПениПропорциональноЗадолженности(ПараметрыОперации, ДанныеДокумента, НадоСписать)
	...
	ПериодыЗадолженности 									= ДанныеЗадолженности.Скопировать();
	ПериодыЗадолженности.Свернуть("ПериодНачисления,ПериодРасчета","СуммаЗадолженности");
	ПериодыЗадолженности.Сортировать("ПериодРасчета,ПериодНачисления"); 	
	
	ТаблицаВзаиморасчетыПоПени 							= ДанныеДокумента.НаборТаблиц.энргВзаиморасчетыПоПени;
	ТаблицаОплатаПени 									= ДанныеДокумента.НаборТаблиц.энргОплатаПени;
		
	СтрокиУдалить 											= Новый Массив;
	
	Для Сч = 0 По ПериодыЗадолженности.Количество() - 1 Цикл
		//СтрокиВПериоде 										= ДанныеЗадолженности.НайтиСтроки(Новый Структура("ПериодРасчета", ПериодыЗадолженности[Сч].ПериодРасчета));
		СтрокиВПериоде 										= ДанныеЗадолженности.НайтиСтроки(Новый Структура("ПериодНачисления", ПериодыЗадолженности[Сч].ПериодНачисления));
		ОстатокСуммы 										= НадоСписать;
		
		Для ИндексСтроки = 0 По СтрокиВПериоде.ВГраница() Цикл			
			ОбщаяСуммаЗаПериод								= СтрокиВПериоде[ИндексСтроки].СуммаЗадолженности;//ПериодыЗадолженности[Сч].СуммаЗадолженности;
			Если ОбщаяСуммаЗаПериод = 0 Тогда
				Продолжить;
			КонецЕсли;
			...
			Если ДанныеДокумента.ЭтоОплата тогда						
				СтрокаОплатаУслуг 							= ТаблицаОплатаПени.добавить();
				СтрокаОплатаУслуг.Активность				= истина;  				
				СтрокаОплатаУслуг.Организация				= ПараметрыОперации.Организация;
				СтрокаОплатаУслуг.Район 					= ПараметрыОперации.Район;
				СтрокаОплатаУслуг.Период 					= ДанныеДокумента.Период;
				...
				СтрокаОплатаУслуг.ПериодРасчета 			= СтрокиВПериоде[ИндексСтроки].ПериодРасчета;				
			иначе
...
Процедура РаспределитьОплатуПени(Параметры, ДанныеОплатыПени, ДанныеДокумента)
	Если ДанныеОплатыПени = Неопределено Тогда 
		Возврат;
	КонецЕсли;
	
	РасшифровкаОплаты = ДанныеОплатыПени.Расшифровка;
	Для Каждого СтрокаОплаты Из РасшифровкаОплаты Цикл
		СписатьЗадолженностьПениПоМесяцам(Параметры, ДанныеДокумента, СтрокаОплаты.Сумма, СтрокаОплаты.Услуга, СтрокаОплаты.Поставщик, СтрокаОплаты.ПериодВзаиморасчетов);
	КонецЦикла; 	
КонецПроцедуры


Процедура СписатьЗадолженностьПениПоМесяцам(ПараметрыОперации, ДанныеДокумента, НадоСписать, Услуга, Поставщик, ПериодРасчета)
	перем яПериодРасчета, яУслуга, яПоставщик;
	ДанныеЗадолженности 						= ПараметрыОперации.ДанныеЗадолженностиПени.Расшифровка;		
...
Процедура СторнироватьАвансыИОплату(ПараметрыОперации, ДанныеДокумента, ДанныеНачислений)	
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ТекСтрока Из ПараметрыОперации.Авансы Цикл
			Если ТекСтрока.ДокументОплаты = Выборка.ДокументОплаты Тогда 
...
Функция ДанныеНачисленийПени(Абонент, ВыборкаДанныеНачислений)
	перем ТекРасшифровкаНачислений;	
...
Функция ЗадолженностьНаГраницуПени(Абонент, ВыборкаЗадолженность)
	перем ТекПериодыЗадолженность,ТекУслуга,ЗадолженностьУслуга;
	
	РасшифровкаЗадолженности						= ТаблицаРасшифровкиЗадолженностиПени();	
	ОписаниеЗадолженности  							= ОписаниеЗадолженности();	
	ЗадолженностьВсего  							= 0;
...
Функция ТаблицаРасшифровкиЗадолженностиПени()		
	ПериодыЗадолженности  							= Новый ТаблицаЗначений;
	Колонки  										= ПериодыЗадолженности.Колонки;
	Колонки.Добавить("ПорядокПериодРасчета",		Новый ОписаниеТипов("Число"));
	Колонки.Добавить("ПорядокПогашенияУслуги",		Новый ОписаниеТипов("Число"));	
...
Процедура РаспределитьОплату(Параметры, ДанныеОплаты, ДанныеДокумента) 
	РасшифровкаОплаты 	= ДанныеОплаты.Расшифровка;
	Для Каждого СтрокаОплаты Из РасшифровкаОплаты Цикл		
		СписатьЗадолженность(Параметры, ДанныеДокумента, СтрокаОплаты.Сумма, СтрокаОплаты.Услуга, СтрокаОплаты.Поставщик, СтрокаОплаты.ПериодВзаиморасчетов);
		СписатьЗадолженность(Параметры, ДанныеДокумента, СтрокаОплаты.Сумма, Неопределено, Неопределено, Неопределено); 				
...
Функция РезультатДанныеНачисленийПени(МВТ)	
	Запрос  						= Новый Запрос;
	Запрос.УстановитьПараметр("СначалаТекущиеНачисления", Константы.энргСначалаГаситьТекущиеНачисления.Получить());
	Запрос.МенеджерВременныхТаблиц 	= МВТ;
	Запрос.Текст 					= ТекстДанныеНачисленийПени();	
...
Функция ТекстЗадолженностьНаГраницуПени()
	...
	|	энргВзаиморасчетыПоПениОбороты.ПериодРасчета КАК ПериодРасчета
...
Функция ТекстДанныеНачислений()
	...
	|		СУММА(энргАвансыИспользованныеОбороты.Сумма)
	|	ИЗ
	|		КорректировкаОплаты КАК КорректировкаОплаты
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.энргАвансыИспользованные КАК энргАвансыИспользованныеОбороты
	...
	|				И КорректировкаОплаты.ДокументОплаты = энргАвансыИспользованныеОбороты.ДокументОплаты
	|				И КорректировкаОплаты.ДатаРегистратора >= энргАвансыИспользованныеОбороты.Период
	|	
	|	СГРУППИРОВАТЬ ПО
	|		КорректировкаОплаты.ПериодНачисления,
	|		КорректировкаОплаты.Район,
	...
	|		энргАвансыИспользованныеОбороты.ПериодРасчета,
	|		энргАвансыИспользованныеОбороты.Поставщик,
	|		энргАвансыИспользованныеОбороты.Услуга) КАК ВложенныйЗапрос
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПорядокПогашенияУслуг КАК ПорядокПогашенияУслуг
	|		ПО ВложенныйЗапрос.Услуга = ПорядокПогашенияУслуг.Услуга
	|		ЛЕВОЕ СОЕДИНЕНИЕ СпособРаспределенияОплаты КАК СпособРаспределенияОплаты
	|		ПО (ИСТИНА)
...
Функция ТекстДанныеНачисленийПени()
	Текст 		=
	"ВЫБРАТЬ
	|	ВложенныйЗапрос.ПериодНачисления КАК ПериодНачисления,
	|	ВложенныйЗапрос.Район КАК Район,
	...
	|				И (НАЧАЛОПЕРИОДА(ДокументыДляПроведения.датаРегистратора, ДЕНЬ) = НАЧАЛОПЕРИОДА(энргНачислениеПени.Период, ДЕНЬ))
...
Функция ТекстОплата()
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.энргПерерасчет КАК энргПерерасчет
	|			ПО ДокументыДляПроведения.ДокРегистратор = энргПерерасчет.Ссылка
	|				И (энргПерерасчет.ВидОперации = ЗНАЧЕНИЕ(Перечисление.энргВидыОпераций.энргПерерасчет_ПереносВозвратОплаты))) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Район,
	|	ВложенныйЗапрос.Абонент,
	...
	|ВЫБРАТЬ
	|	ДокументыДляПроведения.ДокРегистратор,
	|	СУММА(энргПлатежноеПоручениеВходящиеОплата.СуммаОплаты),
	|	ДокументыДляПроведения.Абонент,
	|	энргПлатежноеПоручениеВходящиеОплата.IDСтроки,
	|	энргПлатежноеПоручениеВходящиеОплата.ПериодОплаты,
	|	ДокументыДляПроведения.Район,
	|	ДокументыДляПроведения.МКД,
	|	ДокументыДляПроведения.ПериодНачисления
	|ИЗ
	...
	|	ОплатаПоУслугам.ВидЗадолженности КАК ВидЗадолженности
	|ПОМЕСТИТЬ ВМ
	|ИЗ
	|	ДокументыДляПроведения КАК ДокументыДляПроведения
	...
	|	ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка),
	|	ЗНАЧЕНИЕ(Справочник.энргКонтрагенты.ПустаяСсылка),
	|	ШапкаПерерасчета.Сумма - ЕСТЬNULL(РаспределеноПоУслугамВПерерасчетеГруппировка.СуммаОплаты, 0),
	|	ЗНАЧЕНИЕ(Перечисление.энргВидыЗадолженности.ОсновнойДолг)
	|ИЗ
	|	ДокументыДляПроведения КАК ДокументыДляПроведения
	...
	|				СУММА(РаспределеноПоУслугамВПерерасчете.СуммаОплаты) КАК СуммаОплаты
	|			ИЗ
...
Функция СтабильныеПериодыДокументыНачисления(ПериодНачисления, Строение, Помещение)
	...
	Если ПериодНачисления = НачалоГода(ПериодНачисления) Тогда
		...
		Текст  	= Текст +
		"ВЫБРАТЬ
		| "+ энргНачисления.СформироватьСтрокуДляЗапроса("энргСтабильныеПериоды","ВсеПараметрыРасчета","КлючНорматива,КлючСоцНорматива",Истина,Ложь) + "
		|	КлючиНорматива.КлючНорматива КАК КлючНорматива,
		|	КлючиНорматива.КлючСоцНорматива КАК КлючСоцНорматива
