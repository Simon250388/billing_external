...
Функция ТекстЗапросаСреднийОбъемАбонентов()
	...
	|	КОНЕЦ - 1 КАК КоличествоМесяцевИсключая
	...
	|						КОГДА ВложенныйЗапрос.Сезонность = ЗНАЧЕНИЕ(Справочник.энргВидыСезонности.ПустаяСсылка)
	|							ТОГДА 0
	|						ИНАЧЕ РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиНачалоГода.КонецСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), НАЧАЛОПЕРИОДА(ЕСТЬNULL(ПериодыСезонностиПериодНачисления.НачалоСезонности, ДАТАВРЕМЯ(1, 1, 1)), МЕСЯЦ), МЕСЯЦ) - 1
