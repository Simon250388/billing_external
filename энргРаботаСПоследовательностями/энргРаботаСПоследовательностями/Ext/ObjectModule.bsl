...
Функция ТекстЗапросаСреднийОбъемАбонентов()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	энргДатыОтчетногоПериодыОбщиеНастройки.Организация КАК Организация,
	|	энргДатыОтчетногоПериодыОбщиеНастройки.Завершение КАК Завершение
	|ПОМЕСТИТЬ ДатыОтчетногоПериодыОбщиеНастройки
	|ИЗ
	|	РегистрСведений.энргДатыОтчетногоПериодыОбщиеНастройки КАК энргДатыОтчетногоПериодыОбщиеНастройки
	|ГДЕ
	|	энргДатыОтчетногоПериодыОбщиеНастройки.Организация = &Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				И Район = &Район
	|				И (МКД = &Строение
	|					ИЛИ МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.ПустаяСсылка))) КАК энргНастройкаСезонностиУслугСрезПоследних
	|ГДЕ
	...
	|ПОМЕСТИТЬ ВТ_СпособыРасчетаПоНаправлениям
	|ИЗ
	|	СпособыРасчетаПоНаправлениям КАК СпособыРасчетаПоНаправлениям
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НастройкаСезонностиУслуг КАК НастройкаСезонностиУслуг
	|		ПО СпособыРасчетаПоНаправлениям.Организация = НастройкаСезонностиУслуг.Организация
	|			И СпособыРасчетаПоНаправлениям.Район = НастройкаСезонностиУслуг.Район
	|			И СпособыРасчетаПоНаправлениям.Услуга = НастройкаСезонностиУслуг.Услуга
	|			И СпособыРасчетаПоНаправлениям.НаправлениеИспользованияТУ = НастройкаСезонностиУслуг.НаправлениеИспользованияТУ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			ВЫБОР
	|				КОГДА энргСтабильныеПериоды.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|							КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	|								ТОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27)
	|							ИНАЧЕ ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	|						КОНЕЦ)
	|					ТОГДА НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ)
	|				ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), МЕСЯЦ, 1)
	...
	|					И (энргСтабильныеПериоды.ПриборВведен)
	|				ЛЕВОЕ СОЕДИНЕНИЕ ДатыОтчетногоПериодыОбщиеНастройки КАК ДатыОтчетногоПериодыОбщиеНастройки
	|				ПО энргСтабильныеПериоды.Организация = ДатыОтчетногоПериодыОбщиеНастройки.Организация
	|		ГДЕ
	|			РАЗНОСТЬДАТ(ВЫБОР
	|						КОГДА энргСтабильныеПериоды.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|									КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	...
	|							ТОГДА НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ)
	|						ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), МЕСЯЦ, 1)
	|					КОНЕЦ, &ПериодНачисления, МЕСЯЦ) >= НастройкиРасчетаСреднегоПотребления.КоличествоПоказаний
	|		
	|		СГРУППИРОВАТЬ ПО
	...
	|			энргСтабильныеПериоды.ДатаСостоянияЛицевогоСчета,
	|			НастройкиРасчетаСреднегоПотребления.ЗаПрошлыйГод,
	|			НастройкиРасчетаСреднегоПотребления.ПоВсемНачислениям,
	|			НастройкиРасчетаСреднегоПотребления.КоличествоПоказаний,
	|			НастройкиРасчетаСреднегоПотребления.КоличествоПериодов,
	|			НастройкиРасчетаСреднегоПотребления.НачислятьСреднийОбъемНеБолееМесяцев,
	|			ВЫБОР
	|				КОГДА энргСтабильныеПериоды.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|							КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	|								ТОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27)
	...
	|			энргСтабильныеПериоды.НаправлениеИспользованияТУ) КАК СтабильныеПериоды
	...
	|							ТОГДА НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ)
	|						ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), МЕСЯЦ, 1)
	|					КОНЕЦ, &ПериодНачисления, МЕСЯЦ) >= НастройкиРасчетаСреднегоПотребления.КоличествоПоказанийОбъемПотребленный
	|		
	|		СГРУППИРОВАТЬ ПО
	|			энргСтабильныеПериоды.Район,
	|			энргСтабильныеПериоды.Строение,
	...
	|			энргСтабильныеПериоды.Услуга,
	|			энргСтабильныеПериоды.ДатаСостоянияЛицевогоСчета,
	|			НастройкиРасчетаСреднегоПотребления.ЗаПрошлыйГодОбъемПотребленный,
	|			НастройкиРасчетаСреднегоПотребления.ПоВсемНачислениямОбъемПотребленный,
	|			НастройкиРасчетаСреднегоПотребления.КоличествоПоказанийОбъемПотребленный,
	|			НастройкиРасчетаСреднегоПотребления.КоличествоПериодовОбъемПотребленный,
	|			НастройкиРасчетаСреднегоПотребления.НачислятьСреднийОбъемНеБолееМесяцевОбъемПотребленный,
	|			ВЫБОР
	|				КОГДА энргСтабильныеПериоды.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|							КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериоды.ДатаСостоянияПрибора, МЕСЯЦ))
	|								ТОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27)
	...
	|			ТочкиДляРасчета.Организация,
	|			ТочкиДляРасчета.НаправлениеИспользованияТУ,
	|			ТочкиДляРасчета.КоличествоПоказаний) КАК ВложенныйЗапрос
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВложенныйЗапрос.КонецПериода,
	|		ВложенныйЗапрос.ДатаСостоянияЛицевогоСчета,
	...
	|ГДЕ
	|	РАЗНОСТЬДАТ(ВложенныйЗапрос.НачалоПериода, ВложенныйЗапрос.КонецПериода, МЕСЯЦ) >= ВложенныйЗапрос.КоличествоПоказаний
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Район,
	|	ВложенныйЗапрос.ЧастныйСектор,
	...
	|	ВТ_СпособыРасчетаПоНаправлениям.Сезонность КАК Сезонность,
	|	ПериодыРасчетаСреднего.НаправлениеИспользованияТУ КАК НаправлениеИспользованияТУ,
	|	ПериодыРасчетаСреднего.Организация КАК Организация,
	|	ВТ_СпособыРасчетаПоНаправлениям.МКД КАК МКД
	|ПОМЕСТИТЬ СтрокиСОтоплениеМКД
	|ИЗ
	|	ПериодыРасчетаСреднего КАК ПериодыРасчетаСреднего
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СпособыРасчетаПоНаправлениям КАК ВТ_СпособыРасчетаПоНаправлениям
	|		ПО ПериодыРасчетаСреднего.Район = ВТ_СпособыРасчетаПоНаправлениям.Район
	|			И ПериодыРасчетаСреднего.Услуга = ВТ_СпособыРасчетаПоНаправлениям.Услуга
	|			И ПериодыРасчетаСреднего.НаправлениеИспользованияТУ = ВТ_СпособыРасчетаПоНаправлениям.НаправлениеИспользованияТУ
	|			И (ПериодыРасчетаСреднего.ЭтоОбъемПотребленный)
	|			И ПериодыРасчетаСреднего.Организация = ВТ_СпособыРасчетаПоНаправлениям.Организация
	|			И ПериодыРасчетаСреднего.Строение = ВТ_СпособыРасчетаПоНаправлениям.МКД
	|			И (НЕ ВТ_СпособыРасчетаПоНаправлениям.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И (ПериодыРасчетаСреднего.ЭтоОбъемПотребленный)
	|			И ПериодыРасчетаСреднего.Организация = ВТ_СпособыРасчетаПоНаправлениям.Организация
	|			И (ВТ_СпособыРасчетаПоНаправлениям.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.ПустаяСсылка))
	|ГДЕ
	|	НЕ (ПериодыРасчетаСреднего.Организация, ПериодыРасчетаСреднего.Район, ПериодыРасчетаСреднего.Строение) В
	|				(ВЫБРАТЬ
	|					СтрокиСОтоплениеМКД.Организация КАК Организация,
	|					СтрокиСОтоплениеМКД.Район КАК Район,
	|					СтрокиСОтоплениеМКД.МКД КАК МКД
	|				ИЗ
	|					СтрокиСОтоплениеМКД КАК СтрокиСОтоплениеМКД)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СтрокиСОтоплениеМКД.Район,
	|	СтрокиСОтоплениеМКД.Услуга,
	|	СтрокиСОтоплениеМКД.ТочкаУчета,
	|	СтрокиСОтоплениеМКД.КонецПериода,
	|	СтрокиСОтоплениеМКД.НачалоПериода,
	|	СтрокиСОтоплениеМКД.Сезонность,
	|	СтрокиСОтоплениеМКД.НаправлениеИспользованияТУ,
	|	СтрокиСОтоплениеМКД.Организация
	|ИЗ
	|	СтрокиСОтоплениеМКД КАК СтрокиСОтоплениеМКД
	|;
	|
	...
	|	СУММА(ВложенныйЗапрос.РазностьПериодов) КАК РазностьПериодов,
	|	ВложенныйЗапрос.НаправлениеИспользованияТУ КАК НаправлениеИспользованияТУ,
	|	ВложенныйЗапрос.Организация КАК Организация
	|ПОМЕСТИТЬ ОтопительныеПериоды
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВложенныйЗапрос.Район КАК Район,
	|		ВложенныйЗапрос.Услуга КАК Услуга,
	|		ВложенныйЗапрос.ТочкаУчета КАК ТочкаУчета,
	|		ВЫБОР
	|			КОГДА ВложенныйЗапрос.НачалоПериода = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ВложенныйЗапрос.НачалоСезонности
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ВложенныйЗапрос.НачалоСезонности >= ВложенныйЗапрос.НачалоПериода
	|						ТОГДА ВложенныйЗапрос.НачалоСезонности
	|					ИНАЧЕ ВложенныйЗапрос.НачалоПериода
	|				КОНЕЦ
	|		КОНЕЦ КАК НачалоСезона,
	|		ВЫБОР
	|			КОГДА ВложенныйЗапрос.КонецПериода = ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ВложенныйЗапрос.КонецСезонности
	|			ИНАЧЕ ВЫБОР
	|					КОГДА ВложенныйЗапрос.КонецСезонности <= ВложенныйЗапрос.КонецПериода
	|						ТОГДА ВложенныйЗапрос.КонецСезонности
	|					ИНАЧЕ ВложенныйЗапрос.КонецПериода
	|				КОНЕЦ
	|		КОНЕЦ КАК КонецСезона,
	...
	|			ВложенныйЗапрос.Услуга КАК Услуга,
	|			ВложенныйЗапрос.ТочкаУчета КАК ТочкаУчета,
	|			НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(1, 1, 1), ГОД, ГОД(ВложенныйЗапрос.НачалоПериода) - ВЫБОР
	|								КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|										И МЕСЯЦ(энргДлительностиПериодов.КонецПериода) < МЕСЯЦ(ВложенныйЗапрос.НачалоПериода)
	|										И МЕСЯЦ(ВложенныйЗапрос.НачалоПериода) <= МЕСЯЦ(энргДлительностиПериодов.НачалоПериода)
	|									ТОГДА 1
	|								КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|										И МЕСЯЦ(ВложенныйЗапрос.НачалоПериода) <= МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|									ТОГДА 2
	|								КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|										И МЕСЯЦ(ВложенныйЗапрос.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.НачалоПериода)
	|									ТОГДА 1
	...
	|									ТОГДА 2
	|								ИНАЧЕ 1
	|							КОНЕЦ), МЕСЯЦ, МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) - 1), ДЕНЬ, ДЕНЬ(энргДлительностиПериодов.НачалоПериода) - 1), МЕСЯЦ) КАК НачалоСезонности,
	|			ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(1, 1, 1), ГОД, ГОД(ВложенныйЗапрос.НачалоПериода) - ВЫБОР
	|									КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|											И МЕСЯЦ(энргДлительностиПериодов.КонецПериода) < МЕСЯЦ(ВложенныйЗапрос.НачалоПериода)
	|											И МЕСЯЦ(ВложенныйЗапрос.НачалоПериода) <= МЕСЯЦ(энргДлительностиПериодов.НачалоПериода)
	|										ТОГДА 0
	...
	|										ТОГДА 1
	|									ИНАЧЕ 1
	|								КОНЕЦ), МЕСЯЦ, МЕСЯЦ(энргДлительностиПериодов.КонецПериода) - 1), ДЕНЬ, ДЕНЬ(энргДлительностиПериодов.КонецПериода)), МЕСЯЦ), МЕСЯЦ, 1) КАК КонецСезонности,
	|			ВложенныйЗапрос.НачалоПериода КАК НачалоПериода,
	...
	|		ИЗ
	|			(ВЫБРАТЬ
	|				СтрокиСОтоплением.Район КАК Район,
	|				СтрокиСОтоплением.Услуга КАК Услуга,
	|				СтрокиСОтоплением.ТочкаУчета КАК ТочкаУчета,
	|				МАКСИМУМ(энргДлительностиПериодов.Период) КАК Период,
	|				СтрокиСОтоплением.КонецПериода КАК КонецПериода,
	|				СтрокиСОтоплением.НачалоПериода КАК НачалоПериода,
	|				СтрокиСОтоплением.Сезонность КАК Сезонность,
	|				СтрокиСОтоплением.НаправлениеИспользованияТУ КАК НаправлениеИспользованияТУ,
	|				СтрокиСОтоплением.Организация КАК Организация
	|			ИЗ
	|				СтрокиСОтоплением КАК СтрокиСОтоплением
	|					ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргДлительностиПериодов КАК энргДлительностиПериодов
	|					ПО СтрокиСОтоплением.Район = энргДлительностиПериодов.Район
	|						И СтрокиСОтоплением.НачалоПериода >= энргДлительностиПериодов.Период
	|						И СтрокиСОтоплением.Сезонность = энргДлительностиПериодов.Сезонность
	|						И СтрокиСОтоплением.Организация = энргДлительностиПериодов.Организация
	|			
	|			СГРУППИРОВАТЬ ПО
	|				СтрокиСОтоплением.Услуга,
	|				СтрокиСОтоплением.ТочкаУчета,
	|				СтрокиСОтоплением.Район,
	|				СтрокиСОтоплением.КонецПериода,
	|				СтрокиСОтоплением.НачалоПериода,
	|				СтрокиСОтоплением.Сезонность,
	|				СтрокиСОтоплением.НаправлениеИспользованияТУ,
	|				СтрокиСОтоплением.Организация) КАК ВложенныйЗапрос
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргДлительностиПериодов КАК энргДлительностиПериодов
	|				ПО ВложенныйЗапрос.Район = энргДлительностиПериодов.Район
	|					И ВложенныйЗапрос.Период = энргДлительностиПериодов.Период
	|					И ВложенныйЗапрос.Сезонность = энргДлительностиПериодов.Сезонность
	|					И ВложенныйЗапрос.Организация = энргДлительностиПериодов.Организация
	|		
	|		ОБЪЕДИНИТЬ ВСЕ
	|		
	|		ВЫБРАТЬ
	...
	|			ВложенныйЗапрос.Услуга,
	|			ВложенныйЗапрос.ТочкаУчета,
	|			НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(1, 1, 1), ГОД, ГОД(ВложенныйЗапрос.КонецПериода) - ВЫБОР
	|								КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|										И МЕСЯЦ(энргДлительностиПериодов.КонецПериода) < МЕСЯЦ(ВложенныйЗапрос.КонецПериода)
	|										И МЕСЯЦ(ВложенныйЗапрос.КонецПериода) <= МЕСЯЦ(энргДлительностиПериодов.НачалоПериода)
	|									ТОГДА 1
	|								КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|										И МЕСЯЦ(ВложенныйЗапрос.КонецПериода) <= МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|									ТОГДА 2
	|								КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|										И МЕСЯЦ(ВложенныйЗапрос.КонецПериода) > МЕСЯЦ(энргДлительностиПериодов.НачалоПериода)
	|									ТОГДА 1
	...
	|									ТОГДА 2
	|								ИНАЧЕ 1
	|							КОНЕЦ), МЕСЯЦ, МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) - 1), ДЕНЬ, ДЕНЬ(энргДлительностиПериодов.НачалоПериода) - 1), МЕСЯЦ),
	|			ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДАТАВРЕМЯ(1, 1, 1), ГОД, ГОД(ВложенныйЗапрос.КонецПериода) - ВЫБОР
	|									КОГДА МЕСЯЦ(энргДлительностиПериодов.НачалоПериода) > МЕСЯЦ(энргДлительностиПериодов.КонецПериода)
	|											И МЕСЯЦ(энргДлительностиПериодов.КонецПериода) < МЕСЯЦ(ВложенныйЗапрос.КонецПериода)
	|											И МЕСЯЦ(ВложенныйЗапрос.КонецПериода) <= МЕСЯЦ(энргДлительностиПериодов.НачалоПериода)
	|										ТОГДА 0
	...
	|										ТОГДА 1
	|									ИНАЧЕ 1
	|								КОНЕЦ), МЕСЯЦ, МЕСЯЦ(энргДлительностиПериодов.КонецПериода) - 1), ДЕНЬ, ДЕНЬ(энргДлительностиПериодов.КонецПериода)), МЕСЯЦ), МЕСЯЦ, 1),
	|			ДАТАВРЕМЯ(1, 1, 1),
	|			ВложенныйЗапрос.КонецПериода,
	...
	|					ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргДлительностиПериодов КАК энргДлительностиПериодов
	|					ПО СтрокиСОтоплением.Район = энргДлительностиПериодов.Район
	|						И СтрокиСОтоплением.КонецПериода >= энргДлительностиПериодов.Период
	|						И СтрокиСОтоплением.Сезонность = энргДлительностиПериодов.Сезонность
	|						И СтрокиСОтоплением.Организация = энргДлительностиПериодов.Организация
	|			
	|			СГРУППИРОВАТЬ ПО
	...
	|				ПО ВложенныйЗапрос.Район = энргДлительностиПериодов.Район
	|					И ВложенныйЗапрос.Период = энргДлительностиПериодов.Период
	|					И ВложенныйЗапрос.Сезонность = энргДлительностиПериодов.Сезонность) КАК ВложенныйЗапрос) КАК ВложенныйЗапрос
	|ГДЕ
	|	ВложенныйЗапрос.РазностьПериодов > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Район,
	|	ВложенныйЗапрос.Услуга,
	...
	|				ТОГДА СУММА(энргРасчетСреднегоОбъема.ОбъемУслуги) / ЕСТЬNULL(ОтопительныеПериоды.РазностьПериодов, ПериодыРасчетаСреднего.КоличествоМесяцев)
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК ОбъемПотребленный,
	|		ВЫБОР
	|			КОГДА НЕ ПериодыРасчетаСреднего.ЭтоОбъемПотребленный
	|				ТОГДА СУММА(энргРасчетСреднегоОбъема.ОбъемУслуги) / ПериодыРасчетаСреднего.КоличествоМесяцев
	|			ИНАЧЕ 0
	|		КОНЕЦ КАК Объем,
	...
	|				И (энргРасчетСреднегоОбъема.Строение = &Строение)
	|				И (энргРасчетСреднегоОбъема.Помещение = &Помещение)
	|			ЛЕВОЕ СОЕДИНЕНИЕ ОтопительныеПериоды КАК ОтопительныеПериоды
	|			ПО ПериодыРасчетаСреднего.Организация = ОтопительныеПериоды.Организация
	|				И ПериодыРасчетаСреднего.Район = ОтопительныеПериоды.Район
	|				И ПериодыРасчетаСреднего.Услуга = ОтопительныеПериоды.Услуга
	|				И ПериодыРасчетаСреднего.ТочкаУчета = ОтопительныеПериоды.ТочкаУчета
	|				И ПериодыРасчетаСреднего.НаправлениеИспользованияТУ = ОтопительныеПериоды.НаправлениеИспользованияТУ
	|				И (ПериодыРасчетаСреднего.ЭтоОбъемПотребленный)
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|		ОтопительныеПериоды.РазностьПериодов) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.Район,
	|	ВложенныйЗапрос.ЧастныйСектор,
...
Функция ТекстЗапросаСреднийОбъемМКД()
	...
	|	СтабильныеПериодыМКД.ДатаСостоянияПрибора КАК ДатаСостоянияПрибора,
	|	СтабильныеПериодыМКД.КоличествоПоказаний КАК КоличествоПоказаний
	|ПОМЕСТИТЬ ТочкиДляРасчета
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		НастройкиРасчетаСреднегоПотребления.ПоВсемНачислениям КАК ПоВсемНачислениям,
	|		ВЫБОР
	|			КОГДА энргСтабильныеПериодыМКД.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериодыМКД.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|						КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериодыМКД.ДатаСостоянияПрибора, МЕСЯЦ))
	|							ТОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27)
	|						ИНАЧЕ ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериодыМКД.ДатаСостоянияПрибора, МЕСЯЦ))
	|					КОНЕЦ)
	|				ТОГДА НАЧАЛОПЕРИОДА(энргСтабильныеПериодыМКД.ДатаСостоянияПрибора, МЕСЯЦ)
	|			ИНАЧЕ ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериодыМКД.ДатаСостоянияПрибора, МЕСЯЦ), МЕСЯЦ, 1)
	|		КОНЕЦ КАК ДатаСостоянияПрибора,
	|		НастройкиРасчетаСреднегоПотребления.КоличествоПоказаний КАК КоличествоПоказаний
	|	ИЗ
	...
	|				И (энргСтабильныеПериодыМКД.ПриборВведен)
	|			ЛЕВОЕ СОЕДИНЕНИЕ ДатыОтчетногоПериодыОбщиеНастройки КАК ДатыОтчетногоПериодыОбщиеНастройки
	|			ПО энргСтабильныеПериодыМКД.Организация = ДатыОтчетногоПериодыОбщиеНастройки.Организация
	|	ГДЕ
	|		РАЗНОСТЬДАТ(ВЫБОР
	|					КОГДА энргСтабильныеПериодыМКД.ДатаСостоянияПрибора < ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(энргСтабильныеПериодыМКД.ДатаСостоянияПрибора, МЕСЯЦ), ДЕНЬ, ВЫБОР
	|								КОГДА ЕСТЬNULL(ДатыОтчетногоПериодыОбщиеНастройки.Завершение, 27) <= ДЕНЬ(КОНЕЦПЕРИОДА(энргСтабильныеПериодыМКД.ДатаСостоянияПрибора, МЕСЯЦ))
	...
	|		энргСтабильныеПериодыМКД.ТочкаУчета,
	...
	|		КОНЕЦ,
	|		НастройкиРасчетаСреднегоПотребления.КоличествоПериодов,
	|		НастройкиРасчетаСреднегоПотребления.КоличествоПоказаний) КАК СтабильныеПериодыМКД
	...
	|	СтабильныеПериодыМКД.ДатаСостоянияПрибора,
	|	СтабильныеПериодыМКД.КоличествоПоказаний
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(энргРасчетСреднегоОбъемаМКД.ПериодНачисления, МЕСЯЦ)) >= 1
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ТочкиДляРасчета.ТарифнаяЗона КАК ТарифнаяЗона,
	|		МАКСИМУМ(энргРасчетСреднегоОбъемаМКД.ПериодНачисления) КАК КонецПериода,
	|		МИНИМУМ(энргРасчетСреднегоОбъемаМКД.ПериодНачисления) КАК НачалоПериода,
	|		ТочкиДляРасчета.КоличествоПоказаний КАК КоличествоПоказаний
	|	ИЗ
	|		ТочкиДляРасчета КАК ТочкиДляРасчета
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргРасчетСреднегоОбъемаМКД КАК энргРасчетСреднегоОбъемаМКД
	...
	|			ТочкиДляРасчета.КоличествоПоказаний
	|		ИЗ
	|			ТочкиДляРасчета КАК ТочкиДляРасчета) КАК ВложенныйЗапрос) КАК ВложенныйЗапрос
	|ГДЕ
	|	РАЗНОСТЬДАТ(ВложенныйЗапрос.НачалоПериода, ВложенныйЗапрос.КонецПериода, МЕСЯЦ) >= ВложенныйЗапрос.КоличествоПоказаний
	|
	|СГРУППИРОВАТЬ ПО
