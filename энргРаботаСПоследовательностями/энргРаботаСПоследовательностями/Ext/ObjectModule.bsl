...
Функция ТекстЗапросаСреднийОбъемАбонентов()
	...
	|
	|ИМЕЮЩИЕ
	|	(КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(энргРасчетСреднегоОбъема.ПериодНачисления, МЕСЯЦ)) >= МАКСИМУМ(СтабильныеПериоды.КоличествоПоказаний)
	...
	|		ИЛИ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ НАЧАЛОПЕРИОДА(энргРасчетСреднегоОбъема.ПериодНачисления, МЕСЯЦ)) >= МАКСИМУМ(СтабильныеПериоды.КоличествоПоказанийОбъемПотребленный)
	...
	|			МАКСИМУМ(РАЗНОСТЬДАТ(ПериодыРасчетаСреднего.НачалоПериода, ПериодыРасчетаСреднего.КонецПериода, МЕСЯЦ) + 1) КАК КолВоМесяцев,
	|			0 КАК КолВоМесяцевОбъемПотребленный
	|		ИЗ
	...
	|			0,
	|			МАКСИМУМ(РАЗНОСТЬДАТ(ПериодыРасчетаСреднего.НачалоПериодаОбъемПотребленный, ПериодыРасчетаСреднего.КонецПериода, МЕСЯЦ) + 1)
	|		ИЗ
	|			ПериодыРасчетаСреднего КАК ПериодыРасчетаСреднего
	|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргРасчетСреднегоОбъема КАК энргРасчетСреднегоОбъема
	|				ПО ПериодыРасчетаСреднего.Район = энргРасчетСреднегоОбъема.Район
	...
	|			ПериодыРасчетаСреднего.ТочкаУчета,
	|			ПериодыРасчетаСреднего.Шкала,
	|			ПериодыРасчетаСреднего.ТарифнаяЗона) КАК ВложенныйЗапрос
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВложенныйЗапрос.Район,
	|		ВложенныйЗапрос.ЧастныйСектор,
