	...
	Пока Выборка.Следующий() цикл		
		...
		Если ТекСтроение <> Строение тогда  						
			Если ТекСтроение <>Неопределено тогда
				...
				Пока ?(НЕ ВыборкаПоказания.ЧастныйСектор,ВыборкаПоказания.Строение,ПустоеСтроение) = ТекСтроение и ЕстьСледующийПоказания Цикл
...
Процедура ВосстановитьПоследовательностьСтабильныеПериоды(Организация, ПериодНачисления,Район,Строение=Неопределено,Помещение=Неопределено) Экспорт 
	...
	Пока Выборка.следующий() цикл  		
		...
		//очистить наборы по помещению с разделенным ЛС
		...
		Набор.Отбор.ЧастныйСектор.установить(Выборка.ЧастныйСектор);
		Набор.Отбор.Строение.установить(Выборка.Строение);
		Набор.Отбор.Помещение.установить(Выборка.Помещение.Родитель);
		Набор.Записать();
