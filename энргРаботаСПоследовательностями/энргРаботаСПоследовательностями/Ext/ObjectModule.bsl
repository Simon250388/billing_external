		...
		Если НЕ РезультатДокументыДляПроведения.Пустой() Тогда
			...
			ВыборкаДокументыДляПроведения 								= РезультатДокументыДляПроведения.Выбрать();
			 ...
			 Пока ВыборкаДокументыДляПроведения.Следующий() Цикл
				 ...
				 НаборСтабильныеПериоды.Отбор.Организация 				= ВыборкаДокументыДляПроведения.Организация;
				 НаборСтабильныеПериоды.Отбор.ПериодНачисления 			= ПериодНачисления;
				 НаборСтабильныеПериоды.Отбор.Район 					= ВыборкаДокументыДляПроведения.Район;
				 НаборСтабильныеПериоды.Отбор.ЧастныйСектор 			= ВыборкаДокументыДляПроведения.ЧастныйСектор;
				 Если Не ВыборкаДокументыДляПроведения.ЧастныйСектор Тогда
					 НаборСтабильныеПериоды.Отбор.Строение 				= ВыборкаДокументыДляПроведения.Строение;
				 КонецЕсли;
				 НаборСтабильныеПериоды.Отбор.ДокРегистратор 			= ВыборкаДокументыДляПроведения.ДокРегистратор;
				 
				 НаборСтабильныеПериодыШкалы							= РегистрыСведений.энргСтабильныеПериодыШкалы.СоздатьНаборЗаписей();
				 НаборСтабильныеПериодыШкалы.Отбор.Организация 			= ВыборкаДокументыДляПроведения.Организация;
				 НаборСтабильныеПериодыШкалы.Отбор.ПериодНачисления		= ПериодНачисления;
				 НаборСтабильныеПериодыШкалы.Отбор.Район 				= ВыборкаДокументыДляПроведения.Район;
				 НаборСтабильныеПериодыШкалы.Отбор.ЧастныйСектор 		= ВыборкаДокументыДляПроведения.ЧастныйСектор;
				 Если Не ВыборкаДокументыДляПроведения.ЧастныйСектор Тогда
					 НаборСтабильныеПериодыШкалы.Отбор.Строение 		= ВыборкаДокументыДляПроведения.Строение;
				 КонецЕсли;
				 НаборСтабильныеПериодыШкалы.Отбор.ДокРегистратор 		= ВыборкаДокументыДляПроведения.ДокРегистратор;				 
