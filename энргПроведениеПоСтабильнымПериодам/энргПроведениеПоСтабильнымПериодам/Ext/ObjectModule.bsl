Перем СервисСтабильныеПериоды; 
Перем ИзмеренияИсключая;

Процедура ВосстановитьПоследовательность(яСервисСтабильныеПериоды) Экспорт  	
	
	Если яСервисСтабильныеПериоды = Неопределено Тогда
		ВызватьИсключение "Не установлен сервис";
	КонецЕсли;
	
	СервисСтабильныеПериоды 					= яСервисСтабильныеПериоды;
	
	ЗначенияПараметровПоНаправлениямУслуг 		= СервисСтабильныеПериоды.ЗначенияПараметровПоНаправлениямУслуг();
	...
	СоотвествиеРазрезовНорматива 				= СоотвествиеРазрезовНорматива();	
	...
	СервисСтабильныеПериоды.Инициализировать();
			
	РезультатДокументыДляПроведения  			= СервисСтабильныеПериоды.ДокументыДляПроведения();
	...
	НаборыДанных.Вставить("ДанныеДокументов", 					СервисСтабильныеПериоды.ДанныеДокументов());
	НаборыДанных.Вставить("СостояниеНаГраницу", 				СервисСтабильныеПериоды.СостояниеНаГраницу());
	НаборыДанных.Вставить("СостояниеНаГраницуШкалы", 			СервисСтабильныеПериоды.СостояниеНаГраницуШкалы());
	НаборыДанных.Вставить("СостояниеНаГраницуИзмерители", 		СервисСтабильныеПериоды.СостояниеНаГраницуИзмерители());
	НаборыДанных.Вставить("ДанныеДокументовИзмененияПлощади", 	СервисСтабильныеПериоды.ДанныеДокументовИзмененияПлощади());
	НаборыДанных.Вставить("ДанныеИзмененияСоставаШкал", 		СервисСтабильныеПериоды.ДанныеИзмененияСоставаШкал());
	НаборыДанных.Вставить("ИзменяемыеПараметрыПоТипам", 		СервисСтабильныеПериоды.ИзменяемыеПараметрыПоТипам());
	
	ЗаполнитьНаборы(НаборыДанных, ПараметрыОпределенияКлючаНорматива, ЗначенияПараметровПоНаправлениямУслуг);
...
Процедура ЗаполнитьНаборы(НаборыДанных, ПараметрыОпределенияКлючаНорматива, ЗначенияПараметровПоНаправлениямУслуг)
	...
	ОписаниеРесурсовИзмерители						= Новый ФиксированнаяСтруктура(РесурсыИзмерители);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ текПериодНачисления = Выборка.ПериодНачисления
			...
			Пока Выборка.ЧастныйСектор = ВыборкаДанныеДокументов.ЧастныйСектор
				...
				Если НЕ текРегистратор = ВыборкаДанныеДокументов.ДокРегистратор Тогда
					Если НЕ текРегистратор = Неопределено Тогда
						ДанныеДокументов.Вставить(текРегистратор, ДанныеДокумента);
					КонецЕсли;				
					ДанныеДокумента 				= Новый Структура("ВидОперации,Порядок,ДанныеДляПроведения", ВыборкаДанныеДокументов.ВидОперации,ВыборкаДанныеДокументов.Порядок,Новый Массив);
				КонецЕсли;					
				...
				текРегистратор 						= ВыборкаДанныеДокументов.ДокРегистратор;
				ЕстьСледующиеДанные 				= ВыборкаДанныеДокументов.Следующий();
			КонецЦикла;
			...
			Пока Выборка.ЧастныйСектор = ВыборкаСостояниеНаГраницуИзмерители.ЧастныйСектор
				...
				РесурсыИзмеретели							= Новый Структура(ОписаниеРесурсовИзмерители);
				РесурсыИзмеретели.Вставить("Измеритель", Справочники.энргНаправлениеИспользованияТочекУчета.ПустаяСсылка());
				ЗаполнитьЗначенияСвойств(РесурсыИзмеретели,ВыборкаСостояниеНаГраницуШкалы);
		...
		ДанныеДокумента 							= ДанныеДокументов[Выборка.ДокРегистратор];
		...
		ИначеЕсли ДанныеДокумента = Неопределено И Не ИзменяемыеПараметрыПоТипам[ТипЗнч(Выборка.ДокРегистратор)] = Неопределено Тогда
			ИзменяемыеПараметрыРасчета		 		= ИзменяемыеПараметрыПоТипам[ТипЗнч(Выборка.ДокРегистратор)][Перечисления.энргВидыОпераций.ПустаяСсылка()];
...
Процедура ИзменитьСвойстваВсехТочекУчета(ТекущиеДанныеПомещения,Порядок,ИзменяемыеПараметры, Параметры, ДанныеДокумента)
	...
	Для Каждого КлючЗначениеТочка Из ТекущиеДанныеПомещения Цикл
		Для Каждого КлючЗначениеТочкаУслуга Из КлючЗначениеТочка.Значение Цикл
			...
			Если НЕ ИндексСоставЖильцов = Неопределено Тогда
				 УстановитьКоличествоЖильцов(ЗначенияРесурсов, яДАнныеДокумента.СоставЖильцов, яДАнныеДокумента.КлючПомещения);
			КонецЕсли;			
...
Процедура ДобавитьСтрокуНабора(Параметры,Ресурсы, ДанныеСтрокиДокумента, Услуга, ТочкаУчета = Неопределено)
	...
	УстановитьЗначениеПлощади(Параметры, Ресурсы, Услуга);
...
Процедура ИзменитьПараметрыТочкиУчета(ТекущиеДанныеПомещения, Порядок, ИзменяемыПараметры, Параметры, ДанныеДокумента)
	...
	Для Сч = 0 По ДанныеДокумента.ВГраница() Цикл 		
		...
		Для Каждого КлючЗначениеУслуга Из ДанныеТочкиУчета Цикл
			...
			ДобавитьНовыйСоставШкал(Параметры,ТекущиеДанныеПомещения[ДанныеДокумента[Сч].ТочкаУчета][КлючЗначениеУслуга.Ключ],ДанныеДокумента[Сч].ТочкаУчета);			
			Если мТочкиУчета.Найти(ДанныеДокумента[Сч].ТочкаУчета) = Неопределено Тогда			
				ДобавитьСтрокуНабораШкал(Параметры,ТекущиеДанныеПомещения[ДанныеДокумента[Сч].ТочкаУчета][КлючЗначениеУслуга.Ключ].Шкалы,ДанныеДокумента[Сч],ТекущиеДанныеПомещения[ДанныеДокумента[Сч].ТочкаУчета][КлючЗначениеУслуга.Ключ].НомерПозиции);
				мТочкиУчета.Добавить(ДанныеДокумента[Сч].ТочкаУчета);
			КонецЕсли;			
		КонецЦикла;
	КонецЦикла; 
...
Процедура УстановитьЗначениеПлощади(Параметры, Ресурсы, Услуга)
	
	Если Параметры.ИзмененияПлощади.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЗначенияПараметров 					= Параметры.ЗначенияПараметровПоНаправлениямУслуг[Услуга][Ресурсы.НаправлениеИспользованияТУ];
...
Процедура УстановитьКоличествоЖильцов(ЗначенияРесурсов, КоличествоЖильцов, КлючПомещения)
	...
	Если СоставЖильцов[IDКлючПомещения] = Неопределено Тогда
		СоставЖильцов.Вставить(IDКлючПомещения, КоличествоЖильцов);
	Иначе
	...
	Для Каждого КлючЗначение Из СоставЖильцов Цикл
		НовоеЗначение 					= НовоеЗначение +  КлючЗначение.Значение;
	КонецЦикла;	
...
Функция СоотвествиеРазрезовНорматива() 
	...
	РезультатРазрезыНорматива 				= СервисСтабильныеПериоды.ДействующуюРазрезыНорматива();
...
Функция ОпределитьКлючНорматива(Услуга,Ресурсы,СоциальныйНорматив,ПараметрыОпределенияКлючаНорматива)  
	...
	ВидыРазрезовСоциальныйНорматива  		= СоотвествиеРазрезовНорматива[Услуга][Ресурсы.НаправлениеИспользованияТУ];
