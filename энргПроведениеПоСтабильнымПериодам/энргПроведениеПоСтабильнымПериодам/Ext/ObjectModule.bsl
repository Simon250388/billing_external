...
Процедура ВосстановитьПоследовательность(СервисСтабильныеПериоды,Ошибки,ЗаполнитьСостояниеПриПустомНаборе = Ложь) Экспорт  	
		...
		Если НЕ текПериодНачисления = Выборка.ПериодНачисления
			...
			Если Не ЕстьСледующиеСостояниеНаГраницу Тогда
				Для Каждого Запись Из СервисСтабильныеПериоды.НаборПериоды Цикл
					Если  Запись.ПериодНачисления = Выборка.ПериодНачисления И Запись.НомерПОзиции = 0 Тогда
			...
			Если НЕ ЕстьСледующиеСостояниеНаГраницуШкалы Тогда
				Для Каждого ЗаписьШкалы Из СервисСтабильныеПериоды.НаборШкалы Цикл
					Если  ЗаписьШкалы.ПериодНачисления = Выборка.ПериодНачисления И ЗаписьШкалы.НомерПОзиции = 0 Тогда
			...
			Если Не ЕстьСледующиеСостояниеНаГраницуИзмерители Тогда
				Для Каждого ЗаписьИзмерители Из СервисСтабильныеПериоды.НаборИзмерители Цикл
					Если  ЗаписьИзмерители.ПериодНачисления = Выборка.ПериодНачисления И ЗаписьИзмерители.НомерПозиции = 0 Тогда
...
Процедура УстановитьЗначениеПлощади(Параметры, Ресурсы, ПериодНачисления, Услуга)
	...
	Если НЕ ЗначениеЗаполнено(ВидПлощади) Тогда
		ВидПлощади 						= Перечисления.энргВидыПлощадей.ОбщаяПлощадь;
	КонецЕсли;
