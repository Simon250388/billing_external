...
Процедура Начислить(Организация,ПериодНачисления,Район,МКД,Отдельностоящие, Договор,Документ) Экспорт
	...
	//Договор = Справочники.бестДоговорыКонтрагентов.НайтиПоНаименованию("Договор № 2469-ТВБ-Ну от 30.01.2020 0:00:00");
	//Договор = Справочники.бестДоговорыКонтрагентов.НайтиПоНаименованию("Договор № 2464-ТВБ-Ну от 01.01.2020 0:00:00");
	Договор = Справочники.бестДоговорыКонтрагентов.НайтиПоНаименованию("Договор № 0006-ТВЮЛ-Ну от 01.05.2017");
		...
		Пока ВыборкаДок.Следующий() Цикл
			...
			Пока Выборка.следующий() цикл 
				...
				Если НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
					...
					СтрокаОписанияФормулы.Поставщик 						= Константы.энргПоставщикПоУмолчанию.Получить();;
...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(Отдельностоящие, МКД, Договор)
	...
	Если Отдельностоящие = Ложь И Не ЗначениеЗаполнено(МКД) Тогда
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП(ФормируемВременную,ИмяДокумента); 
	...
	|					ИНАЧЕ ДанныеДляРасчета.КонПок - ДанныеДляРасчета.НачПок
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 6)) КАК ОбъемПоПрибору,
	...
	|		ИНАЧЕ 0
	|	КОНЕЦ,
	|	Выбор когда НЕ ДанныеДляРасчета.НеПрименятьСезонность или ДанныеДляРасчета.Договор.ТипДоговора <> Значение(Перечисления.бестТипыДоговоров.УправляющаяКомпания) Тогда ВЫБОР
	|		КОГДА ДанныеДляРасчета.СпособРасчетаНорматива = ЗНАЧЕНИЕ(Перечисление.бестТипыНачисленияПоНормативуНП.НормативСНиП)
	|			ТОГДА ВЫРАЗИТЬ(РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(ДанныеДляРасчета.КонецПериода, ДЕНЬ), ДЕНЬ) * ДанныеДляРасчета.ЗначениеНормативаЗаДень / 1000 КАК ЧИСЛО(15, 6))
	|		КОГДА ДанныеДляРасчета.СпособРасчетаНорматива = ЗНАЧЕНИЕ(Перечисление.бестТипыНачисленияПоНормативуНП.ДоговорнаяВеличина)
	|			ТОГДА ДанныеДляРасчета.ГарантированныйОбъем * (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ЧАС), НАЧАЛОПЕРИОДА(ДанныеДляРасчета.КонецПериода, ЧАС), ЧАС) / ДанныеДляРасчета.ДнейВПериоде / 24)
