...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(ИсключаяОтдельностоящие, МКД, Договор)
	...
	|			&КонецПериодаРасчета,
	|			Организация = &Организация
	|				И Район = &Район" + ОтборОбъектаРасчета + ") КАК бестСтруктураСетиТочекУчетаНПСрезПоследних
...
Функция ПолучитьРезультатЗапросаДополнительныеНачисленияЮЛ(ПериодНачисления,Организация,Район,ИсключаяОтдельностоящие, МКД,НачалоПериодаРасчета,КонецПериодаРасчета)
	...
	Если ИсключаяОтдельностоящие = Ложь И МКД = Неопределено тогда
		Текст 					= "";
		ОтборОбъектаРасчета 	= "";
	Иначе
		Текст = бестНачисленияНП.ПолучитьОтборПоМКД(Новый Структура("ИсключаяОтдельностоящие,МКД", ИсключаяОтдельностоящие,МКД));  
		ОтборОбъектаРасчета 	= " И ОбъектРасчета В (ВЫБРАТЬ АктивныеОбъектыРасчета.ОбъектРасчета КАК ОбъектРасчета ИЗ АктивныеОбъектыРасчета КАК АктивныеОбъектыРасчета)" + Символы.ПС;		
	КонецЕсли;
