...
Функция ТекстЗапросаДополнительныеНачисленияЮЛ(ОтборДоговора)
	Возврат 
		"ВЫБРАТЬ
		|	бестПревышениеПДКСрезПоследних.Организация КАК Организация,
		|	бестПревышениеПДКСрезПоследних.Район КАК Район,
		|	бестПревышениеПДКСрезПоследних.Договор КАК Договор,
		...
		|			Организация = &Организация
		|				И Район = &Район
		|				И ПериодНачисления = &ПериодНачисления" + ОтборДоговора + ") КАК бестПревышениеПДКСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	бестПромывкаОтопительнойСистемыСрезПоследних.Договор,
		...
		|			Организация = &Организация
		|				И Район = &Район
		|				И ПериодНачисления = &ПериодНачисления" + ОтборДоговора + ") КАК бестПромывкаОтопительнойСистемыСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	бестПриемЛивневыхСтоковСрезПоследних.Договор,
		...
		|			Организация = &Организация
		|				И Район = &Район
		|				И ПериодНачисления = &ПериодНачисления" + ОтборДоговора + ") КАК бестПриемЛивневыхСтоковСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	бестОбъемНачисленийНП.ОбъемУслуги * ДанныеДляДопНачисления.Коэффициент КАК ОбъемУслуги,
		|	бестОбъемНачисленийНП.ОбъемУслугиПотребленный * ДанныеДляДопНачисления.Коэффициент КАК ОбъемУслугиПотребленный,
		|	бестОбъемНачисленийНП.Сумма * ДанныеДляДопНачисления.Коэффициент КАК Сумма,
		|	бестОбъемНачисленийНП.СуммаНДС * ДанныеДляДопНачисления.Коэффициент КАК СуммаНДС,
		...
		|			И бестОбъемНачисленийНП.Договор = ДанныеДляДопНачисления.Договор
...
Функция ПолучитьРезультатЗапросаДополнительныеНачисленияЮЛ(ПериодНачисления,Организация,Район,ИсключаяОтдельностоящие, МКД, Договор,НачалоПериодаРасчета,КонецПериодаРасчета)
	...
	Иначе
		Текст = бестНачисленияНП.ПолучитьОтборПоМКД(Новый Структура("ИсключаяОтдельностоящие,МКД,Договор", ИсключаяОтдельностоящие,МКД,Договор),,Истина);  
		ОтборДоговора 	= " И Договор В (ВЫБРАТЬ ОтборДоговор.Договор КАК Договор ИЗ ОтборДоговор КАК ОтборДоговор)" + Символы.ПС;		
	КонецЕсли;
	
	Запрос.Текст = Текст + ТекстЗапросаДополнительныеНачисленияЮЛ(ОтборДоговора);
