...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП(ФормируемВременную,ИмяДокумента); 
	...
	|						ТОГДА (ДанныеДляРасчета.ЗначениеНагрузки * 18 - ДанныеДляРасчета.СреднемесячнаяТемпература) / (18 - ДанныеДляРасчета.ПроектнаяТемператураНаружногоВоздуха) * РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(ДанныеДляРасчета.КонецПериода, ДЕНЬ), ДЕНЬ) * ДанныеДляРасчета.КоличествоЧасовРаботы
	...
	|		КОГДА ДанныеДляРасчета.СпособРасчетаНорматива = ЗНАЧЕНИЕ(Перечисление.бестТипыНачисленияПоНормативуНП.ПоНагрузке)
	|			ТОГДА ДанныеДляРасчета.ЗначениеНагрузки * (18 - ДанныеДляРасчета.СреднемесячнаяТемпература) / (18 - ДанныеДляРасчета.ПроектнаяТемператураНаружногоВоздуха) * РАЗНОСТЬДАТ(ДанныеДляРасчета.НачалоПериода, ДанныеДляРасчета.КонецПериода, ЧАС)
	|		КОГДА ДанныеДляРасчета.СпособРасчетаНорматива = ЗНАЧЕНИЕ(Перечисление.бестТипыНачисленияПоНормативуНП.Норматив)
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ВЫБОР
	|					КОГДА НЕ ДанныеДляРасчета.ВидПлощади = ЗНАЧЕНИЕ(Перечисление.энргВидыПлощадей.ПустаяСсылка)
