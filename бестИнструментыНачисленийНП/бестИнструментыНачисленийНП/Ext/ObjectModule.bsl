			...
			Пока Выборка.следующий() цикл
				...
				Если НЕ ЗначениеЗаполнено(Выборка.ФормулаРасчета) Тогда
					...
					ИначеЕсли ЗначениеЗаполнено(Выборка.ЗависитОтУслуги) и НЕ Выборка.ПривязанаКШкале и НЕ Выборка.РасчетПоФормуле Тогда
						Если Выборка.СпособРасчетаНорматива = Перечисления.бестТипыНачисленияПоНормативуНП.ТемпературныйКоэффициент Тогда
...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(ОтборПоДоговору,Отдельностоящие)
	...
	|			И СтабильныеПериодыНачало.ПриборУчета = СтабильныеПериодыШкалы.ПриборУчета
...
Функция ТекстЗапросаПолучитьДанныеДляРасчетаНП(РасчетПоФормуле, ИмяДокумента,Отдельностоящие)
	...
	|	Периоды.ПриборУчетаРесурс КАК ПриборУчетаРесурс,
