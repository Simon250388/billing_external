		...
		Пока ВыборкаДок.Следующий() Цикл
			...
			Пока Выборка.следующий() цикл 
				...
				Если НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
					...
					СтрокаОписанияФормулы.УзелПриборУчета 					= Выборка.ПриборУчета;
				...
				//	ПлотностиВоды.Вставить(Выборка.Услуга, 					Выборка.Плотность);	
				////	ПлотностиВоды.Вставить(Выборка.Услуга, 					0);	
					...
					ПлотностьВоды = Выборка.Плотность;
...
Процедура ПолучитьСтабильныеПериодыНП(МВТ,ЭтоПерерасчет,Организация,ПериодНачисления,Район,Договор,НачалоПериодаРасчета,КонецПериодаРасчета,Отдельностоящие)  
...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(Отдельностоящие, МКД, Договор)
	...
	|			И бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.ВидПараметра = бестТипРегистрируемойТемпературыПриборовУчетаНПСрезПоследних.ВидПараметра
	 |ГДЕ
	          ...
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.УзелУчета КАК УзелУчета,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.ПриборУчета КАК ПриборУчета,
	          |	СУММА(бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.ЗначениеПараметра) КАК Объем,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.Шкала КАК Шкала
	          |ПОМЕСТИТЬ ОбъемыИзСуточнойВедомости
	          |ИЗ
	          |	РегистрСведений.бестИтоговыеЗначенияПараметровСуточнойВедомости.СрезПоследних(
	          |			&КонецПериодаРасчета,
	          |			Организация = &Организация
	          ...
	          |		ПО бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.ПриборУчета = энргПриборыУчетаАбонента.Ссылка
	          |			И (энргПриборыУчетаАбонента.РегистрируетОбъем)
	          |
	          |СГРУППИРОВАТЬ ПО
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.Организация,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.Район,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.Договор,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.ОбъектРасчета,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.УзелУчета,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.Шкала,
	          |	бестИтоговыеЗначенияПараметровСуточнойВедомостиСрезПоследних.ПриборУчета
	          |;
	          |
	          |////////////////////////////////////////////////////////////////////////////////
	          |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|////////////////////////////////////////////////////////////////////////////////	
	|ВЫБРАТЬ
	|	" + бестНачисленияНП.СформироватьСтрокуДляЗапроса("бестСтабильныеПериодыНП","СтабильныеПериодыНачалоСоставУзла","Шкала",Истина,Ложь) + "	
	...
	|	СтабильныеПериодыНачалоСоставУзла.ЧастьУзлаПриборУчета,
	|	СтабильныеПериодыНачалоСоставУзла.ЧастьУзлаПриборВведен,
	...
	|			И СтабильныеПериодыНачалоСоставУзла.ЧастьУзлаПриборУчета = СтабильныеПериодыШкалы.ПриборУчета
	|			И (ВЫБОР
	|				КОГДА СтабильныеПериодыНачалоСоставУзла.Шкала = ЗНАЧЕНИЕ(Справочник.энргШкалыПриборовУчета.ПустаяСсылка)
	|						И СтабильныеПериодыНачалоСоставУзла.ПриборУчета = ЗНАЧЕНИЕ(Справочник.энргПриборыУчетаАбонента.ПустаяСсылка)
	|					ТОГДА ИСТИНА
...
Функция ТекстПолучитьСтабильныеПериоды(ЭтоПерерасче,Отдельностоящие)  
	...
	|	СтабильныеПериодыНачалоШкалы.ЧастьУзлаПриборУчета,
	|	СтабильныеПериодыНачалоШкалы.ЧастьУзлаПриборВведен,
	|	СтабильныеПериодыНачалоШкалы.КоэффициентПУ
	...
	|	СтабильныеКонецСезонностиТипНачисления.ЧастьУзлаПриборУчета КАК ЧастьУзлаПриборУчета,
	|	СтабильныеКонецСезонностиТипНачисления.ЧастьУзлаПриборВведен КАК ЧастьУзлаПриборВведен, 
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.Организация КАК Организация,
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.Район КАК Район,
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.Договор КАК Договор,
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.ОбъектРасчета КАК ОбъектРасчета,
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.Услуга КАК Услуга,
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.ТочкаУчета КАК ТочкаУчета,
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.ПриборУчета КАК ПриборУчета,
	|	бестЗарегистрированныеОбъемыПотребленияНПОбороты.ОбъемИтоговыйОборот КАК ЗарегистрированныйОбъем,
	|	МаксНомерПозицииПериоды.НомерПозиции КАК НомерПозиции
	|ПОМЕСТИТЬ ЗарегистрированныеОбъемы
	|ИЗ
	|	РегистрНакопления.бестЗарегистрированныеОбъемыПотребленияНП.Обороты(
	|			&НачалоПериодаРасчета,
	|			&КонецПериодаРасчета,
	|			Период,
	|			Организация = &Организация
	|				И Район = &Район
	|				И ПериодНачисления = &ПериодНачисления) КАК бестЗарегистрированныеОбъемыПотребленияНПОбороты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ МаксНомерПозицииПериоды КАК МаксНомерПозицииПериоды
	|		ПО бестЗарегистрированныеОбъемыПотребленияНПОбороты.Организация = МаксНомерПозицииПериоды.Организация
	|			И бестЗарегистрированныеОбъемыПотребленияНПОбороты.Район = МаксНомерПозицииПериоды.Район
	|			И бестЗарегистрированныеОбъемыПотребленияНПОбороты.Договор = МаксНомерПозицииПериоды.Договор
	|			И бестЗарегистрированныеОбъемыПотребленияНПОбороты.ОбъектРасчета = МаксНомерПозицииПериоды.ОбъектРасчета
	|			И бестЗарегистрированныеОбъемыПотребленияНПОбороты.Услуга = МаксНомерПозицииПериоды.Услуга
	|			И бестЗарегистрированныеОбъемыПотребленияНПОбороты.ТочкаУчета = МаксНомерПозицииПериоды.ТочкаУчета
	|			И бестЗарегистрированныеОбъемыПотребленияНПОбороты.ПериодНачисления = МаксНомерПозицииПериоды.ПериодНачисления";
...
Функция ТекстЗапросаПолучитьДанныеДляРасчетаНП(РасчетПоФормуле, ИмяДокумента,Отдельностоящие)
	...
	|				ТОГДА ИСТИНА
	|			КОГДА НЕ ОбъемыИзСуточнойВедомости.УзелУчета ЕСТЬ NULL
	|				ТОГДА ИСТИНА
	...
	|	Периоды.ЧастьУзлаУчета КАК ЧастьУзлаУчета,
	|	ВЫБОР
	...
	|	ЕСТЬNULL(ОбъемыИзСуточнойВедомости.Объем, 0) КАК ОбъемПУПоСуточнойВедомости
	...
	|			И Периоды.ЧастьУзлаПриборУчета = ПредоставленныеПоказанияПоДоговору.ПриборУчета
	...
	|			И Периоды.ЧастьУзлаПриборУчета = ПредоставленныеПоказанияПредПериод.ПриборУчета
	...
	|			И Периоды.НомерПозиции = ЗарегистрированныеОбъемы.НомерПозиции
	|				ЛЕВОЕ СОЕДИНЕНИЕ ОбъемыИзСуточнойВедомости КАК ОбъемыИзСуточнойВедомости
	|				ПО Периоды.Организация = ОбъемыИзСуточнойВедомости.Организация
	|					И Периоды.Район = ОбъемыИзСуточнойВедомости.Район
	|					И Периоды.Договор = ОбъемыИзСуточнойВедомости.Договор
	|					И Периоды.ОбъектРасчета = ОбъемыИзСуточнойВедомости.ОбъектРасчета
	|					И Периоды.ПриборУчета = ОбъемыИзСуточнойВедомости.УзелУчета
	|					И Периоды.ЧастьУзлаПриборУчета = ОбъемыИзСуточнойВедомости.ПриборУчета
	|					И Периоды.Шкала = ОбъемыИзСуточнойВедомости.Шкала
	|ГДЕ
...
//	|	энргВидыУслуг.ИспользоватьПовыщающийКоэффициент,
	...
	|	ЕСТЬNULL(ОбъемыИзСуточнойВедомости.Объем, 0)
	|";   	
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП(ФормируемВременную,ИмяДокумента,Отдельностоящие); 
	...
	|	ДанныеДляРасчетаПередНормой.ОбъемПУПоСуточнойВедомости КАК ОбъемПУПоСуточнойВедомости,
	|		ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ТемпературыПодачиИзСуточнойВедомости.ПриборУчета ЕСТЬ NULL
	|				ТОГДА ЕСТЬNULL(ТемпературыИсточникаТочекУчета.ТемператураПодачиИсточника, 0)
	|			ИНАЧЕ ЕСТЬNULL(ТемпературыПодачиИзСуточнойВедомости.ЗначениеПараметра, 0)
	|		КОНЕЦ КАК ЧИСЛО(10, 1)) КАК ТемператураДляРасчетаПлотности
	...
	|			И ДанныеДляРасчетаПередНормой.ПриборУчета = ТемпературыПодачиИзСуточнойВедомости.УзелУчета
	|			И ДанныеДляРасчетаПередНормой.ЧастьУзлаПриборУчета = ТемпературыПодачиИзСуточнойВедомости.ПриборУчета
	|;  
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|				ТОГДА ДанныеДляРасчета.ЗарегистрированныйОбъем
	|			КОГДА ДанныеДляРасчета.ОбъемПУПоСуточнойВедомости <> 0
	|						ТОГДА ДанныеДляРасчета.ОбъемПУПоСуточнойВедомости
	|			ИНАЧЕ ВЫБОР
	...
	|	ДанныеДляРасчета.ОбъемПУПоСуточнойВедомости КАК ОбъемПУПоСуточнойВедомости,
	|	ДанныеДляРасчета.ТемператураДляРасчетаПлотности КАК ТемператураДляРасчетаПлотности
	...
	|	ДанныеДляРасчета.ОбъемПУПоСуточнойВедомости,
	|	ДанныеДляРасчета.ТемператураДляРасчетаПлотности
	|ИЗ
	|	ДанныеДляРасчетабестНачислениеНП КАК ДанныеДляРасчета
	|ГДЕ
	|	НЕ ДанныеДляРасчета.ПриборВведен
	|	И НЕ ДанныеДляРасчета.ЧастьУзлаПриборВведен
	...
	|	ГДЕ
	|	НЕ ПредварительныйРасчет.РасчетПоФормуле
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПредварительныйРасчет.ЧастьУзлаПриборУчета,
	|	ПредварительныйРасчет.ЧастьУзлаПриборВведен, 
	|	ПредварительныйРасчет.КоэффициентПУ,
	|	ВЫБОР
	...
	|	ПредварительныйРасчет.ТемператураДляРасчетаПлотности КАК ТемператураДляРасчетаПлотности,
	|	ЕСТЬNULL(бестПлотностиВоды.Плотность, 0) КАК Плотность
	|ИЗ
	|	ПредварительныйРасчет КАК ПредварительныйРасчет
	...
	|	ПО ПредварительныйРасчет.ТемператураДляРасчетаПлотности = бестПлотностиВоды.Температура
	|";
...
Функция РезультатЗапросаИндивидуальныеНачисленияНП(ПериодНачисления,Организация,Район,Отдельностоящие, МКД, Договор,НачалоПериодаРасчета,КонецПериодаРасчета,ПериодНовогоТарифа,ДнейВПериоде)
	...
	ПолучитьСтабильныеПериодыНП(МВТ,Ложь,Организация, ПериодНачисления,Район,Договор,НачалоПериодаРасчета,КонецПериодаРасчета,Отдельностоящие);
...
Функция ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам(Отдельностоящие,МКД,Договор = Неопределено)
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |	ТочкиСубабонентов.Коэффициент КАК КоэффициентДолевогоРаспределения,
	        ...
	        |			И (бестДопускПриборовУчетаККоммерческомуУчетуСрезПоследних.КонецПериодаКоммерскогоУчета < &КонецПериода)
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        ...
	        |			И (бестДатыПоверкиПрибораУчетаНПСрезПоследних.ДатаПоверки < &КонецПериода)
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
