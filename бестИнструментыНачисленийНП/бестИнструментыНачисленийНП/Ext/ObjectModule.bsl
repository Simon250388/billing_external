...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП(); 
	...
	|	И ВЫБОР
	|		КОГДА ДанныеДляРасчета.ЯвляетсяОтдельностоящим И ДанныеДляРасчета.СпособДосчётаРесурса = ЗНАЧЕНИЕ(Перечисление.бестСпособыДосчётаРесурса.ПоФактическомуРасходуЗаПредыдущийПериод) 
