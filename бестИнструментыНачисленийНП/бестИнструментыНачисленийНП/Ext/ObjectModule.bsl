...
Процедура Начислить(Организация,ПериодНачисления,Район,МКД,ИсключаяОтдельностоящие, Договор,Документ) Экспорт
	...
	ПараметрыРасчетаФормулы							= Новый Структура("НормативНаПодогрев,ТемпературныйКоэффициентПоВоздуху,ПлотностьВоды,ТемператураПодачи,ТемператураХолоднойВоды,КолвоЧасовРаботыТУ,Объем_ПоПриборуПред,РасчетПоНормативу,Договор",0,0,0,0,0,0,0,0,Справочники.энргДоговорыКонтрагентов.ПустаяСсылка());
	...
	Пока ВыборкаМКД.Следующий() Цикл 		
		...
		Пока ВыборкаДок.Следующий() Цикл
			...
			Пока Выборка.следующий() цикл 
				...
				Если НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
					...
					Если НЕ ТекОбъектРасчета = Неопределено Тогда
						...
						ДобавитьСтрокиНачисленияНППоФормуле(ТЗДанныеСуточнойВедомости, ПериодНачисления, Организация, Район, ТекМКД, ТекОбъектРасчета, ТекТочкаУчета, ТекУзелПриборУчета, КонецПериодаРасчета, ДокументСсылка, МассивФормулРасчета, НормативыПоУслугам, ПараметрыРасчетаФормулы, Набор);
					КонецЕсли;
				...
				ПараметрыРасчетаФормулы.КолвоЧасовРаботыТУ      = (Выборка.КонецПериода - Выборка.НачалоПериода) / 3600;
			...
			Если Не МассивФормулРасчета = Неопределено Тогда
				...
				ДобавитьСтрокиНачисленияНППоФормуле(ТЗДанныеСуточнойВедомости, ПериодНачисления, Организация, Район, ВыборкаМКД.МКД, ТекОбъектРасчета, ТекТочкаУчета, ТекУзелПриборУчета, КонецПериодаРасчета, ДокументСсылка, МассивФормулРасчета, НормативыПоУслугам, ПараметрыРасчетаФормулы, Набор);
			КонецЕсли;
...
Процедура ДобавитьСтрокиНачисленияНППоФормуле(ТЗДанныеСуточнойВедомости, ПериодНачисления, Организация, Район, МКД, ОбъектРасчета, ТочкаУчета, ТекУзелПриборУчета, КонецПериодаРасчета, ДокументСсылка, ОписаниеУслуг, НормативыПоУслугам, ПараметрыРасчетаФормулы, Набор)
	...
	Для Каждого СтрокаФормулы Из ОписаниеУслуг Цикл			
		...
		Для Каждого стр Из мТекСтрокДанныеСуточнойВедомости Цикл
			...
			ТекФормулаОбъемУслуги 				= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",стр.ЗначениеПараметра);
			ТекФормулаОбъемУслугиПотребленный 	= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",стр.ЗначениеПараметра);
		...
		ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.ПлотностьВоды);
		ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.ПлотностьВоды);
		
		
		Ключ 									= Новый Структура("ТипРасхода, Услуга", ТипыРасходов.ЗначениеТемпературы, "ТемператураПодачи");
		ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.ТемператураПодачи);
		ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.ТемператураПодачи);

		
		Ключ 									= Новый Структура("ТипРасхода, Услуга", ТипыРасходов.ЗначениеТемпературы, "ТемператураХолоднойВоды");	
		ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.ТемператураХолоднойВоды);
		ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.ТемператураХолоднойВоды);
		
		
		Ключ 									= Новый Структура("ТипРасхода, Услуга", ТипыРасходов.ЧасыРаботы, "КолвоЧасовРаботыТУ");	
		ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.КолвоЧасовРаботыТУ);
		ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.КолвоЧасовРаботыТУ);
		
		
		Ключ 									= Новый Структура("ТипРасхода, Услуга", ТипыРасходов.ЧасыРаботы, "КолвоЧасовШтатнойРаботыПУ");	
		ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ВНР);
		ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ВНР);
		
		Ключ 									= Новый Структура("ТипРасхода, Услуга", ТипыРасходов.РасходИзмерителяПоШкале, "Объем_ПоПриборуПред");	
		ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.Объем_ПоПриборуПред);
		ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.Объем_ПоПриборуПред);

		Ключ 									= Новый Структура("ТипРасхода, Услуга", ТипыРасходов.РасходИзмерителяПоШкале, "РасчетПоНормативу");	
		ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.РасчетПоНормативу);
		ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + ЗначениеВСтрокуВнутр(Ключ) + "]",ПараметрыРасчетаФормулы.РасчетПоНормативу);
		...
		СтрокаНабора.ВидНачисления 					= Перечисления.энргВидыНачислений.Начисление;
		СтрокаНабора.СпособНачисления 				= ?(ПараметрыРасчетаФормулы.РасчетПоНормативу = РезультатПоФормуламОбъемУслуги,СтрокаФормулы.СпособНачисления, Перечисления.энргСпособНачислений.ПоПриборам);
...
Функция ТекстЗапросаПолучитьДанныеДляРасчетаНП(РасчетПоФормуле)
	...
	|	Периоды.ФормулаРасчета КАК ФормулаРасчета,
