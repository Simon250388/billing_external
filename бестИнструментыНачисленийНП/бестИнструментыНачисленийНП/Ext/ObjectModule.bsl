			...
			Пока Выборка.следующий() цикл 
				...
				Если НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
					...
					Если НЕ ТекДокумент = Неопределено Тогда
						...
						ДобавитьСтрокиНачисленияНППоФормуле(ПериодНачисления, Организация, Район, ТекДоговор, ТекОбъектРасчета, ТекТочкаУчета, ОписаниеУслуг, НормативыПоУслугам, Набор);
					...
					Ключ 				= Новый Структура("ТипРасхода, Услуга", ТипыРасходов["ЗначениеНормативаПоУслуге"], XMLСтрока(Выборка.Услуга));
					НормативыПоУслугам.Вставить(ЗначениеВСтрокуВнутр(Ключ), 		Выборка.НормаПотребления);	
				...
				Если НЕ ПараметрыФормулы.Получить(КлючШкалы) = Неопределено Тогда
					...
					Ключ 				= Новый Структура("ТипРасхода, ПриборУчета, Шкала", ТипыРасходов["РасходИзмерителяПоШкале"], XMLСтрока(Выборка.ПриборУчета),XMLСтрока(Выборка.Шкала));
				Иначе
					Ключ 				= Новый Структура("ТипРасхода, ПриборУчета, Шкала", ТипыРасходов["РасходИзмерителяПоШкале"], XMLСтрока(Выборка.ПриборУчета),XMLСтрока(Выборка.Шкала));
					ПараметрыФормулы.Вставить(ЗначениеВСтрокуВнутр(Ключ), 	Новый Структура("ОбъемУслуги, ОбъемУслугиПотребленный", Выборка.ОбъемУслуги, Выборка.ОбъемУслугиПотребленный));
				КонецЕсли; 		
...
Процедура ДобавитьСтрокиНачисленияНППоФормуле(ПериодНачисления, Организация, Район, Договор, ОбъектРасчета, ТочкаУчета, ОписаниеУслуг, НормативыПоУслугам, Набор)
	...
	Для Каждого СтрокаФормулы Из ОписаниеУслуг Цикл			
		...
		//ТекФормула = СтрЗаменить(ТекФормулаОбъемУслуги,"[Результат]=","");
		//
		...
		//ТекФормула = СтрЗаменить(ТекФормула,",]","]");
		//ТекФормула = СтрЗаменить(ТекФормула," ","");
		//ТекФормула = СтрЗаменить(ТекФормула,Символы.НПП,"");
		//
		//МассивОперандов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТекФормула,"]",Истина);
		//
		//Для Сч = 0 По МассивОперандов.ВГраница() Цикл
		//	СтрОперанд  = СтрЗаменить(МассивОперандов[Сч],"[","");
		//	Если Сч <> МассивОперандов.ВГраница() Тогда
		//		ДопСкобки = СтрОперанд + Символы.ПС + "}" + Символы.ПС+ "}";
		//		МассивОперандов[Сч] = ЗначениеИзСтрокиВнутр(ДопСкобки);
		//	Иначе
		//		МассивОперандов[Сч] = ЗначениеИзСтрокиВнутр(СтрОперанд);
		//	КонецЕсли;
		//КонецЦикла;
		...
		Для Каждого ПараметрФормулы Из ПараметрыФормулы Цикл
			...
			КлючПараметра = ПараметрФормулы.Ключ;

			ОписаниеОбъема 							= ПараметрФормулы.Значение;
			ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + КлючПараметра + "]",ОписаниеОбъема.ОбъемУслуги);
			ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + КлючПараметра + "]",ОписаниеОбъема.ОбъемУслугиПотребленный);
		КонецЦикла;
		
		Для Каждого ПараметрФормулы Из НормативыПоУслугам Цикл
			
			КлючНорматива = ПараметрФормулы.Ключ;

			ТекФормулаОбъемУслуги 					= СтрЗаменить(ТекФормулаОбъемУслуги,"[" + КлючНорматива + "]",ПараметрФормулы.Значение);
			ТекФормулаОбъемУслугиПотребленный 		= СтрЗаменить(ТекФормулаОбъемУслугиПотребленный,"[" + КлючНорматива + "]",ПараметрФормулы.Значение);
		КонецЦикла;
...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(ОтборПоДоговору,Отдельностоящие)
	...
	|	" + бестНачисленияНП.СформироватьСтрокуДляЗапроса("бестСтабильныеПериодыНП","СтабильныеПериодыНачало","ПриборУчета,Шкала",Истина,Ложь) + "	
 	|	ЕСТЬNULL(СтабильныеПериодыПриборыУзловУчета.ПриборУчета, ЗНАЧЕНИЕ(Справочник.энргПриборыУчетаАбонента.ПустаяСсылка)) КАК ПриборУчета,
	...
	|	ЕСТЬNULL(СтабильныеПериодыПриборыУзловУчета.ПриборВведен, ЛОЖЬ) КАК ПриборУчетаВведен
