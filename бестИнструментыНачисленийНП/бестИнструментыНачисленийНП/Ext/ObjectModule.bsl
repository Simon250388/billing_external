...
Процедура СнятьСредниеНачисленияНП(Организация, ПериодНачисления,Район,МКД,ИсключаяОтдельностоящие,Договор,Документ) Экспорт
	...
	Если Выборка.Количество() = 0 Тогда				
		ВыборкаДокументыОтменить 					= РезультатОтменитьСнятьСредниеНачисления(ПериодНачисления,Организация,Район,МКД,ИсключаяОтдельностоящие,Договор,НачалоПериодаРасчета,КонецПериодаРасчета);
...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(ИсключаяОтдельностоящие, МКД, Договор)
	...
	Иначе
		...
		ОтборОбъектаРасчета_бестПредоставленныеПоказанияНП 			= " И бестПредоставленныеПоказанияНП.ОбъектРасчета В (ВЫБРАТЬ ДоговорыОбъектовРасчета.ОбъектРасчета КАК ОбъектРасчета ИЗ ДоговорыОбъектовРасчета КАК ДоговорыОбъектовРасчета)" + Символы.ПС;
  		ОтборОбъектаРасчета_бестСреднийОбъемПотребленияНП 			= " И бестСреднийОбъемПотребленияНП.ОбъектРасчета В (ВЫБРАТЬ ДоговорыОбъектовРасчета.ОбъектРасчета КАК ОбъектРасчета ИЗ ДоговорыОбъектовРасчета КАК ДоговорыОбъектовРасчета)" + Символы.ПС;
		ОтборОбъектаРасчета_бестНомераСтрокСтабильногоПериодаНП 	= " И бестНомераСтрокСтабильногоПериодаНП.ОбъектРасчета В (ВЫБРАТЬ ДоговорыОбъектовРасчета.ОбъектРасчета КАК ОбъектРасчета ИЗ ДоговорыОбъектовРасчета КАК ДоговорыОбъектовРасчета)"  + Символы.ПС;
	...
	|	И бестСреднийОбъемПотребленияНП.Район = &Район" + ОтборОбъектаРасчета_бестСреднийОбъемПотребленияНП + "
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			И (ДлительностьПериодов.НачалоФактическогоПериода <> &КонецПериодаРасчета)" + ОтборОбъектаРасчета_бестНомераСтрокСтабильногоПериодаНП + "
	|ГДЕ
	...
	|			И (ИндивидуальнаяСезонностьСтабильныеПериоды.НачалоПериода <> &КонецПериодаРасчета)" + ОтборОбъектаРасчета_бестНомераСтрокСтабильногоПериодаНП + "
	|
	|СГРУППИРОВАТЬ ПО
	|	бестНомераСтрокСтабильногоПериодаНП.Организация,
	|	бестНомераСтрокСтабильногоПериодаНП.ПериодНачисления,
	...
	|			И (ДлительностьПериодов.КонецФактическогоПериода <> &КонецПериодаРасчета)" + ОтборОбъектаРасчета_бестНомераСтрокСтабильногоПериодаНП + "
	|ГДЕ
	|	НЕ (бестНомераСтрокСтабильногоПериодаНП.ОбъектРасчета, бестНомераСтрокСтабильногоПериодаНП.Услуга, бестНомераСтрокСтабильногоПериодаНП.ТочкаУчета) В
	|				(ВЫБРАТЬ
	|					ИзменениеИндивидуальнойСезонности.ОбъектРасчета КАК ОбъектРасчета,
	...
	|			И (ИндивидуальнаяСезонностьСтабильныеПериоды.КонецПериода <> &КонецПериодаРасчета)" + ОтборОбъектаРасчета_бестНомераСтрокСтабильногоПериодаНП + "
	|
	|СГРУППИРОВАТЬ ПО
	|	бестНомераСтрокСтабильногоПериодаНП.Организация,
	|	бестНомераСтрокСтабильногоПериодаНП.ПериодНачисления,
	...
	|			И бестНомераСтрокСтабильногоПериодаНП.Услуга = ЗначенияТарифов.Услуга" + ОтборОбъектаРасчета_бестНомераСтрокСтабильногоПериодаНП + "
	|
	|СГРУППИРОВАТЬ ПО
	|	бестНомераСтрокСтабильногоПериодаНП.Организация,
	|	бестНомераСтрокСтабильногоПериодаНП.ПериодНачисления,
	...
	|			&КонецПериодаРасчета,
	|			Организация = &Организация
	|				И Район = &Район" + ОтборОбъектаРасчета + ") КАК бестТарифныеКатегорииНПСрезПоследних";
...
Функция ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам(ИсключаяОтдельностоящие,МКД,Договор = Неопределено)
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |	СтабПериодыПоМинПозиции.Договор КАК Договор,
...
Процедура ПрочитатьНеСписанныйСреднийНП(МВТ,ПериодНачисления,Организация,Район,МКД,ИсключаяОтдельностоящие,Договор,НачалоПериодаРасчета,КонецПериодаРасчета,ТаблицаПриемник = Неопределено)
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	        ...
	        |ПОМЕСТИТЬ ДоговорыОбъектовРасчета
	        |ИЗ
	        |	РегистрСведений.бестАктивностьОбъектовРасчета.СрезПоследних(
	        |			&НачалоПериодаРасчета,
	        |			Организация = &Организация И ОбъектРасчета.МКД = &МКД
	        ...
	        |
	        |ВЫБРАТЬ
	        |	бестАктивностьОбъектовРасчета.ОбъектРасчета,
	        |	бестАктивностьОбъектовРасчета.Организация,
	        |	бестАктивностьОбъектовРасчета.Район
	        |ИЗ
	        |	РегистрСведений.бестАктивностьОбъектовРасчета КАК бестАктивностьОбъектовРасчета
	        |ГДЕ
	        |	бестАктивностьОбъектовРасчета.Период МЕЖДУ &НачалоПериодаРасчета И &КонецПериода
	        |	И бестАктивностьОбъектовРасчета.Организация = &Организация
	        |	И бестАктивностьОбъектовРасчета.Район = &Район
	        |	И бестАктивностьОбъектовРасчета.Состояние = ЗНАЧЕНИЕ(перечисление.энргСостояниеТочекУчета.включена)
	        |	И бестАктивностьОбъектовРасчета.ОбъектРасчета.МКД = &МКД
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ РАЗЛИЧНЫЕ
	        ...
	        |ПОМЕСТИТЬ Договоры
	        |ИЗ
	        |	ДоговорыОбъектовРасчета КАК ДоговорыОбъектовРасчета
	        |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.бестВладельцыОбъектовРасчета.СрезПоследних(
	        |				&НачалоПериодаРасчета,
	        |				Организация = &Организация И Договор = &Договор
	        |					И Район = &Район) КАК бестВладельцыОбъектовРасчетаСрезПоследних
	        |		ПО ДоговорыОбъектовРасчета.ОбъектРасчета = бестВладельцыОбъектовРасчетаСрезПоследних.ОбъектРасчета
	        |			И ДоговорыОбъектовРасчета.Организация = бестВладельцыОбъектовРасчетаСрезПоследних.Организация
	        |			И ДоговорыОбъектовРасчета.Район = бестВладельцыОбъектовРасчетаСрезПоследних.Район
	        |
	        |ОБЪЕДИНИТЬ
	        |
	        |ВЫБРАТЬ РАЗЛИЧНЫЕ
	        ...
	        |		ПО ДоговорыОбъектовРасчета.Организация = бестВладельцыОбъектовРасчета.Организация
	        |			И ДоговорыОбъектовРасчета.Район = бестВладельцыОбъектовРасчета.Район
	        |			И ДоговорыОбъектовРасчета.ОбъектРасчета = бестВладельцыОбъектовРасчета.ОбъектРасчета
	        |ГДЕ
	        |	ДоговорыОбъектовРасчета.Организация = &Организация
	        |	И ДоговорыОбъектовРасчета.Район = &Район
	        |	И ДоговорыОбъектовРасчета.ОбъектРасчета.МКД = &МКД
	        |	И бестВладельцыОбъектовРасчета.Договор = &Договор
	        |	И бестВладельцыОбъектовРасчета.Период МЕЖДУ &НачалоПериодаРасчета И &КонецПериода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО бестНачислениеНП.Договор = Договоры.Договор
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Договоры КАК Договоры
	|		ПО бестПерерасчетНП.Договор = Договоры.Договор
	|ГДЕ
	...
	Если НЕ ЗначениеЗаполнено(Договор) тогда
		Текст 		= СтрЗаменить(Текст,"И Договор = &Договор","");
		Текст 		= СтрЗаменить(Текст,"И бестВладельцыОбъектовРасчета.Договор = &Договор","");
	КонецЕсли;  
...
Функция РезультатОтменитьСнятьСредниеНачисления(ПериодНачисления, Организация, Район, МКД, ИсключаяОтдельностоящие, Договор, НачалоПериодаРасчета, КонецПериодаРасчета)
	...
	Запрос.Текст 	=
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
	        ...
	        |	РегистрСведений.бестАктивностьОбъектовРасчета КАК бестАктивностьОбъектовРасчета
	        |ГДЕ
	        |	бестАктивностьОбъектовРасчета.Период МЕЖДУ &НачалоПериодаРасчета И &КонецПериодаРасчета
	        |	И бестАктивностьОбъектовРасчета.Организация = &Организация
	        |	И бестАктивностьОбъектовРасчета.Район = &Район
	        |	И бестАктивностьОбъектовРасчета.Состояние = ЗНАЧЕНИЕ(перечисление.энргСостояниеТочекУчета.включена)
	        |	И бестАктивностьОбъектовРасчета.ОбъектРасчета.МКД = &МКД
	        ...
	        |	И ДоговорыОбъектовРасчета.ОбъектРасчета.МКД = &МКД
			|	И бестВладельцыОбъектовРасчета.Договор = &Договор
			|	И бестВладельцыОбъектовРасчета.Период МЕЖДУ &НачалоПериодаРасчета И &КонецПериодаРасчета
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
