		...
		Пока ВыборкаДок.Следующий() Цикл
			...
			Пока Выборка.следующий() цикл
				...
				Если НЕ ЗначениеЗаполнено(Выборка.ФормулаРасчета) Тогда
					...
					ИначеЕсли ЗначениеЗаполнено(Выборка.ЗависитОтУслуги) и НЕ Выборка.ПривязанаКШкале и НЕ Выборка.РасчетПоФормуле 
						И Не Выборка.СпособРасчетаНорматива = Перечисления.бестТипыНачисленияПоНормативуНП.ДоговорнаяВеличина Тогда
						ОбъемУслуги 										= 0;
						ОбъемУслугиПотребленный 							= 0;
