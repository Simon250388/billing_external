						...
						Если НЕ НачисленияПоУслугамТочки[Выборка.ЗависитОтУслуги] = Неопределено Тогда
							Если НЕ НачисленияПоУслугамТочки[Выборка.ЗависитОтУслуги][Выборка.Шкала] = Неопределено Тогда
								...
								Если Выборка.УслугаСОбратнымРасчетом Тогда
									ОбъемУслуги 								= ?(Выборка.НормаПотребления = 0, 0, ОбъемУслугиОсновной.ОбъемУслуги / Выборка.НормаПотребления);
									ОбъемУслугиПотребленный 					= ?(Выборка.НормаПотребления = 0, 0, ОбъемУслугиОсновной.ОбъемУслугиПотребленный / Выборка.НормаПотребления);
								Иначе
