	...
	Пока ВыборкаДолевойОбъем.Следующий() Цикл
		...
		Если НЕ ТекДоговор = ВыборкаДолевойОбъем.Договор Тогда
			...
			Пока ВыборкаТочкиУчета.Следующий() Цикл
				...
				Если НЕ ТекТочкаУчета = ВыборкаТочкиУчета.ТочкаУчета Или НЕ ТекУслуга = ВыборкаТочкиУчета.Услуга Тогда
					...
					Если Не ТекУслуга  = Неопределено Тогда
						НачислитьОбъемПоСубабонентам(ПериодНачисления, Организация, Район, ?(ТекДоговор = Неопределено, ВыборкаДолевойОбъем.Договор, ТекДоговор), ТекТочкаУчета, ТекУслуга, ГраницыПериода, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей);
					КонецЕсли;
					...
					Пока ВыборкаСубабоненты.ГоловнаяТочкаУчета = ВыборкаТочкиУчета.ТочкаУчета и ВыборкаСубабоненты.Услуга = ВыборкаТочкиУчета.Услуга Цикл
						Если НЕ ТекСубабонент = ВыборкаСубабоненты.ТочкаУчета Тогда
							...
							ДанныеСубабонента 								= Новый Структура("Договор, ОбъектРасчета, ТочкаУчета, ОбъемУслуги, ЗначениеПлощади, КоэффициентРасчета, СпособРаспределенияОбъемаДолевойТУ, СпособРаспределенияОстаткаДолевыеСхемы, ЗависимаяТочкаУчета, ЗначениеНагрузки, ДоговорнойОбъем",
							ВыборкаСубабоненты.Договор, ВыборкаСубабоненты.ОбъектРасчета, ВыборкаСубабоненты.ТочкаУчета, ВыборкаСубабоненты.ОбъемУслуги, ВыборкаСубабоненты.ЗначениеПлощади, ВыборкаСубабоненты.КоэффициентРасчета, ВыборкаСубабоненты.СпособРаспределенияОбъемаДолевойТУ, 
						...
						Иначе
							Если ВыборкаСубабоненты.ЕстьИндивидуальныеНачисления И Не ВыборкаСубабоненты.РасчетИндивидуальногоПотребления  Тогда
								СтрокаСписатьИндивидуальные  				= ДанныеПоУслуге.ДанныеНачисленийСубабонентов.Добавить();
								ЗаполнитьЗначенияСвойств(СтрокаСписатьИндивидуальные, ВыборкаСубабоненты);
								СтрокаСписатьИндивидуальные.ОбъемУслуги		= ВыборкаСубабоненты.ОбъемУслуги;
							КонецЕсли;
			...
			Если Не НаборЗаписей = Неопределено Тогда
				...
				НачислитьОбъемПоСубабонентам(ПериодНачисления, Организация, Район, ТекДоговор, ТекТочкаУчета, ТекУслуга, ГраницыПериода, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей);
...
Процедура НачислитьОбъемПоСубабонентам(ПериодНачисления, Организация, Район, Договор, ТочкаУчета, Услуга, ГраницыПериода, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей)
	...
	Если ДанныеПоУслуге.РасчетИндивидуальногоПотребления Тогда
		Если ДанныеПоУслуге.СуммарныйОбъемСубабонентов >= ДанныеПоУслуге.ОбъемУслуги Тогда
			ОсталосьРаспределить = ДанныеПоУслуге.ОбъемУслуги;
		Иначе
			ОсталосьРаспределить = ДанныеПоУслуге.ОбъемУслуги - ДанныеПоУслуге.СуммарныйОбъемСубабонентов;
		КонецЕсли;
	Иначе
		ОсталосьРаспределить = ДанныеПоУслуге.ОбъемУслуги;
	КонецЕсли;
...
Функция ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам(Договор = Неопределено)
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |		ПО УслугиДолевойТУ.Организация = бестОбъемНачисленийНПОбороты.Организация
	        |			И УслугиДолевойТУ.Район = бестОбъемНачисленийНПОбороты.Район
	        |			И УслугиДолевойТУ.Договор = бестОбъемНачисленийНПОбороты.Договор
	        |			И УслугиДолевойТУ.ОбъектРасчета = бестОбъемНачисленийНПОбороты.ОбъектРасчета
	        |			И УслугиДолевойТУ.Услуга = бестОбъемНачисленийНПОбороты.Услуга
	        |			И УслугиДолевойТУ.ТочкаУчета = бестОбъемНачисленийНПОбороты.ТочкаУчета
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        ...
	        |	ОбъемНачисленийСубабонентов.Шкала КАК Шкала,
	        ...
	        |	ОбъемНачисленийСубабонентов.Шкала,
	        |	ОбъемНачисленийСубабонентов.ТарифнаяЗона,
	        ...
	        |			ТОГДА ПараметрыСубабонентов.ЗначениеНагрузки
	        |		ИНАЧЕ ВЫБОР
	        ...
	        |				ПараметрыСубабонентов.ТочкаУчета КАК ТочкаУчета
	        |			ИЗ
	        |				ПараметрыСубабонентов КАК ПараметрыСубабонентов) КАК ВложенныйЗапрос
	        |		
	        |		СГРУППИРОВАТЬ ПО
	        ...
	        |		ПО УслугиДолевойТУ.Договор = ПлощадьСубабонентов.ДоговорДолевойТУ
	        |			И УслугиДолевойТУ.ОбъектРасчета = ПлощадьСубабонентов.ОбъектРасчетаДолевойТУ
	        |			И УслугиДолевойТУ.ТочкаУчета = ПлощадьСубабонентов.ГоловнаяТочкаУчета
	        |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	        ...
	        |		ПО УслугиДолевойТУ.Договор = ДоговорныеОбъемыНагрузка.ДоговорДолевойТУ
	        |			И УслугиДолевойТУ.ОбъектРасчета = ДоговорныеОбъемыНагрузка.ОбъектРасчетаДолевойТУ
	        |			И УслугиДолевойТУ.ТочкаУчета = ДоговорныеОбъемыНагрузка.ГоловнаяТочкаУчета
	        |			И УслугиДолевойТУ.Услуга = ДоговорныеОбъемыНагрузка.Услуга
	        ...
	        |		ПО УслугиДолевойТУ.Организация = КоличествоПриборов.Организация
	        |			И УслугиДолевойТУ.Район = КоличествоПриборов.Район
	        |			И УслугиДолевойТУ.Договор = КоличествоПриборов.ДоговорДолевойТУ
	        |			И УслугиДолевойТУ.ОбъектРасчета = КоличествоПриборов.ОбъектРасчетаДолевойТУ";
...
Функция ТекстЗапросаДанныеДляНачисленияПоДолевымСхемам(Договор)
	...
	|	ДанныеПоСубабонентам.Шкала КАК Шкала,
