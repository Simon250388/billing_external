...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(ОтборПоДоговору,Отдельностоящие)
	...
	|	ГДЕ
	|		энргДлительностиПериодов.Сезонность = ЗНАЧЕНИЕ(Перечисление.энргВидыСезонности.ОтопительныйПериод)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		РегистрСведений.энргДлительностиПериодов.СрезПоследних(&НачалоПериодаРасчета, Организация = &Организация) КАК энргДлительностиПериодов
	|	ГДЕ
	|		энргДлительностиПериодов.Сезонность = ЗНАЧЕНИЕ(Перечисление.энргВидыСезонности.ОтопительныйПериод)) КАК ВложенныйЗапрос
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
