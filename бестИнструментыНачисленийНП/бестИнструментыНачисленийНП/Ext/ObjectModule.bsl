...
Функция ТекстПолучитьСтабильныеПериоды(ЭтоПерерасчет)  
	...
	|   ТемператураКатегорииПомещения.ТемператураОптимальная КАК ТемператураВнутриПомещения,
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП(ФормируемВременную,ИмяДокумента); 
	...
	|	ДанныеДляРасчета.ПриборВведен,
	 |	ВЫБОР
	         ...
	         |						ТОГДА ДанныеДляРасчета.ЗначениеНагрузки * (ЕСТЬNULL(ДанныеДляРасчета.ТемператураВнутриПомещения, 18) - ДанныеДляРасчета.СреднемесячнаяТемпература) / (ЕСТЬNULL(ДанныеДляРасчета.ТемператураВнутриПомещения, 18) - ДанныеДляРасчета.ПроектнаяТемператураНаружногоВоздуха) * РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(ДанныеДляРасчета.КонецПериода, ДЕНЬ), ДЕНЬ) * ДанныеДляРасчета.КоличествоЧасовРаботы
