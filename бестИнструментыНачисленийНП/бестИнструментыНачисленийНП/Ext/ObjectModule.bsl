				...
				Если НЕ ТекТочкаУчета = ВыборкаТочкиУчета.ТочкаУчета Или НЕ ТекУслуга = ВыборкаТочкиУчета.Услуга Тогда
					...
					Пока ВыборкаСубабоненты.ГоловнаяТочкаУчета = ВыборкаТочкиУчета.ТочкаУчета и ВыборкаСубабоненты.Услуга = ВыборкаТочкиУчета.Услуга Цикл
						Если НЕ ТекСубабонент = ВыборкаСубабоненты.ТочкаУчета Тогда
							...
							ДанныеСубабонента 								= Новый Структура("Договор, ОбъектРасчета, ТочкаУчета, ЗначениеПлощади, КоэффициентРасчета, СпособРаспределенияОбъемаДолевойТУ, СпособРаспределенияОстаткаДолевыеСхемы, ЗависимаяТочкаУчета, ЗначениеНагрузки",
							...
							ВыборкаСубабоненты.СпособРаспределенияОстаткаДолевыеСхемы, ВыборкаСубабоненты.ЗависимаяТочкаУчета, ВыборкаСубабоненты.ЗначениеНагрузки);
	...
	Для Сч = 0 По Субабоненты.ВГраница() Цикл
		...
		Иначе
			Если ДанныеСубабонента.СпособРаспределенияОбъемаДолевойТУ = Перечисления.бестСпособыРаспределенияДолевыхСхем.ДолевоеРаспределение Тогда
				ОбъемУслуги 				= ОсталосьРаспределить * ДанныеСубабонента.КоэффициентРасчета / 100;	
			ИначеЕсли ДанныеСубабонента.СпособРаспределенияОбъемаДолевойТУ = Перечисления.бестСпособыРаспределенияДолевыхСхем.ПропорциональноПлощади Тогда
				...
				ОбъемУслуги 				= ОсталосьРаспределить * ДанныеСубабонента.ЗначениеПлощади / ДанныеПоУслуге.ЗначениеПлощади;		
			КонецЕсли;
...
Функция ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам(Договор = Неопределено)
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |			И СтабПериодыПоМинПозиции.Услуга = бестФормулыРасчетаТочекУчетаНПСрезПоследних.Услуга
	        |;
	        |
	        |////////////////////////////////////////////////////////////////////////////////
	        |ВЫБРАТЬ
	        ...
	        |			И бестНомераСтрокСтабильногоПериодаНП.Услуга = ТочкиСубабонентов.Услуга
	        ...
	        |						ИЗ
	        |							ТочкиСубабонентов КАК ТочкиСубабонентов)) КАК бестОбъемНачисленийНПОбороты
...
Функция ТекстЗапросаДанныеДляНачисленияПоДолевымСхемам(Договор)
	...
	|			ПО УслугиДолевойТУ.Район = ОбъемыДолевойТУДляРаспределения.Район
	|				И УслугиДолевойТУ.Договор = ОбъемыДолевойТУДляРаспределения.Договор
	|				И УслугиДолевойТУ.ОбъектРасчета = ОбъемыДолевойТУДляРаспределения.ОбъектРасчета
	|				И УслугиДолевойТУ.Услуга = ОбъемыДолевойТУДляРаспределения.Услуга
	|				И УслугиДолевойТУ.ТочкаУчета = ОбъемыДолевойТУДляРаспределения.ТочкаУчета
	...
	|				И УслугиДолевойТУ.Организация = ДокументНачисления.Организация
	|				И УслугиДолевойТУ.Район = ДокументНачисления.Район
	|				И УслугиДолевойТУ.Договор = ДокументНачисления.Договор
	|			ЛЕВОЕ СОЕДИНЕНИЕ СуммарныйОбъемСубабонентов КАК СуммарныйОбъемСубабонентов
	|			ПО УслугиДолевойТУ.Договор = СуммарныйОбъемСубабонентов.ДоговорДолевойТУ
	|				И УслугиДолевойТУ.ОбъектРасчета = СуммарныйОбъемСубабонентов.ОбъектРасчетаДолевойТУ
	|				И УслугиДолевойТУ.ТочкаУчета = СуммарныйОбъемСубабонентов.ГоловнаяТочкаУчета) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	Договор,
	|	ОбъектРасчета,
