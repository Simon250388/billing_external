				...
				Если НЕ ТекТочкаУчета = ВыборкаТочкиУчета.ТочкаУчета Или НЕ ТекУслуга = ВыборкаТочкиУчета.Услуга Тогда
					...
					Пока ВыборкаСубабоненты.ГоловнаяТочкаУчета = ВыборкаТочкиУчета.ТочкаУчета и ВыборкаСубабоненты.Услуга = ВыборкаТочкиУчета.Услуга Цикл
						Если НЕ ТекСубабонент = ВыборкаСубабоненты.ТочкаУчета Тогда
							...
							ДанныеСубабонента 								= Новый Структура("Договор, ОбъектРасчета, ТочкаУчета, ЗначениеПлощади, КоэффициентРасчета, СпособРаспределенияОбъемаДолевойТУ, СпособРаспределенияОстаткаДолевыеСхемы, ЗависимаяТочкаУчета, ЗначениеНагрузки, ДоговорнойОбъем",
							ВыборкаСубабоненты.Договор, ВыборкаСубабоненты.ОбъектРасчета, ВыборкаСубабоненты.ТочкаУчета, ВыборкаСубабоненты.ЗначениеПлощади, ВыборкаСубабоненты.КоэффициентРасчета, ВыборкаСубабоненты.СпособРаспределенияОбъемаДолевойТУ, 
						...
						Если ВыборкаСубабоненты.ЕстьИндивидуальныеНачисления  Тогда 
							...
							СтрокаСписатьИндивидуальные.ОбъемУслуги		= ВыборкаСубабоненты.ОбъемУслугиОборот;
						КонецЕсли;

						
						ПотреблениеАбонентов 							= ВыборкаСубабоненты.ОбъемУслугиОборот;
...
Функция ТекстЗапросаПолучитьДанныеДляРасчетаНП(РасчетПоФормуле, ИмяДокумента)
	...
	|				ТОГДА Периоды.ТочкаУчета = Периоды.ЗависимаяТочкаУчета
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП(ФормируемВременную,ИмяДокумента); 
	...
	|					ИНАЧЕ ДанныеДляРасчета.ЗначениеНагрузки * (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ЧАС), НАЧАЛОПЕРИОДА(ДанныеДляРасчета.КонецПериода, ЧАС), ЧАС))
	|				КОНЕЦ
...
Функция ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам(Договор = Неопределено)
	...
	Текст = "ВЫБРАТЬ
	        ...
	        |				Организация = &Организация
	        |					И Район = &Район
	        |					И Договор = &Договор) КАК бестФормулыРасчетаТочекУчетаНПСрезПоследних
	        ...
	        |	бестСтабильныеПериодыНП.СпособРасчетаНорматива КАК СпособРасчетаНорматива,
	        |	ЕСТЬNULL(ГарантированныеОбъемы.ОбъемУслуги, 0) КАК ДоговорнойОбъем
	        ...
	        |	ВложенныйЗапрос.ЗначениеПлощади КАК СуммарноеЗначениеПлощадиСубабонентов,
	        |	ВложенныйЗапрос.ЗначениеНагрузки КАК СуммарноеЗначениеНагрузкиСубабонентов,
	        |	ДоговорныеОбъемы.ДоговорнойОбъем КАК СуммарноеЗначениеДоговорногоОбъемаСуб
	        ...
	        |			СУММА(ВложенныйЗапрос.ЗначениеНагрузки) КАК ЗначениеНагрузки
	        |		ИЗ
	        |			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	        |				ПараметрыСубабонентов.ОбъектРасчетаДолевойТУ КАК ОбъектРасчетаДолевойТУ,
	        |				ПараметрыСубабонентов.ГоловнаяТочкаУчета КАК ГоловнаяТочкаУчета,
	        ...
	        |			ВложенныйЗапрос.ДоговорДолевойТУ) КАК ВложенныйЗапрос
	        |		ПО УслугиДолевойТУ.Договор = ВложенныйЗапрос.ДоговорДолевойТУ
	        |			И УслугиДолевойТУ.ОбъектРасчета = ВложенныйЗапрос.ОбъектРасчетаДолевойТУ
	        |			И УслугиДолевойТУ.ТочкаУчета = ВложенныйЗапрос.ГоловнаяТочкаУчета
	        |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	        |			ВложенныйЗапрос.ГоловнаяТочкаУчета КАК ГоловнаяТочкаУчета,
	        |			ВложенныйЗапрос.ДоговорДолевойТУ КАК ДоговорДолевойТУ,
	        |			ВложенныйЗапрос.ОбъектРасчетаДолевойТУ КАК ОбъектРасчетаДолевойТУ,
	        |			СУММА(ВложенныйЗапрос.ДоговорнойОбъем) КАК ДоговорнойОбъем,
	        ...
	        |			ВложенныйЗапрос.Услуга) КАК ДоговорныеОбъемы
	        |		ПО УслугиДолевойТУ.Договор = ДоговорныеОбъемы.ДоговорДолевойТУ
	        |			И УслугиДолевойТУ.ОбъектРасчета = ДоговорныеОбъемы.ОбъектРасчетаДолевойТУ
	        |			И УслугиДолевойТУ.ТочкаУчета = ДоговорныеОбъемы.ГоловнаяТочкаУчета
	        |			И УслугиДолевойТУ.Услуга = ДоговорныеОбъемы.Услуга";
...
Функция ТекстЗапросаДанныеДляНачисленияПоДолевымСхемам(Договор)
	...
	|				И ДанныеПоДолевымТУ.ТочкаУчета = СуммарныйОбъемСубабонентов.ГоловнаяТочкаУчета) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	Договор,
	|	ОбъектРасчета,
	...
	|			ТОГДА ЗначенияПараметровОсновныхТочек.ЗначениеНагрузки
	|		ИНАЧЕ ЗначенияПараметровОсновныхТочек.ЗначениеНагрузки
