...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП(ФормируемВременную,ИмяДокумента,Отдельностоящие); 
	...
	|	(ЕСТЬNULL(ТемпературыИсточникаТочекУчета.ТемператураПодачиИсточника, 0) - ЕСТЬNULL(ТемпературыИсточникаТочекУчета.ТемператураХолоднойВодыИсточника, 0)) / 1000 КАК РасчетныйНормативНаПодогрев
