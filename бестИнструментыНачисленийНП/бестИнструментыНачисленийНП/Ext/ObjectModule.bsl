	...
	Пока ВыборкаМКД.Следующий() Цикл 		
		...
		Пока ВыборкаДок.Следующий() Цикл
			...
			Пока Выборка.следующий() цикл 
				...
				Если НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
					...
					Если НЕ ТекОбъектРасчета = Неопределено Тогда
						...
						ДобавитьСтрокиНачисленияНППоФормуле(ПериодНачисления, Организация, Район, ТекМКД, ТекОбъектРасчета, ТекТочкаУчета, КонецПериодаРасчета, ДокументСсылка, МассивФормулРасчета, НормативыПоУслугам, НормативНаПодогрев,  ТемпературныйКоэффициентПоВоздуху, ПлотностьВоды, Набор);
					КонецЕсли;
			...
			Если Не МассивФормулРасчета = Неопределено Тогда
				...
				ДобавитьСтрокиНачисленияНППоФормуле(ПериодНачисления, Организация, Район, ВыборкаМКД.МКД, ТекОбъектРасчета, ТекТочкаУчета, КонецПериодаРасчета, ДокументСсылка, МассивФормулРасчета, НормативыПоУслугам, НормативНаПодогрев, ТемпературныйКоэффициентПоВоздуху, ПлотностьВоды, Набор);
			КонецЕсли;
...
Процедура ДобавитьСтрокиНачисленияНППоФормуле(ПериодНачисления, Организация, Район, МКД, ОбъектРасчета, ТочкаУчета, КонецПериодаРасчета, ДокументСсылка, ОписаниеУслуг, НормативыПоУслугам, НормативНаПодогрев, ТемпературныйКоэффициентПоВоздуху, ПлотностьВоды, Набор)
...
Функция ТекстЗапросаПолучитьДанныеДляРасчетаНП(РасчетПоФормуле)
	...
	|	МАКСИМУМ(Периоды.ЗначениеНагрузки) КАК ЗначениеНагрузки,
	|	МАКСИМУМ(Периоды.ЗначениеМесячнойНагрузки) КАК ЗначениеМесячнойНагрузки,
	|	МАКСИМУМ(Периоды.ЗначениеПлощади) КАК ЗначениеПлощади,
	|	Периоды.ТарифнаяГруппа КАК ТарифнаяГруппа,
	|	Периоды.ДатаПоверки КАК ДатаПоверки,
	|	МАКСИМУМ(Периоды.ЗависимаяТочкаУчета) КАК ЗависимаяТочкаУчета,
	...
	|	МАКСИМУМ(Периоды.ЗначениеНормативаЗаДень) КАК ЗначениеНормативаЗаДень,
	...
	|	МАКСИМУМ(Периоды.ТемператураВнутриПомещения) КАК ТемператураВнутриПомещения,
	...
	|	МИНИМУМ(Периоды.НачальныеПоказания) КАК НачПок,
	...
	|	МАКСИМУМ(энргПриборыУчетаАбонента.МаксимальноВозможныеПоказания) КАК МаксимальноВозможныеПоказания,
	|	ВЫБОР
	...
	|	МАКСИМУМ(Периоды.КоэффициентНорматива) КАК КоэффициентНорматива,
	|	МАКСИМУМ(Периоды.КоэффициентНормативаДляФакта) КАК КоэффициентНормативаДляФакта,
	|	МАКСИМУМ(Периоды.ГарантированныйОбъем) КАК ГарантированныйОбъем,
	|	МАКСИМУМ(ЕСТЬNULL(ФактическаяСреднемесячнаяТемпература.СреднемесячнаяТемпература, 0)) КАК СреднемесячнаяТемпература,
	|	МАКСИМУМ(ЕСТЬNULL(ТемператураНаружногоВоздуха.ПроектнаяТемпература, &ПроектнаяТемператураПоУмолчанию)) КАК ПроектнаяТемператураНаружногоВоздуха,
	...
	|	МАКСИМУМ(ЕСТЬNULL(РасходЗаПредПериод.КоличествоЧасовШтатнойРаботыПУОборот, 0)) КАК ВремяШтатнойРаботыПУЗаПредыдущийПериод,
	|	МАКСИМУМ(ЕСТЬNULL(РасходЗаПредПериод.КоличествоЧасовРаботыТУОборот, 0)) КАК КоличествоЧасовРаботыТУЗаПредыдущийПериод,
	|	МАКСИМУМ(ЕСТЬNULL(ЗначенияПлощадейСтроений.ПлощадьМОП, 0)) КАК ПлощадьМОП,
	|	МАКСИМУМ(ЕСТЬNULL(ЗначенияПлощадейСтроений.ПлощадьАбонентов, 0)) КАК ПлощадьАбонентов,
	|	МАКСИМУМ(ЕСТЬNULL(ЗначенияПлощадейСтроений.ПлощадьСобственнаяПоставка, 0)) КАК ПлощадьСобственнаяПоставка,
	|	МАКСИМУМ(ЕСТЬNULL(ЗарегистрированныеОбъемы.ЗарегистрированныйОбъем, -1)) КАК ЗарегистрированныйОбъем,
	|	МАКСИМУМ(ЕСТЬNULL(ЗарегистрированныеОбъемы.КоличествоЧасовРаботыТУ, 0)) КАК КолЧасовРаботыТУЗарегОбъем,
	|	МАКСИМУМ(ЕСТЬNULL(ЗарегистрированныеОбъемы.КоличествоЧасовШтатнойРаботыПУ, 0)) КАК КоличествоЧасовШтатнойРаботыПУЗарегОбъем,
	|	МАКСИМУМ(ЕСТЬNULL(ЗарегистрированныеОбъемы.ЗарегистрированныйОбъемПоПрибору, 0)) КАК ЗарегистрированныйОбъемПоПрибору,
	|	МАКСИМУМ(ЕСТЬNULL(ЗарегистрированныеОбъемы.ОбъемМГВ, 0)) КАК ОбъемМГВ,
	|	МАКСИМУМ(ЕСТЬNULL(ЗарегистрированныеОбъемы.СпособНачисления, ЗНАЧЕНИЕ(Перечисление.энргСпособНачислений.Пустаяссылка))) КАК СпособНачисления,
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ЗарегистрированныеОбъемы.ТочкаУчета ЕСТЬ NULL
	|				ТОГДА ЛОЖЬ
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ) КАК НаличиеЗарегистрированногоОбъема,
	|	МАКСИМУМ(Периоды.РасчетПоФактическомуПотреблению) КАК РасчетПоФактическомуПотреблению,
	|	МАКСИМУМ(Периоды.ДиаметрСечения) КАК ДиаметрСечения,
	|	МАКСИМУМ(ЕСТЬNULL(НормаСкоростиДвиженияВоды.СкоростьДвижения, 0)) КАК СкоростьДвижения
	...
	|	Периоды.ТочкаУчета,
	|	Периоды.УзелУчета,
	|	ВЫБОР КОГДА КоличествоПУВУзлеУчета.КоличествоПУ > 1 ТОГДА Периоды.УзелУчета ИНАЧЕ Периоды.ПриборУчета КОНЕЦ,
	|	Периоды.ДоговорЗаключен,
	...
	|	Периоды.Поставщик,
	|	ВЫБОР КОГДА КоличествоПУВУзлеУчета.КоличествоПУ > 1 ТОГДА ЗНАЧЕНИЕ(Справочник.энргШкалыПриборовУчета.Основная) ИНАЧЕ Периоды.Шкала КОНЕЦ,
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияНП() 
	...
	//|	И ДанныеДляРасчетаПередНормой.Договор.ТипДоговора <> Значение(Перечисление.бестТипыДоговоров.УправляющаяКомпания)
	...
	//|	ЕСТЬNULL(ДанныеДляРасчетаГруппировкаПоУзлу.ДиаметрСечения,0),
	...
	|	РезультатБезЗависимости.ПривязанаКШкале КАК ПривязанаКШкале,
