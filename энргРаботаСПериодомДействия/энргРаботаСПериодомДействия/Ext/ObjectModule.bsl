...
Процедура СтабПериодыУстановитьЗначениеПлощадиМКД(ссылка,Параметры) 
	...
	|			И (НЕ энргУстановитьЗначениеПлощадиМКДЗначениеПлощадей.Услуга = ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка))";
...
Функция ТекстЗапросаПодключениеУслугиМКД()
	...
	|			Организация = &Организация
	|				И МКД = &МКД
	|				И Район = &Район) КАК энргЗначениеПлощадейМКДСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаСтабПериодыАктВыводаПрибораУчетаМКД()
	...
	|	ШкалыПриборовУчетаНачало.КоэффициентПУ КАК КоэффициентПУ,
