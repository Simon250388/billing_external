...
Функция ТекстЗапросаШкалыПриборовУчетаМКД() 
	...
	|	0,
	|	энргОперацияСУслугамиМКДШкалыПриборовУчета.Шкала,
	|	энргОперацияСУслугамиМКДШкалыПриборовУчета.ВидДифференцированности,
	|	&ссылка
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТочкиУчета.ТочкаУчета КАК ТочкаУчета,
	...
	|		ТочкиУчета.Ссылка = &Ссылка
	|		И НЕ ТочкиУчета.ПриборУчета = ЗНАЧЕНИЕ(Справочник.энргПриборыУчетаАбонента.ПустаяСсылка)) КАК ВложенныйЗапрос
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.энргОперацияСУслугамиМКД.ШкалыПриборовУчета КАК энргОперацияСУслугамиМКДШкалыПриборовУчета
	|		ПО ВложенныйЗапрос.Ссылка = энргОперацияСУслугамиМКДШкалыПриборовУчета.Ссылка
	|			И ВложенныйЗапрос.IDСтроки = энргОперацияСУслугамиМКДШкалыПриборовУчета.IDСтрокиТочкиУчета
	|			И (энргОперацияСУслугамиМКДШкалыПриборовУчета.Ссылка = &Ссылка)";
