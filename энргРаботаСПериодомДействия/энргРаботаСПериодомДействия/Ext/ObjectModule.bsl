...
Функция ОпределитьПериодНачисления(Организация, ДатаДокумента) Экспорт
	...
	ДатаКонстанта 				= Константы.энргДеньЗавершенияРасчетногоПериода.Получить();
...
Процедура ПровестиПоСтабильнымПериодам(ссылка,Параметры) Экспорт 
	...
	иначеЕсли  ТипЗнч(ссылка)  =Тип("ДокументСсылка.энргОперацияСЛицевымСчетом") и ссылка.ВидОперации = Перечисления.энргВидыОпераций.энргОперацияСЛС_ОткрытиеЛС тогда
		...
		Если Параметры.Свойство("РазделенныйЛС") И Параметры.РазделенныйЛС Тогда
...
Процедура ЗаполнитьДанныеДляПерерасчета(Объект) Экспорт 
	...
	Если ПерерасчетыПоОснованию.Количество() > 1 тогда
		ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю("Найдено более одного документа перерасчет");
...
Процедура СтабПериодыУстановитьКоличествоКомнат(ссылка,Параметры) 
	...
	|ПОМЕСТИТЬ ДанныеКоличестваКомнат
	|ИЗ
	...
	ЗаполнитьНомераПозиций(МВТ,Параметры,"ДанныеКоличестваКомнат"); 	
...
Процедура СтабПериодыУстановитьКоличествоПрописанных(ссылка,Параметры) 
	...
	|ПОМЕСТИТЬ ДанныеКоличестваПрописанных
	|ИЗ
	|	ДанныеДокумента КАК ДанныеДокумента";	
	...
	ЗаполнитьНомераПозиций(МВТ,Параметры,"ДанныеКоличестваПрописанных"); 	
...
Процедура СтабПериодыУстановитьФормулуРасчетаТочекУчета(ссылка,Параметры) 
	...
	Запрос.УстановитьПараметр("Район",					ПараметрыЗапроса.Район);
	Запрос.УстановитьПараметр("Абонент",				ПараметрыЗапроса.Абонент);
	...
	Запрос.УстановитьПараметр("ПериодНачисления",	Параметры.ПериодНачисления); 	
	Запрос.Текст 					="ВЫБРАТЬ
	|	энргВладельцыТочекУчетаСрезПоследних.ТочкаУчета
	|ПОМЕСТИТЬ ТочкиАбонента
	|ИЗ
	|	РегистрСведений.энргВладельцыТочекУчета.СрезПоследних(&МоментВремени, Организация = &Организация И Район = &Район) КАК энргВладельцыТочекУчетаСрезПоследних
	|ГДЕ
	...
	|				И Абонент = &Абонент) КАК энргАктивностьТочекУчетаСрезПоследних
	|ГДЕ
	...
	|			Организация = &Организация и Район = &Район
	|				И Абонент = &Абонент
	|				И ТочкаУчета = &РасчитываемаяТочка) КАК энргРасчетныеСпособыТочекУчетаСрезПоследних
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВключенныеТочки.ТочкаУчета
	|ПОМЕСТИТЬ ДействующиеТочки
	|ИЗ
	|	ВключенныеТочки КАК ВключенныеТочки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТочкиАбонента КАК ТочкиАбонента
	|		ПО ВключенныеТочки.ТочкаУчета = ТочкиАбонента.ТочкаУчета
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	энргФормулыРасчетаТочекУчетаСрезПоследних.ТочкаИсточник
	|ПОМЕСТИТЬ ТочкиПредыдущейФормулы
	|ИЗ
	|	ДокументыУстановкиПредыдущейФормулы КАК ДокументыУстановкиПредыдущейФормулы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргФормулыРасчетаТочекУчета.СрезПоследних КАК энргФормулыРасчетаТочекУчетаСрезПоследних
	|		ПО ДокументыУстановкиПредыдущейФормулы.Период = энргФормулыРасчетаТочекУчетаСрезПоследних.Период
	|			И ДокументыУстановкиПредыдущейФормулы.Регистратор = энргФормулыРасчетаТочекУчетаСрезПоследних.Регистратор
	|			И ДокументыУстановкиПредыдущейФормулы.Район = энргФормулыРасчетаТочекУчетаСрезПоследних.Район
	|			И ДокументыУстановкиПредыдущейФормулы.Абонент = энргФормулыРасчетаТочекУчетаСрезПоследних.Абонент
	|			И ДокументыУстановкиПредыдущейФормулы.ТочкаУчета = энргФормулыРасчетаТочекУчетаСрезПоследних.РассчитываемяТочка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	энргУстановитьФормулуРасчетаТочекУчета.Организация,
	|	энргУстановитьФормулуРасчетаТочекУчета.ПериодНачисления,
	|	энргУстановитьФормулуРасчетаТочекУчета.Район,
	|	энргУстановитьФормулуРасчетаТочекУчета.МКД,
	|	энргУстановитьФормулуРасчетаТочекУчета.Абонент,
	...
	|	энргУстановитьФормулуРасчетаТочекУчета.МКД,
	|	энргУстановитьФормулуРасчетаТочекУчета.Абонент,
	|	энргУстановитьФормулуРасчетаТочекУчетаТочкиУчета.ТочкаУчета,
	|	ЛОЖЬ,
	|	энргУстановитьФормулуРасчетаТочекУчета.РассчитываемаяТочкаУчета,
	|	энргУстановитьФормулуРасчетаТочекУчетаТочкиУчета.Коэффициент
	|ИЗ
	|	Документ.энргУстановитьФормулуРасчетаТочекУчета КАК энргУстановитьФормулуРасчетаТочекУчета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.энргУстановитьФормулуРасчетаТочекУчета.ТочкиУчета КАК энргУстановитьФормулуРасчетаТочекУчетаТочкиУчета
	|		ПО энргУстановитьФормулуРасчетаТочекУчета.Ссылка = энргУстановитьФормулуРасчетаТочекУчетаТочкиУчета.Ссылка
	|			И (энргУстановитьФормулуРасчетаТочекУчета.Ссылка = &ссылка)
	...
	|	Шапка.Абонент,
	|	УслугиТочек.Услуга,
	|	Шапка.РасчетПоФормуле,
	|	Шапка.ТочкаУчета,
	|	Шапка.РассчитываемаяТочкаУчета,
	|	Шапка.КоэффициентРасчета
	...
	|	ВозможноСоставные.МКД,
	|	ВозможноСоставные.Абонент,
	|	ВЫБОР
	|		КОГДА энргВидыУслугСоставляющиеУслуг.Ссылка ЕСТЬ NULL 
	...
	|	ВозможноСоставные.КоэффициентРасчета,
	|	ВозможноСоставные.РасчетПоФормуле,
	...
	ЗаполнитьНомераПозиций(МВТ,Параметры,"ТочкаПоФормуле");
...
Процедура СтабПериодыУстановитьСпособРаспределенияОбъемаПомещения(ссылка,Параметры)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДанныеШапкиУслуги.Организация КАК Организация,
	|	ДанныеШапкиУслуги.Район КАК Район,
	|	ДанныеШапкиУслуги.КлючПомещения КАК КлючПомещения,
	|	ДанныеШапкиУслуги.ПериодНачисления КАК ПериодНачисления,
	|	ДанныеШапкиУслуги.Услуга КАК Услуга,
	...
	|	ДанныеШапкиУслуги.Ссылка КАК Ссылка,
	|	ТЧАбоненты.IDСтроки КАК IDСтроки,
	|	ДанныеШапкиУслуги.ЧастныйСектор КАК ЧастныйСектор,
	|	ДанныеШапкиУслуги.Строение КАК Строение,
	|	ДанныеШапкиУслуги.Помещение КАК Помещение
	|ПОМЕСТИТЬ ДанныеАбонентов
	|ИЗ
	|	Документ.энргСоглашениеОПотребленииВПомещении.Абоненты КАК ТЧАбоненты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеШапкиУслуги КАК ДанныеШапкиУслуги
	|		ПО (ТЧАбоненты.Ссылка = &Ссылка)
	|			И (ДанныеШапкиУслуги.Ссылка = &Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаОткрытиеЛС() 
	...
	|				И Услуги.НаправлениеИспользованияТУ = СпособыРасчетаПоНаправлениям.НаправлениеИспользования
...
Функция ТекстЗапросаПодключениеУслуги()
	...
	|				И Район = &Район
	|				И ВидПлощади В (ЗНАЧЕНИЕ(Перечисление.энргВидыПлощадей.ОбщаяПлощадь), ЗНАЧЕНИЕ(Перечисление.энргВидыПлощадей.Поливная))) КАК энргЗначенияПлощадейСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
