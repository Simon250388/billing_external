...
Перем ИзменяемыеПараметрыПоТипам, ПоляПорядка, ИзмеренияИсключая, ТаблицыДанных, ТаблицыИзмененияПлощади, ТаблицыИзмененияШкал;
...
Функция ЗначенияПараметровПоНаправлениямУслуг() Экспорт 
	Возврат РегистрыСведений.энргСпособыРасчетаПоНаправлениям.ЗначенияПараметровПоНаправлениямУслугКоллекцияПериодов(Организация,Район,)
КонецФункции
...
Функция ДействующуюРазрезыНорматива() Экспорт 
	Схема 									= Неопределено;
...
Процедура ЗаполнитьСхемуЗапросаПромежуточнымиДанными(Схема, СоотвествиеПолей)
	...
	РегистрыСведений.энргСтабильныеПериодыПоследовательность.НаборДокументовДляПроведенияПоФактическомуПериоду(Схема,Параметры,"ДокументыДляПроведения");
