...
Процедура Инициализировать(МВТ, ПолучаемыеНаборы)  Экспорт 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("слжСлужебная") Тогда		
		Для ИндексТаблицы = 0 По ПолучаемыеНаборы.ВГраница() Цикл
			...
			ИмяМетода 					= "ДелегатНачислений.Инициализировать" + "." + ПолучаемыеНаборы[ИндексТаблицы].ТаблицаДляПомещения;
...
Процедура ПослеПроведенияМКД(Ошибки) Экспорт 
	...
	Попытка
		...
		Если ТЗОрганизаций.Количество() > 1 Тогда
			НаборОбъемНачислений.ДополнительныеСвойства.Вставить("Организация",		ТЗОрганизаций.ВыгрузитьКолонку("Организация"));	
		КонецЕсли;
