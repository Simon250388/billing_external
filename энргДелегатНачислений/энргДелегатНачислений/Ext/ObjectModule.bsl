...
Процедура ПередПроведениемМКД(МКД, яДокументСсылка, ПометкаУдаления) Экспорт 
	...
	Если Не МенеджерОбъемаНачисленийПомещений = Неопределено Тогда
		...
		НаборОбъемНачисленийПомещений.ДополнительныеСвойства.Вставить("Строение",			Строение);
	...
	Если Не МассивПомещений = Неопределено Тогда		
		...
		МенеджерОбъемаНачислений.УдалитьЗаписиПоПомещению(НаборОбъемНачислений, НаборПрочитан);
...
Процедура ПослеПроведенияМКД(Ошибки) Экспорт 
	...
	Попытка
		...
		МенеджерОбъемаНачислений.ОчиститьВременныеНачисления(Организация, ПериодНачисления, Район, ЧастныйСектор, Строение);
