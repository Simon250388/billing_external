...
Процедура СтабПериодыНПАктВводаПрибораУчета(Ссылка,Параметры)  
	...
	Запрос.Текст 	="ВЫБРАТЬ
	             	 ...
	             	 |			И ОтменаФормулыТекущимДокументом.ТочкаУчета = бестФормулыРасчетаТочекУчетаНП.ГоловнаяТочкаУчета
	             	 |;
	             	 |
	             	 |////////////////////////////////////////////////////////////////////////////////
	             	 |ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура СтабПериодыНПУстановитьФормулуРасчетаТочекУчета(ссылка,Параметры) 
	...
	//МассивТочек.добавить(ссылка.ДолеваяТочкаУчета);
	ПараметрыЗапроса  							= Параметры.ПараметрыЗапроса;	
	МВТ 										= Новый МенеджерВременныхТаблиц;
	Запрос  									= Новый Запрос;
	Запрос.УстановитьПараметр("ссылка",					ссылка);
	...
	//Запрос.УстановитьПараметр("МассивТочек",			МассивТочек);
	//Запрос.УстановитьПараметр("РасчитываемаяТочка",     ссылка.ДолеваяТочкаУчета);
	Запрос.МенеджерВременныхТаблиц 	= МВТ;     	
