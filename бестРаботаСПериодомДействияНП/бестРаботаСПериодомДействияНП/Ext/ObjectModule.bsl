...
Процедура СтабПериодыНПАктВводаПрибораУчета(Ссылка,Параметры)  
	...
	|		ТочкиУчета.УзелУчета КАК УзелУчета,
	...
	|	ТочкиВводаПрибора.Услуга КАК Услуга,
	...
	|	ТочкиВводаПрибора.МенялосьСостояниеПУ КАК МенялосьСостояниеПУ,
	|	ТочкиВводаПрибора.IDСтроки КАК IDСтроки,
	|	ТочкиВводаПрибора.ОбъектРасчета КАК ОбъектРасчета,
	|	ТочкиВводаПрибора.УзелУчета КАК УзелУчета,
	|	ТочкиВводаПрибора.МКД КАК МКД,
	|	ПриборыУзловУчета.ПриборУчета КАК ПриборУчета,
	|	ПриборыУзловУчета.IDСтрокиШкалы КАК IDСтрокиШкалы
	|ПОМЕСТИТЬ ТочкиВводаУзлыПриборы
	|ИЗ
	|	ТочкиВводаПрибора КАК ТочкиВводаПрибора
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.бестОперацияСПриборомУчетаНП.ПриборыУзловУчета КАК ПриборыУзловУчета
	|		ПО (ПриборыУзловУчета.Ссылка = &Ссылка)
	|			И ТочкиВводаПрибора.IDСтроки = ПриборыУзловУчета.IDСтроки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТочкиВводаУзлыПриборы.Организация КАК Организация,
	|	ТочкиВводаУзлыПриборы.ПериодНачисления КАК ПериодНачисления,
	|	ТочкиВводаУзлыПриборы.Район КАК Район,
	|	ТочкиВводаУзлыПриборы.Договор КАК Договор,
	|	ТочкиВводаУзлыПриборы.ТочкаУчета КАК ТочкаУчета,
	...
	|	ТочкиВводаУзлыПриборы.ОбъектРасчета КАК ОбъектРасчета,
	|	ТочкиВводаУзлыПриборы.УзелУчета КАК УзелУчета,
	|	ТочкиВводаУзлыПриборы.ПриборУчета КАК ПриборУчета,
	|	ТочкиВводаУзлыПриборы.МКД КАК МКД
	|ПОМЕСТИТЬ ШкалыПриборовУчета
	|ИЗ
	|	ТочкиВводаУзлыПриборы КАК ТочкиВводаУзлыПриборы
	...
	|		ПО ТочкиВводаУзлыПриборы.IDСтроки = бестОперацияСПриборомУчетаНПШкалыПриборовУчета.IDСтрокиТочкиУчета
	...
	ЗаполнитьНомераПозицийНП(МВТ,Параметры,"ТочкиВводаУзлыПриборы"); 
	...
	|	НомераПозиций.УзелУчета КАК УзелУчета,
	|	НомераПозиций.ПриборУчета КАК ПриборУчета,
	...
	|		И НомераПозиций.УзелУчета = ШкалыПриборовУчета.УзелУчета
...
Процедура СтабПериодыНПУстановитьФормулуРасчетаТочекУчета(ссылка,Параметры) 
	...
	Запрос.Текст 					="ВЫБРАТЬ
	             					 ...
	             					 |	бестУстановитьФормулуРасчетаТочекУчетаНПТочкиУчета.СпособРасчетаПриВыходеПУ КАК СпособРасчетаПриВыходеПУ,
	             					 ...
	             					 |	Шапка.СпособРасчетаПриВыходеПУ КАК СпособРасчетаПриВыходеПУ,
	...
	Ресурсы.Добавить("СпособРасчетаПриВыходеПУ");
...
Функция ТекстЗапросаЗаключениеДоговора() 
	...
	|		бестЗаключениеДоговоровСЮридическимиЛицамиТочкиУчета.ПриборУчета КАК УзелУчета,
	...
	|	ДанныеТочекУчета.УзелУчета КАК УзелУчета
	|ПОМЕСТИТЬ Услуги
	|ИЗ
	...
	|	Услуги.УзелУчета КАК УзелУчета,
	|	бестЗаключениеДоговоровСЮридическимиЛицамиПриборыУзловУчета.ПриборУчета КАК ПриборУчета,
	|	ВЫБОР
	|		КОГДА бестЗаключениеДоговоровСЮридическимиЛицамиПриборыУзловУчета.ПриборУчета ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ПриборВведен
	|ПОМЕСТИТЬ УслугиПриборы
	|ИЗ
	|	Услуги КАК Услуги
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.бестЗаключениеДоговоровСЮридическимиЛицами.ПриборыУзловУчета КАК бестЗаключениеДоговоровСЮридическимиЛицамиПриборыУзловУчета
	|		ПО Услуги.IDСтроки = бестЗаключениеДоговоровСЮридическимиЛицамиПриборыУзловУчета.IDСтроки
	|			И Услуги.ДокРегистратор = бестЗаключениеДоговоровСЮридическимиЛицамиПриборыУзловУчета.Ссылка
	|			И Услуги.Услуга = бестЗаключениеДоговоровСЮридическимиЛицамиПриборыУзловУчета.Услуга
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УслугиПриборы.ПериодНачисления КАК ПериодНачисления,
	|	УслугиПриборы.Район КАК Район,
	|	УслугиПриборы.ТочкаУчета КАК ТочкаУчета,
	|	УслугиПриборы.НомерПозиции КАК НомерПозиции,
	|	УслугиПриборы.ДатаРегистратора КАК ДатаРегистратора,
	|	УслугиПриборы.ДокРегистратор КАК ДокРегистратор,
	|	УслугиПриборы.УслугаПодключена КАК УслугаПодключена,
	|	УслугиПриборы.КоэффициентПУ КАК КоэффициентПУ,
	|	УслугиПриборы.IDСтроки КАК IDСтроки,
	|	УслугиПриборы.Организация КАК Организация,
	|	УслугиПриборы.ДоговорЗаключен КАК ДоговорЗаключен,
	|	УслугиПриборы.ОбъектРасчета КАК ОбъектРасчета,
	|	УслугиПриборы.Договор КАК Договор,
	|	УслугиПриборы.ОбъектРасчетаПодключен КАК ОбъектРасчетаПодключен,
	|	УслугиПриборы.ЭтоСоставнаяУслуга КАК ЭтоСоставнаяУслуга,
	|	УслугиПриборы.Услуга КАК Услуга,
	|	УслугиПриборы.СоставнаяУслуга КАК СоставнаяУслуга,
	|	ВЫБОР
	|		КОГДА бестЗаключениеДоговоровСЮридическимиЛицамиСпособыРасчетаНормативаСоставныхУслуг.СпособРасчетаНорматива ЕСТЬ NULL
	|			ТОГДА УслугиПриборы.СпособРасчетаНорматива
	...
	|	УслугиПриборы.IDСтрокиВладельца КАК IDСтрокиВладельца,
	|	УслугиПриборы.ВидБлагоустройства КАК ВидБлагоустройства,
	|	УслугиПриборы.Поставщик КАК Поставщик,
	|	УслугиПриборы.УзелУчета КАК УзелУчета,
	|	УслугиПриборы.ПриборУчета КАК ПриборУчета,
	|	УслугиПриборы.ПриборВведен КАК ПриборВведен
	|ПОМЕСТИТЬ УслугиСпособРасчетаНорматива
	|ИЗ
	|	УслугиПриборы КАК УслугиПриборы
	...
	|		ПО УслугиПриборы.ДокРегистратор = бестЗаключениеДоговоровСЮридическимиЛицамиСпособыРасчетаНормативаСоставныхУслуг.Ссылка
	|			И УслугиПриборы.IDСтроки = бестЗаключениеДоговоровСЮридическимиЛицамиСпособыРасчетаНормативаСоставныхУслуг.IDСтроки
	|			И УслугиПриборы.Услуга = бестЗаключениеДоговоровСЮридическимиЛицамиСпособыРасчетаНормативаСоставныхУслуг.ЧастьСоставнойУслуги
	|			И (УслугиПриборы.ЭтоСоставнаяУслуга)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		УслугиСпособРасчетаНорматива.УзелУчета КАК УзелУчета,
...
Функция ТекстЗапросаШкалыПриборовУчета() 
	...
	|		ПриборыУзловУчета.ПриборУчета КАК ПриборУчета,
	|		ТочкиУчета.ПриборУчета КАК УзелУчета,
	|		ПриборыУзловУчета.IDСтрокиШкалы КАК IDСтрокиШкалы
	|	ИЗ
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.бестЗаключениеДоговоровСЮридическимиЛицами.ПриборыУзловУчета КАК ПриборыУзловУчета
	|			ПО ТочкиУчета.Ссылка = ПриборыУзловУчета.Ссылка
	|				И ТочкиУчета.IDСтроки = ПриборыУзловУчета.IDСтроки
	|	ГДЕ
	...
	|			И ВложенныйЗапрос.IDСтрокиШкалы = ШкалыПриборовУчета.IDСтроки
	...
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.бестОперацияСУслугамиНП.ПриборыУзловУчета КАК ПриборыУзловУчета
	|			ПО ТочкиУчета.Ссылка = ПриборыУзловУчета.Ссылка
	|				И ТочкиУчета.IDСтроки = ПриборыУзловУчета.IDСтроки
	|	ГДЕ
	|		ТочкиУчета.Ссылка = &Ссылка
	|		И НЕ ТочкиУчета.ПриборУчета = ЗНАЧЕНИЕ(Справочник.энргПриборыУчетаАбонента.ПустаяСсылка)
	|		И ПриборыУзловУчета.Ссылка = &Ссылка) КАК ВложенныйЗапрос
...
Процедура ДобавитьСтрокуСтабильногоПериодаНП(МВТ,Параметры,ИмяТаблицы,Увеличивать)
	...
	//|			И НомераПозиций.УзелУчета = бестСтабильныеПериодыНП.УзелУчета
	...
	Если ДобавлятьШкалы Тогда
		...
		|			И НомераПозиций.УзелУчета = бестСтабильныеПериодыШкалыНП.УзелУчета
