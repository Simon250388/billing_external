...
Процедура ПровестиПоСтабильнымПериодамНП(ссылка,Параметры) Экспорт 
	...
	ИначеЕсли  ТипЗнч(ссылка)  =Тип("ДокументСсылка.бестЗаключениеДоговоровСЮридическимиЛицами") и (ссылка.ВидОперации = Перечисления.бестВидыОпераций.ОперацияСДоговором_ПерезаключениеДоговора 
		ИЛИ ссылка.ВидОперации = Перечисления.бестВидыОпераций.ОперацияСДоговором_ЗаменаДоговораУОбъекта) тогда
...
Процедура СтабПериодыНПОтключениеУслуги(ссылка,параметры)
	...
	Запрос.Текст 	="ВЫБРАТЬ
	             	 ...
	             	 |				И бестОперацияСУслугамиНПТочкиУчета.Флаг)
	             	 |;
	             	 |
	             	 |////////////////////////////////////////////////////////////////////////////////
	             	 |ВЫБРАТЬ
...
Процедура СтабПериодыИзменитьДоговорПоУслуге(Ссылка,Параметры)
	...
	Запрос.Текст 	="ВЫБРАТЬ
	             	 ...
	             	 |	бестИзменитьДоговорПоУслуге.ПериодНачисления КАК ПериодНачисления,
...
Функция ТекстЗапросаЗаключениеДоговора() 
	...
	|	ВложенныйЗапрос.ПриборУчета КАК ПриборУчета,
	|	ВложенныйЗапрос.ПриборВведен КАК ПриборВведен,
	|	ВложенныйЗапрос.ДиаметрСечения * 10 КАК ДиаметрСечения
	...
	|				И УслугиСДоговорами.IDСтроки = бестЗаключениеДоговоровСЮридическимиЛицамиФормулыРасчетаУслуг.IDСтроки
...
Функция ТекстЗапросаПодключениеУслуги() 
	...
	|		ПО ДанныеШапкиСоставныеУслугиДоговоры.IDСтроки = бестОперацияСУслугамиНПФормулыРасчетаУслуг.IDСтроки
...
Процедура ДобавитьСтрокуСтабильногоПериодаНП(МВТ,Параметры,ИмяТаблицы,Увеличивать)
	...
	|			И НомераПозиций.МКД = бестСтабильныеПериодыПриборыУзловУчета.МКД
	...
	|	НомераПозицийПриборыУзловУчета.МКД КАК МКД,
	...
	|	НомераПозицийПриборыУзловУчета.МКД,
