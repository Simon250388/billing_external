		...
		Для Каждого СтрокаПериод Из СтрокаПоФормуле.Значение Цикл
			...
			Для Каждого ПараметрФормулы Из ПараметрыФормулы Цикл
				...
				Если НачисленияПоУслугам[ПараметрФормулы.Услуга] = Неопределено Тогда
					НачисленияПоУслугам.Вставить(ПараметрФормулы.Услуга, Новый Структура("ОбъемУслуги,ОбъемУслугиПотребленный,ВсеНачисленияПоПрибору",0,0,Ложь));
				КонецЕсли;  				
			...
			Для Сч = 0 По ИндексыСтрок.ВГраница() Цикл
				...
				Если ЗначениеЗаполнено(СтрокаНабора.ЗависитОтУслуги) Тогда
				...
				Если НЕ СтрокаНабора.ОбъемУслуги = 0 Тогда
					СтрокаНабора.ОбъемУслуги						= ОбъемУслуги;
				КонецЕсли;
				Если НЕ СтрокаНабора.ОбъемУслугиПотребленный = 0 Тогда
