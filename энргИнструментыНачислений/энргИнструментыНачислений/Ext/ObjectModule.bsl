...
Процедура НачислениеПоДолевымСхемам(Организация,ПериодНачисления,Район,Строение,Абонент,Документ)  Экспорт 
	...
	Запрос.УстановитьПараметр("НачалоПериода",		ГраницыПериода.НачалоОП);
	Запрос.УстановитьПараметр("КонецПериода",		ГраницыПериода.ЗавершениеОП);
	Запрос.Текст 					= ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам();
	...
	Пока ВыборкаДок.Следующий() Цикл
		...
		ДокументСсылка 							= ВыборкаДок.ДокументНачисленияПоДолевымСхемам;			
		
		Если НЕ ЗначениеЗаполнено(ДокументСсылка) Тогда			
			ДокументСсылка 						= Документы.энргНачислениеПоФормуламРасчета.ПолучитьСсылку(Новый УникальныйИдентификатор());
			ДокументОбъект 						= Документы.энргНачислениеПоФормуламРасчета.СоздатьДокумент();
			ДокументОбъект.Район 				= Район;
			ДокументОбъект.МКД 					= ВыборкаДок.Строение;
			ДокументОбъект.Дата 				= ГраницыПериода.ЗавершениеОП-1;
			ДокументОбъект.Организация			= Организация;
			ДокументОбъект.УстановитьСсылкуНового(ДокументСсылка);	
	...
	Если Не НаборЗаписей = Неопределено Тогда
		...
		Если НаборЗаписей.Количество() > 0 Тогда
			...
			Попытка				
				...
				НаборЗаписей.ДополнительныеСвойства.Вставить("Строение",    		Выборка.Строение);
...
Функция ТекстПолучитьСтабильныеПериоды(ЭтоПерерасчет)
	...
	|	СтабильныеПериодыТариф.ПредыдущееЗначениеТарифаСверхНорматива КАК ПредыдущееЗначениеТарифаСверхНорматива,
	|	ВЫБОР
	...
	|	ЕСТЬNULL(ДлительностьПериодов.НормативПоДням, ЛОЖЬ) КАК НормативПоДням
	...
	|	СтабильныеПериодыНачалоСезонности.ПредыдущееЗначениеТарифаСверхНорматива КАК ПредыдущееЗначениеТарифаСверхНорматива,
	...
	|	КОНЕЦ КАК УвеличениеПозиции,
	|	СтабильныеПериодыНачалоСезонности.НормативПоДням
	...
	|	СтабильныеПериодыСезонность.ПредыдущееЗначениеТарифаСверхНорматива КАК ПредыдущееЗначениеТарифаСверхНорматива,
	...
	|	КОНЕЦ КАК УвеличениеПозиции,
	|	СтабильныеПериодыСезонность.НормативПоДням
	...
	|	СтабильныеКонецСезонности.ПредыдущееЗначениеТарифаСверхНорматива КАК ПредыдущееЗначениеТарифаСверхНорматива,
	...
	|	СтабильныеКонецСезонности.НормативПоДням
	...
	|	СтабильныеКонецСезонностиТарифнаяЗона.ПредыдущееЗначениеТарифаСверхНорматива КАК ПредыдущееЗначениеТарифаСверхНорматива,
	...
	|	СтабильныеКонецСезонностиТарифнаяЗона.НормативПоДням
	...
	|	СтабильныеПериодыПоказанияНачало.НормативПоДням
	|ПОМЕСТИТЬ Периоды
	|ИЗ
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	МИНИМУМ(ДанныеДляРасчета.НачалоПериода) КАК НачалоПериода
	|ПОМЕСТИТЬ НачалоУслугСечения
	|ИЗ
	|	ДанныеДляРасчета КАК ДанныеДляРасчета
	|ГДЕ
	|	ДанныеДляРасчета.ИспользоватьДиаметрСечения
	|	И ДанныеДляРасчета.УслугаПодключена
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НачалоУслугСечения.Организация КАК Организация,
	|	НачалоУслугСечения.Район КАК Район,
	|	НачалоУслугСечения.Строение КАК Строение,
	|	НачалоУслугСечения.Помещение КАК Помещение,
	|	НачалоУслугСечения.Абонент КАК Абонент,
	|	НачалоУслугСечения.ЧастныйСектор КАК ЧастныйСектор,
	|	НачалоУслугСечения.ТочкаУчета КАК ТочкаУчета,
	|	НачалоУслугСечения.Услуга КАК Услуга,
	|	ДанныеДляРасчета.НачалоПериода КАК НачалоПериода,
	|	ДанныеДляРасчета.КонецПериода КАК КонецПериода
	|ПОМЕСТИТЬ СтабильныеПериодыСечение
	|ИЗ
	|	ДанныеДляРасчета КАК ДанныеДляРасчета
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НачалоУслугСечения КАК НачалоУслугСечения
	|		ПО ДанныеДляРасчета.Организация = НачалоУслугСечения.Организация
	|			И ДанныеДляРасчета.Район = НачалоУслугСечения.Район
	|			И ДанныеДляРасчета.Строение = НачалоУслугСечения.Строение
	|			И ДанныеДляРасчета.Помещение = НачалоУслугСечения.Помещение
	|			И ДанныеДляРасчета.Абонент = НачалоУслугСечения.Абонент
	|			И ДанныеДляРасчета.ЧастныйСектор = НачалоУслугСечения.ЧастныйСектор
	|			И ДанныеДляРасчета.ТочкаУчета = НачалоУслугСечения.ТочкаУчета
	|			И ДанныеДляРасчета.Услуга = НачалоУслугСечения.Услуга
	|			И ДанныеДляРасчета.НачалоПериода >= НачалоУслугСечения.НачалоПериода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СтабильныеПериодыСечение.Организация КАК Организация,
	|	СтабильныеПериодыСечение.Район КАК Район,
	|	СтабильныеПериодыСечение.Строение КАК Строение,
	|	СтабильныеПериодыСечение.Помещение КАК Помещение,
	|	СтабильныеПериодыСечение.Абонент КАК Абонент,
	|	СтабильныеПериодыСечение.ЧастныйСектор КАК ЧастныйСектор,
	|	СтабильныеПериодыСечение.ТочкаУчета КАК ТочкаУчета,
	|	СтабильныеПериодыСечение.Услуга КАК Услуга,
	|	СтабильныеПериодыСечение.НачалоПериода КАК НачалоПериода,
	|	МИНИМУМ(СтабильныеПериодыСечение1.НачалоПериода) КАК КонецПериодаНовый
	|ПОМЕСТИТЬ СеченияКонецПериода
	|ИЗ
	|	СтабильныеПериодыСечение КАК СтабильныеПериодыСечение
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СтабильныеПериодыСечение КАК СтабильныеПериодыСечение1
	|		ПО СтабильныеПериодыСечение.Организация = СтабильныеПериодыСечение1.Организация
	|			И СтабильныеПериодыСечение.Район = СтабильныеПериодыСечение1.Район
	|			И СтабильныеПериодыСечение.Строение = СтабильныеПериодыСечение1.Строение
	|			И СтабильныеПериодыСечение.Помещение = СтабильныеПериодыСечение1.Помещение
	|			И СтабильныеПериодыСечение.Абонент = СтабильныеПериодыСечение1.Абонент
	|			И СтабильныеПериодыСечение.ЧастныйСектор = СтабильныеПериодыСечение1.ЧастныйСектор
	|			И СтабильныеПериодыСечение.ТочкаУчета = СтабильныеПериодыСечение1.ТочкаУчета
	|			И СтабильныеПериодыСечение.НачалоПериода < СтабильныеПериодыСечение1.НачалоПериода
	|
	|СГРУППИРОВАТЬ ПО
	|	СтабильныеПериодыСечение.Организация,
	|	СтабильныеПериодыСечение.Район,
	|	СтабильныеПериодыСечение.Строение,
	|	СтабильныеПериодыСечение.Помещение,
	|	СтабильныеПериодыСечение.Абонент,
	|	СтабильныеПериодыСечение.ЧастныйСектор,
	|	СтабильныеПериодыСечение.ТочкаУчета,
	|	СтабильныеПериодыСечение.Услуга,
	|	СтабильныеПериодыСечение.НачалоПериода
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДанныеДляРасчета.НачалоПериода КАК НачалоПериода,
	|	ЕСТЬNULL(СеченияКонецПериода.КонецПериодаНовый, ДанныеДляРасчета.КонецПериода) КАК КонецПериода,
	|	ДанныеДляРасчета.Район КАК Район,
	...
	|	ВЫБОР
	|		КОГДА ДанныеДляРасчета.ИспользоватьДиаметрСечения
	|				И ДанныеДляРасчета.ДиаметрСечения <> 0
	|			ТОГДА ВЫБОР
	|					КОГДА ДанныеДляРасчета.УслугаПодключена
	|						ТОГДА РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(ЕСТЬNULL(СеченияКонецПериода.КонецПериодаНовый, ДанныеДляРасчета.КонецПериода), ДЕНЬ), СЕКУНДА) * 3.14 * ДанныеДляРасчета.ДиаметрСечения * ДанныеДляРасчета.ДиаметрСечения * ДанныеДляРасчета.СкоростьДвижения / (4 * 1000000)
	|					ИНАЧЕ 0
	|				КОНЕЦ
	...
	|	КОНЕЦ * (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(ЕСТЬNULL(СеченияКонецПериода.КонецПериодаНовый, ДанныеДляРасчета.КонецПериода), ДЕНЬ), ДЕНЬ) / ВЫБОР
	...
	|ИЗ
	|	ДанныеДляРасчета КАК ДанныеДляРасчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ СеченияКонецПериода КАК СеченияКонецПериода
	|		ПО ДанныеДляРасчета.Организация = СеченияКонецПериода.Организация
	|			И ДанныеДляРасчета.Район = СеченияКонецПериода.Район
	|			И ДанныеДляРасчета.Строение = СеченияКонецПериода.Строение
	|			И ДанныеДляРасчета.Помещение = СеченияКонецПериода.Помещение
	|			И ДанныеДляРасчета.Абонент = СеченияКонецПериода.Абонент
	|			И ДанныеДляРасчета.ЧастныйСектор = СеченияКонецПериода.ЧастныйСектор
	|			И ДанныеДляРасчета.ТочкаУчета = СеченияКонецПериода.ТочкаУчета
	|			И ДанныеДляРасчета.Услуга = СеченияКонецПериода.Услуга
	|			И ДанныеДляРасчета.НачалоПериода = СеченияКонецПериода.НачалоПериода
	|ГДЕ
	...
	|					ИНАЧЕ ИСТИНА
	|				КОНЕЦ
	|			ИЛИ ДанныеДляРасчета.ИспользоватьДиаметрСечения
	|				И ДанныеДляРасчета.ДиаметрСечения <> 0)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	И НЕ ДанныеДляРасчета.СпособРаспределенияОбъемаПомещения = ЗНАЧЕНИЕ(Перечисление.энргСпособыРаспределенияОбъема.ФиксированныйОбъем)
	|	И (НЕ ДанныеДляРасчета.ИспользоватьДиаметрСечения
	...
	|	ОбъемыНачислений.КоэффициентРасчета КАК КоэффициентРасчета,
	...
	|	ОбъемыНачислений.КоэффициентРасчета,
...
Процедура ОбработатьРезультатЗапросаНачислений(РезультатЗапроса, Набор, НаборОбъемПомещений, ЗаписыватьНаборыЗаписей, Помещение = Неопределено)
	...
	Пока Выборка.следующий() цикл
		...
		Если НЕ ТекПомещениеРодитель = Выборка.ПомещениеРодитель Тогда
			ПересчитатьСтрокиНачисление(Выборка, Набор, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам, ТекПомещениеРодитель);
			ПересчитатьСтроки 				= Новый Массив;
			ПересчитатьСтрокиПоПлощади 		= Новый Массив;
			ПлощадиПомещенийПоУслугам 		= Новый Соответствие;
			НачисленияПоУслугамПомещения 	= Новый Соответствие;
		...
		ЗаполнитьНаборыЗаписейНачисление(Выборка, ДокументСсылка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам);
...
Процедура ПересчитатьСтрокиНачисление(Выборка, Набор, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам, ТекПомещениеРодитель)
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, ДокументСсылка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам)
...
Процедура ПрочитатьВспомогательныеДанныеНачислениеПоДолевымСхемам(МВТ, Организация,ПериодНачисления,Район, ГраницыПериода)	
	Запрос  						= Новый Запрос;
	Запрос.МенеджерВременныхТаблиц 	= МВТ;
	Запрос.УстановитьПараметр("Организация",		Организация);
	Запрос.УстановитьПараметр("ПериодНачисления",	ПериодНачисления);
...
Функция ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам()
	...
	Текст = 
		...
		|	энргФормулыРасчетаТочекУчетаСрезПоследних.РассчитываемяТочка КАК РассчитываемяТочка,
		...
		|ПОМЕСТИТЬ ДолевыеСхемы
		|ИЗ
		|	РегистрСведений.энргФормулыРасчетаТочекУчета.СрезПоследних(
		|			&КонецПериода,
		|			Организация = &Организация
		|				И Район = &Район) КАК энргФормулыРасчетаТочекУчетаСрезПоследних
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргКлючиПомещений КАК энргКлючиПомещений
		|		ПО энргФормулыРасчетаТочекУчетаСрезПоследних.КлючПомещения = энргКлючиПомещений.КлючПомещения
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.энргАбоненты КАК энргАбоненты
		|		ПО энргФормулыРасчетаТочекУчетаСрезПоследних.КлючПомещения = энргАбоненты.КлючПомещения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	СУММА(энргОбъемНачисленийОбороты.ОбъемУслугиОборот) КАК ОбъемУслугиОборот,
		|	СУММА(энргОбъемНачисленийОбороты.ОбъемУслугиПотребленныйОборот) КАК ОбъемУслугиПотребленныйОборот,
		|	СУММА(энргОбъемНачисленийОбороты.СуммаПоПовышенномуКоэффициентуОборот) КАК СуммаПоПовышенномуКоэффициентуОборот,
		|	энргОбъемНачисленийОбороты.ПериодРасчета КАК ПериодРасчета,
		|	энргОбъемНачисленийОбороты.ПериодНачисления КАК ПериодНачисления,
		|	энргОбъемНачисленийОбороты.Распределение КАК Распределение,
		|	энргОбъемНачисленийОбороты.Поставщик КАК Поставщик
		|ПОМЕСТИТЬ ОбъемНачислений
		|ИЗ
		|	ДолевыеСхемы КАК ДолевыеСхемы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.энргОбъемНачислений.Обороты(
		|				&НачалоПериода,
		|				&КонецПериода,
		|				Месяц,
		|				Организация = &Организация
		...
		|					И ТочкаУчета В
		|						(ВЫБРАТЬ
		|							ДолевыеСхемы.ТочкаИсточник
		|						ИЗ
		|							ДолевыеСхемы)) КАК энргОбъемНачисленийОбороты
		|		ПО ДолевыеСхемы.Организация = энргОбъемНачисленийОбороты.Организация
		|			И ДолевыеСхемы.Район = энргОбъемНачисленийОбороты.Район
		|			И ДолевыеСхемы.ТочкаИсточник = энргОбъемНачисленийОбороты.ТочкаУчета
		|
		|СГРУППИРОВАТЬ ПО
		|	энргОбъемНачисленийОбороты.Организация,
		|	энргОбъемНачисленийОбороты.Район,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДолевыеСхемы.Организация КАК Организация,
		|	ДолевыеСхемы.Район КАК Район,
		|	ДолевыеСхемы.КлючПомещения КАК КлючПомещения,
		|	ДолевыеСхемы.Коэффициент КАК Коэффициент,
		|	ОбъемНачислений.ЗначениеТарифа КАК ЗначениеТарифа,
		|	ОбъемНачислений.СуммаОборот * ДолевыеСхемы.Коэффициент КАК Сумма,
		|	ОбъемНачислений.ОбъемУслугиОборот * ДолевыеСхемы.Коэффициент КАК Объем,
		|	ОбъемНачислений.ОбъемУслугиПотребленныйОборот * ДолевыеСхемы.Коэффициент КАК ОбъемУслугиПотребленный,
		|	ОбъемНачислений.СуммаПоПовышенномуКоэффициентуОборот * ДолевыеСхемы.Коэффициент КАК СуммаПоПовышенномуКоэффициенту,
		|	ДолевыеСхемы.РассчитываемяТочка КАК ТочкаУчета,
		|	ОбъемНачислений.ПериодРасчета КАК ПериодРасчета,
		|	ОбъемНачислений.ПериодНачисления КАК ПериодНачисления,
		|	ОбъемНачислений.Распределение КАК Распределение,
		|	ОбъемНачислений.Поставщик КАК Поставщик,
		|	ДолевыеСхемы.Строение КАК Строение,
		|	ДолевыеСхемы.Помещение КАК Помещение,
		|	ОбъемНачислений.Услуга КАК Услуга,
		|	ДолевыеСхемы.Абонент КАК Абонент
		|ПОМЕСТИТЬ РассчитанныеОбъемы
		|ИЗ
		|	ДолевыеСхемы КАК ДолевыеСхемы
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ОбъемНачислений КАК ОбъемНачислений
		|		ПО ДолевыеСхемы.Организация = ОбъемНачислений.Организация
		|			И ДолевыеСхемы.Район = ОбъемНачислений.Район
		|			И ДолевыеСхемы.ТочкаИсточник = ОбъемНачислений.ТочкаУчета
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РассчитанныеОбъемы.Организация КАК Организация,
		|	РассчитанныеОбъемы.Район КАК Район,
		|	МАКСИМУМ(РассчитанныеОбъемы.ЗначениеТарифа) КАК ЗначениеТарифа,
		|	СУММА(РассчитанныеОбъемы.Сумма) КАК Сумма,
		|	СУММА(РассчитанныеОбъемы.Объем) КАК Объем,
		|	СУММА(РассчитанныеОбъемы.ОбъемУслугиПотребленный) КАК ОбъемУслугиПотребленный,
		|	СУММА(РассчитанныеОбъемы.СуммаПоПовышенномуКоэффициенту) КАК СуммаПоПовышенномуКоэффициенту,
		|	РассчитанныеОбъемы.ТочкаУчета КАК ТочкаУчета,
		|	РассчитанныеОбъемы.ПериодРасчета КАК ПериодРасчета,
		|	РассчитанныеОбъемы.ПериодНачисления КАК ПериодНачисления,
		|	РассчитанныеОбъемы.Распределение КАК Распределение,
		|	РассчитанныеОбъемы.Поставщик КАК Поставщик,
		|	РассчитанныеОбъемы.Строение КАК Строение,
		|	РассчитанныеОбъемы.Помещение КАК Помещение,
		|	РассчитанныеОбъемы.Услуга КАК Услуга,
		|	РассчитанныеОбъемы.Абонент КАК Абонент
		|ПОМЕСТИТЬ РассчитанныеОбъемыСумма
		|ИЗ
		|	РассчитанныеОбъемы КАК РассчитанныеОбъемы
		|
		|СГРУППИРОВАТЬ ПО
		|	РассчитанныеОбъемы.Организация,
		|	РассчитанныеОбъемы.Район,
		|	РассчитанныеОбъемы.ТочкаУчета,
		|	РассчитанныеОбъемы.Строение,
		|	РассчитанныеОбъемы.ПериодРасчета,
		|	РассчитанныеОбъемы.Поставщик,
		|	РассчитанныеОбъемы.Помещение,
		|	РассчитанныеОбъемы.Распределение,
		|	РассчитанныеОбъемы.ПериодНачисления,
		|	РассчитанныеОбъемы.Услуга,
		|	РассчитанныеОбъемы.Абонент
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РассчитанныеОбъемыСумма.Организация КАК Организация,
		|	РассчитанныеОбъемыСумма.Район КАК Район,
		|	РассчитанныеОбъемыСумма.ЗначениеТарифа КАК ЗначениеТарифа,
		|	РассчитанныеОбъемыСумма.Сумма КАК Сумма,
		|	РассчитанныеОбъемыСумма.Объем КАК ОбъемУслуги,
		|	РассчитанныеОбъемыСумма.ОбъемУслугиПотребленный КАК ОбъемУслугиПотребленный,
		|	РассчитанныеОбъемыСумма.СуммаПоПовышенномуКоэффициенту КАК СуммаПоПовышенномуКоэффициенту,
		|	РассчитанныеОбъемыСумма.ТочкаУчета КАК ТочкаУчета,
		|	РассчитанныеОбъемыСумма.ПериодРасчета КАК ПериодРасчета,
		|	РассчитанныеОбъемыСумма.ПериодНачисления КАК ПериодНачисления,
		|	РассчитанныеОбъемыСумма.Распределение КАК Распределение,
		|	РассчитанныеОбъемыСумма.Поставщик КАК Поставщик,
		|	РассчитанныеОбъемыСумма.Строение КАК Строение,
		|	РассчитанныеОбъемыСумма.Помещение КАК Помещение,
		|	энргНачислениеПоФормуламРасчета.Ссылка КАК ДокументНачисленияПоДолевымСхемам,
		|	РассчитанныеОбъемыСумма.Услуга КАК Услуга,
		|	РассчитанныеОбъемыСумма.Абонент КАК Абонент
		|ИЗ
		|	РассчитанныеОбъемыСумма КАК РассчитанныеОбъемыСумма
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.энргНачислениеПоФормуламРасчета КАК энргНачислениеПоФормуламРасчета
		|		ПО РассчитанныеОбъемыСумма.Организация = энргНачислениеПоФормуламРасчета.Организация
		|			И РассчитанныеОбъемыСумма.Район = энргНачислениеПоФормуламРасчета.Район
		|			И РассчитанныеОбъемыСумма.Строение = энргНачислениеПоФормуламРасчета.МКД
		|			И (энргНачислениеПоФормуламРасчета.Дата МЕЖДУ &НачалоПериода И &КонецПериода)
		|ИТОГИ
