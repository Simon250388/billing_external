...
Процедура РаспредениеОбъемаМКД(Организация,ПериодНачисления, Район, Строение, Документ)  Экспорт 
	...
	Пока ВыборкаОбъемМКД.Следующий() Цикл
		...
		Если НЕ ТекСтроение = ВыборкаОбъемМКД.Строение Или НЕ ТекУслуга = ВыборкаОбъемМКД.Услуга Тогда
			...
			Пока НЕ ВыборкаОбъемПоПриборуКорректировать.Строение = ВыборкаОбъемМКД.Строение ИЛИ НЕ ВыборкаОбъемПоПриборуКорректировать.Услуга = ВыборкаОбъемМКД.Услуга Цикл
			...
			Пока ВыборкаАбоненты.Строение = ВыборкаОбъемМКД.Строение и ВыборкаАбоненты.Услуга = ВыборкаОбъемМКД.Услуга Цикл
				Если НЕ ТекАбонент = ВыборкаАбоненты.Абонент Тогда
					...
					ДанныеАбонента 									= Новый Структура("Договор, Абонент, Помещение,ЗначениеПлощади, ТарифнаяГруппа, Поставщик, ОбъемУслуги, НП",
					ВыборкаАбоненты.Помещение, ВыборкаАбоненты.Абонент, ВыборкаАбоненты.Помещение, ВыборкаАбоненты.ЗначениеПлощадиДляРаспределения,ВыборкаАбоненты.ТарифнаяГруппаДляРаспределения, ВыборкаАбоненты.ПоставщикДляРаспределения, 0, ВыборкаАбоненты.НП);
					ЕстьРасходПоПрибору 							= Ложь;					
				КонецЕсли;				
				...
				Если НЕ ВыборкаАбоненты.СпособНачисления = Перечисления.энргСпособНачислений.ПоПриборам И НЕ ВыборкаАбоненты.СпособНачисления = Перечисления.энргСпособНачислений.НачислениеПоСреднему Тогда
					ВсеПоПрибору 								= Ложь;
				Иначе
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	МАКСИМУМ(ДанныеДляРасчетаПередНормой.ПриборВведен) КАК ПриборВведен,
	|	МАКСИМУМ(ДанныеДляРасчетаПередНормой.ПоказанияВСрок) КАК ПоказанияВСрок
	...
	|
	|ИМЕЮЩИЕ
	|	(МАКСИМУМ(ДанныеДляРасчетаПередНормой.ПриборВведен) = ЛОЖЬ
	|		ИЛИ МАКСИМУМ(ДанныеДляРасчетаПередНормой.ПоказанияВСрок) = ЛОЖЬ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДанныеДляРасчетаПередНормой.ФизЛицо КАК ФизЛицо,
	|	ДанныеДляРасчетаПередНормой.ЗависитОтУслуги КАК ЗависитОтУслуги,
	|	ДанныеДляРасчетаПередНормой.СоциальныйНормативРавенНормативуПотребления,
	|	ДанныеДляРасчетаПередНормой.УмножатьНаНормуРодительскойУслуги КАК УмножатьНаНормуРодительскойУслуги,
	|	ДанныеДляРасчетаПередНормой.ИспользоватьКоэффициентСдерживанияРостаПлаты КАК ИспользоватьКоэффициентСдерживанияРостаПлаты,
	|	ДанныеДляРасчетаПередНормой.ПлощадьМОП КАК ПлощадьМОП,
	|	ДанныеДляРасчетаПередНормой.ПлощадьАбонентов КАК ПлощадьАбонентов,
...
Функция РезультатЗапросаПерерасчетСреднего(МВТ,Организация,ПериодНачисления,Район,Строение,Помещение,НачисленныйСреднийОбъем=Неопределено)
	...
	|					ТОГДА энргПерерасчет.КлючПомещения.Строение = ЗНАЧЕНИЕ(Справочник.энргСтроения.ПустаяСсылка)
	|				ИНАЧЕ ДанныеДляСписанияМинРегистратор.Строение = энргПерерасчет.КлючПомещения.Строение
	|			КОНЕЦ)
	|			И (энргПерерасчет.Организация = &Организация)
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП, НаборЗаписейСверхНорма)
	...
	Если Не ДанныеПоУслуге.РаспределятьПолностью Тогда
...
Функция ТекстЗапросаВспомогательныеДанныеРаспределениеМКД(Строение)
	...
	|	ЕСТЬNULL(энргИсторияСоглашенийОРаспределенииСверхнормативногоОДНСрезПервых.РаспределятьПолностью, ЛОЖЬ) КАК РаспределятьПолностью
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.энргИсторияСоглашенийОРаспределенииСверхнормативногоОДН.СрезПервых(&КонецПериода, ) КАК энргИсторияСоглашенийОРаспределенииСверхнормативногоОДНСрезПервых
	|		ПО энргОбъемНачисленийМКДОбороты.Организация = энргИсторияСоглашенийОРаспределенииСверхнормативногоОДНСрезПервых.Организация
	|			И энргОбъемНачисленийМКДОбороты.Район = энргИсторияСоглашенийОРаспределенииСверхнормативногоОДНСрезПервых.Район
	|			И энргОбъемНачисленийМКДОбороты.МКД = энргИсторияСоглашенийОРаспределенииСверхнормативногоОДНСрезПервых.МКД
	|			И энргОбъемНачисленийМКДОбороты.Услуга = энргИсторияСоглашенийОРаспределенииСверхнормативногоОДНСрезПервых.Услуга
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаДанныеДляРаспределения(Строение)
	...
	|	энргОбъемНачислений.ПлощадьСобственнаяПоставка КАК ПлощадьСобственнаяПоставка
	|ИЗ
	|	ОбъемНачисленийПоПриборуУчетаКорректироватьИндивидуальныйОбъем КАК энргОбъемНачислений
	|
	|УПОРЯДОЧИТЬ ПО
	|	Строение,
	|	Услуга,
