...
Процедура Начислить(Делегат) Экспорт
	...
	РезультатЗапроса 	= РезультатЗапросаИндивидуальныеНачисления(МВТ); 	
...
Процедура РаспредениеОбъемаМКД(Организация,ПериодНачисления, Район, Строение, Документ)  Экспорт 
	...
	ВыборкаОтрицательныйОбъемРаспределения	= Результат[6].Выбрать();
...
Процедура НачислитьПеню(Организация,ПериодНачисления,Район,МКД,Абонент,Документ) Экспорт  
	...
	|	ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(ДОБАВИТЬКДАТЕ(КОНЕЦПЕРИОДА(ДатаНачала.Период, МЕСЯЦ), СЕКУНДА, 1), МЕСЯЦ, &ГлубинаОтсрочки), ДЕНЬ, &КрайнийСрокОплаты), ДЕНЬ, &КоличествоДней) КАК ДатаНачалаНачисленияПени
		...
		Пока ВыборкаПериодНачисления.Следующий() Цикл
			...
			Пока ВыборкаУслуга.Следующий() Цикл
				...
				Пока ВыборкаАбонент.Следующий() Цикл
					...
					ДнейНаДеньНачалаДляРасчета 	= ВыборкаАбонент.ДнейНаДеньНачала+1;
...
Процедура ПолучитьСтабильныеПериоды(МВТ)  
...
Процедура ДобавитьРасчетОбъемаУслугиПоНормативу(Схема,ВыбираемыеПоля,ВычисляемыеПоля,МВТ,ТаблицаДанныеДляРасчета)
	...
	|ИЛИ НЕ СоставнаяУслуга = ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка)");
...
Процедура ДобавитьРасчетОбъемаПотребленногоПоСреднемуНормативу(Схема,ВыбираемыеПоля, ВычисляемыеПоля, МВТ, ТаблицаДанныеДляРасчета)
	...
	ПереопределяемыеПоля.Вставить("ОбъемУслуги", 				"0");
	ПереопределяемыеПоля.Вставить("ОбъемУслугиПотребленный",	
	"__ОбъемУслугиПотребленныйПоНорме__ * __ДлительностьПериодаПоНормативу__"); 	
	ПереопределяемыеПоля.Вставить("Сумма", 						"0");
	ПереопределяемыеПоля.Вставить("СниматьНачисленныйСредний", 	"ИСТИНА");
	ПереопределяемыеПоля.Вставить("СпособНачисления", 			"ЗНАЧЕНИЕ(Перечисление.энргСпособНачислений.СреднееПоНормативу)");
	ПереопределяемыеПоля.Вставить("НачальныеПоказания", 		"0");
...
Процедура ДобавитьРасчетОбъемаУслугиПоСреднемуЗаПрошлыйГод(Схема,ВыбираемыеПоля, ВычисляемыеПоля, МВТ, ТаблицаДанныеДляРасчета)
	...
	ПереопределяемыеПоля.Вставить("ОбъемУслуги", 				"__ОбъемУслугиПоСреднему__ * __ДлительностьПериода__");
	ПереопределяемыеПоля.Вставить("ОбъемУслугиПотребленный",	"0");
...
Процедура ДобавитьРасчетОбъемаПотребленногоПоСреднему(Схема,ВыбираемыеПоля, ВычисляемыеПоля, МВТ, ТаблицаДанныеДляРасчета)
	...
	ПереопределяемыеПоля.Вставить("ОбъемУслуги", 				"0");
	ПереопределяемыеПоля.Вставить("ОбъемУслугиПотребленный",	"__ОбъемУслугиПотребленныйПоСреднему__ * __ДлительностьПериода__");
	ПереопределяемыеПоля.Вставить("Сумма", 						"0");
	ПереопределяемыеПоля.Вставить("СниматьНачисленныйСредний", 	"ИСТИНА");
	ПереопределяемыеПоля.Вставить("СпособНачисления", 			"ЗНАЧЕНИЕ(Перечисление.энргСпособНачислений.НачислениеПоСреднему)");
	ПереопределяемыеПоля.Вставить("НачальныеПоказания", 		"0");
...
Функция РезультатЗапросаИндивидуальныеНачисления(МВТ) 			
	ПолучитьСтабильныеПериоды(МВТ);
...
Процедура ОбработатьРезультатЗапросаНачислений(РезультатЗапроса, Делегат)
	...
	НеПрименятьЗависимостьЕслиБольшеОднойТочки	= Константы.энргНеПрименятьЗависимостьЕслиБольшеОднойТочки.Получить();
	
	Набор 										= Неопределено;
	НаборОбъемПомещений 						= Неопределено;
	
	//Если НЕ РезультатЗапроса.Пустой() Тогда
	//	ГраницыПериодаНачисления 	= энргРаботаСПериодомДействия.ГраницыРасчетногоПериода(Делегат.Организация, Делегат.ПериодНачисления,Ложь);	
	//	АктуальныеЗначенияПотерь 	= энргРаботаСТочкамиУчетаСервер.ПолучитьАктуальныеЗначенияПотерь(ГраницыПериодаНачисления.ЗавершениеОП, Делегат.Организация, Делегат.Район);
	//КонецЕсли;
	ОбъемПотерьТочекУчета 			= Новый Соответствие;	
 	...
 	Пока Выборка.следующий() цикл
		...
		Если НЕ ТекПериодНачисления = Выборка.ПериодНачисления
			...
			Если Не ТекПомещениеРодитель = Неопределено Тогда
				ПересчитатьСтрокиНачисление(Набор, НаборОбъемПомещений, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек, ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ОстатокСоциальнойНормы, Ошибки, ИнформацияДляОшибки);
			КонецЕсли;
			ПересчитатьСтроки 						= Новый Массив;
			ПересчитатьСтрокиПоПлощади 				= Новый Массив;
			ПересчитатьСтрокиПоКоличествуЧеловек 	= Новый Соответствие;
		...
		ИнформацияДляОшибки = "Остановились на строении '" + Выборка.Строение + "'" + ?(ЗначениеЗаполнено(Выборка.Помещение)," и помещении '" + Выборка.Помещение + "'", "");		
	КонецЦикла;	
...
Функция ПолучитьОбъемПотерь(Выборка, АктуальныеЗначенияПотерь, ОбъемПотерьТочекУчета)
	
	Если НЕ Выборка.ОбъемУслуги > 0 или (Выборка.ОбъемУслуги > 0 И НЕ (Выборка.СпособНачисления = Перечисления.энргСпособНачислений.ПоПриборам или Выборка.СпособНачисления = Перечисления.энргСпособНачислений.НачислениеПоСреднему)) Тогда
		Возврат 0;
	КонецЕсли;	
	
	//ДанныеПотерьТочки 	= ОбъемПотерьТочекУчета[Выборка.ТочкаУчета];
	//Если ДанныеПотерьТочки = Неопределено Тогда
	//	ОбъемПотерь = энргРаботаСТочкамиУчетаСервер.ПолучитьОбъемПотерь(Выборка.ОбъемУслуги, АктуальныеЗначенияПотерь, Новый Структура("КлючПомещения, ТочкаУчета", Выборка.КлючПомещения, Выборка.ТочкаУчета));		
	//	ОбъемПотерьТочекУчета.Вставить(Выборка.ТочкаУчета,СоотвествиеТарифнаяЗона);
	//Иначе 
	//	ОбъемПотерь			= ДанныеПотерьТочки;
	//КонецЕсли;
...
Процедура ПересчитатьСтрокиНачисление(Набор, НаборОбъемПомещений, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек, ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ОстатокСоциальнойНормы, Ошибки, ИнформацияДляОшибки)
	...
	Попытка
					...
					Для Каждого КлючЗначениеНачалоПериода Из КлючЗначениеТарифнаяЗона.Значение Цикл
						...
						Для Каждого КлючЗначениеСоставнаяУслуга Из КлючЗначениеНачалоПериода.Значение Цикл
							...
							Для Сч = 0 По КлючЗначениеСоставнаяУслуга.Значение.СтрокиНабораПомещения.ВГраница() Цикл
								...
								Для Каждого КлючЗначениеПомещение Из ПересчитатьСтрокиПоКоличествуЧеловек Цикл
									...
									Иначе
										РаспределяемОбъем 								= ОсталосьРаспределитьОбъем * КоличествоЖильцов /  КоличествоЖильцовПомещения;
										РаспределяемОбъемПотребленный					= ОсталосьРаспределитьОбъемПотребленный * КоличествоЖильцов /  КоличествоЖильцовПомещения;
									КонецЕсли;
						...
						Для Каждого КлючЗначениеПомещение Из ПересчитатьСтрокиПоКоличествуЧеловек Цикл
							...
							Для Каждого КлючЗначениеСоставнаяУслуга Из СоовествиеСоставныеУслуги Цикл
								Если НЕ мСоставныеУслуги.Найти(КлючЗначениеСоставнаяУслуга.Ключ) = Неопределено Тогда
									Продолжить;
								КонецЕсли;						
						...
						Для Каждого КлючЗначениеНачалоПериода Из КлючЗначениеТарифнаяЗона.Значение Цикл
							Для Каждого КлючЗначениеСоставнаяУслуга Из КлючЗначениеНачалоПериода.Значение Цикл
								...
								Для Индекс = 0 По СтрокиНабораПомещения.ВГраница() Цикл
									...
									Иначе	                               	
										СтрокаНабора 							= Набор.Получить(КлючЗначениеСоставнаяУслуга.Значение.СтрокиНабора[ИндексСтроки]);
									КонецЕсли;									
		...
		Если ПересчитатьСтроки.Количество() > 0 Тогда
			Для Сч = 0 По ПересчитатьСтроки.ВГраница() Цикл
				...
				СтрокНабора.ОбъемУслуги 					= (НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода].ОбъемУслуги - НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода].ОбъемУслугиОстаток) * СтрокНабора.КоэффициентРаспределенияОстатка;
				СтрокНабора.ОбъемУслугиПотребленный			= (НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода].ОбъемУслугиПотребленный - НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода].ОбъемУслугиПотребленныйОстаток)* СтрокНабора.КоэффициентРаспределенияОстатка;
			КонецЦикла;				
		КонецЕсли;
	Исключение
		ИнформацияОбОшибке  					= ИнформацияОбОшибке();
	...
	Для Каждого СтрокаПоФормуле Из ПересчитатьСтрокиПоФормуле Цикл 																			 
		Для Каждого СтрокаПериод Из СтрокаПоФормуле.Значение Цикл
			...
			Для Сч = 0 По ИндексыСтрок.ВГраница() Цикл
				...
				ИначеЕсли ЗначениеЗаполнено(СтрокаНабора.ЗависитОтУслуги) Тогда
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек,ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле, ПересчитатьТехническиеТочки, ПересчитатьТехническиеТочкиПомещения,ДеревоТочекУчета, ОстатокСоциальнойНормы, ЗаписиСтороннихПоставщиков, ОбъемПотерь, НеПрименятьЗависимостьЕслиБольшеОднойТочки)
	...
	Если ЗначениеЗаполнено(Выборка.Абонент) И Выборка.ПомещениеРодитель = Выборка.Помещение Тогда		 
		...
		СтрокНабора.ОбъемУслуги 							= Выборка.ОбъемУслуги + ОбъемПотерь;
		ЕстьАбонент 										= Истина;
	...
	Иначе
		...
		Если НачисленияПоУслугамПомещения[Выборка.Услуга] = Неопределено Тогда
			...
			ЗначениеСоотвествия 							= Новый Структура("СтрокиНабораПомещения,КоличествоЧеловек,ОбъемУслуги,ОбъемУслугиПотребленный,ОбъемУслугиОстаток,ОбъемУслугиПотребленныйОстаток,ЗначениеСоциальнойНормы,ПовыщающийКоэффициент,СниматьНачисленныйСредний", СтрокиНабораПомещения,Выборка.КоличествоЖильцов,Выборка.ОбъемУслуги, Выборка.ОбъемУслугиПотребленный,0,0, Выборка.ЗначениеСоциальнойНормы, Выборка.ПовыщающийКоэффициент, Выборка.СниматьНачисленныйСредний);
	...
	Если Выборка.ПомещениеРодитель = Выборка.Помещение И ЗначениеЗаполнено(Выборка.ЗависитОтУслуги) И НЕ Выборка.РасчетПоФормуле И НЕ ЗначениеЗаполнено(Выборка.ЗависимаяТочкаУчета) Тогда
		...
		Если НЕ НачисленияПоУслугам.Получить(Выборка.ТочкаУчета) = Неопределено 
			...
			Если Не НеПрименятьЗависимостьЕслиБольшеОднойТочки Тогда
