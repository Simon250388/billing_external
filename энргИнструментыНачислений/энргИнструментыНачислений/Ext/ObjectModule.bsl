...
Функция ТекстЗапросаДанныеДляРаспределения(Строение)
	...
	|	ЗНАЧЕНИЕ(Перечисление.энргСпособРасчетаНорматива.ПустаяСсылка) КАК СпособРасчетаСоциальногоНорматива
	|ИЗ
	|	АбонентыДляРаспределения КАК АбонентыДляРаспределения
 	...
 	Пока Выборка.следующий() цикл
		...
		Если НЕ ТекПериодНачисления = Выборка.ПериодНачисления
			...
			Если Не ТекПомещениеРодитель = Неопределено Тогда
				ПересчитатьСтрокиНачисление(Набор, НаборОбъемПомещений, НачисленияПоУслугамПомещения, ПлощадиПомещенийПоУслугам, ПараметрыРаспределенияОбъемаПомещения, ОстатокСоциальнойНормы, Ошибки, ИнформацияДляОшибки);
			КонецЕсли;
			ПараметрыРаспределенияОбъемаПомещения 	= Новый Соответствие;
			ПлощадиПомещенийПоУслугам 				= Новый Соответствие;
			НачисленияПоУслугамПомещения 			= Новый Соответствие;			
		...
		ЗаполнитьНаборыЗаписейНачисление(Выборка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле, ПересчитатьТехническиеТочки, ПересчитатьТехническиеТочкиПомещ, ДеревоТочекУчета, ОстатокСоциальнойНормы, ЗаписиСтороннихПоставщиков);
		ЗаполнитьПараметрыРаспределенияОбъемаПомещения(ДанныеВыборки, ПараметрыРаспределенияОбъемаПомещения);
...
Процедура ПересчитатьСтрокиНачисление(Набор, НаборОбъемПомещений, НачисленияПоУслугамПомещения, ПлощадиПомещенийПоУслугам, ПараметрыРаспределенияОбъемаПомещения, ОстатокСоциальнойНормы, Ошибки, ИнформацияДляОшибки)
			...
			Для Каждого КлючЗначениеТарифнаяЗона Из КлючЗначениеУслуга.Значение Цикл
				Для Каждого КлючЗначениеНачалоПериода Из КлючЗначениеТарифнаяЗона.Значение Цикл
					Для Каждого КлючЗначениеСоставнаяУслуга Из КлючЗначениеНачалоПериода.Значение Цикл
						ЗначениеСтруктуры 									= КлючЗначениеСоставнаяУслуга.Значение;						
						...
						РаспредеделитьОбъемПомещенияПоУслугеНаПотомков(Набор, ЗначениеСтруктуры, ПараметрыРаспределенияОбъемаПомещения,КлючЗначениеУслуга.Ключ, КлючЗначениеТарифнаяЗона.Ключ, КлючЗначениеНачалоПериода.Ключ, КлючЗначениеСоставнаяУслуга.Ключ, ПомещенияСОстаткомРаспределения, ПлощадиПомещенийПоУслугам);
						РаспределитьОстатокНаПомещенияСоСпособомРаспределенияОстаток(Набор, ЗначениеСтруктуры,ПомещенияСОстаткомРаспределения,ПараметрыРаспределенияОбъемаПомещения, КлючЗначениеУслуга.Ключ, КлючЗначениеТарифнаяЗона.Ключ, КлючЗначениеНачалоПериода.Ключ, КлючЗначениеСоставнаяУслуга.Ключ);
					КонецЦикла;					
				КонецЦикла;
			КонецЦикла;
		КонецЦикла;		
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле, ПересчитатьТехническиеТочки, ПересчитатьТехническиеТочкиПомещения,ДеревоТочекУчета, ОстатокСоциальнойНормы, ЗаписиСтороннихПоставщиков)
	...
	Если Выборка.ПомещениеРодитель = Выборка.Помещение И ЗначениеЗаполнено(Выборка.ЗависитОтУслуги) И НЕ Выборка.РасчетПоФормуле И НЕ ЗначениеЗаполнено(Выборка.ЗависимаяТочкаУчета) Тогда
		...
		Если Не ЗначениеЗаполнено(Выборка.Абонент) Тогда				
			...
			НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПоПрибору 				= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПоПрибору + ОбъемУслугиПоПриборуПоСтроке - ОбъемУслугиПоПрибору;
			НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйПоПрибору 	= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйПоПрибору + ОбъемУслугиПотребленныйПоПриборуПоСтроке - ОбъемУслугиПотребленныйПоПрибору;
			НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиБезПрибора 				= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиБезПрибора + ОбъемУслугиБезПрибораПоСтроке - ОбъемУслугиБезПрибора;
			НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйБезПрибора 	= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйБезПрибора + ОбъемУслугиПотребленныйБезПрибораПоСтроке - ОбъемУслугиПотребленныйБезПрибора;			
		КонецЕсли;	  
	КонецЕсли; 
...
Процедура УстановитьСниматьНачисленныйСреднийПотомковРекурсия(Набор, Потомки, Услуга, ПриборУчетаГоловнойТочкиВведен)
	...
	Для Каждого СтрокаПотомка Из Потомки Цикл
		...
		Для Сч = 0 По СтрокиНабора.ВГраница() Цикл
			...
			ДанныеСтрокиНачислений.СпособНачисления 			= Перечисления.энргСпособНачислений.ПоНормативу; 
...
Процедура ЗаполнитьНачисленияПоУслугамПомещения(ДанныеВыборки, НачисленияПоУслугамПомещения)
	...
	|ПовыщающийКоэффициент,СниматьНачисленныйСредний,СтрокиРезультатаЗапроса");
	...
	Если НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга] = Неопределено Тогда
				
		СоотвествиеСоставнаяУслуга 						= Новый Соответствие;
		СоотвествиеСоставнаяУслуга.Вставить(ДанныеВыборки.СоставнаяУслуга, ЗначениеСоотвествия); 
		
		СоотвествиеНачалоПериода 						= Новый Соответствие;
		СоотвествиеНачалоПериода.Вставить(ДанныеВыборки.НачалоПериода, СоотвествиеСоставнаяУслуга);
		
		СоотвествиеТарифнаяЗона 						= Новый Соответствие;
		СоотвествиеТарифнаяЗона.Вставить(ДанныеВыборки.ТарифнаяЗона, СоотвествиеНачалоПериода);	
		
		НачисленияПоУслугамПомещения.Вставить(ДанныеВыборки.Услуга,СоотвествиеТарифнаяЗона);			
		
	ИначеЕсли НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона] = Неопределено Тогда
		...
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга].Вставить(ДанныеВыборки.ТарифнаяЗона, СоотвествиеНачалоПериода);
		
	ИначеЕсли НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода] = Неопределено Тогда
		
		СоотвествиеСоставнаяУслуга 						= Новый Соответствие;
		СоотвествиеСоставнаяУслуга.Вставить(ДанныеВыборки.СоставнаяУслуга, ЗначениеСоотвествия);
		
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона].Вставить(ДанныеВыборки.НачалоПериода, СоотвествиеСоставнаяУслуга);
		
	ИначеЕсли НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга] = Неопределено Тогда
		
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода].Вставить(ДанныеВыборки.СоставнаяУслуга, ЗначениеСоотвествия);
		
	Иначе		
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].КоличествоЧеловек 				= ДанныеВыборки.КоличествоЖильцов;
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслуги 						= НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслуги + ДанныеВыборки.ОбъемУслуги;
		...
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПотребленный			= НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПотребленный + ДанныеВыборки.ОбъемУслугиПотребленный;
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ЗначениеСоциальнойНормы 			= ДанныеВыборки.ЗначениеСоциальнойНормы;
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ВидПлощади 						= ДанныеВыборки.ВидПлощади;
		
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПоПрибору				= НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПоПрибору + ОбъемУслугиПоПрибору;
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПотребленныйПоПрибору	= НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПотребленныйПоПрибору + ОбъемУслугиПотребленныйПоПрибору;
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиБезПрибора			= НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиБезПрибора + ОбъемУслугиБезПрибора;
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПотребленныйБезПрибора= НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].ОбъемУслугиПотребленныйБезПрибора + ОбъемУслугиПотребленныйБезПрибора;
		НачисленияПоУслугамПомещения[ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга].СтрокиРезультатаЗапроса.Добавить(ДанныеСтрокиРезультата);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыРаспределенияОбъемаПомещения(ДанныеВыборки, ПараметрыРаспределенияОбъемаПомещения)
	...
	ЗначениеСоотвествия 						= Новый Структура(
	"ДанныеСтроки, ДоляПотребленияЧислитель,ДоляПотребленияЗнаменатель,КоэффициентРаспределенияОстатка,СпособРаспределенияОстатка",
	ДанныеСтрокиОбъема, ДанныеВыборки.ДоляПотребленияЧислитель, ДанныеВыборки.ДоляПотребленияЗнаменатель, ДанныеВыборки.КоэффициентРаспределенияОстатка, ДанныеВыборки.СпособРаспределенияОбъемаПомещения);
	...
	Если ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение] = Неопределено Тогда
		...
		СоотвествиеУслуга 						= Новый Соответствие;
		СоотвествиеУслуга.Вставить(ДанныеВыборки.Услуга, СоотвествиеТарифнаяЗона);			
		
		ПараметрыРаспределенияОбъемаПомещения.Вставить(ДанныеВыборки.Помещение, СоотвествиеУслуга);
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение][ДанныеВыборки.Услуга] = Неопределено Тогда
		...
		ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение].Вставить(ДанныеВыборки.Услуга, СоотвествиеТарифнаяЗона);
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение][ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона] = Неопределено Тогда
		...
		ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение][ДанныеВыборки.Услуга].Вставить(ДанныеВыборки.ТарифнаяЗона,СоотвествиеНачалоПериода)
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение][ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода] = Неопределено Тогда
		
		СоотвествиеСоставнаяУслуга 					= Новый Соответствие;
		СоотвествиеСоставнаяУслуга.Вставить(ДанныеВыборки.СоставнаяУслуга, ЗначениеСоотвествия);
		
		ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение][ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона].Вставить(ДанныеВыборки.НачалоПериода, СоотвествиеСоставнаяУслуга);
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение][ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода][ДанныеВыборки.СоставнаяУслуга] = Неопределено Тогда
		
		ПараметрыРаспределенияОбъемаПомещения[ДанныеВыборки.Помещение][ДанныеВыборки.Услуга][ДанныеВыборки.ТарифнаяЗона][ДанныеВыборки.НачалоПериода].Вставить(ДанныеВыборки.СоставнаяУслуга, ЗначениеСоотвествия)			
	КонецЕсли;
КонецПроцедуры
...
Процедура РаспредеделитьОбъемПомещенияПоУслугеНаПотомков(Набор, ДанныеДляРаспределения, ПараметрыРаспределенияОбъемаПомещения, Услуга, ТарифнаяЗона, НачалоПериода, СоставнаяУслуга, ПомещенияСОстаткомРаспределения, ПлощадиПомещенийПоУслугам)
	...
	Для Каждого КлючЗначениеПомещение Из ПараметрыРаспределенияОбъемаПомещения Цикл
		...
		Если КлючЗначениеПомещение.Значение[Услуга] = Неопределено
			ИЛИ КлючЗначениеПомещение.Значение[Услуга][ТарифнаяЗона] = Неопределено Тогда
			ИЛИ КлючЗначениеПомещение.Значение[Услуга][ТарифнаяЗона][НачалоПериода] = Неопределено
			ИЛИ КлючЗначениеПомещение.Значение[Услуга][ТарифнаяЗона][НачалоПериода][СоставнаяУслуга] = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ПараметрыРаспределения 														= КлючЗначениеПомещение.Значение[Услуга][ТарифнаяЗона][НачалоПериода][СоставнаяУслуга];
		ПлощадиПомещений	 														= ПлощадиПомещенийПоУслугам[Услуга][НачалоПериода];

		Если НЕ ДанныеДляРаспределения.ОбъемУслугиБезПрибора = 0 Тогда
			Если НЕ ПараметрыРаспределения.СпособРаспределенияОстатка = Перечисления.энргСпособыРаспределенияОбъема.Остаток Тогда
				Если НЕ ПараметрыРаспределения.ДоляПотребленияЧислитель И НЕ ПараметрыРаспределения.ДоляПотребленияЗнаменатель = 0 Тогда
					ОбъемУслугиПотомка 												= Окр(ДанныеДляРаспределения.ОбъемУслугиБезПрибора * ПараметрыРаспределения.ДоляПотребленияЧислитель / ПараметрыРаспределения.ДоляПотребленияЗнаменатель, РазрядностьДробнойЧасти);
					ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, ДанныеДляРаспределения.СтрокиРезультатаЗапроса, ПараметрыРаспределения.ДанныеСтроки, ОбъемУслугиПотомка, 0);					
				ИначеЕсли ЗначениеЗаполнено(ДанныеДляРаспределения.ВидПлощади) Тогда
					ОбъемУслугиПотомка 												= Окр(ДанныеДляРаспределения.ОбъемУслугиБезПрибора * ПараметрыРаспределения.ДанныеСтроки.ЗначениеПлощади / ПлощадиПомещений.Площадь);
					ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, ДанныеДляРаспределения.СтрокиРезультатаЗапроса, ПараметрыРаспределения.ДанныеСтроки, ОбъемУслугиПотомка, 0);
				Иначе
					КоличествоЧеловек 												= энргРаботаСПараметрамиРасчетаАбонентаСервер.КоличествоЧеловек(ПараметрыРаспределения.ДанныеСтроки);
					ОбъемУслугиПотомка 												= Окр(ДанныеДляРаспределения.ОбъемУслугиБезПрибора * КоличествоЧеловек / ДанныеДляРаспределения.КоличествоЧеловек, РазрядностьДробнойЧасти);
					ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, ДанныеДляРаспределения.СтрокиРезультатаЗапроса, ПараметрыРаспределения.ДанныеСтроки, ОбъемУслугиПотомка, 0);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если НЕ ДанныеДляРаспределения.ОбъемУслугиПотребленныйБезПрибора = 0 Тогда
			Если НЕ ПараметрыРаспределения.СпособРаспределенияОстатка = Перечисления.энргСпособыРаспределенияОбъема.Остаток Тогда
				Если НЕ ПараметрыРаспределения.ДоляПотребленияЧислитель И НЕ ПараметрыРаспределения.ДоляПотребленияЗнаменатель = 0 Тогда
					ОбъемУслугиПотребленныйПотомка 									= Окр(ДанныеДляРаспределения.ОбъемУслугиПотребленныйБезПрибора * ПараметрыРаспределения.ДоляПотребленияЧислитель / ПараметрыРаспределения.ДоляПотребленияЗнаменатель, РазрядностьДробнойЧасти);
					ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, ДанныеДляРаспределения.СтрокиРезультатаЗапроса, ПараметрыРаспределения.ДанныеСтроки, 0, ОбъемУслугиПотребленныйПотомка);					
				ИначеЕсли ЗначениеЗаполнено(ДанныеДляРаспределения.ВидПлощади) Тогда
					ОбъемУслугиПотребленныйПотомка 									= Окр(ДанныеДляРаспределения.ОбъемУслугиПотребленныйБезПрибора * ПараметрыРаспределения.ДанныеСтроки.ЗначениеПлощади / ПлощадиПомещений.Площадь, РазрядностьДробнойЧасти);
					ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, ДанныеДляРаспределения.СтрокиРезультатаЗапроса, ПараметрыРаспределения.ДанныеСтроки, 0, ОбъемУслугиПотребленныйПотомка);
				Иначе
					КоличествоЧеловек 												= энргРаботаСПараметрамиРасчетаАбонентаСервер.КоличествоЧеловек(ПараметрыРаспределения.ДанныеСтроки);
					ОбъемУслугиПотребленныйПотомка 									= Окр(ДанныеДляРаспределения.ОбъемУслугиПотребленныйБезПрибора * КоличествоЧеловек / ДанныеДляРаспределения.КоличествоЧеловек, РазрядностьДробнойЧасти);
					ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, ДанныеДляРаспределения.СтрокиРезультатаЗапроса, ПараметрыРаспределения.ДанныеСтроки, 0, ОбъемУслугиПотребленныйПотомка);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		Если НЕ ДанныеДляРаспределения.ОбъемУслугиОстаток = 0 Тогда
			Если НЕ ПараметрыРаспределения.СпособРаспределенияОстатка = Перечисления.энргСпособыРаспределенияОбъема.Остаток Тогда
		...
		Если НЕ ДанныеДляРаспределения.ОбъемУслугиПотребленныйОстаток = 0 Тогда
			Если НЕ ПараметрыРаспределения.СпособРаспределенияОстатка = Перечисления.энргСпособыРаспределенияОбъема.Остаток Тогда
...
Процедура РаспределитьОстатокНаПомещенияСоСпособомРаспределенияОстаток(Набор, ДанныеДляРаспределения,ПомещенияСОстаткомРаспределения, ПараметрыРаспределенияОбъемаПомещения, Услуга, ТарифнаяЗона, НачалоПериода, СоставнаяУслуга)
		
	Для Индекс  = 0 По ПомещенияСОстаткомРаспределения.ВГраница() Цикл
		ПараметрыРаспределения 								= ПараметрыРаспределенияОбъемаПомещения[ПомещенияСОстаткомРаспределения[Индекс]][Услуга][ТарифнаяЗона][НачалоПериода][СоставнаяУслуга];
		...
		ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, ДанныеДляРаспределения.СтрокиРезультатаЗапроса, ПараметрыРаспределения.ДанныеСтроки, ОбъемУслугиПотомка, ОбъемУслугиПотребленныйПотомка);
...
Процедура ЗаполнитьСтрокуНабораНачисленияПотомка(Набор, СтрокиРезультатаЗапросаРодитель, ДанныеСтрокиРезультатаЗапросаПомещеия, ОбъемУслуги, ОбъемУслугиПотребленный)
	
	СтрокаРезультатаЗапросаПомещения 			= СтрокиРезультатаЗапросаРодитель[0];
	Если НЕ ОбъемУслуги = 0 Тогда
		СтрокаНабора 							= Набор.Добавить();		
		ЗаполнитьЗначенияСвойств(СтрокаНабора,ДанныеСтрокиРезультатаЗапросаПомещеия);		
		СтрокаНабора.Период						= ДанныеСтрокиРезультатаЗапросаПомещеия.ЗавершениеОП-1;
		СтрокаНабора.ВидНачисления 				= Перечисления.энргВидыНачислений.Начисление;
		СтрокаНабора.ПериодРасчета 				= ДанныеСтрокиРезультатаЗапросаПомещеия.ПериодНачисления;
		СтрокаНабора.ОбъемУслуги 				= ОбъемУслуги;
		СтрокаНабора.ОбъемУслугиПотребленный 	= 0;
	КонецЕсли;
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, РаспределениеОбъемаПоУслугам, НаборОтрицательныйОбъемРаспределения, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП, НаборЗаписейСверхНорма, ИспользоватьТарифРек)
	...
	ДанныеОбъемаКРаспределению 						= ДанныеОбъемаКРаспределению(ДанныеПоУслуге, Услуга, ГраницыПериода, ЗначенияТарифов, НаборЗаписей, НаборЗаписейПомещение, НаборЗаписейНП, ОбщийОбъемНачислений, ОбщийОбъемНачисленийПомещений);
...
Функция ДанныеОбъемаКРаспределению(ДанныеПоУслуге, Услуга, ГраницыПериода, ЗначенияТарифов, НаборЗаписей, НаборЗаписейПомещение, НаборЗаписейНП, ОбщийОбъемНачислений, ОбщийОбъемНачисленийПомещений)
	...
	ИначеЕсли ЕстьПотреблениеПоПрибору и ДанныеПоУслуге.ОбъемПоПриборуТарифныЗоны.Количество() = ДанныеПоУслуге.ПотреблениеАбонентовТарифныеЗоны.Количество() Тогда
		Для Каждого КлючЗначениеТарифнаяЗона Из ДанныеПоУслуге.ОбъемПоПриборуТарифныЗоны Цикл
			ОсталосьРаспределить.Вставить(КлючЗначениеТарифнаяЗона.Ключ, КлючЗначениеТарифнаяЗона.Значение - ДанныеПоУслуге.ПотреблениеАбонентовТарифныеЗоны[КлючЗначениеТарифнаяЗона.Ключ]);
		КонецЦикла;
		СпособНачисленияОДН 								= ДанныеПоУслуге.СпособНачисленияМКД;
