...
Функция ТекстПолучитьСтабильныеПериоды(ЭтоПерерасчет)
	...
	|	ВЫБОР
	|		КОГДА НЕ УслугиНеПрименятьСезонностьМКД.НеПрименятьСезонность есть null и УслугиНеПрименятьСезонностьМКД.НеПрименятьСезонность
	|			ТОГДА 1
	|		КОГДА НЕ УслугиНеПрименятьСезонностьРайон.НеПрименятьСезонность есть Null и УслугиНеПрименятьСезонностьРайон.НеПрименятьСезонность
	|			ТОГДА 1	
...
Функция ТекстПолучитьДанныеДляРасчета()
	...
	|	ВЫБОР
	|		КОГДА Периоды.ЧастныйСектор И энргПомещения.Родитель ЕСТЬ NULL
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.энргПомещения.ПустаяСсылка)
	|		КОГДА Периоды.ЧастныйСектор И энргПомещения.Родитель = ЗНАЧЕНИЕ(Справочник.энргПомещения.ПустаяСсылка)
