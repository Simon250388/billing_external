...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|			И ДанныеДляРасчетаПередНормой.ПериодаНачисления = ПроцентВетхостиСтроений.ПериодаНачисления
		...
		Для Каждого СтрокаПериод Из СтрокаПоФормуле.Значение Цикл
			...
			Для Сч = 0 По ИндексыСтрок.ВГраница() Цикл
				...
				ИначеЕсли ЗначениеЗаполнено(СтрокаНабора.ЗависитОтУслуги) Тогда
					ОбъемУслуги 									= Макс(НачисленияПоУслугам[СтрокаНабора.ЗависитОтУслуги].ОбъемУслуги,0);
					ОбъемУслугиПотребленный 						= Макс(НачисленияПоУслугам[СтрокаНабора.ЗависитОтУслуги].ОбъемУслугиПотребленный,0);
					ВсеНачисленияПоПрибору 							= НачисленияПоУслугам[СтрокаНабора.ЗависитОтУслуги].ВсеНачисленияПоПрибору;
				Иначе	
					ОбъемУслуги 									= Макс(НачисленияПоУслугам[СтрокаНабора.Услуга].ОбъемУслуги,0);
					ОбъемУслугиПотребленный 						= Макс(НачисленияПоУслугам[СтрокаНабора.Услуга].ОбъемУслугиПотребленный,0);
					ВсеНачисленияПоПрибору 							= НачисленияПоУслугам[СтрокаНабора.Услуга].ВсеНачисленияПоПрибору;
				КонецЕсли;
				СтрокаНабора.СпособНачисления 						= СпособПоФормуле;
				Если НЕ ОбъемУслугиБыло = 0 Тогда
					СтрокаНабора.ОбъемУслуги						= Макс(ОбъемУслуги,0);
				КонецЕсли;
				Если Не ОбъемУслугиПотребленныйБыло = 0 Тогда
					СтрокаНабора.ОбъемУслугиПотребленный			= Макс(ОбъемУслугиПотребленный,0);
				КонецЕсли;				
