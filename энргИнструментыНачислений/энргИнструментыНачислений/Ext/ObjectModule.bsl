...
// Процедура - НачислитьПомещенияСРазделеннымиЛС
//
// Параметры:
//  ПериодНачисления - 	 - 
//  Район			 - 	 - 
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	ДанныеДляРасчетаПередНормой.ФизЛицо,
	...
	|		КОГДА ДанныеДляРасчета.ИспользоватьКоэффициентСдерживанияРостаПлаты И НЕ ДанныеДляРасчета.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.Пустаяссылка)
	|			ТОГДА ВЫБОР
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребленияПомещенияСРазделеннымиЛС()
	...
	|	ЗНАЧЕНИЕ(Справочник.энргТарифныеЗоны.БезРазбивки),
	|	ДанныеДляРасчета.ПриборВведен,
	|	ВЫРАЗИТЬ(ДанныеДляРасчета.КоличествоЖильцов * ДанныеДляРасчета.КоэффициентСезоности * ДанныеДляРасчета.НормаПотребления * ДанныеДляРасчета.КоэффициентНорматива * ВЫБОР
	|		КОГДА ДанныеДляРасчета.ИспользоватьКоэффициентСдерживанияРостаПлаты и НЕ ДанныеДляРасчета.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.Пустаяссылка)
	|			ТОГДА ВЫБОР
	|					КОГДА ДанныеДляРасчета.ПлощадьАбонентов - ДанныеДляРасчета.ПлощадьСобственнаяПоставка + ДанныеДляРасчета.ПлощадьМОП = 0
	|						ТОГДА 0
	...
	|	ДанныеДляРасчета.ТарифнаяЗона,
	|	ДанныеДляРасчета.ПриборВведен,
	|	ВЫРАЗИТЬ(ДанныеДляРасчета.КоличествоЖильцов * ДанныеДляРасчета.КоэффициентСезоности * ДанныеДляРасчета.НормаПотребления * ВЫБОР
	|		КОГДА ДанныеДляРасчета.ИспользоватьКоэффициентСдерживанияРостаПлаты и НЕ ДанныеДляРасчета.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.Пустаяссылка)
	|			ТОГДА ВЫБОР
	|					КОГДА ДанныеДляРасчета.ПлощадьАбонентов - ДанныеДляРасчета.ПлощадьСобственнаяПоставка + ДанныеДляРасчета.ПлощадьМОП = 0
	|						ТОГДА 0
	...
	|			КОГДА ДанныеДляРасчета.ОбъемЗаПрошлыйГод
	|				ТОГДА 0
	|			ИНАЧЕ ВЫРАЗИТЬ(ДанныеДляРасчета.КоличествоЖильцов * ДанныеДляРасчета.КоэффициентСезоности * ДанныеДляРасчета.НормаПотребления * ВЫБОР
	|		КОГДА ДанныеДляРасчета.ИспользоватьКоэффициентСдерживанияРостаПлаты и НЕ ДанныеДляРасчета.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.Пустаяссылка)
	|			ТОГДА ВЫБОР
	|					КОГДА ДанныеДляРасчета.ПлощадьАбонентов - ДанныеДляРасчета.ПлощадьСобственнаяПоставка + ДанныеДляРасчета.ПлощадьМОП = 0
	|						ТОГДА 0
