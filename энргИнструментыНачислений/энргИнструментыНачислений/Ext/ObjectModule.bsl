...
Функция ТекстПолучитьСтабильныеПериоды()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	" + энргНачисления.СформироватьСтрокуДляЗапроса("энргСтабильныеПериоды","СтабильныеПериодыПослеИзмененияСезонности","Шкала,РасчетТолькоПоПоказаниям",Истина,Ложь) + "	
							...
							Для Каждого КлючЗначениеСоставнаяУслуга Из КлючЗначениеНачалоПериода.Значение Цикл
								...
								Для Индекс = 0 По СтрокиНабораПомещения.ВГраница() Цикл
									...
									Если СтрокаНабора.СпособНачисления = Перечисления.энргСпособНачислений.ПоПриборам Тогда
										...
										Если ОбъемыПомещенийССоглашением[СтрокаНабора.Услуга] <> Неопределено и ОбъемыПомещенийССоглашением[СтрокаНабора.Услуга][СтрокаНабора.ПомещениеРодитель] <> Неопределено и ОбъемыПомещенийССоглашением[СтрокаНабора.Услуга][СтрокаНабора.ПомещениеРодитель].ВсеПоПриборам Тогда 
											Если ЕстьПотребленный Тогда
												СтрокаНабора.ОбъемУслугиПотребленный		= СтрокаНабора.ОбъемУслугиПотребленный + (СтрокаНабораПомещения.ОбъемУслугиПотребленный - ОбъемыПомещенийССоглашением[СтрокаНабора.Услуга][СтрокаНабора.ПомещениеРодитель].ОбъемУслугиПотребленный) * КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЧислитель / КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЗнаменатель;
											Иначе
												СтрокаНабора.ОбъемУслуги 					= СтрокаНабора.ОбъемУслуги + (СтрокаНабораПомещения.ОбъемУслуги - ОбъемыПомещенийССоглашением[СтрокаНабора.Услуга][СтрокаНабора.ПомещениеРодитель].ОбъемУслуги) * КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЧислитель / КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЗнаменатель;
											КонецЕсли;
										Иначе
											Если ЕстьПотребленный Тогда
												СтрокаНабора.ОбъемУслугиПотребленный		= СтрокаНабора.ОбъемУслугиПотребленный + СтрокаНабораПомещения.ОбъемУслугиПотребленный  * КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЧислитель / КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЗнаменатель;
											Иначе
												СтрокаНабора.ОбъемУслуги 					= СтрокаНабора.ОбъемУслуги + СтрокаНабораПомещения.ОбъемУслуги * КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЧислитель / КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЗнаменатель;
											КонецЕсли;
										КонецЕсли;
