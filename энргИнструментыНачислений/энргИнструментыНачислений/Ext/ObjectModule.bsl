...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(Строение, Помещение)
	...
	|	РегистрСведений.энргСтабильныеПериоды КАК энргСтабильныеПериоды
	// TODO FIX ME
	// Срочно убери это от меня, я вся чешусь......
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДиаметрСеченийТочекУчета КАК ДиаметрСеченийТочекУчета
	|		ПО энргСтабильныеПериоды.Организация = ДиаметрСеченийТочекУчета.Организация
	|			И энргСтабильныеПериоды.ПериодНачисления = ДиаметрСеченийТочекУчета.ПериодНачисления
	|			И энргСтабильныеПериоды.Район = ДиаметрСеченийТочекУчета.Район
	|			И энргСтабильныеПериоды.ЧастныйСектор = ДиаметрСеченийТочекУчета.ЧастныйСектор
	|			И энргСтабильныеПериоды.Строение = ДиаметрСеченийТочекУчета.Строение
	|			И энргСтабильныеПериоды.Помещение = ДиаметрСеченийТочекУчета.Помещение
	|			И энргСтабильныеПериоды.ТочкаУчета = ДиаметрСеченийТочекУчета.ТочкаУчета
	// TODO FIX ME	
	|ГДЕ
	|	энргСтабильныеПериоды.Организация = &Организация
	|	И энргСтабильныеПериоды.ПериодНачисления = &ПериодНачисления
...
Функция ТекстПолучитьСтабильныеПериоды(ЭтоПерерасчет)
	...
	|	СтабильныеПериодыНачало.ИспользоватьСечение КАК ИспользоватьСечение,
	|	СтабильныеПериодыНачало.ДиаметрСечения КАК ДиаметрСечения,
	...
	|	СтабильныеПериодыТариф.ИспользоватьСечение КАК ИспользоватьСечение,
	|	СтабильныеПериодыТариф.ДиаметрСечения КАК ДиаметрСечения
	...
	|	СтабильныеПериодыНачалоСезонности.ИспользоватьСечение КАК ИспользоватьСечение,
	|	СтабильныеПериодыНачалоСезонности.ДиаметрСечения КАК ДиаметрСечения,
	...
	|	СтабильныеПериодыСезонность.ИспользоватьСечение КАК ИспользоватьСечение,
	|	СтабильныеПериодыСезонность.ДиаметрСечения КАК ДиаметрСечения,
	|	ВЫБОР
	|		КОГДА НомераСтрокКонецСезонностиБезФормулы.НомерПозиции ЕСТЬ NULL
	...
	|	СтабильныеКонецСезонности.ИспользоватьСечение КАК ИспользоватьСечение,
	|	СтабильныеКонецСезонности.ДиаметрСечения КАК ДиаметрСечения,
	...
	|	СтабильныеКонецСезонностиТарифнаяЗона.ИспользоватьСечение КАК ИспользоватьСечение,
	|	СтабильныеКонецСезонностиТарифнаяЗона.ДиаметрСечения КАК ДиаметрСечения,
	...
	|	СтабильныеПериодыПоказанияНачало.ИспользоватьСечение КАК ИспользоватьСечение,
	|	СтабильныеПериодыПоказанияНачало.ДиаметрСечения КАК ДиаметрСечения,
...
Функция ТекстПолучитьДанныеДляРасчета()
	...
	|	Периоды.ИспользоватьСечение КАК ИспользоватьСечение,
	...
	|	ДанныеДляРасчетаПередНормой.ИспользоватьСечение КАК ИспользоватьСечение,
	|	ДанныеДляРасчетаПередНормой.ДиаметрСечения КАК ДиаметрСечения,
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|			ТОГДА ОбъемыНачислений.ОбъемУслугиСоцНорма
	|		ИНАЧЕ ОбъемыНачислений.ОбъемУслуги
	|	КОНЕЦ * ОбъемыНачислений.ЗначениеТарифа КАК Сумма,
...
Функция ТекстЗапросаДанныеДляРаспределения(Строение)
	...
	|	ЕСТЬNULL(ОбъемНачислений.Сумма, 0) КАК Сумма,
	...
	|	ЕСТЬNULL(ОбъемНачислений.Сумма, 0),
...
Процедура ПрочитатьСтабильныеПериодыПерерасчет(МВТ,Организация,ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,ДанныеСтроения,Абонент)
	...
	|	ЕСТЬNULL(НаборэнргДиаметрСеченияТочекУчетаГраницы.Использовать, ЛОЖЬ) КАК ИспользоватьСечение,
	...
	|	ДанныеПриборовУчета.ИспользоватьСечение КАК ИспользоватьСечение,
	...
	|	ВсеСведения.ИспользоватьСечение КАК ИспользоватьСечение,
	...
	|	ДанныеНормТарифов.ИспользоватьСечение КАК ИспользоватьСечение,
	...
	|	СтабильныеПериодыБезНомераНачало.ИспользоватьСечение КАК ИспользоватьСечение,
	...
	|	СтабильныеПериодыБезНомераНачало.ИспользоватьСечение,
...
Процедура РассчитатьНормативПоФормуле(ПараметрыРасчетаНорматива, РассчитыватьНорматив, НачисленияПоУслугам, ЗначенияТарифов, ГраницыПериода,Отказ, НаборЗаписей, НаборЗаписейНП, НаборЗаписейПомещений)
	...
	Для Сч = 0 По РассчитыватьНорматив.ВГраница() Цикл
		...
		Для Каждого СтрокаНачислений ИЗ НачисленияПоУслуге Цикл
			...
			СтрокаНачисляем.ЗначениеТарифа 					= ЗначениеТарифа.Значение;
...
Функция ТекстЗапросаВспомогательныеДанныеНачислениеПоДолевымСхемам()
	...
	Текст = 
		...
		|	РассчитанныеОбъемыСумма.Объем КАК Объем,
