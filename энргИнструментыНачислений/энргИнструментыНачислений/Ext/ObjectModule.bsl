...
Функция РезультатЗапросаПерерасчетСреднего(МВТ,Организация,ПериодНачисления,НачалоПериодаРасчета, КонецПериодаРасчета,Район,Строение,Помещение,НачисленныйСреднийОбъем=Неопределено)
	...
	|	ЕСТЬNULL(НачисленныйОбъем.ОбъемУслугиОборот,0) КАК ОбъемУслугиНачислено,
	|	ЕСТЬNULL(НачисленныйОбъем.ОбъемУслугиПотребленныйОборот,0) КАК ОбъемУслугиПотребленныйНачислено,
	|	ЕСТЬNULL(НачисленныйОбъем.ЗначениеТарифа,0) КАК ЗначениеТарифаНачислено
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ НачисленныйОбъем КАК НачисленныйОбъем
