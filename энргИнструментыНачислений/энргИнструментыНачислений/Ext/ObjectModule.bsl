...
Процедура НачислениеНаОДН(Организация,ПериодНачисления, Район,МКД, Документ)  Экспорт 
	...
	Пока Выборка.следующий() цикл
		...
		Если Не ТекМКД = Выборка.МКД Или НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
			...
			|Услуга, ФормулаРасчета, Поставщик, СпособНачисления, ПриборУчета, НачалоПериода, КонецПериода, ДокументСсылка, КонецПериодаРасчета, ОбъемУслугиПотребленный, ОбъемЗаПрошлыйГод",
			Выборка.Услуга, Выборка.ФормулаРасчета, Выборка.Поставщик, Выборка.СпособНачисления, Выборка.ПриборУчета, Выборка.НачалоПериода, Выборка.КонецПериода, ДокументСсылка, КонецПериодаРасчета, Выборка.ОбъемУслугиПотребленный, Выборка.ОбъемЗаПрошлыйГод); 			
...
Процедура РаспредениеОбъемаМКД(Организация,ПериодНачисления, Район, Строение, Документ)  Экспорт 
	...
	Пока ВыборкаОбъемМКД.Следующий() Цикл
		...
		Если НЕ ТекСтроение = ВыборкаОбъемМКД.Строение Или НЕ ТекУслуга = ВыборкаОбъемМКД.Услуга Тогда
			...
			Если Не ТекУслуга  = Неопределено Тогда
				РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, ТекСтроение, ТекУслуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейНП);
			КонецЕсли;
			
			Если НЕ ТекСтроение = ВыборкаОбъемМКД.Строение Тогда
				Если Не НаборЗаписей = Неопределено и НаборЗаписей.Количество() > 0 Или 
					Не НаборЗаписейНП = Неопределено и НаборЗаписейНП.Количество() > 0 Тогда
			...
			Пока ВыборкаАбоненты.Строение = ВыборкаОбъемМКД.Строение и ВыборкаАбоненты.Услуга = ВыборкаОбъемМКД.Услуга Цикл
				Если НЕ ТекАбонент = ВыборкаАбоненты.Абонент Тогда
					Если Не ТекАбонент = Неопределено Тогда
						мАбонентыДляРаспределения.Добавить(ДанныеАбонента);
						ДанныеПоУслуге.ПлощадиАбонентов.Вставить(ТекАбонент,ДанныеАбонента.ЗначениеПлощади);
					...
					ДанныеАбонента 									= Новый Структура("Договор, Абонент, ЗначениеПлощади, ТарифнаяГруппа, Поставщик, ОбъемУслуги, НП",
					ВыборкаАбоненты.Помещение, ВыборкаАбоненты.Абонент, ВыборкаАбоненты.ЗначениеПлощадиДляРаспределения,ВыборкаАбоненты.ТарифнаяГруппаДляРаспределения, ВыборкаАбоненты.ПоставщикДляРаспределения, 0, ВыборкаАбоненты.НП);
					ЕстьРасходПоПрибору 							= Ложь;					
				КонецЕсли;				
				...
				Если ВыборкаАбоненты.ЕстьИндивидуальныеНачисления Тогда 
					...
					Иначе
						...
						Если ВыборкаАбоненты.СпособНачисления = Перечисления.энргСпособНачислений.ПоНормативу
							ИЛИ ВыборкаАбоненты.СпособНачисления = Перечисления.энргСпособНачислений.СреднееПоНормативу Тогда							
				...
				СпособНачисления 								= ВыборкаАбоненты.СпособНачисления;
				ПотреблениеАбонентов 							= ПотреблениеАбонентов + ВыборкаАбоненты.ОбъемУслугиОборот;
								
				Если ВыборкаАбоненты.СпособНачисления = Перечисления.энргСпособНачислений.ПоНормативу ИЛИ ВыборкаАбоненты.СпособНачисления = Перечисления.энргСпособНачислений.СреднееПоНормативу Тогда
					ВсеПоПрибору 								= Ложь;
				Иначе
				...
				ТекАбонент 									 	= ВыборкаАбоненты.Абонент;
		...
		Если ВыборкаОбъемМКД.СпособНачисления = Перечисления.энргСпособНачислений.ПоНормативу 
			ИЛИ ВыборкаОбъемМКД.СпособНачисления = Перечисления.энргСпособНачислений.СреднееПоНормативу Тогда
	...
	Если Не НаборЗаписей = Неопределено Или НаборЗаписейНП <> Неопределено Тогда
		...
		Если НаборЗаписей.Количество() > 0 Или НаборЗаписейНП.Количество() > 0 Тогда
			НачатьТранзакцию();
			Попытка				
				Если Не ДокументОбъект = Неопределено Тогда
					ДокументОбъект.ДополнительныеСвойства.Вставить("ПроведениеРазрешено",Истина);
...
Процедура СнятьСредниеНачисления(Организация,ПериодНачисления,Район,Строение,Помещение,Документ) Экспорт
	...
	результатЗапроса 								= РезультатЗапросаПерерасчетСреднего(МВТ,Организация, ПериодНачисления,Район,Строение,Помещение); 	
	...
	Пока Выборка.следующий() цикл
		...
		СтрокаНабора.Сумма		 					= -Выборка.Сумма;
...
Функция ДанныеПерерасчета(Организация,Район,МКД,Абонент,ПериодНачисленияНачало,ПериодНачисленияКонец,Набор,КорректировкаПоказаний) Экспорт 
	...
	РезультатЗапроса  								= ПересчитатьИндивидуальныйОбъем(МВТ,Организация, ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,МКД,Абонент,КорректировкаПоказаний,ПериодНовогоТарифа);	
	Выборка  										= РезультатЗапроса.Выбрать();
...
Процедура НачислитьПомещенияСРазделеннымиЛС(Организация, ПериодНачисления,Район,Строение,Помещение,Документ) Экспорт
	...
	ДнейВПериоде  									= ГраницыПериода.ДнейВПериоде; 
	ПериодНовогоТарифа 								= Константы.энргДеньНачалаПримененияНовогоТарифа.Получить();
	результатЗапроса 								= РезультатЗапросаНачисленияПомещенияСРазделеннымиЛС(Организация, ПериодНачисления,Район,Строение,Помещение,НачалоПериодаРасчета,КонецПериодаРасчета,ПериодНовогоТарифа,ДнейВПериоде); 	
	КонецПериода  									= КонецМесяца(ПериодНачисления);
	...
	Пока Выборка.следующий() цикл
		...
		Если Документ = Неопределено тогда			
			...
			Если НЕ ТекМКД = Выборка.МКД Тогда
				...
				ЗамещатьНаборВыполненные 					= Истина;
				Если НЕ ЗначениеЗаполнено(ДокументСсылка)  Тогда
					ДокументСсылка 							= Документы.энргНачислениеПоПомещениямСРазделеннымиЛС.ПолучитьСсылку(Новый УникальныйИдентификатор());
					ДокументОбъект  						= Документы.энргНачислениеПоПомещениямСРазделеннымиЛС.СоздатьДокумент();
					ДокументОбъект.УстановитьСсылкуНового(ДокументСсылка);
					ДокументОбъект.район  					= Район;
					ДокументОбъект.МКД 						= Выборка.МКД;
					ДокументОбъект.Дата						= КонецПериодаРасчета-1;
					...
					ЗаписьВыполненные.Район		 			= Район;
					ЗаписьВыполненные.ПериодНачисления 		= ПериодНачисления;
					ЗаписьВыполненные.ВидОперацийНачисления = Перечисления.энргВидыОперацийНачисления.ПомещенияСРазделеннымиЛС;
					ЗаписьВыполненные.МКД 					= Выборка.МКД;
					ЗаписьВыполненные.ДокументНачисления 	= ДокументСсылка;
					ЗамещатьНаборВыполненные 				= Ложь;
				ИначеЕсли ПометкаУдаления Тогда
				...
				Набор 								= РегистрыНакопления.энргОбъемНачисленийПомещенийСРазделеннымиЛС.СоздатьНаборЗаписей();
				Набор.Отбор.Регистратор.Установить(ДокументСсылка);
			...
			КонецЕсли;
		иначе
			Набор 									= Документ.Движения.энргОбъемНачисленийПомещенийСРазделеннымиЛС; 
		КонецЕсли;		
...
Процедура ВспомогательныеДанныеПерерасчет(МВТ,Организация,ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,МКД,Абонент,ПериодНовогоТарифа)
	...
	|	энргСреднийОбъемПотребленияАбонентов.МКД КАК МКД,
	|	энргСреднийОбъемПотребленияАбонентов.Абонент КАК Абонент,
	...
	|	И энргСреднийОбъемПотребленияАбонентов.МКД = &МКД
	|	И энргСреднийОбъемПотребленияАбонентов.Абонент = &Абонент
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СтабильныеПериодыНачало.Абонент КАК Абонент,
	...
	|	СтабильныеПериодыНачало.Абонент,
...
Процедура ПоказанияПерерасчет(МВТ,Организация,ПериодНачисленияНачало,ПериодНачисленияКонец,Район,МКД,Абонент,КорректировкаПоказаний)
	...
	|	И энргМаксимальныеПоказанияСрез.МКД = &МКД
	|	И энргМаксимальныеПоказанияСрез.Абонент = &Абонент";
	...
	|	яПредоставленныеПоказания.Абонент КАК Абонент,
	...
	|		И энргМаксимальныеПоказанияСрез.Район = &Район
	|		И энргМаксимальныеПоказанияСрез.МКД = &МКД
	|		И энргМаксимальныеПоказанияСрез.Абонент = &Абонент) КАК ВложенныйЗапрос
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ГраницыОтчетногоПериода КАК ГраницыОтчетногоПериода
...
Функция ТекстПолучитьСтабильныеПериоды(ЭтоПерерасчет)
	...
	|	СтабильныеКонецСезонностиТарифнаяЗона.УвеличениеПозиции,
	...
	|////////////////////////////////////////////////////////////////////////////////	
	|ВЫБРАТЬ
	|	" + энргНачисления.СформироватьСтрокуДляЗапроса("энргСтабильныеПериоды","СтабильныеПериодыПоказанияНачало","ПериодРегистрации,ДатаРегистратора,ДокРегистратор",Истина,Ложь) + "	
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	Текст  = 
	"ВЫБРАТЬ
	|	ДанныеДляРасчетаПередНормойИзмерители.Организация КАК Организация,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПериодНачисления КАК ПериодНачисления,
	|	ДанныеДляРасчетаПередНормойИзмерители.Район КАК Район,
	|	ДанныеДляРасчетаПередНормойИзмерители.ЧастныйСектор КАК ЧастныйСектор,
	|	ДанныеДляРасчетаПередНормойИзмерители.Строение КАК Строение,
	|	ДанныеДляРасчетаПередНормойИзмерители.Помещение КАК Помещение,
	|	ДанныеДляРасчетаПередНормойИзмерители.Услуга КАК Услуга,
	|	ДанныеДляРасчетаПередНормойИзмерители.НаправлениеИспользованияТУ КАК НаправлениеИспользованияТУ,
	|	МИНИМУМ(ВЫБОР
	|			КОГДА ДанныеДляРасчетаПередНормойИзмерители.ЗависимаяТочкаУчета <> ЗНАЧЕНИЕ(Справочник.энргТочкиУчета.ПустаяСсылка)
	|					И НЕ ДанныеДляРасчетаПередНормойИзмерители.ПриборВведен
	|				ТОГДА 1
	|			КОГДА НЕ ДанныеДляРасчетаПередНормойИзмерители.ПриборВведен
	|				ТОГДА 2
	|			ИНАЧЕ 3
	|		КОНЕЦ) КАК Приоритет,
	|	МИНИМУМ(ВЫБОР
	|			КОГДА ДанныеДляРасчетаПередНормойИзмерители.СреднийОбъем >= 0
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	...
	|ГДЕ
	|	ДанныеДляРасчетаПередНормойИзмерители.КоэффициентРасчета <= 0
	|	И (НЕ ДанныеДляРасчетаПередНормойИзмерители.ПриборВведен
	|			ИЛИ НЕ ДанныеДляРасчетаПередНормойИзмерители.ПоказанияВСрок)
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДляРасчетаПередНормойИзмерители.Организация,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПериодНачисления,
	|	ДанныеДляРасчетаПередНормойИзмерители.Район,
	|	ДанныеДляРасчетаПередНормойИзмерители.ЧастныйСектор,
	|	ДанныеДляРасчетаПередНормойИзмерители.Строение,
	|	ДанныеДляРасчетаПередНормойИзмерители.Помещение,
	|	ДанныеДляРасчетаПередНормойИзмерители.Услуга,
	|   ДанныеДляРасчетаПередНормойИзмерители.НаправлениеИспользованияТУ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДанныеДляРасчетаПередНормойИзмерители.Услуга КАК Услуга,
	|	ДанныеДляРасчетаПередНормойИзмерители.НаправлениеИспользованияТУ,
	|	МИНИМУМ(ДанныеДляРасчетаПередНормойИзмерители.ТочкаУчета) КАК ТочкаУчета,
	...
	|		ПО ДанныеДляРасчетаПередНормойИзмерители.Организация = ПриоритетНормы.Организация
	|			И ДанныеДляРасчетаПередНормойИзмерители.ПериодНачисления = ПриоритетНормы.ПериодНачисления
	|			И ДанныеДляРасчетаПередНормойИзмерители.Район = ПриоритетНормы.Район
	|			И ДанныеДляРасчетаПередНормойИзмерители.ЧастныйСектор = ПриоритетНормы.ЧастныйСектор
	|			И ДанныеДляРасчетаПередНормойИзмерители.Строение = ПриоритетНормы.Строение
	|			И ДанныеДляРасчетаПередНормойИзмерители.Помещение = ПриоритетНормы.Помещение
	|			И ДанныеДляРасчетаПередНормойИзмерители.Услуга = ПриоритетНормы.Услуга
	|			И (ДанныеДляРасчетаПередНормойИзмерители.КоэффициентРасчета <= 0)
	|			И (НЕ ДанныеДляРасчетаПередНормойИзмерители.ПриборВведен
	|				ИЛИ НЕ ДанныеДляРасчетаПередНормойИзмерители.ПоказанияВСрок)
	|			И (ВЫБОР
	|				КОГДА ПриоритетНормы.Приоритет = 1
	|					ТОГДА ДанныеДляРасчетаПередНормойИзмерители.ЗависимаяТочкаУчета <> ЗНАЧЕНИЕ(Справочник.энргТочкиУчета.ПустаяСсылка)
	|							И НЕ ДанныеДляРасчетаПередНормойИзмерители.ПриборВведен
	|				КОГДА ПриоритетНормы.Приоритет = 2
	|					ТОГДА НЕ ДанныеДляРасчетаПередНормойИзмерители.ПриборВведен
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	" + энргНачисления.СформироватьСтрокуДляЗапроса("энргСтабильныеПериоды","ДанныеДляРасчетаПередНормойИзмерители","НомерПозиции,ПериодРегистрации,ДатаРегистратора,ДокРегистратор",Истина,Ложь) + "
	|	ДанныеДляРасчетаПередНормойИзмерители.МКД КАК МКД,
	|	ДанныеДляРасчетаПередНормойИзмерители.НачалоПериода КАК НачалоПериода,
	|	ДанныеДляРасчетаПередНормойИзмерители.КонецПериода КАК КонецПериода,
	|	ДанныеДляРасчетаПередНормойИзмерители.ВидДифференцированности КАК ВидДифференцированности,
	|	ДанныеДляРасчетаПередНормойИзмерители.Измеритель КАК Измеритель,
	|	ДанныеДляРасчетаПередНормойИзмерители.ТарифнаяЗона КАК ТарифнаяЗона,
	|	ДанныеДляРасчетаПередНормойИзмерители.НачПок КАК НачПок,
	|	ДанныеДляРасчетаПередНормойИзмерители.КонПок КАК КонПок,
	|	ДанныеДляРасчетаПередНормойИзмерители.МаксимальноВозможныеПоказания КАК МаксимальноВозможныеПоказания,
	|	ЕСТЬNULL(ЗначенияТекущихНормПотребления.ЗначениеНорматива, 0) КАК НормаПотребления,
	|	ДанныеДляРасчетаПередНормойИзмерители.СреднийОбъем КАК СреднийОбъем,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПоказанияВСрок КАК ПоказанияВСрок,
	|	ДанныеДляРасчетаПередНормойИзмерители.ЕстьПоказанияАбонента КАК ЕстьПоказанияАбонента,
	|	ДанныеДляРасчетаПередНормойИзмерители.ВыводПрибораУчета КАК ВыводПрибораУчета,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПоказанияПредоставлены КАК ДанныеДляРасчетаПЕредНормой,
	|	ЕСТЬNULL(ЗначенияТекущихТарифов.Значение, 0) КАК ЗначениеТарифа,
	|	ДанныеДляРасчетаПередНормойИзмерители.ДокументНачисления КАК ДокументНачисления,
	|	ДанныеДляРасчетаПередНормойИзмерители.ДокументНачисленияПометкаУдаления КАК ДокументНачисленияПометкаУдаления,
	|	ДанныеДляРасчетаПередНормойИзмерители.КоэффициентСезоности КАК КоэффициентСезоности,
	|	ДанныеДляРасчетаПередНормойИзмерители.НеПрименятьСезонность КАК НеПрименятьСезонность,
	|	ДанныеДляРасчетаПередНормойИзмерители.ОбъемЗаПрошлыйГод КАК ОбъемЗаПрошлыйГод,
	|	ДанныеДляРасчетаПередНормойИзмерители.ВидПлощади КАК ВидПлощади,
	|	ВЫБОР
	|		КОГДА ДанныеДляРасчетаПередНормойИзмерители.КоэффициентРасчета = 1
	|			ТОГДА ИСТИНА
	|		КОГДА ДанныеДляРасчетаПередНормойИзмерители.ТочкаУчета = ТочкиНачисленияНормы.ТочкаУчета
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	ДанныеДляРасчетаПередНормойИзмерители.ДнейВПериоде КАК ДнейВПериоде,
	|	ДанныеДляРасчетаПередНормойИзмерители.СледующийРегистратор КАК СледующийРегистратор,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПредыдущееЗначениеТарифа КАК ПредыдущееЗначениеТарифа,
	|	ДанныеДляРасчетаПередНормойИзмерители.КоэффициентНорматива КАК КоэффициентНорматива,
	|	ДанныеДляРасчетаПередНормойИзмерители.ИспользоватьПовыщающийКоэффициент КАК ИспользоватьПовыщающийКоэффициент,
	...
	|	ДанныеДляРасчетаПередНормойИзмерители.ФизЛицо КАК ФизЛицо,
	|	ДанныеДляРасчетаПередНормойИзмерители.ЗависитОтУслуги КАК ЗависитОтУслуги,
	|	ДанныеДляРасчетаПередНормойИзмерители.УмножатьНаНормуРодительскойУслуги КАК УмножатьНаНормуРодительскойУслуги,
	|	ДанныеДляРасчетаПередНормойИзмерители.ИспользоватьКоэффициентСдерживанияРостаПлаты КАК ИспользоватьКоэффициентСдерживанияРостаПлаты,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПлощадьМОП КАК ПлощадьМОП,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПлощадьАбонентов КАК ПлощадьАбонентов,
	|	ДанныеДляРасчетаПередНормойИзмерители.ПлощадьСобственнаяПоставка КАК ПлощадьСобственнаяПоставка,
	|	ВЫБОР
	...
	|		ПО ДанныеДляРасчетаПередНормойИзмерители.Организация = ТочкиНачисленияНормы.Организация
	|			И ДанныеДляРасчетаПередНормойИзмерители.ПериодНачисления = ТочкиНачисленияНормы.ПериодНачисления
	|			И ДанныеДляРасчетаПередНормойИзмерители.Район = ТочкиНачисленияНормы.Район
	|			И ДанныеДляРасчетаПередНормойИзмерители.ЧастныйСектор = ТочкиНачисленияНормы.ЧастныйСектор
	|			И ДанныеДляРасчетаПередНормойИзмерители.Строение = ТочкиНачисленияНормы.Строение
	|			И ДанныеДляРасчетаПередНормойИзмерители.Помещение = ТочкиНачисленияНормы.Помещение
	|			И ДанныеДляРасчетаПередНормойИзмерители.Услуга = ТочкиНачисленияНормы.Услуга
	|			И ДанныеДляРасчетаПередНормойИзмерители.ТочкаУчета = ТочкиНачисленияНормы.ТочкаУчета
	|			И ДанныеДляРасчетаПередНормойИзмерители.НаправлениеИспользованияТУ = ТочкиНачисленияНормы.НаправлениеИспользованияТУ
	...
	|			И ДанныеДляРасчетаПередНормойИзмерители.Помещение = ПриоритетНормы.Помещение
	|			И ДанныеДляРасчетаПередНормойИзмерители.Услуга = ПриоритетНормы.Услуга
	|			И ДанныеДляРасчетаПередНормойИзмерители.НаправлениеИспользованияТУ = ПриоритетНормы.НаправлениеИспользованияТУ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗначенияТекущихТарифов КАК ЗначенияТекущихТарифов
	|		ПО ДанныеДляРасчетаПередНормойИзмерители.Организация = ЗначенияТекущихТарифов.Организация
	|			И ДанныеДляРасчетаПередНормойИзмерители.Район = ЗначенияТекущихТарифов.Район
	|			И ДанныеДляРасчетаПередНормойИзмерители.ПериодНачисления = ЗначенияТекущихТарифов.ПериодНачисления
	|			И ДанныеДляРасчетаПередНормойИзмерители.ТарифнаяГруппа = ЗначенияТекущихТарифов.ГруппаПотребителей
	|			И ДанныеДляРасчетаПередНормойИзмерители.Услуга = ЗначенияТекущихТарифов.Услуга
	|			И ДанныеДляРасчетаПередНормойИзмерители.Поставщик = ЗначенияТекущихТарифов.Поставщик
	|			И ДанныеДляРасчетаПередНормойИзмерители.ТарифнаяЗона = ЗначенияТекущихТарифов.ТарифнаяЗона
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗначенияТекущихНормПотребления КАК ЗначенияТекущихНормПотребления
	|		ПО ДанныеДляРасчетаПередНормойИзмерители.Организация = ЗначенияТекущихНормПотребления.Организация
	|			И ДанныеДляРасчетаПередНормойИзмерители.Район = ЗначенияТекущихНормПотребления.Район
	|			И ДанныеДляРасчетаПередНормойИзмерители.ПериодНачисления = ЗначенияТекущихНормПотребления.ПериодНачисления
	|			И ДанныеДляРасчетаПередНормойИзмерители.Услуга = ЗначенияТекущихНормПотребления.Услуга
	|			И ДанныеДляРасчетаПередНормойИзмерители.КлючНорматива = ЗначенияТекущихНормПотребления.КлючНорматива
	|			И ДанныеДляРасчетаПередНормойИзмерители.НаправлениеИспользованияТУ = ЗначенияТекущихНормПотребления.НаправлениеИспользованияТУ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗначенияПовышающихКоэффициентов КАК ЗначенияПовышающихКоэффициентов
	|		ПО ДанныеДляРасчетаПередНормойИзмерители.Организация = ЗначенияПовышающихКоэффициентов.Организация
	|			И ДанныеДляРасчетаПередНормойИзмерители.Район = ЗначенияПовышающихКоэффициентов.Район
	|			И ДанныеДляРасчетаПередНормойИзмерители.ПериодНачисления = ЗначенияПовышающихКоэффициентов.ПериодНачисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ МКДНеНачислятьТЭ КАК МКДНеНачислятьТЭМКД
	|		ПО ДанныеДляРасчетаПередНормойИзмерители.Организация = МКДНеНачислятьТЭМКД.Организация
	|			И ДанныеДляРасчетаПередНормойИзмерители.Район = МКДНеНачислятьТЭМКД.Район
	|			И ДанныеДляРасчетаПередНормойИзмерители.Услуга = МКДНеНачислятьТЭМКД.Услуга
	|			И ДанныеДляРасчетаПередНормойИзмерители.Строение = МКДНеНачислятьТЭМКД.Строение
	|			И ДанныеДляРасчетаПередНормойИзмерители.ТипСхемы = МКДНеНачислятьТЭМКД.ТипСхемы
	|			И ДанныеДляРасчетаПередНормойИзмерители.ВидТепловогоПункта = МКДНеНачислятьТЭМКД.ПринадлежностьКТепловомуПункту
	|		ЛЕВОЕ СОЕДИНЕНИЕ МКДНеНачислятьТЭ КАК МКДНеНачислятьТЭ
	|		ПО ДанныеДляРасчетаПередНормойИзмерители.Организация = МКДНеНачислятьТЭ.Организация
	|			И ДанныеДляРасчетаПередНормойИзмерители.Район = МКДНеНачислятьТЭ.Район
	|			И ДанныеДляРасчетаПередНормойИзмерители.Услуга = МКДНеНачислятьТЭ.Услуга
	|			И (ДанныеДляРасчетаПередНормойИзмерители.Строение = ЗНАЧЕНИЕ(Справочник.энргСтроения.ПустаяСсылка))
	|			И ДанныеДляРасчетаПередНормойИзмерители.ТипСхемы = МКДНеНачислятьТЭ.ТипСхемы
	|			И ДанныеДляРасчетаПередНормойИзмерители.ВидТепловогоПункта = МКДНеНачислятьТЭ.ПринадлежностьКТепловомуПункту
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////	
	|ВЫБРАТЬ
...
Функция ТекстЗапросаРассчитатьОбъемПотребленияМКД()
	...
	|	ДанныеДляРасчета.ТарифнаяЗона,
	|	ДанныеДляРасчета.ПриборВведен,
	|	ВЫРАЗИТЬ(ДанныеДляРасчета.НормаПотребления * ВЫБОР
	|			КОГДА НЕ ДанныеДляРасчета.ВидПлощади = ЗНАЧЕНИЕ(Перечисление.энргВидыПлощадей.ПустаяСсылка)
	|				ТОГДА ДанныеДляРасчета.ЗначениеПлощади
	|			ИНАЧЕ 1
	|		КОНЕЦ * ВЫБОР
	...
	|		КОГДА ДанныеДляРасчета.ОбъемЗаПрошлыйГод
	|			ТОГДА 0
	|		ИНАЧЕ ВЫРАЗИТЬ(ДанныеДляРасчета.НормаПотребления * ВЫБОР
	|					КОГДА НЕ ДанныеДляРасчета.ВидПлощади = ЗНАЧЕНИЕ(Перечисление.энргВидыПлощадей.ПустаяСсылка)
	|						ТОГДА ДанныеДляРасчета.ЗначениеПлощади
	|					ИНАЧЕ 1
	|				КОНЕЦ * ВЫБОР
...
Функция РезультатЗапросаПерерасчетСреднего(МВТ,Организация,ПериодНачисления,Район,Строение,Помещение,НачисленныйСреднийОбъем=Неопределено)
	ТекстРазделить 						= " 
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|					ТОГДА энргПерерасчет.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.ПустаяСсылка)
	|				ИНАЧЕ ДанныеДляСписанияМинРегистратор.Строение = энргПерерасчет.МКД
	|			КОНЕЦ)
	|			И (энргПерерасчет.Организация = &Организация)
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейНП)
	...
	Если ДанныеПоУслуге.КорректироватьИндивидуальныйОбъемСУчетомОДПУ И ДанныеПоУслуге.ВсеПоПрибору Тогда
	...
	ИначеЕсли ДанныеПоУслуге.КорректироватьИндивидуальныйОбъемСУчетомОДПУ и ДанныеПоУслуге.ВсегоПлощадьАбонентовПоПрибору > 0 Тогда
		...
		Для Каждого СтрокаНачисленийАбонента ИЗ ДанныеПоУслуге.НачисленияАбонентовБезПрибора Цикл
			...
			ЗначениеПлощади 					= ДанныеПоУслуге.ПлощадиАбонентов[СтрокаНачисленийАбонента.Абонент];
			...
			ЗаполнитьЗначенияСвойств(СтрокаНабора, СтрокаНачисленийАбонента);
			СтрокаНабора.Распределение 			= Истина;
			СтрокаНабора.ОбъемУслуги 			= ЗначениеПлощади * ОбъемНаКвМ;
	...
	Для Сч = 0 По АбонентыДляРаспределения.ВГраница() Цикл
		...
		Иначе
			...
			СтрокаНабора.Помещение				= ДанныеАбонента.Абонент.КлючПомещения.Помещение; // TODO FIX ME
...
Процедура ПрочитатьСтабильныеПериодыПерерасчет(МВТ,Организация,ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,МКД,Абонент)
	...
	|Условие, УсловиеТаблицы, СвойствоТочки", "АбонентВладелец = &Абонент", "ТаблицаРС.АбонентВладелец = &Абонент", "ТочкаУчета"));  	
	
	Объекты.Вставить("энргАктивностьТочекУчета",						Новый Структура("
	|Условие, УсловиеТаблицы, СвойствоТочки", "Абонент = &Абонент", "ТаблицаРС.Абонент = &Абонент", "ТочкаУчета"));
	...
	|Условие, УсловиеТаблицы, СвойствоТочки", "Абонент = &Абонент", "ТаблицаРС.Абонент = &Абонент", "РассчитываемяТочка" ));
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ) КАК КоличествоПрописанных,
	|	МАКСИМУМ(ВЫБОР
	...
	|ПОМЕСТИТЬ РазрезыНормативаУслуг
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВложенныйЗАпрос.ПериодНачисления КАК ПериодНачисления,
	|		ВложенныйЗАпрос.Услуга КАК Услуга,
	...
	|				ПО ГраницыОтчетногоПериода.Организация = энргУстановленныеПараметрыРасчетаНорматива.Организация
	|					И ГраницыОтчетногоПериода.ЗавершениеОП > энргУстановленныеПараметрыРасчетаНорматива.Период
	...
	|			энргУстановленныеПараметрыРасчетаНорматива.ТипНачисления) КАК ВложенныйЗАпрос
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргУстановленныеПараметрыРасчетаНорматива КАК энргУстановленныеПараметрыРасчетаНорматива
	...
	|				И (энргУстановленныеПараметрыРасчетаНорматива.Организация = &Организация)
	|				И (энргУстановленныеПараметрыРасчетаНорматива.Район = &Район)
	|				И ВложенныйЗАпрос.ТипНачисления = энргУстановленныеПараметрыРасчетаНорматива.ТипНачисления) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.ПериодНачисления,
	|	ВложенныйЗапрос.Услуга
	...
	|	энргВыводПриборовУчета.ПериодНачисления КАК ПериодНачисления,
	|	энргВыводПриборовУчета.Район КАК Район,
	|	энргВыводПриборовУчета.Строение КАК МКД,
	|	энргВыводПриборовУчета.УдалитьАбонент КАК Абонент,
	|	энргВыводПриборовУчета.ТочкаУчета КАК ТочкаУчета,
	|	энргВыводПриборовУчета.ПриборУчета КАК ПриборУчета
	|ПОМЕСТИТЬ ДанныеВыводаПрибораУчета
	|ИЗ
	...
	|			И (энргВыводПриборовУчета.УдалитьАбонент = &Абонент)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(НаборэнргГруппыПотребителейАбонентовГраницы.ГруппаПотребителей, ЗНАЧЕНИЕ(Справочник.энргГруппыПотребителей.ПустаяССылка)) КАК ГруппаПотребителей,
	...
	|		ПО (ВЫБОР
	|				КОГДА ГраницыПериодов.НачПер = ГраницыПериодов.КонПер
	|					ТОГДА ГраницыПериодов.ДокРегистратор = НаборэнргГруппыПотребителейАбонентовГраницы.ДокРегистратор
	|				ИНАЧЕ НаборэнргГруппыПотребителейАбонентовГраницы.ПериодС <= ГраницыПериодов.НачПер
	|						И НаборэнргГруппыПотребителейАбонентовГраницы.ПериодПо >= ГраницыПериодов.КонПер
	|			КОНЕЦ)
	...
	|		ПО (ВЫБОР
	|				КОГДА ГраницыПериодов.НачПер = ГраницыПериодов.КонПер
	|					ТОГДА ГраницыПериодов.ДокРегистратор = НаборэнргВидБлагоустройствАбонентаГраницы.ДокРегистратор
	|				ИНАЧЕ НаборэнргВидБлагоустройствАбонентаГраницы.ПериодС <= ГраницыПериодов.НачПер
	|						И НаборэнргВидБлагоустройствАбонентаГраницы.ПериодПо >= ГраницыПериодов.КонПер
	|			КОНЕЦ)
	...
	|	ОбщиеДанные.ГруппаПотребителей КАК ГруппаПотребителей,
	|	ВЫБОР
	...
	|	ДанныеПриборовУчета.ГруппаПотребителей КАК ГруппаПотребителей,
	...
	|	ДанныеПриборовУчета.ВидБлагоустройства КАК ВидБлагоустройства,
	...
	|	ВсеСведения.ГруппаПотребителей КАК ГруппаПотребителей,
	|	ВсеСведения.ВидБлагоустройства КАК ВидБлагоустройства,
	...
	|	РазрезыНормативаУслуг.ВидБлагоустройства КАК ЕстьРазрезВидБлагоустройства,
	|	РазрезыНормативаУслуг.ВидСобственности КАК ЕстьРазрезВидСобственности,
	|	РазрезыНормативаУслуг.ГодПостройки КАК ЕстьРазрезГодПостройки,
	|	РазрезыНормативаУслуг.КоличествоКомнат КАК ЕстьРазрезКоличествоКомнат,
	|	РазрезыНормативаУслуг.КоличествоПрописанных КАК ЕстьРазрезКоличествоПрописанных,
	|	РазрезыНормативаУслуг.КоличествоЭтажей КАК ЕстьРазрезКоличествоЭтажей,
	|	энргАбоненты.ВидОбъекта КАК ВидСобственности,
	...
	|		ПО ВсеСведения.МКД = энргСтроения.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РазрезыНормативаУслуг КАК РазрезыНормативаУслуг
	|		ПО ВсеСведения.ПериодНачисления = РазрезыНормативаУслуг.ПериодНачисления
	|			И ВсеСведения.Услуга = РазрезыНормативаУслуг.Услуга
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ДанныеНормТарифов.ГруппаПотребителей КАК ТарифнаяГруппа,
	...
	|	ЕСТЬNULL(энргКлючиНормативов.Ссылка, ЗНАЧЕНИЕ(Справочник.энргКлючиНормативов.ПустаяСсылка)) КАК КлючНорматива
	...
	|		ПО (ВЫБОР
	|				КОГДА ДанныеНормТарифов.ЕстьРазрезВидБлагоустройства
	|					ТОГДА ДанныеНормТарифов.ВидБлагоустройства = энргКлючиНормативов.ВидБлагоустройства
	|				ИНАЧЕ энргКлючиНормативов.ВидБлагоустройства = ЗНАЧЕНИЕ(Справочник.энргВидыБлагоустройств.ПустаяСсылка)
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ДанныеНормТарифов.ЕстьРазрезВидСобственности
	|					ТОГДА ДанныеНормТарифов.ВидСобственности = энргКлючиНормативов.ВидСобственности
	|				ИНАЧЕ энргКлючиНормативов.ВидСобственности = ЗНАЧЕНИЕ(Перечисление.энргТипыСобственниковЖилья.ПустаяСсылка)
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ДанныеНормТарифов.ЕстьРазрезГодПостройки
	|					ТОГДА ДанныеНормТарифов.ГодПостройки = энргКлючиНормативов.ГодПостройки
	|				ИНАЧЕ энргКлючиНормативов.ГодПостройки = 0
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ДанныеНормТарифов.ЕстьРазрезКоличествоКомнат
	|					ТОГДА ВЫБОР
	|							КОГДА энргКлючиНормативов.КоличествоКомнатДо = 0
	|								ТОГДА ДанныеНормТарифов.КоличествоКомнат >= энргКлючиНормативов.КоличествоКомнатОт
	|										И энргКлючиНормативов.КоличествоКомнатОт > 0
	|							ИНАЧЕ ДанныеНормТарифов.КоличествоКомнат >= энргКлючиНормативов.КоличествоКомнатОт
	|									И ДанныеНормТарифов.КоличествоКомнат < энргКлючиНормативов.КоличествоКомнатДо
	|									И энргКлючиНормативов.КоличествоКомнатОт > 0
	|						КОНЕЦ
	...
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ДанныеНормТарифов.ЕстьРазрезКоличествоПрописанных
	|					ТОГДА ВЫБОР
	|							КОГДА энргКлючиНормативов.КоличествоПрописанныхДо = 0
	|								ТОГДА ДанныеНормТарифов.КоличествоПрописанных >= энргКлючиНормативов.КоличествоПрописанныхОт
	|										И энргКлючиНормативов.КоличествоПрописанныхОт > 0
	|							ИНАЧЕ ДанныеНормТарифов.КоличествоПрописанных >= энргКлючиНормативов.КоличествоПрописанныхОт
	|									И ДанныеНормТарифов.КоличествоПрописанных < энргКлючиНормативов.КоличествоПрописанныхДо
	|									И энргКлючиНормативов.КоличествоПрописанныхОт > 0
	|						КОНЕЦ
	...
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА ДанныеНормТарифов.ЕстьРазрезКоличествоЭтажей
	|					ТОГДА ВЫБОР
	|							КОГДА энргКлючиНормативов.КоличествоЭтажейДо = 0
	|								ТОГДА ДанныеНормТарифов.КоличествоЭтажей >= энргКлючиНормативов.КоличествоЭтажейОт
	|										И энргКлючиНормативов.КоличествоЭтажейОт > 0
	|							ИНАЧЕ ДанныеНормТарифов.КоличествоЭтажей >= энргКлючиНормативов.КоличествоЭтажейОт
	|									И ДанныеНормТарифов.КоличествоЭтажей < энргКлючиНормативов.КоличествоЭтажейДо
	|									И энргКлючиНормативов.КоличествоЭтажейОт > 0
	|						КОНЕЦ
	...
	|			И СтабильныеПериодыБезНомераНачало.Абонент = СтабильныеПериодыБезНомераКонец.Абонент
	...
	|		ПО СтабильныеПериодыНачало.Район = НаборэнргВидыДифференцированностиПриборовУчетаГраницы.Район
	|			И СтабильныеПериодыНачало.Абонент = НаборэнргВидыДифференцированностиПриборовУчетаГраницы.Абонент
	|			И СтабильныеПериодыНачало.ПриборУчета = НаборэнргВидыДифференцированностиПриборовУчетаГраницы.ПриборУчета
	|			И (ВЫБОР
...
Функция ПересчитатьИндивидуальныйОбъем(МВТ,Организация,ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,МКД,Абонент,КорректировкаПоказаний,ПериодНовогоТарифа)
	
	ПрочитатьСтабильныеПериодыПерерасчет(МВТ,Организация, ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,МКД,Абонент);
	
	ПоказанияПерерасчет(МВТ,Организация, ПериодНачисленияНачало,ПериодНачисленияКонец,Район,МКД,Абонент,КорректировкаПоказаний.выгрузить()); // Вспомогательные данные по показаниям
	
	ВспомогательныеДанныеПерерасчет(МВТ,Организация, ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,МКД,Абонент,ПериодНовогоТарифа);
...
Функция ТекстЗапросаДанныеДляРаспределения(Строение)
	...
	|		ДанныеМКДПоУслугам.ДокументРаспределения) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВложенныйЗапрос.Строение,
	|	ВложенныйЗапрос.Услуга
