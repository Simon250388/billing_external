...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	И (ДанныеДляРасчета.НачислятьПоПрибору ИЛИ НЕ ДанныеДляРасчета.ТочкаУчетаРодитель = ЗНАЧЕНИЕ(Справочник.энргТочкиУчета.ПустаяСсылка))
	...
	Пока Выборка.следующий() цикл
		...
		Если НЕ ТекПериодНачисления = Выборка.ПериодНачисления
			...
			Если НЕ ТекПериодНачисления = Неопределено и ДеревоТочекУчета.Количество() > 1 Тогда
				РассчитатьДеревоТочекУчета(ДеревоТочекУчета, Набор);
			КонецЕсли; 			
			ДеревоТочекУчета 				= Новый Соответствие;						
		КонецЕсли;		
...
Процедура РассчитатьДеревоТочекУчета(ДеревоТочекУчета, Набор)
	...
	Для Каждого Вершина Из Дерево Цикл
		...
		Иначе
			...
			ОбъемУслугиНорматива 								= СуммаНормативовПотомковРекурсия(Набор, Вершина.Потомки);
