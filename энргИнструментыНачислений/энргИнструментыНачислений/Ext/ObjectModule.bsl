...
Процедура Начислить(Организация, ПериодНачисления,Район,Строение,Помещение,Документ) Экспорт
	...
	Пока Выборка.следующий() цикл
		...
		ИначеЕсли  ЗначениеЗаполнено(Выборка.Абонент) и Выборка.СпособРаспределенияОбъемаПомещения = Перечисления.энргСпособыРаспределенияОбъема.ДолевоеРаспределение Тогда
			...
			СтрокНабора.ВидНачисления 							= ВидНачисления;
			СтрокНабора.ПериодРасчета 							= ПериодНачисления;
			СтрокНабора.ОбъемУслуги 							= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона].ОбъемУслуги * Выборка.ДоляПотребленияЧислитель / Выборка.ДоляПотребленияЧислитель;
			СтрокНабора.ОбъемУслугиПотребленный					= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона].ОбъемУслугиПотребленный * Выборка.ДоляПотребленияЧислитель / Выборка.ДоляПотребленияЧислитель;
			СтрокНабора.Сумма 									= СтрокНабора.ОбъемУслуги * СтрокНабора.ЗначениеТарифа;
		...
		Иначе
			...
			Если НачисленияПоУслугамПомещения[Выборка.Услуга] = Неопределено Тогда
				...
				СоотвествиеПоТарифнамЗонам.Вставить(Выборка.ТарифнаяЗона,  Новый Структура("ОбъемУслуги,ОбъемУслугиПотребленный,ОбъемУслугиОстаток,ОбъемУслугиПотребленныйОстаток", Выборка.ОбъемУслуги, Выборка.ОбъемУслугиПотребленный,0,0));
			...
			ИначеЕсли НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона] = Неопределено Тогда
				НачисленияПоУслугамПомещения[Выборка.Услуга].Вставить(Выборка.ТарифнаяЗона, Новый Структура("ОбъемУслуги,ОбъемУслугиПотребленный,ОбъемУслугиОстаток,ОбъемУслугиПотребленныйОстаток", Выборка.ОбъемУслуги, Выборка.ОбъемУслугиПотребленный,0,0));
			Иначе	
...
=======
	|	ДанныеДляРасчетаПередНормой.КоэффициентНорматива КАК КоэффициентНорматива
>>>>>>> 9c10e48e0be825533e327a4be0060cdb0dbdb2dc
	|ПОМЕСТИТЬ ДанныеДляРасчета
	|ИЗ
	|	ДанныеДляРасчетаПередНормой КАК ДанныеДляРасчетаПередНормой
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТочкиНачисленияНормы КАК ТочкиНачисленияНормы
	...
	|	ДанныеДляРасчета.ПлощадьСобственнаяПоставка КАК ПлощадьСобственнаяПоставка,
<<<<<<< HEAD
	|	ДанныеДляРасчета.ОбъемПоНорме КАК ОбъемПоНорме
=======
