...
Функция ТекстЗапросаВспомогательныеДанныеНачисления(Строение, Помещение)
	...
	|ГДЕ
	|	СпособыРасчетаПоНаправлениям.Организация = &Организация
	|	И СпособыРасчетаПоНаправлениям.Район = &Район
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Функция ТекстПолучитьДанныеДляРасчета()
	...
	|			И Периоды.НаправлениеИспользованияТУ = НаличиеПриборовУчетаСНачалаГода.НаправлениеИспользованияТУ
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|			И ДанныеДляРасчетаПередНормой.ТочкаУчетаРодитель = ПриоритетНормы.ТочкаУчетаРодитель
	...
	|			И ДанныеДляРасчетаПередНормой.ТочкаУчетаРодитель = ТочкиНачисленияНормы.ТочкаУчетаРодитель	
...
Функция РезультатЗапросаПерерасчетСреднего(МВТ,Организация,ПериодНачисления,НачалоПериодаРасчета, КонецПериодаРасчета,Район,Строение,Помещение,НачисленныйСреднийОбъем=Неопределено)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НачисленныйОбъем КАК НачисленныйОбъем
	|		ПО ДанныеДляСписанияМинРегистратор.Организация = НачисленныйОбъем.Организация
	|			И ДанныеДляСписанияМинРегистратор.Район = НачисленныйОбъем.Район
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеДляСписанияМинРегистратор.Организация,
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, РаспределениеОбъемаПоУслугам, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП, НаборЗаписейСверхНорма, ИспользоватьТарифРек)
	...
	ИначеЕсли ДанныеПоУслуге.КорректироватьИндивидуальныйОбъемСУчетомОДПУ и ЕстьПотреблениеПоПрибору Тогда
		...
		Для Каждого СтрокаНачисленийАбонента ИЗ ДанныеПоУслуге.НачисленияАбонентовБезПрибора Цикл
				...
				Если СтрокаНачисленийАбонента.ПомещениеРодитель = СтрокаНачисленийАбонента.Помещение и НЕ СтрокаНачисленийАбонента.СверхНорматива  Тогда
					ОбъемУслуги 								= ЗначениеПлощади * ОбъемНаКвМ;
					ПотреблениеАбонентов 						= ПотреблениеАбонентов + ОбъемУслуги;
					СуммаПлощадейПомещения 						= 0;
					Если ОбъемУслуги > СтрокаНачисленийАбонента.ЗначениеСоциальнойНормы Тогда
						РазбитьПоСоцНорме(яНабор,яНаборОбщий,НаборЗаписей,СтрокаНачисленийАбонента,СтрокаНачисленийАбонента.ЗначениеСоциальнойНормы,СпособНачисленияОДН,ГраницыПериода,ДанныеПоУслуге,ЕстьСуммаНачислений,ЗначенияТарифов,Услуга,ПотреблениеАбонентов,СуммаПлощадейПомещения,Ложь);
						РазбитьПоСоцНорме(яНабор,яНаборОбщий,НаборЗаписей,СтрокаНачисленийАбонента,ОбъемУслуги - СтрокаНачисленийАбонента.ЗначениеСоциальнойНормы,СпособНачисленияОДН,ГраницыПериода,ДанныеПоУслуге,ЕстьСуммаНачислений,ЗначенияТарифов,Услуга,ПотреблениеАбонентов,СуммаПлощадейПомещения,Истина);
					Иначе
						РазбитьПоСоцНорме(яНабор,яНаборОбщий,НаборЗаписей,СтрокаНачисленийАбонента,ОбъемУслуги,СпособНачисленияОДН,ГраницыПериода,ДанныеПоУслуге,ЕстьСуммаНачислений,ЗначенияТарифов,Услуга,ПотреблениеАбонентов,СуммаПлощадейПомещения,Ложь)
					КонецЕсли;
...
Процедура РазбитьПоСоцНорме(яНабор,яНаборОбщий,НаборЗаписей,СтрокаНачисленийАбонента,ОбъемУслуги,СпособНачисленияОДН,ГраницыПериода,ДанныеПоУслуге,ЕстьСуммаНачислений,ЗначенияТарифов,Услуга,ПотреблениеАбонентов,СуммаПлощадейПомещения,СверхНорматива)
	СтрокаНабора  								= яНабор.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаНабора, СтрокаНачисленийАбонента);
				СтрокаНабора.Распределение 					= Истина;
				СтрокаНабора.СпособНачисления				= СпособНачисленияОДН;
