...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	ВЫБОР
	|		КОГДА ОбъемыНачислений.РасчетПоФормуле
	|				ИЛИ НЕ ОбъемыНачислений.ЗависитОтУслуги = ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка)
	|            ТОГДА ОбъемыНачислений.ОбъемУслуги
	...
	|	ОбъемыНачислений.ЗначениеТарифаСверхНорматива КАК ЗначениеТарифаСверхНорматива,
	|	ВЫБОР
	|		КОГДА ОбъемыНачислений.ОбъемУслугиСоцНорма = 0
	|				ИЛИ ОбъемыНачислений.РасчетПоФормуле
	|				ИЛИ НЕ ОбъемыНачислений.ЗависитОтУслуги = ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка)
	|            ТОГДА ОбъемыНачислений.ОбъемУслуги
	|		КОГДА ОбъемыНачислений.ОбъемУслуги > ОбъемыНачислений.ОбъемУслугиСоцНорма
	|				И ОбъемыНачислений.ОбъемУслугиСоцНорма > 0
	...
	|	И НЕ ОбъемыНачислений.ЗависитОтУслуги = ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОбъемыНачислений.ПериодНачисления,
	|	ОбъемыНачислений.Район,
...
Процедура ПересчитатьСтрокиПоФормуле(Набор, НачисленияПоУслугамПомещения, ПересчитатьСтрокиПоФормуле)
	...
	Для Каждого СтрокаПоФормуле Из ПересчитатьСтрокиПоФормуле Цикл
		СтрокаНабора 							= Набор.Получить(СтрокаПоФормуле.Значение.ИндексСтроки);
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, ДокументСсылка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле)
	...
	Если Выборка.ПомещениеРодитель = Выборка.Помещение И ЗначениеЗаполнено(Выборка.ЗависитОтУслуги) Тогда
		...
		Если ОбъемУслуги > Выборка.ОбъемУслугиСоцНорма и НЕ Выборка.ОбъемУслугиСоцНорма = 0 Тогда
			ОбъемУслугиСверхНормы 							= ОбъемУслуги - Выборка.ОбъемУслугиСоцНорма;			
			ОбъемУслуги 									= Выборка.ОбъемУслугиСоцНорма;			
		КонецЕсли;	
		...
		Если ОбъемУслугиСверхНормы > 0 Тогда
			СтрокаСверхНормы 								= Набор.добавить();
			ЗаполнитьЗначенияСвойств(СтрокаСверхНормы, СтрокНабора);
			СтрокаСверхНормы.ОбъемУслуги 					= ОбъемУслугиСверхНормы;
			СтрокаСверхНормы.ОбъемУслугиПотребленный 		= 0;
			СтрокаСверхНормы.ЗначениеТарифа 				= Выборка.ЗначениеТарифаСверхНорматива;
			Если ЕстьСумма Тогда
