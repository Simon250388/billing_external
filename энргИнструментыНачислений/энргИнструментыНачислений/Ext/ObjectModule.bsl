...
Процедура СкорректироватьПотреблениеСезонныхУслугМКД(Организация, ПериодНачисления,Район,Строение,Набор) Экспорт 
	...
	// ЗаписатьНаборыЗаписейНачислениеМКД(ДокументОбъект, ЗамещатьНаборВыполненные, ТекЗавершениеОП, Набор, НаборВыполненные, Организация, Ошибки, ТекПериодНачисления, Район, ТекМКД, ТекЧастныйСектор);
...
Функция ТекстЗапросаВспомогательныеДанныеНачисленияМКД(ОтборПоМКД)
	...
	|	СтабильныеПериодыНачалоСоставУзла.ДатаРегистратора КАК ДатаРегистратора,
	...
	|	СтабильныеПериодыНачалоСоставУзла.Бойлер КАК Бойлер,
