...
Процедура НачислениеНаОДН(Организация,ПериодНачисления, Район,МКД, Документ)  Экспорт 
	...
	Пока Выборка.следующий() цикл
		...
		Если Не ТекМКД = Выборка.МКД Или НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
			...
			|Услуга, ФормулаРасчета, Поставщик, СпособНачисления, ЧастьУзлаПриборУчета, НачалоПериода, КонецПериода, ДокументСсылка, КонецПериодаРасчета, ОбъемУслугиПотребленный, ОбъемЗаПрошлыйГод, НеПрименятьСезонность",
			Выборка.Услуга, Выборка.ФормулаРасчета, Выборка.Поставщик, Выборка.СпособНачисления, Выборка.ЧастьУзлаПриборУчета, Выборка.НачалоПериода, Выборка.КонецПериода, ДокументСсылка, КонецПериодаРасчета, Выборка.ОбъемУслугиПотребленный, Выборка.ОбъемЗаПрошлыйГод, Выборка.НеПрименятьСезонность);			
...
Процедура ВспомогательныеДанныеПерерасчет(МВТ,Организация,ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,ДанныеСтроения,Абонент,ПериодНовогоТарифа)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	энргЗарегистрированныйОбъемПриборовОбороты.Организация КАК Организация,
	|	энргЗарегистрированныйОбъемПриборовОбороты.Район КАК Район,
	|	энргЗарегистрированныйОбъемПриборовОбороты.ЧастныйСектор КАК ЧастныйСектор,
	|	энргЗарегистрированныйОбъемПриборовОбороты.Строение КАК Строение,
	|	энргЗарегистрированныйОбъемПриборовОбороты.Помещение КАК Помещение,
	|	энргЗарегистрированныйОбъемПриборовОбороты.ТочкаУчета КАК ТочкаУчета,
	|	энргЗарегистрированныйОбъемПриборовОбороты.Шкала КАК Шкала,
	|	энргЗарегистрированныйОбъемПриборовОбороты.ТарифнаяЗона КАК ТарифнаяЗона,
	|	энргЗарегистрированныйОбъемПриборовОбороты.ОбъемУслугиПотребленныйОборот КАК ЗарегистрированныйОбъемПрибора
	|ПОМЕСТИТЬ ЗарегистрированныйОбъемПриборов
	|ИЗ
	|	РегистрНакопления.энргЗарегистрированныйОбъемПриборов.Обороты(
	|		&ПериодНачисленияНачало,
	|		&ПериодНачисленияКонец,
	|		МЕСЯЦ,
	|		Организация = &Организация
	|			И Район = &Район
	|			И ЧастныйСектор = &ЧастныйСектор
	|			И Строение = &Строение
	|			И Помещение = &Помещение) КАК энргЗарегистрированныйОбъемПриборовОбороты 	
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстПолучитьДанныеДляРасчета()
	...
	|	МАКСИМУМ(ВЫБОР
	|				КОГДА ЕСТЬNULL(энргПриборыУчетаАбонента.РегистрируетОбъем, ЛОЖЬ)
	|					И НЕ ЗарегистрированныйОбъемПриборов.ТочкаУчета ЕСТЬ NULL
	|						ТОГДА ИСТИНА
	|				ИНАЧЕ ЕСТЬNULL(ПредоставленныеПоказанияПоАбоненту.ПоказанияТекущегоПериода, ЛОЖЬ)
	...
	|	ЕСТЬNULL(ЗарегистрированныйОбъемПриборов.ЗарегистрированныйОбъемПрибора, 0)  КАК ЗарегистрированныйОбъемПрибора,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗарегистрированныйОбъемПриборов КАК ЗарегистрированныйОбъемПриборов
	|		ПО Периоды.Организация = ЗарегистрированныйОбъемПриборов.Организация
	|			И Периоды.Район = ЗарегистрированныйОбъемПриборов.Район
	|			И Периоды.ЧастныйСектор = ЗарегистрированныйОбъемПриборов.ЧастныйСектор
	|			И Периоды.Строение = ЗарегистрированныйОбъемПриборов.Строение
	|			И Периоды.Помещение = ЗарегистрированныйОбъемПриборов.Помещение
	|			И Периоды.ТочкаУчета = ЗарегистрированныйОбъемПриборов.ТочкаУчета
	|			И Периоды.Шкала = ЗарегистрированныйОбъемПриборов.Шкала
	|			И Периоды.ТарифнаяЗона = ЗарегистрированныйОбъемПриборов.ТарифнаяЗона
	...
	|	ЕСТЬNULL(ЗарегистрированныйОбъемПриборов.ЗарегистрированныйОбъемПрибора, 0),
	...
	|	ДанныеДляРасчетаПередНормой.ЗарегистрированныйОбъемПрибора,
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|				ТОГДА ДанныеДляРасчета.ЗарегистрированныйОбъемПрибора
	...
	|	ДанныеДляРасчета.КоэффициентСезоности КАК КоэффициентСезоности1,
	...
	|			ИНАЧЕ ДанныеДляРасчета.ДнейВПериоде
	|		КОНЕЦ) КАК ЧИСЛО(15, 6)),
	|	ВЫРАЗИТЬ(ВЫРАЗИТЬ(ДанныеДляРасчета.СреднийОбъем * ДанныеДляРасчета.КоэффициентСезоности * (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(ДанныеДляРасчета.КонецПериода, ДЕНЬ), ДЕНЬ) / ВЫБОР
	|				КОГДА ДанныеДляРасчета.НормативПоДням
	|						И НЕ ДанныеДляРасчета.НеПрименятьСезонность
	|					ТОГДА 1
	|				ИНАЧЕ ДанныеДляРасчета.ДнейВПериоде
	|			КОНЕЦ) КАК ЧИСЛО(15, 6)) КАК ЧИСЛО(15, 6)),
	|	ДанныеДляРасчета.КоэффициентСезоности * ДанныеДляРасчета.КоэффициентНорматива * ВЫБОР
	|		КОГДА ДанныеДляРасчета.СпособОпределенияСоциальнойНормы = ЗНАЧЕНИЕ(Перечисление.энргСпособыОпределенияСоциальнойНормы.ПустаяСсылка)
	|			ТОГДА 0
	|		КОГДА ДанныеДляРасчета.СпособОпределенияСоциальнойНормы = ЗНАЧЕНИЕ(Перечисление.энргСпособыОпределенияСоциальнойНормы.СоциальнаяНормаРавнаНормативуПотребления)
	...
	|		КОГДА ОбъемыНачислений.ОбъемУслуги > ОбъемыНачислений.ЗначениеСоциальнойНормы
	|				И ОбъемыНачислений.ЗначениеСоциальнойНормы > 0
	|			ТОГДА ОбъемыНачислений.ЗначениеСоциальнойНормы
	|		ИНАЧЕ ОбъемыНачислений.ОбъемУслуги
	|	КОНЕЦ КАК ОбъемУслуги,
	|	ОбъемыНачислений.ЗначениеСоциальнойНормы КАК ОбъемУслугиСоцНорма,
	|	ОбъемыНачислений.ЗначениеСоциальнойНормы КАК ЗначениеСоциальнойНормы,
	...
	|	ОбъемыНачислений.ЗначениеТарифаРЭК КАК ЗначениеРЭК,
	|	ВЫБОР
	|		КОГДА ОбъемыНачислений.ЗначениеСоциальнойНормы = 0
	...
	|	ОбъемыНачислений.НачислятьНорму КАК НачислятьНорму,
	|	ОбъемыНачислений.НачислятьПоПрибору КАК НачислятьПоПрибору,
	|	ОбъемыНачислений.Техническая КАК Техническая,
	|	ОбъемыНачислений.ТочкаУчетаРодитель КАК ТочкаУчетаРодитель
	|ИЗ
	...
	|			ТОГДА 0
	|		ИНАЧЕ ОбъемыНачислений.ОбъемУслуги - ОбъемыНачислений.ЗначениеСоциальнойНормы
	|	КОНЕЦ,
	|	0,
	|	ОбъемыНачислений.ЗначениеСоциальнойНормы,
	|	ОбъемыНачислений.СпособНачисления,
	|	ОбъемыНачислений.ЗначениеТарифаСверхНорматива,
	|	ОбъемыНачислений.ЗначениеТарифаСверхНорматива,
	|	ОбъемыНачислений.ЗначениеТарифаРЭК,
	|	(ОбъемыНачислений.ОбъемУслуги - ОбъемыНачислений.ЗначениеСоциальнойНормы) * ОбъемыНачислений.ЗначениеТарифаСверхНорматива,
	...
	|	ИСТИНА,
	|	ОбъемыНачислений.НачислятьНорму,
	|	ОбъемыНачислений.НачислятьПоПрибору,
	|	ОбъемыНачислений.Техническая,
	|	ОбъемыНачислений.ТочкаУчетаРодитель
	|ИЗ
	|	ОбъемыНачислений КАК ОбъемыНачислений
	|ГДЕ
	|	ОбъемыНачислений.ОбъемУслуги - ОбъемыНачислений.ЗначениеСоциальнойНормы > 0
	|	И НЕ ОбъемыНачислений.ЗначениеСоциальнойНормы = 0
...
Функция ТекстЗапросаДанныеДляРаспределения(Строение)
	...
	|	ВложенныйЗапрос.НеНачислятьУслугуАбонентам
	|ИЗ
	|	(ВЫБРАТЬ
...
Процедура ОбработатьРезультатЗапросаНачислений(РезультатЗапроса, Набор, НаборОбъемПомещений, ЗаписыватьНаборыЗаписей, Помещение = Неопределено)
	...
	Пока Выборка.следующий() цикл
		...
		Если НЕ ТекПериодНачисления = Выборка.ПериодНачисления
			...
			НачисленияПоУслугам 				= Новый Соответствие;
			Если НЕ ТекПериодНачисления = Неопределено и ДеревоТочекУчета.Количество() > 1 Тогда
				РассчитатьДеревоТочекУчета(ДеревоТочекУчета, Набор, ПересчитатьТехническиеТочки);
			КонецЕсли; 			
			ДеревоТочекУчета 				= Новый Соответствие;						
		КонецЕсли;		
		...
		Если НЕ ТекМКД = Выборка.МКД И ЗаписыватьНаборыЗаписей Тогда
			...
			Если НЕ ТекМКД = Неопределено Тогда
				ПересчитатьСтрокиПоФормуле(Набор, НачисленияПоУслугамПомещения, ПересчитатьСтрокиПоФормуле);
				УдалитьТехническиеТочкиУчета(Набор, НачисленияПоУслугамПомещения, ПересчитатьТехническиеТочки);
		...
		ЗаполнитьНаборыЗаписейНачисление(Выборка, ДокументСсылка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле, ПересчитатьТехническиеТочки, ДеревоТочекУчета);
...
Процедура УдалитьТехническиеТочкиУчета(Набор, НачисленияПоУслугамПомещения, ПересчитатьТехническиеТочки)
	
	СтрокиУдалить  		= Новый Массив;
	Для Сч = 0 По ПересчитатьТехническиеТочки.ВГраница() Цикл
		СтрокиУдалить.Добавить(Набор.Получить(ПересчитатьТехническиеТочки[Сч]));
	КонецЦикла;
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, ДокументСсылка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле, ПересчитатьТехническиеТочки, ДеревоТочекУчета)
	...
	Если Выборка.Техническая Тогда
		ПересчитатьТехническиеТочки.Добавить(Набор.Индекс(СтрокНабора));
	КонецЕсли;	
	
	Если Не Выборка.СверхНорматива Тогда
		Если ДеревоТочекУчета[Выборка.ТочкаУчетаРодитель] = Неопределено Тогда
			ОписаниеТочкиУчета 								= Новый Структура("ТочкаУчета,ИндексНабора", Выборка.ТочкаУчета, Набор.Индекс(СтрокНабора));
...
Процедура РассчитатьДеревоТочекУчета(ДеревоТочекУчета, Набор, ПересчитатьТехническиеТочки)
	...
	Для Каждого Вершины Из ДеревоТочекУчета Цикл
		...
		Для Каждого КлючЗначение ИЗ Вершины.Значение Цикл			
			Дерево.Добавить(Новый Структура("ТочкаУчета, Потомки, ИндексСтроки", КлючЗначение.ТочкаУчета, Новый Массив, КлючЗначение.ИндексНабора));	
		КонецЦикла;			
	КонецЦикла;
	...
	Для Каждого Вершина Из Дерево Цикл
		ДанныеСтрокиНачислений 		=  Набор.Получить(Вершина.ИндексСтроки);
		Если (ДанныеСтрокиНачислений.СпособНачисления  = Перечисления.энргСпособНачислений.ПоПриборам
			...
			Для Сч = 0 По Вершина.Потомки.ВГраница() Цикл
				СтрокаНачисленийПотомка  						= Набор.Получить(Вершина.Потомки[Сч].ИндексСтроки);
			...
			Иначе
				СтрокаНачисленийПотомка 						= Набор.Получить(Вершина.Потомки[ИндексГруппаТочекБезПрибора].ИндексСтроки);
				СтрокаНачисленийПотомка.ОбъемУслуги 			= ОбъемКРаспределению - РасходПотомковПоПрибору;
				СтрокаНачисленийПотомка.ОбъемУслугиПотребленный = ОбъемКРаспределению - РасходПотомковПоПрибору;
				СтрокаНачисленийПотомка.Сумма  					= СтрокаНачисленийПотомка.ОбъемУслуги * СтрокаНачисленийПотомка.ЗначениеТарифа;
		...
		Иначе
			...
			ДанныеСтрокиНачислений.ОбъемУслуги 					= СуммаНормативовПотомковРекурсия(Набор, Вершина.Потомки);
			...
			ОчиститьОбъемУслугиТочекУчетаДереревоРекурсия(Набор, Вершина.Потомки);
			ИндексТехническойТочки 								= ПересчитатьТехническиеТочки.Найти(Вершина.ИндексСтроки);
			Если НЕ ИндексТехническойТочки = Неопределено Тогда
				  ПересчитатьТехническиеТочки.Удалить(ИндексТехническойТочки);
			КонецЕсли;			
		КонецЕсли;		
	КонецЦикла;	
...
Процедура ЗаполнитьДеревоТочекУчетаРекурсивно(ДеревоТочекУчета,Родитель,Потомки)
	...
	Для Каждого КлючЗначение Из Коллекция Цикл
		...
		Потомки.Добавить(Новый Структура("ТочкаУчета,Потомки, ИндексСтроки", КлючЗначение.ТочкаУчета, врПотомки, КлючЗначение.ИндексНабора)); 		
	КонецЦикла;
КонецПроцедуры
...
Процедура ОчиститьОбъемУслугиТочекУчетаДереревоРекурсия(Набор, Потомки)
	...
	Для Каждого СтрокаПотомка Из Потомки Цикл
		ДанныеСтрокиНачислений 				=  Набор.Получить(СтрокаПотомка.ИндексСтроки);
...
Функция СуммаНормативовПотомковРекурсия(Набор, Потомки) 
	
	врНорматив = 0;
	Для Каждого СтрокаПотомка Из Потомки Цикл
		Если СтрокаПотомка.Потомки.Количество()  = 0 Тогда
			врНорматив = врНорматив + Набор.Получить(СтрокаПотомка.ИндексСтроки).ОбъемУслугиПотребленный;
		Иначе
			врНорматив = врНорматив + СуммаНормативовПотомковРекурсия(Набор, СтрокаПотомка.Потомки);
		КонецЕсли;		
	КонецЦикла;
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП, НаборЗаписейСверхНорма, ИспользоватьТарифРек)
	...
	ИначеЕсли ДанныеПоУслуге.КорректироватьИндивидуальныйОбъемСУчетомОДПУ и ДанныеПоУслуге.ВсегоПлощадьАбонентовПоПрибору > 0 Тогда
		...
		Для Каждого СтрокаНачисленийНП ИЗ ДанныеПоУслуге.НачисленияНПБезПрибора Цикл
			...
			Если НЕ СтрокаНачисленийНП.ОбъемУслуги = 0 ИЛИ НЕ СтрокаНачисленийНП.Сумма = 0 Тогда
				СтрокаНабора  								= яНабор.Добавить();
	...
	Для Сч = 0 По АбонентыДляРаспределения.ВГраница() Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(ДанныеАбонента.Абонент) Тогда			
			...
			Если ДанныеПоУслуге.КорректироватьИндивидуальныйОбъемСУчетомОДПУ Тогда
				Если ДанныеАбонента.ОбъемУслуги  + ОбъемУслуги > ДанныеАбонента.ОбъемУслугиСоцНорма Тогда
