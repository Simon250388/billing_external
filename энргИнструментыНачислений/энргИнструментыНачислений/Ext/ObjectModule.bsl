...
Процедура Начислить(Делегат) Экспорт
	...
	ПолучаемыеНаборы.Добавить(Новый Структура("ИмяНабора,ТаблицаДляПомещения","ЗначенияДополнительныхСоциальныхНормативов",	"ЗначенияДополнительныхСоциальныхНормативов"));
...
Процедура РаспредениеОбъемаМКД(Организация,ПериодНачисления, Район, Строение, Документ)  Экспорт 
	...
	|ДоляПотребленияЧислитель,ДоляПотребленияЗнаменатель,КоличествоПроживающих,КоличествоПрописанных,КоличествоСобственников,КоличествоЧеловек,ТарифнаяГруппа,Поставщик,ЗначениеСоциальнойНормы";
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	ЕСТЬNULL(ЗначенияДополнительныхСоциальныхНормативов.ЗначениеНорматива, 0) КАК ДопСоциальнаяНормаПотребления,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЗначенияДополнительныхСоциальныхНормативов КАК ЗначенияДополнительныхСоциальныхНормативов
	|		ПО ДанныеДляРасчетаПередНормой.Организация = ЗначенияДополнительныхСоциальныхНормативов.Организация
	|			И ДанныеДляРасчетаПередНормой.Район = ЗначенияДополнительныхСоциальныхНормативов.Район
	|			И ДанныеДляРасчетаПередНормой.ПериодНачисления = ЗначенияДополнительныхСоциальныхНормативов.ПериодНачисления
	|			И ДанныеДляРасчетаПередНормой.ЧастныйСектор = ЗначенияДополнительныхСоциальныхНормативов.ЧастныйСектор
	|			И ДанныеДляРасчетаПередНормой.Строение = ЗначенияДополнительныхСоциальныхНормативов.Строение
	|			И ДанныеДляРасчетаПередНормой.Помещение = ЗначенияДополнительныхСоциальныхНормативов.Помещение
	|			И ДанныеДляРасчетаПередНормой.Услуга = ЗначенияДополнительныхСоциальныхНормативов.Услуга
	|			И ДанныеДляРасчетаПередНормой.НаправлениеИспользованияТУ = ЗначенияДополнительныхСоциальныхНормативов.НаправлениеИспользованияТУ
...
Функция ТекстЗапросаДанныеДляРаспределения(Строение)
	...
	|	АбонентыРазделенныеЛицевыеСчета.КоличествоПрописанных КАК КоличествоПрописанных,
	|	АбонентыРазделенныеЛицевыеСчета.КоличествоСобственников КАК КоличествоСобственников,
		...
		Иначе
			...
			Для Каждого ПомещениеПотомок Из ПомещенияПотомки Цикл
				...
				Если ДанныеПоУслуге.КорректироватьИндивидуальныйОбъемСУчетомОДПУ Тогда
					...
					Если ДанныеПомещения.ЗначениеПлощади > ДанныеПомещения.ЗначениеСоциальнойНормы * ДанныеПомещения.КоличествоЧеловек  Тогда 
...
Процедура ЗаполнитьДанныеАбонентовДляРаспределения(ДанныеПоУслуге, ВыборкаАбоненты, ВыборкаОбъемМКД, ВыборкаОтрицательныйОбъемРаспределения, ВыборкаОбъемПоПриборуКорректировать, ВыборкаОбъемПоПриборуНПКорректировать)
	...
	Пока ВыборкаАбоненты.Строение = ВыборкаОбъемМКД.Строение и ВыборкаАбоненты.Услуга = ВыборкаОбъемМКД.Услуга Цикл
		...
		Если НЕ ТекПомещение = ВыборкаАбоненты.Помещение Тогда
			...
			ВыборкаАбоненты.ЗначениеСоциальнойНормы,ВыборкаАбоненты.КоличествоЖильцов, ВыборкаАбоненты.НП, ВыборкаАбоненты.ПомещениеСРазделенными,0, КолвоЧеловек * ВыборкаАбоненты.СоциальнаяНормаПотребления + ВыборкаАбоненты.ДопСоциальнаяНормаПотребления,ВыборкаАбоненты.ТарифнаяЗона);
...
Процедура РазбитьПоСоцНорме(яНабор,яНаборОбщий,НаборЗаписей,СтрокаНачисленийАбонента,ОбъемУслуги,СоцПлощадь,СпособНачисленияОДН,ГраницыПериода,ДанныеПоУслуге,ЕстьСуммаНачислений,ЗначенияТарифов,Услуга,ПотреблениеАбонентов,СуммаПлощадейПомещения,ЭтоОбъемПотребленный)
	...
	Если НЕ РазделенныеЛицевыеСчета[СтрокаНачисленийАбонента.Помещение] = Неопределено Тогда
		...
		Для Каждого ПомещениеПотомок Из ПомещенияПотомки Цикл
			...
			Если ДанныеПомещения.ЗначениеСоциальнойНормы * ДанныеПомещения.КоличествоЧеловек < ДанныеПомещения.ЗначениеПлощади Тогда 
				ОбъемСоциальнойНормы 			= ОбъемПотомка * (ДанныеПомещения.ЗначениеСоциальнойНормы * ДанныеПомещения.КоличествоЧеловек) / ДанныеПомещения.ЗначениеПлощади;
			Иначе
