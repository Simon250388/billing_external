...
Процедура ПересчитатьСтрокиНачисление(Набор, НаборОбъемПомещений, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек, ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ОстатокСоциальнойНормы, ОбъемыПомещенийССоглашением,Ошибки, ИнформацияДляОшибки)
	...
	Попытка
		...
		// Из общего объема по помещению отнимаем начисленный объем по приборам абонентов
		Если ПересчитатьСтрокиПоДолям.Количество() > 0 Тогда
			...
			ОбъемУслугиПотребленныйОстатокОтОстатка = 0;
			Для Каждого КлючЗначениеПомещение Из ПересчитатьСтрокиПоДолям Цикл
				Для Каждого КлючЗначениеУслуга Из КлючЗначениеПомещение.Значение Цикл				
					Для Каждого КлючЗначениеТарифнаяЗона Из  КлючЗначениеУслуга.Значение Цикл					
						Для Каждого КлючЗначениеНачалоПериода Из КлючЗначениеТарифнаяЗона.Значение Цикл
							Для Каждого КлючЗначениеСоставнаяУслуга Из КлючЗначениеНачалоПериода.Значение Цикл
								...
								Если НЕ ОстатокОбъемУслуги = 0 Тогда
									...
									НоваяСтрокаНабора.ОбъемУслуги 				= ОстатокОбъемУслуги * КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЧислитель / КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЗнаменатель;
									НоваяСтрокаНабора.ОбъемУслугиПотребленный 	= 0;
									
									НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиОстаток = НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиОстаток + НоваяСтрокаНабора.ОбъемУслуги;
								...
								Если НЕ ОстатокОбъемУслугиПотребленный = 0 Тогда
									...
									НоваяСтрокаНабора.ОбъемУслугиПотребленный	= ОстатокОбъемУслугиПотребленный * КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЧислитель / КлючЗначениеСоставнаяУслуга.Значение.ДоляПотребленияЗнаменатель;
									НоваяСтрокаНабора.ОбъемУслуги 				= 0;
									
									//НачислениеПоУслугеПомещения.ОбъемУслугиПотребленныйОстаток = НачислениеПоУслугеПомещения.ОбъемУслугиПотребленныйОстаток - НоваяСтрокаНабора.ОбъемУслугиПотребленный;
									НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиПотребленныйОстаток = НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиПотребленныйОстаток + НоваяСтрокаНабора.ОбъемУслугиПотребленный;
								КонецЕсли;
							КонецЦикла;
						КонецЦикла;
					КонецЦикла;				
		...
		// Обходим абонентов со способом распределения "Остаток"
		Если ПересчитатьСтроки.Количество() > 0 Тогда
			Для Сч = 0 По ПересчитатьСтроки.ВГраница() Цикл
				...
				НачислениеПоУслугеПомещения										= НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода][СтрокНабора.СоставнаяУслуга];
				...
				Если ЗначениеЗаполнено(СтрокНабора.ОбъемУслуги) тогда
					...
					НачислениеПоУслугеПомещения.ОбъемУслугиОстаток              = НачислениеПоУслугеПомещения.ОбъемУслугиОстаток - НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиОстаток;
					НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиОстаток		= 0;// обнуляем, необходимо отнять только один раз
					
					ОбъемУслуги 												= НачислениеПоУслугеПомещения.ОбъемУслугиОстаток * СтрокНабора.КоэффициентРаспределенияОстатка;
					Если НЕ ОбъемУслуги = 0 Тогда
						...
						Если НЕ СтрокНабора.СпособНачисления = Перечисления.энргСпособНачислений.ПоПриборам и НЕ СтрокНабора.СпособНачисления = Перечисления.энргСпособНачислений.НачислениеПоСреднему Тогда
							СтрокНабора.ОбъемУслуги = 0;	
						КонецЕсли;						
					КонецЕсли;
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтрокНабора.ОбъемУслугиПотребленный) тогда
					...
					НачислениеПоУслугеПомещения.ОбъемУслугиПотребленныйОстаток  		= НачислениеПоУслугеПомещения.ОбъемУслугиПотребленныйОстаток - НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиПотребленныйОстаток;
					НачислениеПоУслугеПомещения.ОтнятьОтОбъемУслугиПотребленныйОстаток	= 0;// обнуляем, необходимо отнять только один раз
					
					ОбъемУслугиПотребленный 									= НачислениеПоУслугеПомещения.ОбъемУслугиПотребленныйОстаток * СтрокНабора.КоэффициентРаспределенияОстатка;
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек,ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле, ПересчитатьТехническиеТочки, ПересчитатьТехническиеТочкиПомещения,ДеревоТочекУчета, ОстатокСоциальнойНормы, ЗаписиСтороннихПоставщиков, ОбъемыПомещенийССоглашением)
	...
	Иначе
		...
		Если НачисленияПоУслугамПомещения[Выборка.Услуга] = Неопределено Тогда
			...
			|ПовыщающийКоэффициент,СниматьНачисленныйСредний,ОтнятьОтОбъемУслугиОстаток,ОтнятьОтОбъемУслугиПотребленныйОстаток",
			...
			Выборка.ПовыщающийКоэффициент, Выборка.СниматьНачисленныйСредний,0,0);
		...
		Иначе	
			...
			НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОтнятьОтОбъемУслугиОстаток 				= 0;
			НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОтнятьОтОбъемУслугиПотребленныйОстаток 	= 0;
		КонецЕсли;		
	КонецЕсли;
