	...
	Пока ВыборкаОбъемМКД.Следующий() Цикл
		...
		Если НЕ ТекСтроение = ВыборкаОбъемМКД.Строение Или НЕ ТекУслуга = ВыборкаОбъемМКД.Услуга Тогда
			...
			Если Не ТекУслуга  = Неопределено Тогда
				РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, ТекСтроение, ТекУслуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейПомещений, НаборЗаписейНП);
			КонецЕсли;
			...
			Если НЕ ТекСтроение = ВыборкаОбъемМКД.Строение Тогда
				Если (Не НаборЗаписей = Неопределено и НаборЗаписей.Количество() > 0) ИЛИ 
					...
					(НЕ НаборЗаписейПомещений = Неопределено И НаборЗаписейПомещений.Количество() > 0) Тогда
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП)
