	...
	Пока Выборка.следующий() цикл
		...
		Если Не ТекМКД = Выборка.МКД Или НЕ ТекТочкаУчета = Выборка.ТочкаУчета ИЛИ Не ТекНачалоПериода = Выборка.НачалоПериода Тогда
			...
			Если НЕ ТекМКД = Неопределено Тогда
				...
				ДобавитьСтрокиНачисленияМКДПоФормуле(ПериодНачисления, Организация, Район, ТекМКД, ТекТочкаУчета, КонецПериодаРасчета, ДокументСсылка, МассивФормулРасчета, НормативыПоУслугам, Набор);
			КонецЕсли;
