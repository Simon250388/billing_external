...
Функция ТекстПолучитьСтабильныеПериоды(ЭтоПерерасчет)
	...
	|	КОНЕЦ КАК УвеличениеПозиции,
	|	СтабильныеПериодыНачалоСезонности.НормативПоДням КАК НормативПоДням
	...
	|	СтабильныеКонецСезонности.НормативПоДням КАК НормативПоДням
	...
	|	СтабильныеКонецСезонностиТарифнаяЗона.НормативПоДням КАК НормативПоДням
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	ДанныеДляРасчетаПередНормой.МаксимальноВозможныеПоказания КАК МаксимальноВозможныеПоказания,
	|	ВЫБОР 
	|		КОГДА ДанныеДляРасчетаПередНормой.НормативПоДням
	|			ТОГДА 12/365
	|		ИНАЧЕ 1
	|	КОНЕЦ * ЕСТЬNULL(ЗначенияТекущихНормПотребления.ЗначениеНорматива, 0) КАК НормаПотребления,
...
Процедура ПрочитатьСтабильныеПериодыПерерасчет(МВТ,Организация,ПериодНачисленияНачало,ПериодНачисленияКонец,НачалоПериодаРасчета,КонецПериодаРасчета,Район,ДанныеСтроения,Абонент)
	...
	|	КлючиНорматива.КлючСоцНорматива КАК КлючНорматива
