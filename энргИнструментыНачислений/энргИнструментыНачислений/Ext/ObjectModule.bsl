...
Функция ТекстПолучитьСтабильныеПериоды()
	...
	|	СтабильныеПериодыПоказанияНачало.КлючПомещения КАК КлючПомещения,
	|	СтабильныеПериодыПоказанияНачало.АбонентЗакрытВТекущемПериоде КАК АбонентЗакрытВТекущемПериоде,
	...
	|						ТОГДА СтабильныеПериодыПоказанияНачало.ЗавершениеОП
	|					ИНАЧЕ СтабильныеПериодыПоказанияНачало.ПериодРегистрации
	|				КОНЕЦ = СтабильныеПериодыПоказанияНачало.НачалоОП
	|				И СтабильныеПериодыПоказанияНачало.РегистрируетОбъем
	|			ТОГДА 0
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	,ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ДатаСостоянияПрибора,МЕСЯЦ),МЕСЯЦ,НачислятьСреднийОбъемНеБолееМесяцев) КАК НачислятьСреднийОбъемДо
	|	,ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(ДатаСостоянияПрибора,МЕСЯЦ),МЕСЯЦ,НачислятьСреднийОбъемНеБолееМесяцевОбъемПотребленный) КАК НачислятьСреднийПотребленныйОбъемДо
...
Функция ВычисляемыеПоляРезультатНачисления()
	...
	"(РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Псевдоним.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(Псевдоним.КонецПериода, ДЕНЬ), ДЕНЬ) / ВЫБОР
	|		КОГДА Псевдоним.НормативПоДням
	|				И НЕ Псевдоним.НеПрименятьСезонность
	|			ТОГДА 1
	...
	ВычисляемыеПоля.Вставить("ДлительностьПериода", "(РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Псевдоним.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(Псевдоним.КонецПериода, ДЕНЬ), ДЕНЬ) / Псевдоним.ДнейВПериоде)");
							...
							Для Сч = 0 По КлючЗначениеСоставнаяУслуга.Значение.СтрокиНабораПомещения.ВГраница() Цикл
								...
								Для Каждого КлючЗначениеПомещение Из ПересчитатьСтрокиПоКоличествуЧеловек Цикл
									...
									Для ИндексСтрокиНачисления = 0 По СтрокиНачисленияПотомка.ВГраница() Цикл
										...
										Распределили 									= Ложь;								
										Если СтрокаНабораПотомка.ОбъемУслуги = 0 и СтрокаНабораПотомка.ОбъемУслугиПотребленный = 0 И НЕ ОсталосьРаспределитьОбъем = 0 И НЕ ОсталосьРаспределитьОбъемПотребленный = 0 Тогда
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек,ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле, ПересчитатьТехническиеТочки, ПересчитатьТехническиеТочкиПомещения,ДеревоТочекУчета, ОстатокСоциальнойНормы, ЗаписиСтороннихПоставщиков, ОбъемПотерь)
	...
	ИзменитьОстатокСоциальнойНормы(ОстатокСоциальнойНормы, Выборка.КлючПомещения, Выборка.Услуга, Выборка.НаправлениеИспользованияТУ, Выборка.ЗначениеСоциальнойНормы, Набор, НаборОбъемПомещений, СтрокНабора, Выборка.СоциальнаяНормаЗависитОтРасхода);
...
Процедура ИзменитьОстатокСоциальнойНормы(ОстатокСоциальнойНормы, КлючПомещения, Услуга, НаправлениеИспользованияТУ, ЗначениеСоциальнойНормы, Набор, НаборОбъемПомещений, СтрокНабора, СоциальнаяНормаЗависитОтРасхода);
	...
	КлючиОписанияСоциальнойНормы 							= "КонецПериода,Остаток,ОстатокПотребленный,ИндексыСтрок,ИндексыСтрокПомещения";
	...
	Если ОстатокСоциальнойНормы[КлючПомещения] = Неопределено Тогда
		...
		СоотвествиеУслуга		  							= Новый Соответствие;
		ОписаниеСоциальнойНормы  							= Новый Структура(КлючиОписанияСоциальнойНормы, СтрокНабора.КонецПериода,яЗначениеСоциальнойНормы,яЗначениеСоциальнойНормыОбъемПотребленный,ИндексыСтрок,ИндексыСтрокПомещения);
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, РаспределениеОбъемаПоУслугам, НаборОтрицательныйОбъемРаспределения, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП, НаборЗаписейСверхНорма, ИспользоватьТарифРек)
	...
	ИначеЕсли  ОсталосьРаспределить > 0 Тогда
		...
		ИндексРасшифровки 									= 0;
		Пока ОсталосьРаспределить > 0 и ИндексРасшифровки < ОтрицательныйОбъемРасшифровка.Количество() Цикл
			Снимаем 										= Мин(ОсталосьРаспределить,ОтрицательныйОбъемРасшифровка[ИндексРасшифровки].ОбъемУслуги);
			...
			ОсталосьРаспределить 							= ОсталосьРаспределить - Снимаем;
		КонецЦикла;		
	КонецЕсли;	
...
Процедура РаспределитьОбъемПотребленныйПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, РаспределениеОбъемаПоУслугам, НаборОтрицательныйОбъемРаспределения, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП, НаборЗаписейСверхНорма, ИспользоватьТарифРек)
	...
	ИначеЕсли  ОсталосьРаспределить > 0 Тогда
		...
		ИндексРасшифровки 										= 0;
		Пока ОсталосьРаспределить > 0 и ИндексРасшифровки < ОтрицательныйОбъемРасшифровка.Количество() Цикл
			Снимаем 											= Мин(ОсталосьРаспределить,ОтрицательныйОбъемРасшифровка[ИндексРасшифровки].ОбъемУслугиПотребленный);
			СтрокаНабораОтрицательный  							= НаборОтрицательныйОбъемРаспределения.ДобавитьРасход();
			СтрокаНабораОтрицательный.Период	 				= ГраницыПериода.ЗавершениеОП-1;
			СтрокаНабораОтрицательный.Организация 				= Организация;
			СтрокаНабораОтрицательный.Район 					= Район;
