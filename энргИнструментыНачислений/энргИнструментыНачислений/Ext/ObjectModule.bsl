...
Процедура ОбработатьРезультатЗапросаНачислений(РезультатЗапроса, Делегат)
	...
	НаборОбъемПомещений 						= Неопределено;
 	...
 	Пока Выборка.следующий() цикл
		...
		ЗаполнитьНачисленияПоУслугамПомещения(Выборка, НачисленияПоУслугамПомещения);
		ЗаполнитьПараметрыРаспределенияОбъемаПомещения(Выборка, ПараметрыРаспределенияОбъемаПомещения);
		ИзменитьОстатокОбъемаКРаспределениюВПомещении(Выборка, НачисленияПоУслугамПомещения);
			...
			Для Каждого КлючЗначениеТарифнаяЗона Из КлючЗначениеУслуга.Значение Цикл
				Для Каждого КлючЗначениеНачалоПериода Из КлючЗначениеТарифнаяЗона.Значение Цикл
					Для Каждого КлючЗначениеСоставнаяУслуга Из КлючЗначениеНачалоПериода.Значение Цикл
						...
						РаспредеделитьОбъемПомещенияПоУслугеНаПотомков(ЗначениеСтруктуры, ПараметрыРаспределенияОбъемаПомещения,КлючЗначениеУслуга.Ключ, КлючЗначениеТарифнаяЗона.Ключ, КлючЗначениеНачалоПериода.Ключ, КлючЗначениеСоставнаяУслуга.Ключ, ПомещенияСОстаткомРаспределения);
						РаспределитьОстатокНаПомещенияСоСпособомРаспределенияОстаток(ЗначениеСтруктуры,ПомещенияСОстаткомРаспределения,ПараметрыРаспределенияОбъемаПомещения, КлючЗначениеУслуга.Ключ, КлючЗначениеТарифнаяЗона.Ключ, КлючЗначениеНачалоПериода.Ключ, КлючЗначениеСоставнаяУслуга.Ключ); 						
					КонецЦикла;					
				КонецЦикла;
			КонецЦикла;
		КонецЦикла;		
...
Процедура ЗаполнитьНачисленияПоУслугамПомещения(Выборка, НачисленияПоУслугамПомещения)
		
	Если НЕ Выборка.Помещение  = Выборка.ПомещениеРодитель Тогда
		Возврат;
	КонецЕсли; 
	...
	ЗначениеСоотвествия.КоличествоЧеловек 					= Выборка.КоличествоЖильцов;
	ЗначениеСоотвествия.ОбъемУслуги 						= Выборка.ОбъемУслуги;
	ЗначениеСоотвествия.ОбъемУслугиПотребленный 			= Выборка.ОбъемУслугиПотребленный;
	ЗначениеСоотвествия.ВидПлощади 							= Выборка.ВидПлощади;
	ЗначениеСоотвествия.ОбъемУслугиОстаток 					= Выборка.ОбъемУслуги;
	ЗначениеСоотвествия.ОбъемУслугиПотребленныйОстаток 		= Выборка.ОбъемУслугиПотребленный;
	ЗначениеСоотвествия.ЗначениеСоциальнойНормы 			= Выборка.ЗначениеСоциальнойНормы;
	...
	ЗначениеСоотвествия.ПовыщающийКоэффициент 				= Выборка.ПовыщающийКоэффициент;
	ЗначениеСоотвествия.СниматьНачисленныйСредний 			= Выборка.СниматьНачисленныйСредний;
	...
	Иначе		
		...
		НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ВидПлощади 						= Выборка.ВидПлощади;
		
		НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПоПрибору				= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПоПрибору + ОбъемУслугиПоПрибору;
		НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйПоПрибору	= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйПоПрибору + ОбъемУслугиПотребленныйПоПрибору;
		НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиБезПрибора			= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиБезПрибора + ОбъемУслугиБезПрибора;
		НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйБезПрибора= НачисленияПоУслугамПомещения[Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга].ОбъемУслугиПотребленныйБезПрибора + ОбъемУслугиПотребленныйБезПрибора;
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыРаспределенияОбъемаПомещения(Выборка, ПараметрыРаспределенияОбъемаПомещения)
	
	Если Выборка.Помещение  = Выборка.ПомещениеРодитель Тогда
		Возврат;
	КонецЕсли;
	
	ЗначениеСоотвествия 						= Новый Структура(
	"ДоляПотребленияЧислитель,ДоляПотребленияЗнаменатель,КоэффициентРаспределенияОстатка,СпособРаспределенияОстатка",
	Выборка.ДоляПотребленияЧислитель, Выборка.ДоляПотребленияЗнаменатель, Выборка.КоэффициентРаспределенияОстатка, Выборка.СпособРаспределенияОбъемаПомещения);
	
	Если ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение] = Неопределено Тогда
		СоотвествиеСоставнаяУслуга 					= Новый Соответствие;
		СоотвествиеСоставнаяУслуга.Вставить(Выборка.СоставнаяУслуга, ЗначениеСоотвествия);
		...
		ПараметрыРаспределенияОбъемаПомещения.Вставить(Выборка.Помещение, СоотвествиеУслуга);
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение][Выборка.Услуга] = Неопределено Тогда
		...
		ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение].Вставить(Выборка.Услуга, СоотвествиеТарифнаяЗона);
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение][Выборка.Услуга][Выборка.ТарифнаяЗона] = Неопределено Тогда
		...
		ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение][Выборка.Услуга].Вставить(Выборка.ТарифнаяЗона,СоотвествиеНачалоПериода)
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение][Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода] = Неопределено Тогда
		
		СоотвествиеСоставнаяУслуга 					= Новый Соответствие;
		СоотвествиеСоставнаяУслуга.Вставить(Выборка.СоставнаяУслуга, ЗначениеСоотвествия);
		
		ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение][Выборка.Услуга][Выборка.ТарифнаяЗона].Вставить(Выборка.НачалоПериода, СоотвествиеСоставнаяУслуга);
	ИначеЕсли ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение][Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода][Выборка.СоставнаяУслуга] = Неопределено Тогда
		
		ПараметрыРаспределенияОбъемаПомещения[Выборка.Помещение][Выборка.Услуга][Выборка.ТарифнаяЗона][Выборка.НачалоПериода].Вставить(Выборка.СоставнаяУслуга, ЗначениеСоотвествия)			
	КонецЕсли;
КонецПроцедуры

Процедура ИзменитьОстатокОбъемаКРаспределениюВПомещении(Выборка, НачисленияПоУслугамПомещения) 
...
Процедура РаспредеделитьОбъемПомещенияПоУслугеНаПотомков(ДанныеДляРаспределения, ПараметрыРаспределенияОбъемаПомещения, Услуга, ТарифнаяЗона, НачалоПериода, СоставнаяУслуга, ПомещенияСОстаткомРаспределения)
...
Процедура РаспределитьОстатокНаПомещенияСоСпособомРаспределенияОстаток(ДанныеДляРаспределения,ПомещенияСОстаткомРаспределения, ПараметрыРаспределенияОбъемаПомещения, Услуга, ТарифнаяЗона, НачалоПериода, СоставнаяУслуга)
