...
Процедура РаспредениеОбъемаМКД(Организация,ПериодНачисления, Район, Строение, Документ)  Экспорт 
	...
	|ДоляПотребленияЧислитель,ДоляПотребленияЗнаменатель,КоличествоПроживающих,ТарифнаяГруппа,Поставщик,ЗначениеСоциальнойНормы";
	...
	Для Сч = 0 По АбонентыДляРаспределения.ВГраница() Цикл
		...
		Иначе
			...
			Для Каждого ПомещениеПотомок Из ПомещенияПотомки Цикл
				...
				Если ДанныеПоУслуге.КорректироватьИндивидуальныйОбъемСУчетомОДПУ Тогда
					ЗначениеСоциальнойНормы 				= ОбъемУслугиПотомка * ДанныеПомещения.ЗначениеСоциальнойНормы  / ДанныеПомещения.ЗначениеПлощади 
				КонецЕсли;
...
Процедура РазбитьПоСоцНорме(яНабор,яНаборОбщий,НаборЗаписей,СтрокаНачисленийАбонента,ОбъемУслуги,СпособНачисленияОДН,ГраницыПериода,ДанныеПоУслуге,ЕстьСуммаНачислений,ЗначенияТарифов,Услуга,ПотреблениеАбонентов,СуммаПлощадейПомещения,СверхНорматива, ОбъемУслугиПотребленный,ЭтоОбъемПотребленный)
	...
	Если НЕ РазделенныеЛицевыеСчета[СтрокаНачисленийАбонента.Помещение] = Неопределено Тогда
		...
		Для Каждого ПомещениеПотомок Из ПомещенияПотомки Цикл
			...
			ОбъемСоциальнойНормы 				= ОбъемПотомка * ДанныеПомещения.ЗначениеСоциальнойНормы / ДанныеПомещения.ЗначениеПлощади;
