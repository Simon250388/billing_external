...
Функция ТекстЗапросаВспомогательныеДанныеРаспределениеМКД(Строение)
	...
	|		энргСтабильныеПериоды.ТочкаУчета КАК ТочкаУчета,
	|		МАКСИМУМ(энргСтабильныеПериоды.НомерПозиции) КАК НомерПозиции,
	|		МАКСИМУМ(ДатаСостоянияЛицевогоСчета) КАК ДатаСостоянияЛицевогоСчета
	|	ИЗ
	|		ДокументыДляРаспределения КАК ДокументыДляРаспределения
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.энргСтабильныеПериоды КАК энргСтабильныеПериоды
	...
	|		МАКСИМУМ(ДАТАВРЕМЯ(1,1,1)) КАК ДатаСостоянияЛицевогоСчета
	|	ИЗ
	|		ДокументыДляРаспределения КАК ДокументыДляРаспределения
	...
	|	&Поставщик,
	|	ДАТАВРЕМЯ(1,1,1) КАК ДатаСостоянияЛицевогоСчета,
...
Функция ТекстЗапросаДанныеДляРаспределения(Строение)
	...
	|	0,
	|	АбонентыРазделенныеЛицевыеСчета.ДатаСостоянияЛицевогоСчета КАК ДатаСостоянияЛицевогоСчета, 
	|	ОбъемНачислений.ТочкаУчета,
	|	ОбъемНачислений.ВидНачисления,
	|	ОбъемНачислений.СпособНачисления,
	|	ОбъемНачислений.НаправлениеИспользованияТУ,
...
Процедура ОбработатьРезультатЗапросаНачислений(РезультатЗапроса, Делегат)
	...
	ОбъемПотерьТочекУчета 			= Новый Соответствие;	
 	...
 	Пока Выборка.следующий() цикл
		...
		ОбъемПотерь = ПолучитьОбъемПотерь(Выборка, АктуальныеЗначенияПотерь, ОбъемПотерьТочекУчета); 		
				...
				Для Каждого КлючЗначениеУслуга Из КлючЗначениеПомещение.Значение Цикл				
					Для Каждого КлючЗначениеТарифнаяЗона Из  КлючЗначениеУслуга.Значение Цикл					
						Для Каждого КлючЗначениеНачалоПериода Из КлючЗначениеТарифнаяЗона.Значение Цикл
							...
							НачислениеПоУслугеПомещения						= НачисленияПоУслугамПомещения[КлючЗначениеУслуга.Ключ][КлючЗначениеТарифнаяЗона.Ключ][КлючЗначениеНачалоПериода.Ключ];
							СтрокиНабораПомещения 							= НачислениеПоУслугеПомещения.СтрокиНабораПомещения;
							ИндексСтроки 									= 0;
							ИндексУслуги 									= 0;
							
							Если ИндексУслуги > 0 Тогда
								Для Сч = 0 По КлючЗначениеНачалоПериода.Значение.СтрокиНабора.ВГраница() Цикл
									СтрокаНабора 							= Набор.Получить(КлючЗначениеНачалоПериода.Значение.СтрокиНабора[Сч]);
							...
							Для Индекс = 0 По СтрокиНабораПомещения.ВГраница() Цикл
								...
								Если ИндексСтроки > КлючЗначениеНачалоПериода.Значение.СтрокиНабора.ВГраница() Тогда
									СтрокаНабора 							= Набор.добавить();
									ЗаполнитьЗначенияСвойств(СтрокаНабора, Набор[КлючЗначениеНачалоПериода.Значение.СтрокиНабора[0]]);								
								Иначе	                               	
									СтрокаНабора 							= Набор.Получить(КлючЗначениеНачалоПериода.Значение.СтрокиНабора[ИндексСтроки]);
								КонецЕсли;									
								...
								СтрокаНабора.ОбъемУслугиПотребленный		= СтрокаНабораПомещения.ОбъемУслугиПотребленный * КлючЗначениеНачалоПериода.Значение.ДоляПотребленияЧислитель / КлючЗначениеНачалоПериода.Значение.ДоляПотребленияЗнаменатель;
								СтрокаНабора.ОбъемУслуги 					= СтрокаНабораПомещения.ОбъемУслуги * КлючЗначениеНачалоПериода.Значение.ДоляПотребленияЧислитель / КлючЗначениеНачалоПериода.Значение.ДоляПотребленияЗнаменатель; 
								...
								Иначе
									СтрокаНабора.ЗначениеСоциальнойНормы 	= НачислениеПоУслугеПомещения.ЗначениеСоциальнойНормы * КлючЗначениеНачалоПериода.Значение.ДоляПотребленияЧислитель / КлючЗначениеНачалоПериода.Значение.ДоляПотребленияЗнаменатель;
								КонецЕсли;
							...
							Для Сч = ИндексСтроки По КлючЗначениеНачалоПериода.Значение.СтрокиНабора.ВГраница() Цикл							
								СтрокаНабора 								= Набор.Получить(КлючЗначениеНачалоПериода.Значение.СтрокиНабора[Сч]);
								СтрокаНабора.ОбъемУслугиПотребленный   		= 0;
								СтрокаНабора.ОбъемУслуги               		= 0;
							КонецЦикла;						
