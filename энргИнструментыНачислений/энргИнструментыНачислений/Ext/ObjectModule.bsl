...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ДанныеДляРасчета.РегистрируетОбъем И НЕ ДанныеДляРасчета.КонПок = ДанныеДляРасчета.НачПок
	...
	|	ДанныеДляРасчета.ТарифнаяЗона,
	|	ДанныеДляРасчета.ПриборВведен,
	|	ВЫРАЗИТЬ(ДанныеДляРасчета.СреднийОбъемПотребленный * ДанныеДляРасчета.КоэффициентСезоности * (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(ДанныеДляРасчета.КонецПериода, ДЕНЬ), ДЕНЬ) / ВЫБОР
	|			КОГДА ДанныеДляРасчета.НормативПоДням
	|					И НЕ ДанныеДляРасчета.НеПрименятьСезонность
	|				ТОГДА 1
	|			ИНАЧЕ ДанныеДляРасчета.ДнейВПериоде
