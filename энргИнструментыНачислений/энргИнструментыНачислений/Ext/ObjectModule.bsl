...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	ДанныеДляРасчетаПередНормой.ТарифнаяЗона КАК ТарифнаяЗона,	
	|	ДанныеДляРасчетаПередНормой.Показания КАК Показания,
	|	ДанныеДляРасчетаПередНормой.КоэффициентСезоности * ДанныеДляРасчетаПередНормой.ДополнительныйКоэффициентСезоности КАК КоэффициентСезоности,
	|	ДанныеДляРасчетаПередНормой.КоэффициентСезоностиОбъемПотребленный КАК КоэффициентСезоностиОбъемПотребленный,
	|	ДанныеДляРасчетаПередНормой.ПредыдущееЗначениеТарифа КАК ПредыдущееЗначениеТарифа,
	|	ДанныеДляРасчетаПередНормой.ПредыдущееЗначениеТарифаСверхНорматива КАК ПредыдущееЗначениеТарифаСверхНорматива,
	|	ДанныеДляРасчетаПередНормой.ПредыдущееЗначениеТарифаРЭК КАК ПредыдущееЗначениеТарифаРЭК,	
...
Процедура ДобавитьРасчетОбъемаУслугиПоНормативу(Схема,ВыбираемыеПоля,ВычисляемыеПоля,МВТ,ТаблицаДанныеДляРасчета)
	...
	"__ОбъемУслугиПоНорме__ * (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Псевдоним.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(Псевдоним.КонецПериода, ДЕНЬ), ДЕНЬ) / ВЫБОР
	...
	Условия.Добавить("НЕ ПриборВведен");
...
Процедура ДобавитьРасчетОбъемаПотребленногоПоНормативу(Схема,ВыбираемыеПоля,ВычисляемыеПоля,МВТ,ТаблицаДанныеДляРасчета)
	...
	"__ОбъемУслугиПотребленныйПоНорме__ * (РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Псевдоним.НачалоПериода, ДЕНЬ), НАЧАЛОПЕРИОДА(Псевдоним.КонецПериода, ДЕНЬ), ДЕНЬ) / ВЫБОР
	|		КОГДА Псевдоним.НормативПоДням
	|				И НЕ Псевдоним.НеПрименятьСезонность
	|			ТОГДА 1
	|		ИНАЧЕ Псевдоним.ДнейВПериоде
...
Процедура ДобавитьРасчетОбъемаУслугиПоСреднемуЗаПрошлыйГод(Схема,ВыбираемыеПоля, ВычисляемыеПоля, МВТ, ТаблицаДанныеДляРасчета)
	...
	ПереопределяемыеПоля.Вставить("ОбъемУслуги", 				"__ОбъемУслугиПоСреднему__");
	ПереопределяемыеПоля.Вставить("ОбъемУслугиПотребленный",	"0");
	ПереопределяемыеПоля.Вставить("Сумма", 						"__ОбъемУслугиПоСреднему__ * ЗначениеТарифа");
	ПереопределяемыеПоля.Вставить("СниматьНачисленныйСредний", 	"ЛОЖЬ");
...
Процедура ДобавитьРасчетОбъемаПотребленногоПоСреднему(Схема,ВыбираемыеПоля, ВычисляемыеПоля, МВТ, ТаблицаДанныеДляРасчета)
	...
	ПереопределяемыеПоля.Вставить("ОбъемУслуги", 				"0");
	ПереопределяемыеПоля.Вставить("ОбъемУслугиПотребленный",	"__ОбъемУслугиПотребленныйПоСреднему__");
	ПереопределяемыеПоля.Вставить("Сумма", 						"0");
	ПереопределяемыеПоля.Вставить("СниматьНачисленныйСредний", 	"ИСТИНА");
	ПереопределяемыеПоля.Вставить("СпособНачисления", 			"ЗНАЧЕНИЕ(Перечисление.энргСпособНачислений.НачислениеПоСреднему)");
	ПереопределяемыеПоля.Вставить("НачальныеПоказания", 		"0");
...
Функция ВычисляемыеПоляРезультатНачисления()
	...
	ВычисляемыеПоля.Вставить("НачалоПериода",		"НАЧАЛОПЕРИОДА(ДанныеДляРасчета.НачалоПериода, ДЕНЬ)");	
	
	#Область ПорядокПомещения
	
	ВычисляемыеПоля.Вставить("НормаПотребления", 	"
	|	Псевдоним.НормаПотребления  * ВЫБОР
	...
	"Псевдоним.КоэффициентСезоности * Псевдоним.НормаПотребления * ВЫБОР
	|		КОГДА Псевдоним.НеПрименятьСезонность
	|			ТОГДА Псевдоним.КоэффициентНормативаНеПрименятьСезонность
	|		ИНАЧЕ Псевдоним.КоэффициентНорматива
	|	КОНЕЦ * ВЫБОР
	...
	"Псевдоним.НормаПотребления * Псевдоним.КоэффициентНорматива * ВЫБОР
	|		КОГДА НЕ Псевдоним.НаправлениеИспользованияТУ = ЗНАЧЕНИЕ(Справочник.энргНаправлениеИспользованияТочекУчета.ОсновноеНаправление)
	|				И Псевдоним.ЕстьИзмерителиНаправления
	|			ТОГДА Псевдоним.КоличествоИзмеретелейНаправления
	|		КОГДА НЕ Псевдоним.ВидПлощади = ЗНАЧЕНИЕ(Перечисление.энргВидыПлощадей.ПустаяСсылка)
...
Функция ТекстЗапросаВспомогательныеДанныеДляКорректировкиПотребленияСезонныхУслуг()
	...
	|			И (НастройкиВыполненияГодовойКорректировкиМКД.МКД = ЗНАЧЕНИЕ(Справочник.энргСтроения.ПустаяСсылка))
	|ГДЕ
	|	ВЫБОР
	...
	|					ТОГДА энргОбъемНачисленийОбороты.ОбъемУслугиОборот
	|				ИНАЧЕ энргОбъемНачисленийОбороты.ОбъемУслугиПотребленныйОборот
	|			КОНЕЦ * энргОбъемНачисленийОбороты.ЗначениеТарифа),
	...
	|		энргОбъемНачисленийОбороты.Услуга,
	|		энргОбъемНачисленийОбороты.Поставщик,
	|		энргОбъемНачисленийОбороты.ЗначениеТарифа) КАК ВложенныйЗапрос
	|ГДЕ
	|	ВЫБОР
 	...
 	Пока Выборка.следующий() цикл
		...
		Если НЕ ТекПериодНачисления = Выборка.ПериодНачисления
			...
			Если Не ТекПомещениеРодитель = Неопределено Тогда
				ПересчитатьСтрокиНачисление(Набор, НаборОбъемПомещений, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек, ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ОстатокСоциальнойНормы);
			КонецЕсли;
			ПересчитатьСтроки 						= Новый Массив;
			ПересчитатьСтрокиПоПлощади 				= Новый Массив;
			ПересчитатьСтрокиПоКоличествуЧеловек 	= Новый Соответствие;
		...
		Если НЕ ТекПериодНачисления = Выборка.ПериодНачисления
			...
			Если НЕ ТекПериодНачисления = Неопределено Тогда
				РассчитатьДеревоТочекУчета(ДеревоТочекУчета, Набор, НаборОбъемПомещений, ПересчитатьТехническиеТочки,ПересчитатьТехническиеТочкиПомещ, ОстатокСоциальнойНормы);									
			КонецЕсли;			
			ДеревоТочекУчета 					= Новый Соответствие;			
		КонецЕсли;		
...
Процедура ПересчитатьСтрокиНачисление(Набор, НаборОбъемПомещений, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПересчитатьСтрокиПоКоличествуЧеловек, ПересчитатьСтрокиПоДолям, ПлощадиПомещенийПоУслугам, ОстатокСоциальнойНормы)
	...
	Если ПересчитатьСтрокиПоПлощади.Количество() > 0 Тогда
		Для Сч = 0 По ПересчитатьСтрокиПоПлощади.ВГраница() Цикл
			...
			Если НЕ ПлощадиПомещенийПоУслугам[СтрокНабора.Услуга][СтрокНабора.НачалоПериода].Площадь = 0 Тогда
				СтрокНабора.ОбъемУслуги 				= (НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода][СтрокНабора.СоставнаяУслуга].ОбъемУслуги * СтрокНабора.ЗначениеПлощади / ПлощадиПомещенийПоУслугам[СтрокНабора.Услуга][СтрокНабора.НачалоПериода].Площадь);
				СтрокНабора.ОбъемУслугиПотребленный		= (НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода][СтрокНабора.СоставнаяУслуга].ОбъемУслугиПотребленный * СтрокНабора.ЗначениеПлощади / ПлощадиПомещенийПоУслугам[СтрокНабора.Услуга][СтрокНабора.НачалоПериода].Площадь);
				СтрокНабора.ЗначениеСоциальнойНормы 	= (НачисленияПоУслугамПомещения[СтрокНабора.Услуга][СтрокНабора.ТарифнаяЗона][СтрокНабора.НачалоПериода][СтрокНабора.СоставнаяУслуга].ЗначениеСоциальнойНормы * СтрокНабора.ЗначениеПлощади / ПлощадиПомещенийПоУслугам[СтрокНабора.Услуга][СтрокНабора.НачалоПериода].Площадь);
			Иначе
						...
						Для Сч = 0 По КлючЗначениеСоставнаяУслуга.Значение.СтрокиНабораПомещения.ВГраница() Цикл
							...
							Для Каждого КлючЗначениеПомещение Из ПересчитатьСтрокиПоКоличествуЧеловек Цикл
								...
								Иначе
									...
									СтрокаНабораПотомка 						= Набор.Получить(СтрокиНачисленияПотомка[ИндексСтрокиПотомка]);
								КонецЕсли;								
								...
								СтрокаНабораПотомка.ПовыщающийКоэффициент 		= СтрокаНабораПомещения.ПовыщающийКоэффициент;
								...
								Если ИндексПомещения = ПересчитатьСтрокиПоКоличествуЧеловек.Количество() -1 Тогда
									СтрокаНабораПотомка.ОбъемУслуги		 		= ОсталосьРаспределитьОбъем;
									СтрокаНабораПотомка.ОбъемУслугиПотребленный	= ОсталосьРаспределитьОбъемПотребленный;
								Иначе								
									СтрокаНабораПотомка.ОбъемУслуги		 		= СтрокаНабораПомещения.ОбъемУслуги * КоличествоЖильцов /  КоличествоЖильцовПомещения;
									СтрокаНабораПотомка.ОбъемУслугиПотребленный	= СтрокаНабораПомещения.ОбъемУслугиПотребленный * КоличествоЖильцов /  КоличествоЖильцовПомещения;
								КонецЕсли; 	
								...
								ОсталосьРаспределитьОбъем 						= ОсталосьРаспределитьОбъем - СтрокаНабораПотомка.ОбъемУслуги;
								ОсталосьРаспределитьОбъемПотребленный 			= ОсталосьРаспределитьОбъемПотребленный - СтрокаНабораПотомка.ОбъемУслугиПотребленный;
...
Процедура РассчитатьДеревоТочекУчета(ДеревоТочекУчета,Набор, НаборОбъемПомещений,ПересчитатьТехническиеТочки, ПересчитатьТехническиеТочкиПомещения,ОстатокСоциальнойНормы)
		...
		Для Каждого Вершина Из Дерево Цикл
			...
			Для Каждого УслугаВершины из Вершина.Услуги Цикл 
				...
				Если (ДанныеСтрокиНачислений.СпособНачисления  = Перечисления.энргСпособНачислений.ПоПриборам
					...
					ОчиститьОбъемУслугиТочекУчетаДереревоРекурсия(Набор, Вершина.Потомки, УслугаВершины.Ключ , ДанныеСтрокиНачислений, ОстатокСоциальнойНормы, ОбщаяСуммаЗначениеСоциальнойНормы);
		...
		Для Каждого КлючЗначениеУслуга Из КлючЗначениеПомещение.Значение Цикл
			Для Каждого КлючЗначениеНаправление Из КлючЗначениеУслуга.Значение Цикл
				...
				Если СтруктураСоциальнойНормы.Остаток < 0 Тогда
					Продолжить;
				КонецЕсли;			
				СтруктураСоциальнойНормы.Вставить("ОстатокПотребленный", СтруктураСоциальнойНормы.Остаток);
				...
				Для Сч = 0 По ИндексыСтрок.ВГраница() Цикл
					СтрокаНабора  				= Набор.Получить(ИндексыСтрок[Сч]);
					ПересчитатьСоциальнуюНормуДляНабора(Набор, СтрокаНабора, СтруктураСоциальнойНормы, ИндексыСтрок, Сч, ПересчитатьТехническиеТочки, Истина);									
				КонецЦикла;
				
				Для Сч = 0 По ИндексыСтрокПомещения.ВГраница() Цикл
					СтрокаНабора  						= НаборОбъемПомещений.Получить(ИндексыСтрокПомещения[Сч]);
					ПересчитатьСоциальнуюНормуДляНабора(НаборОбъемПомещений, СтрокаНабора, СтруктураСоциальнойНормы, ИндексыСтрокПомещения, Сч, ПересчитатьТехническиеТочкиПомещения, Ложь);									
				КонецЦикла;			
			КонецЦикла;		
		КонецЦикла;
	КонецЦикла;
...
Процедура ПересчитатьСоциальнуюНормуДляНабора(Набор, СтрокаНабора, СтруктураСоциальнойНормы, ИндексыСтрок, Сч, ПересчитатьТехническиеТочки, ПересчитыватьСумму)	
	...
	Если СтруктураСоциальнойНормы.Остаток = 0 Тогда					
		...
		Если ЕстьОбъемУслугиПотребленный Тогда
			СтрокаНабора.ОбъемУслугиПотребленныйСоцНорма 	= СтрокаНабора.ОбъемУслугиПотребленный;
		КонецЕсли;		
	...
	ОстатокОбъем							= ИспользуемОбъем - ОстатокОбъем;
	ОстатокОбъемПотребленный				= ИспользуемОбъемПотребленный - ОстатокОбъемПотребленный;
	...
	СтруктураСоциальнойНормы.Остаток					= ОстатокОбъем;
	СтруктураСоциальнойНормы.ОстатокПотребленный 		= ОстатокОбъемПотребленный;
...
Процедура ОчиститьОбъемУслугиТочекУчетаДереревоРекурсия(Набор, Потомки, Услуга, ДанныеСтрокиРодителя, ОстатокСоциальнойНормы, ОбщаяСуммаЗначениеСоциальнойНормы)
	...
	Для Каждого СтрокаПотомка Из Потомки Цикл
		...
		КонецЦикла;
		ОчиститьОбъемУслугиТочекУчетаДереревоРекурсия(Набор, СтрокаПотомка.Потомки, Услуга, ДанныеСтрокиРодителя,ОстатокСоциальнойНормы, ОбщаяСуммаЗначениеСоциальнойНормы);
	КонецЦикла;
