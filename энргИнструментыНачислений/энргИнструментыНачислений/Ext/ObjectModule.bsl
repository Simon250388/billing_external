...
Процедура РаспредениеОбъемаМКД(Организация,ПериодНачисления, Район, Строение, Документ)  Экспорт 
	...
	Если ВыборкаЗначенияТарифов.Количество() > 0 Тогда
		Пока ВыборкаЗначенияТарифов.Следующий() Цикл
			...
			ОписаниеЗначения 				= Новый Структура("Значение,ЗначениеТарифаРЭК,СтавкаНДС,НДСВнутри,ГруппаПотребителей,Услуга,Поставщик,ТарифнаяЗона");
	...
	Если РегистрыСведений.энргИспользованиеТарифовРЭК.Получить(Новый Структура("Организация,Район",Организация, Район)).ПрименятьТарифРЭК Тогда
		...
		Если НЕ ТекСтроение = ВыборкаОбъемМКД.Строение 
			...
			Если Не ТекУслуга  = Неопределено Тогда
				Если ПустаяСтрока(ДанныеПоУслуге.ФормулаРасчетаНорматива) Тогда
					РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, ТекСтроение, ТекУслуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейПомещений, НаборЗаписейНП, НаборЗаписейСверхНорма,ИспользоватьТарифРек);
				КонецЕсли;
...
Функция ТекстПолучитьДанныеДляРасчета()
	...
	|	ЕСТЬNULL(СпособыРасчетаПоНаправлениям.СпособОпределенияСоциальнойНормы, энргВидыУслуг.СпособОпределенияСоциальнойНормы) КАК СпособОпределенияСоциальнойНормы,
	...
	|	ЕСТЬNULL(СпособыРасчетаПоНаправлениям.СпособОпределенияСоциальнойНормы, энргВидыУслуг.СпособОпределенияСоциальнойНормы),
...
Функция ТекстРассчитатьИндивидуальныйОбъемПотребления()
	...
	|	ОбъемыНачислений.ЗначениеТарифаРЭК,
	|	ВЫБОР
	...
	|	И ОбъемыНачислений.ЗависитОтУслуги = ЗНАЧЕНИЕ(Справочник.энргВидыУслуг.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОбъемыНачислений.ПериодНачисления,
	|	ОбъемыНачислений.Район,
...
Процедура ЗаполнитьНаборыЗаписейНачисление(Выборка, ДокументСсылка, Набор, НаборОбъемПомещений, НачисленияПоУслугам, НачисленияПоУслугамПомещения, ПересчитатьСтроки, ПересчитатьСтрокиПоПлощади, ПлощадиПомещенийПоУслугам, ПересчитатьСтрокиПоФормуле)
	...
	Если ЗначениеЗаполнено(Выборка.ЗависимаяТочкаУчета) Тогда
		Если ПересчитатьСтрокиПоФормуле.Получить(Выборка.ЗависимаяТочкаУчета) = Неопределено Тогда 				
			ПересчитатьСтрокиПоФормуле.Вставить(Выборка.ЗависимаяТочкаУчета,Новый Структура(КлючФормулы, Набор.Индекс(СтрокНабора),Новый Массив));				
		КонецЕсли;
...
Процедура РаспределитьОбъемПоУслугеМКД(ПериодНачисления, Организация, Район, МКД, Услуга, ГраницыПериода, ЗначенияТарифов, ДанныеПоУслуге, ДокументОбъект, НаборЗаписей, НаборЗаписейПомещение,НаборЗаписейНП, НаборЗаписейСверхНорма, ИспользоватьТарифРек)
	...
	Для Сч = 0 По АбонентыДляРаспределения.ВГраница() Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(ДанныеАбонента.Абонент) Тогда			
			...
			СтрокаНабора.ПериодРасчета						= ПериодНачисления;
			СтрокаНабора.ПериодНачисления					= ПериодНачисления;
			СтрокаНабора.Сумма								= ОбъемУслуги * СтрокаНабора.ЗначениеТарифа;
			СтрокаНабора.ОбъемУслуги						= ОбъемУслуги;
			СтрокаНабора.ЗначениеПлощади 					= ДанныеАбонента.ЗначениеПлощади;
			Распределили 									= Распределили + СтрокаНабора.ОбъемУслуги;
