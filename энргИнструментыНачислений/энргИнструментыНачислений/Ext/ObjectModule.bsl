...
Функция ТекстПолучитьДанныеДляРасчета()
	...
	| ,НЕ энргВидыУслуг.КорректироватьГодовойОбъем КАК КорректироватьГодовойОбъемПроверка,
	| НЕ ЕСТЬNULL(СпособыРасчетаПоНаправлениям.Сезонность, энргВидыУслуг.Сезонность) = ЗНАЧЕНИЕ(Перечисление.энргВидыСезонности.ПустаяСсылка) КАК СезонностьПроверка,
	| ЕСТЬNULL(НаличиеПриборовУчетаСНачалаГода.ПриборВведен, ЛОЖЬ) КАК ПриборВведенПроверка,
	| ДлительностьПериодов.НачалоФактическогоПериода КАК НачалоФактическогоПериодаПроверка,
	|Периоды.ПериодРегистрации КАК ПериодРегистрацииПроверка,
	|Периоды.ПериодРегистрации МЕЖДУ ДлительностьПериодов.НачалоФактическогоПериода И ДлительностьПериодов.КонецФактическогоПериода КАК ПериодРегистрацииМеждуПроверка,
	|УслугиНеПрименятьСезонностьМКД.НеПрименятьСезонность КАК НеПрименятьСезонностьМКДПроверка,
	|УслугиНеПрименятьСезонностьРайон.НеПрименятьСезонность КАК НеПрименятьСезонностьРайонПроверка,
	|Периоды.НомерПозиции КАК НомерПозицииПроверка,
	|Периоды.КоэффициентСезоности КАК КоэффициентСезоностиПроверка
	...
	|		ИНАЧЕ Периоды.КоэффициентСезоности
	|	КОНЕЦ,
	|	ЕСТЬNULL(УслугиНеПрименятьСезонностьМКД.НеПрименятьСезонность, ЕСТЬNULL(УслугиНеПрименятьСезонностьРайон.НеПрименятьСезонность, ЛОЖЬ)),
	...
	|	ЕСТЬNULL(УслугиНеПрименятьСезонностьМКД.КоэффициентНорматива, ЕСТЬNULL(УслугиНеПрименятьСезонностьРайон.КоэффициентНорматива, 1)),
	|	ВЫБОР
	|		КОГДА НЕ СпособыРасчетаПоНаправлениям.Услуга ЕСТЬ NULL
	|			ТОГДА СпособыРасчетаПоНаправлениям.ВидПлощади
	|		ИНАЧЕ ЕСТЬNULL(энргВидыУслуг.ВидПлощади, ЗНАЧЕНИЕ(Перечисление.энргВидыПлощадей.ПустаяСсылка))
	...
	|,НЕ энргВидыУслуг.КорректироватьГодовойОбъем,
	| НЕ ЕСТЬNULL(СпособыРасчетаПоНаправлениям.Сезонность, энргВидыУслуг.Сезонность) = ЗНАЧЕНИЕ(Перечисление.энргВидыСезонности.ПустаяСсылка),
	| ЕСТЬNULL(НаличиеПриборовУчетаСНачалаГода.ПриборВведен, ЛОЖЬ),
	| ДлительностьПериодов.НачалоФактическогоПериода,
	|Периоды.ПериодРегистрации,
	|Периоды.ПериодРегистрации МЕЖДУ ДлительностьПериодов.НачалоФактическогоПериода И ДлительностьПериодов.КонецФактическогоПериода,
	|УслугиНеПрименятьСезонностьМКД.НеПрименятьСезонность,
	|УслугиНеПрименятьСезонностьРайон.НеПрименятьСезонность,
...
Функция ТекстЗапросаПолучитьСтабильныеПериодыМКД()
	...
	|			И ЗНАЧЕНИЕ(Справочник.энргНаправлениеИспользованияТочекУчета.ОсновноеНаправление) = УслугиНеПрименятьСезонностьМКД.НаправлениеИспользования
	|		ЛЕВОЕ СОЕДИНЕНИЕ УслугиНеПрименятьСезонность КАК УслугиНеПрименятьСезонностьРайон
	...
	|			И ЗНАЧЕНИЕ(Справочник.энргНаправлениеИспользованияТочекУчета.ОсновноеНаправление) = УслугиНеПрименятьСезонностьРайон.НаправлениеИспользования
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
