...
Процедура СнятьСредниеНачисления(Делегат) Экспорт
	...
	ОписаниеПоляСуммы 										= Метаданные.РегистрыНакопления.энргОбъемНачислений.Ресурсы.Сумма.Тип;	
	...
	Для Каждого КолонкаРезультат Из ДанныеДляПерерасчета.НачисленныйОбъем.Колонки Цикл
		...
		Если НЕ текОрганизация = Выборка.Организация
			...
			Если НЕ текОрганизация = Неопределено Тогда
				Для Сч = 0 По мИндексыСтрокДляСписания.ВГраница() Цикл
					СписываемОбъемУслуги 						= Мин(НадоСписатьОбъемУслуги, мИндексыСтрокДляСписания[Сч].ОбъемУслуги);
					СписываемОбъемПотребленный					= Мин(НадоСписатьОбъемПотребленный, мИндексыСтрокДляСписания[Сч].ОбъемУслугиПотребленный);
					СписываемОбъемУслугиПотери 					= Мин(НадоСписатьОбъемУслугиПотери, мИндексыСтрокДляСписания[Сч].ОбъемУслугиПотери);
					СписываемОбъемПотребленныйПотери			= Мин(НадоСписатьОбъемПотребленныйПотери, мИндексыСтрокДляСписания[Сч].ОбъемУслугиПотребленныйПотери);					
					СписываемСумма 								= Окр(?(НадоСписатьОбъемУслуги = мИндексыСтрокДляСписания[Сч].ОбъемУслуги,  мИндексыСтрокДляСписания[Сч].Сумма, НадоСписатьОбъемУслуги * мИндексыСтрокДляСписания[Сч].ЗначениеТарифа),ОписаниеПоляСуммы.КвалификаторыЧисла.РазрядностьДробнойЧасти);
					
					СтрокаНабора 								= Набор.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаНабора, мИндексыСтрокДляСписания[Сч]);
					...
					НадоСписатьОбъемУслуги 						= НадоСписатьОбъемУслуги - СписываемОбъемУслуги;
					НадоСписатьОбъемПотребленный				= НадоСписатьОбъемПотребленный - СписываемОбъемПотребленный;
					НадоСписатьОбъемУслугиПотери 				= НадоСписатьОбъемУслугиПотери - СписываемОбъемУслугиПотери;
					НадоСписатьОбъемПотребленныйПотери			= НадоСписатьОбъемПотребленныйПотери - СписываемОбъемПотребленныйПотери;
					
					Если НадоСписатьОбъемУслуги = 0 и НадоСписатьОбъемПотребленный = 0 Тогда
						Прервать;
					КонецЕсли;
				КонецЦикла;				
			КонецЕсли;  			
...
Функция РезультатЗапросаПерерасчетСреднего(МВТ)
	...
	|	ГДЕ
	|		(НеСписанныйСредний.Организация, НеСписанныйСредний.Район, НеСписанныйСредний.Строение, НеСписанныйСредний.Помещение, НеСписанныйСредний.ЧастныйСектор, НеСписанныйСредний.ТочкаУчета, НеСписанныйСредний.Услуга, НеСписанныйСредний.Шкала, НеСписанныйСредний.Абонент) В
	|				(ВЫБРАТЬ
	...
	|		ПО ВложенныйЗапрос.Организация = ДокументыПерерасчет.Организация
	|			И ВложенныйЗапрос.Район = ДокументыПерерасчет.Район
