...
Функция РезультатЗапросаПерерасчетСреднего(МВТ)
	...
	|				ГДЕ
	|					НачисленныйОбъем.ЕстьСреднийПоВсемТочкамУслуги)
	|		И НеСписанныйСредний.СпособНачисления В (ЗНАЧЕНИЕ(ПЕРЕЧИСЛЕНИЕ.энргСпособНачислений.ПоНормативу), ЗНАЧЕНИЕ(ПЕРЕЧИСЛЕНИЕ.энргСпособНачислений.СреднееПоНормативу), ЗНАЧЕНИЕ(ПЕРЕЧИСЛЕНИЕ.энргСпособНачислений.ПоПриборам))) КАК ВложенныйЗапрос
